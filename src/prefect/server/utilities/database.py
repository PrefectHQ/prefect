...
def datetime_or_interval_add_postgresql(
    ...
    **kwargs: Any,
    ...
    match element:
        case date_add() | interval_add():
            operation = operator.add
        case date_diff():
            operation = operator.sub
    return compiler.process(operation(*element.clauses))
...
def date_diff_seconds_postgresql(
    ...
    return compiler.process(sa.func.extract("epoch", operator.sub(*as_utc)))
...
# The UNIX epoch, 1970-01-01T00:00:00Z, expressed as a fractional Julian day
# number.
...
SQLITE_DATETIME_FORMAT = sa.literal("%F %R:%f000", literal_execute=True)
...
def date_add_sqlite(element: date_add, compiler: SQLCompiler, **kwargs: Any) -> str:
    ...
    return compiler.process(_sqlite_strftime(jdn + offset))
...
def interval_add_sqlite(
    ...
    return compiler.process(_sqlite_strfinterval(operator.add(*offsets)))
...
def date_diff_sqlite(element: date_diff, compiler: SQLCompiler, **kwargs: Any) -> str:
    ...
    return compiler.process(_sqlite_strfinterval(operator.sub(*jdns)))
...
def date_diff_seconds_sqlite(
    ...
    if len(dts) == 1:
        dts = [sa.literal("now", literal_execute=True), *dts]
    ...
    return compiler.process(operator.sub(*as_jdn) * SECONDS_PER_DAY)
...
def _postgresql_array_to_json_array(
    ...
    """Replace any postgresql arary() literals with a json_array() function call
    ...
    def _replacer(element: Any, **kwargs: Any) -> Optional[Any]:
        match element:
            ...
            case (
                sa.Cast(clause=postgresql.array(clauses=clauses))
                | postgresql.array(clauses=clauses)
            ):
                return sa.func.json_array(*clauses)
            case _:
                return None
...
def _json_each(elem: sa.ColumnElement[Any]) -> sa.TableValuedAlias:
    """SQLite json_each() table-valued consruct
...
def sqlite_greatest_as_max(
    element: greatest[Any], compiler: SQLCompiler, **kw: Any
) -> str:
    ...
    # been an issue.
    return compiler.process(sa.func.max(*element.clauses), **kw)
