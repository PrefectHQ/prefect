...
- **SQLite `LIKE` requires an explicit `escape` kwarg.** Unlike PostgreSQL, SQLite does not treat `\` as an escape character in `LIKE` expressions unless you pass `escape="\\"` to `column.like()`. Without it, literal `_` and `%` in values act as wildcards, causing false-positive query results. Always escape `\`, `%`, and `_` in the pattern string and declare `escape="\\"`. See `server/events/filters.py:_is_like_match_expression` for the canonical example.
