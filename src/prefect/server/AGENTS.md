...
- **Work pool fields that workers monitor must be in `WORK_POOL_FIELDS_THAT_TRIGGER_SNAPSHOTS`** — defined in `utilities/worker_channel.py`. When `api/workers.py` updates a work pool, it calls `work_pool_update_triggers_snapshot()` against this frozenset to decide whether to push a snapshot to all connected workers via WebSocket. Adding a new work pool field that workers must react to without polling requires adding it to this frozenset; omitting it causes workers to see stale pool state until they reconnect.
