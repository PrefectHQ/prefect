...
- **`block_document_reference` edges must form a DAG.** `create_block_document_reference` (`models/block_documents.py`) traverses the existing reference graph in-memory before inserting and raises `ValueError` if the new edge would introduce a cycle; `api/block_documents.py` maps that `ValueError` to HTTP 400. A CHECK constraint at the database layer independently prevents self-references (`parent_block_document_id != reference_block_document_id`). `_construct_full_block_document` raises `ValueError` when recursion depth exceeds `_BLOCK_DOCUMENT_REFERENCE_MAX_DEPTH` (50) — any refactor of block document nesting must respect this bound.
