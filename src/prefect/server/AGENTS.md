...
- **`State.data` is typed `Any` — never call `.get()` on it directly in orchestration rules.** Clients may submit scalar, list, or dict values for `state.data`, so `.get()` raises `AttributeError` on non-dict values. Use `_has_persisted_result(data)` (defined in `orchestration/core_policy.py`) for any check that must distinguish persisted result metadata from other data. All rules that inspect result persistence must use this helper.
