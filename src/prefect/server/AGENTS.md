...
- **`server`, `server/api`, and `server/schemas` packages use lazy `__getattr__` loading** — submodules load on first access via `importlib.import_module`. When adding a submodule to any of these packages, add it to `__all__` and the `TYPE_CHECKING` import block; do not add a direct `from . import ...` at module level. Direct imports defeat lazy loading and add startup cost.
