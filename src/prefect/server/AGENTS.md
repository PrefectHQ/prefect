...
- **`StreamingResponse` generators execute outside the request's session scope.** The generator passed to `StreamingResponse` is iterated lazily after the endpoint function returns — by then any outer `async with db.session_context()` block in the endpoint has already exited. Any `StreamingResponse` generator that queries the database must open its own `async with db.session_context() as session:` inside the generator body. Reusing the endpoint-scoped session raises errors or returns stale data at streaming time. See `api/flow_runs.py` `download_logs` for the canonical pattern.
