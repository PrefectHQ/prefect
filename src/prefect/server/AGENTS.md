...
- **V2 concurrency lease release must go through `_release_concurrency_lease` — and failures must propagate.** `_release_concurrency_lease()` in `orchestration/core_policy.py` is the canonical helper for releasing a V2 lease. It handles "lease not found in storage" gracefully via `fallback_concurrency_limit_ids` (pass the known limit ID so the slot can still be decremented) and raises `RuntimeError` if the decrement fails. Orchestration rule cleanup and release methods (`cleanup()`, `after_transition()`) must re-raise after logging — bare `except` blocks that swallow these errors silently leave concurrency slots permanently occupied.
