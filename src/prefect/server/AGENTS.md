...
- **Worker channel handshake (`/workers/connect`) executes pool creation, template backfill, and initial heartbeat inside a single database transaction.** Setup failures (e.g., work-queue-not-found) roll back all three atomically. `update_work_pool` is called with `emit_update_event=False` inside the transaction; the work-pool-updated event fires in a separate post-commit session to avoid publishing events for state that may be rolled back. Any new setup step added to `_build_worker_ready_frame` in `api/workers.py` must remain inside this transaction boundary.
