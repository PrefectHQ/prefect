...
- `worker_communication/` — Pluggable server-side messaging to workers: cleanup delivery queue for expired/cancelled flow run teardown. The abstract interface is in `cleanup_queue/__init__.py`; the default in-memory backend (`cleanup_queue/memory.py`) is a process-level singleton and is not restart-safe. Configure `PREFECT_SERVER_WORKER_CHANNEL_CLEANUP_QUEUE_STORAGE` to point to a module exposing a concrete `WorkerCleanupQueue` subclass for HA or Redis-backed deployments. The delivery dispatch layer (`utilities/worker_channel_cleanup.py`) is a separate process-level singleton (`WORKER_CLEANUP_CONNECTION_REGISTRY`) that maintains per-work-pool dispatcher loops and is equally not restart-safe. In-flight cleanup reservations are keyed by `(work_pool_id, consumer_id, worker_name)` — a worker that reconnects with the same identity can ack/release cleanup messages from before disconnect, within the lease window. Do not change this key without auditing reconnect-ack behavior.
