...
- **`mark_deployments_ready` / `mark_deployments_not_ready` use deterministic ORDER BY locking — never add SKIP LOCKED.** Both functions in `models/deployments.py` lock rows via a CTE with `.order_by(db.Deployment.id).with_for_update()`. The `ORDER BY id` ensures concurrent calls acquire locks in the same order, preventing deadlocks. `SKIP LOCKED` is intentionally absent: a fresh poll that encounters a row locked by a concurrent stale transition must *wait* for it to commit, not silently no-op — `SKIP LOCKED` would let the stale transition commit and overwrite the fresh poll's READY status.
