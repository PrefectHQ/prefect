...
- **Block deletion must clear server default result storage when applicable.** `delete_block_document`, `delete_block_schema`, and `delete_block_type` in `models/` each delegate to `models/storage_defaults.py` to clear the server default result storage when the deleted object is the configured default. Any new block deletion path must include this side effect — omitting it leaves a dangling reference in the `Configuration` key-value store.
