...
- **A `session_context(begin_transaction=True)` block that reads rows before writing them needs `with_for_update=True` on SQLite.** SQLite's default DEFERRED transaction only takes the write lock on the first write statement; if another transaction already holds it, the upgrade fails immediately with `SQLITE_BUSY_SNAPSHOT` and does not honor `busy_timeout`, surfacing as an intermittent "database is locked" error under concurrency. `with_for_update=True` (`database/configurations.py::begin_transaction`) issues `BEGIN IMMEDIATE` instead, taking the write lock upfront and blocking correctly; it's a no-op on Postgres, which locks rows explicitly with `SELECT ... FOR UPDATE` instead. See `mark_deployments_ready` in `models/deployments.py` for the pattern.
