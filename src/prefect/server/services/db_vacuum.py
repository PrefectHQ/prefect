...
class DBVacuum(LoopService):
    ...
    async def delete_logs(
        ...
        total_count = 0
        while keep_going:
            deleted_logs = await db.queries.delete_old_logs(session=session, limit=DBVacuum.settings().batch_size)
    ...
    async def delete_artifacts(
        ...
        total_count = 0
        while keep_going:
            deleted_artifacts = await db.queries.delete_old_artifacts(session=session, limit=DBVacuum.settings().batch_size)
    ...
    async def delete_flow_runs(
        ...
        return await db.queries.delete_old_flow_runs(session=session, before=before, limit=DBVacuum.settings().batch_size)
    ...
    async def run_once(self, db: PrefectDBInterface) -> None:
        ...
        self.logger.info(f"Running DB Vacuum: deleting Prefect resources older than {DBVacuum.settings().retention_days} days (before {cutoff_time.isoformat()}).")
        ...
        async with db.session_context(begin_transaction=False) as session:
            ...
            deleted_flow_runs, deleted_task_runs = await self.delete_flow_runs(db=db, session=session, before=cutoff_time)
        ...
        self.logger.info(f"Finished running DB Vacuum: deleted {deleted_flow_runs} flow runs, {deleted_task_runs} task runs, {deleted_logs} logs, {deleted_artifacts} artifacts.")


if __name__ == "__main__":
    asyncio.run(DBVacuum(handle_signals=True,).start())
