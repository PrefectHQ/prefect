...
class BaseQueryComponents(ABC):
    ...
    def _get_jinja_template(self, template_path: str) -> Template:
        return jinja_env.get_template(
            self._get_query_template_path(template_path)
        )
    ...
    async def delete_old_flow_runs(
        ...
        """
        query = sa.text(
            self._get_jinja_template(template_path="delete-old-flow-runs.sql.jinja").render()
        ).bindparams(sa.bindparam("before", before, type_=Timestamp), sa.bindparam("limit", limit, type_=sa.Integer))
    ...
    async def delete_old_artifacts(
        ...
        """
        query = sa.text(
            self._get_jinja_template(template_path="delete-old-artifacts.sql.jinja").render()
        ).bindparams(limit=limit)
