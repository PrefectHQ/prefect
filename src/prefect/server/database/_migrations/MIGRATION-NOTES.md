...
no-op; the index is removed by downgrading `bad1e352c597`. PostgreSQL migration
runs are serialized with a database advisory lock so concurrent API servers
cannot race the catalog check and rebuild.
