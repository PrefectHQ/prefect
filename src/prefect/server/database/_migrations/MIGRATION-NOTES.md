...
no-op; the index is removed by downgrading `bad1e352c597`. PostgreSQL checks the
rebuilt index before recording the repair revision so concurrent API servers
cannot record an invalid index as repaired.
