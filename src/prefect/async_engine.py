"""
Re-implementation of the engine for supporting fully asynchronous work on the main thread.
"""
import asyncio
import logging
import os
...
async def enter_flow_run_engine_from_flow_call(
    flow: Flow,
    ...
    # can do this here because the awaitable we will return to the user is `enter_flow_run...`
...
async def enter_flow_run_engine_from_subprocess(flow_run_id: UUID) -> State:
    """
    ...
    with capture_sigterm():
        state = await retrieve_flow_then_begin_flow_run(flow_run_id)
...
async def enter_task_run_engine(
    task: Task,
    parameters: Dict[str, Any],
    wait_for: Optional[Iterable[PrefectFuture]],
    return_type: EngineReturnType,
    ...
    run_func = begin_task_map if mapped else get_task_call_return_value
    return await run_func(
        task=task,
    ...
    while state.is_running():
        ...
        with task_run_context.copy(
            ...
            try:
                ...
                # TODO: task timeouts won't be enforced here
...
if __name__ == "__main__":
    ...
    try:
        asyncio.run(enter_flow_run_engine_from_subprocess(flow_run_id))
    except Abort as exc:
        engine_logger.info(
