...
- `ProcessWorker` (`process.py`) — runs direct flow runs through `FlowRunExecutor` with `WorkspaceResolvingEngineCommandStarter`; ad hoc bundles still use `Runner.execute_bundle()`
...
- Direct `ProcessWorker.run()` execution uses `FlowRunExecutorContext` with `WorkspaceResolvingEngineCommandStarter` and `propose_submitting=False` because `BaseWorker` already proposed Submitting. The starter prepares the workspace (storage pulls and deployment pull steps) before it selects the launcher command, so a project that a pull step retrieves can engage the auto-`uv run` launcher. Pass `starter.resolve_flow` to `create_executor()`; `load_flow_from_flow_run` would run the pull steps a second time. Consume the executor's normalized infrastructure status, not the raw child exit code. The ad hoc bundle path still uses deprecated `Runner.execute_bundle()` and remains a migration gap (see `runner/AGENTS.md`).
