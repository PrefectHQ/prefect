...
def error_404_if_workers_not_enabled():
    if not PREFECT_BETA_WORKERS_ENABLED:
...
async def read_worker_pool(
    ...
    # if the special agent pool is being read, ensure it exists
    if worker_pool_name == workers_migration.AGENT_WORKER_POOL_NAME:
        async with db.session_context(begin_transaction=True) as session:
            return await workers_migration.get_or_create_worker_pool(session=session)
...
@router.post("/_migrate_all_agent_work_queues", status_code=status.HTTP_204_NO_CONTENT)
async def migrate_work_queues(
    db: OrionDBInterface = Depends(provide_database_interface),
):
    async with db.session_context(begin_transaction=True) as session:
        await workers_migration.migrate_all_work_queues(session=session)
