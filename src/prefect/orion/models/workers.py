...
async def create_worker_pool(
    ...
    # for backwards compatibility, we use lowercase "default" for the reserved
    # agent pool and "Default Pool" otherwise. This way old deployments that target
    # "default" don't create duplicate and confusing queues.
    default_name = (
        "default"
        if worker_pool.name == workers_migration.AGENT_WORKER_POOL_NAME
        else "Default Pool"
    )
    ...
    default_queue = await create_worker_pool_queue(
        ...
        worker_pool_queue=schemas.actions.WorkerPoolQueueCreate(
            name=default_name, description="The worker pool's default queue."
        ),
    )
