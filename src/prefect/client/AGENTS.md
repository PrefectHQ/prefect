...
- **`Optional[X]` fields in `schemas/` need an explicit `Field(default=None, ...)`.** A bare `Optional[X]` annotation is still a required field in Pydantic v2 — omitting the field from a payload raises a validation error instead of defaulting to `None`.
