...
- `prefect._internal.version_checking` — Shared server version compatibility check (once-per-process, keyed by `(api_url, client_version)`). Used by HTTP clients and WebSocket clients (`events/clients.py`, `logging/clients.py`). When adding a new client type that connects to the server, call `check_server_version` from there rather than reimplementing the check.
