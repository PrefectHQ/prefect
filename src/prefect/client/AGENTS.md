...
- **`@client_injector` always prepends a fresh client; never pass a client positionally.** The decorator unconditionally injects a client as the first arg; any positional client you supply shifts all remaining args by one, causing silent type confusion (e.g., a `PrefectClient` received where a `FlowRun` is expected). Use `@inject_client` instead when the caller needs to supply or override the client — it accepts `client=...` as a kwarg and respects the explicit value.
