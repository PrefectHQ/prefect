...
class RunnerDeployment(BaseModel):
    ...
    async def aapply(
        ...
        async with get_client() as client:
            assert self.flow_name is not None, "flow_name was not provided or inferred"
            ...
            await self._acreate_slas(deployment_id, client)
    ...
    def apply(
        ...
        with get_client(sync_client=True) as client:
            ...
            # to remove existing SLAs for the deployment. If the argument is not provided,
            # we will not call the endpoint.
            self._create_slas(deployment_id, client)

            return deployment_id

    async def _acreate_slas(self, deployment_id: UUID, client: PrefectClient):
    ...
    def _create_slas(self, deployment_id: UUID, client: SyncPrefectClient):
        if self._sla is None:
            return
        if not isinstance(self._sla, list):
            self._sla = [self._sla]
