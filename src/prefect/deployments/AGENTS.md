...
- **Step import side effects**: steps are imported dynamically; packages listed in `requires` are installed into the current environment at execution time. The current prefect version is automatically pinned as a constraint during installation to prevent the running prefect from being overwritten or downgraded. This pin is skipped for local/dev builds (i.e., when `packaging.version.Version(prefect.__version__).local` is truthy).
