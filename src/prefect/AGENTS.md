...
- **Flow-run suspension is enforced at engine-side orchestration boundaries.** External and in-process suspension is delivered to the active flow via `FlowRunSuspensionRequest` and raised as `Pause` through `raise_if_flow_run_suspension_requested()`. Check it when control is about to return to flow user code after orchestration work, or immediately before proposing a flow state that could move the run out of `Suspended`. This includes task/child-flow startup, task completion after transactions commit, future wait/result returns, generator yields, and terminal flow state proposals. Keep sync and async paths in lockstep, and do not add per-boundary API reads.
