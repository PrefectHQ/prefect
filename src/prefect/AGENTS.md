...
- **Context serialization must strip platform-specific types.** `ContextModel.serialize()` overrides in `context.py` are cloudpickled and may be deserialized on a different OS than they were created on (e.g. worker dispatch, subprocess execution) — a `pathlib.WindowsPath` can't be unpickled on POSIX and vice versa. Any field holding a `Path` or other non-portable object must be normalized to a plain string before `serialize()` returns; see `SettingsContext.serialize()`'s `visit_collection` pattern.
