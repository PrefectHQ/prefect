...
- **Threads spawned inside the engine must copy the calling thread's `contextvars` context.** `threading.Thread` starts with an empty context, so `SettingsContext.get()` inside the thread falls back to `GLOBAL_SETTINGS_CONTEXT`, which is initialized at import time and can have a stale `api.url=None`. Use `ctx = contextvars.copy_context()` and pass `target=ctx.run, args=(fn,)` when creating the thread. The heartbeat thread in `_send_heartbeats` demonstrates this pattern.
