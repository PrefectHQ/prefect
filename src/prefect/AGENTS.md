...
- **`DateTime` is the required Pydantic field type for datetimes the server stores.** On Python ≤ 3.12 it is pendulum-backed (`PydanticDateTime`); on Python ≥ 3.13 it is `_DateTime` (a `datetime.datetime` subclass with a custom `__get_pydantic_core_schema__`). Both coerce naive datetimes to UTC during Pydantic validation — wall-clock components are treated as UTC, not converted from local time; tz-aware datetimes pass through unchanged. Using bare `datetime.datetime` for server-facing fields passes Python-side validation but silently fails at the DB layer: the server's SQL bind rejects naive datetimes with "Timestamps must have a timezone."
