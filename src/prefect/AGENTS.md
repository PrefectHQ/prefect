...
- **`PrefectConcurrentFuture` isolates `wait()`/`as_completed()` from user callback exceptions.** `__init__` registers `_notify_prefect_done_callbacks` as the *first* done callback on the wrapped future, before any user callbacks. `wait()` and `as_completed()` use `_register_prefect_done_callback`, which routes to this private channel rather than the public `add_done_callback` chain. This guarantees that user callbacks raising `BaseException` (e.g. `KeyboardInterrupt`) — which Python's `concurrent.futures` does *not* catch — cannot block completion detection. Any subclass overriding `__init__` must call `super().__init__()` to preserve this registration order.
