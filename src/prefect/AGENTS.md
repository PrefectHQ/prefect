...
- **External suspension propagates via `FlowRunSuspensionRequest` and must be checked at every new blocking orchestration point.** When a flow run is suspended while executing in-process (locally via `suspend_flow_run()` or externally via the API), the signal is stored in the active `FlowRunSuspensionRequest` (in `_flow_run_suspension.py`) and raised as `Pause` at orchestration boundaries via `raise_if_flow_run_suspension_requested()`. If you add any new blocking point in an engine, task runner, or future (e.g., a new `wait()` call, a state proposal, or a generator yield), call `raise_if_flow_run_suspension_requested()` at that boundary. Omitting it silently delays suspension until the next boundary that has the check.
