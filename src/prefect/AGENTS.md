...
- **Event and log WebSocket subscribers are parallel implementations.** `events/clients.py::PrefectEventSubscriber` and `logging/clients.py::PrefectLogsSubscriber` duplicate the same reconnect/backoff/dedup logic, including the `reconnect_on_clean_close` option (used by `events/subscribers.py::FlowRunSubscriber` so a clean disconnect doesn't hide a terminal event). Mirror any reconnection-behavior change across both.
...
- **Subflow tracking tasks use UUID dynamic keys when called from within a submitted task.** `flow_engine.py` marks the synthetic tracking `Task` with `_is_subflow_tracking_task=True`; `tasks.py` checks this flag to assign UUID-based (unstable) `dynamic_key` values when the tracking task runs inside a submitted task context. This prevents key collisions between concurrent sibling subflows on parent-flow retry. Direct subflow calls (no enclosing task context) keep stable sequential keys to preserve the retry optimization that reuses completed subflow results.
- **Don't catch bare `TimeoutError`/`CancelledError` to detect an internal `timeout_context()` deadline.** User code inside a task can raise plain `TimeoutError` itself, and `_internal/concurrency/cancellation.py` delivers `CancelledError` to whichever frame the supervised thread is currently suspended in — not necessarily the frame that entered the scope (e.g., a generator paused at `yield`, as in `futures.py::as_completed`). Raise a dedicated exception subtype for the scope's own timeout and check the deadline before treating a caught `CancelledError` as belonging to it.
