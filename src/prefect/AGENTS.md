...
- **`load_subflow_run` always deduplicates subflows, regardless of parent state.** The method queries for an existing subflow run under the parent task run in all cases — including when the parent is still Running (e.g. after a process/pod restart). `_return_value` is set (to skip re-execution) only when the parent task run is final. Non-final reattachment returns the found run without caching, letting the engine re-execute the flow code. Removing the non-final lookup causes duplicate subflow runs on process restart.
