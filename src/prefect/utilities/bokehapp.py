...
from bokeh.models import Arrow, CustomJS, NormalHead, LabelSet, Plot, Range1d, Circle, GraphRenderer, StaticLayoutProvider, Oval, TapTool, HoverTool
...
widths = {x: sum([1 for task, depth in depths.items() if depth == x]) for x in range(max_depth + 1)}
inits, depth_counts = {}, {x: 0 for x in widths}
for task, depth in depths.items():
    width = widths[depth]
    depth_count = depth_counts[depth]
    inits[task] =  (1 - (depth_count  + 1) * 2 / (width + 1), 1 - (depth + 1) * 2 / (max_depth + 2))
    depth_counts[depth] += 1

graph_layout = inits
xnoise, ynoise = [random.random() / 25 for _ in runner.flow.tasks], [random.random() / 5 for _ in runner.flow.tasks]
...
plot = figure(title='Prefect Flow Interactive Demonstration', x_range=(-1.0, 1.0), y_range=(-1.0, 1.0),
...
for edge in list(runner.flow.edges):
    ...
    plot.add_layout(Arrow(end=NormalHead(fill_color="grey", size=7, fill_alpha=0.5),
...
labels = LabelSet(x='x', y='y', text='name', source=source, x_offset=10, y_offset=10, render_mode="canvas", text_font_size="8pt")
plot.renderers.append(labels)
hover = HoverTool(tooltips=[("Name:", "@name"), ("State:", "@state"), ("Message:", "@message{safe}")])
...
butt = Button(label="Run", button_type="success")
butt.on_event(ButtonClick, update)
quit = Button(label="Exit", button_type="danger", callback=CustomJS(code="window.close()"))
