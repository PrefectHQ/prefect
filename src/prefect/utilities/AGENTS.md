...
- **Never access `EngineContext.run_results` directly via `id(obj)`.** Always call `get_state_for_result(obj)` — it performs weakref identity verification to reject stale hits caused by CPython recycling a freed memory address after an object is GC'd. Direct `run_results.get(id(obj))` bypasses this check. `run_results` stores 3-tuples `(State, RunType, Optional[weakref.ref])`, not 2-tuples — unpacking with `state, run_type = entry` raises `ValueError`. Objects that don't support `__weakref__` (plain `dict`, `list`, `str`, `int`, `tuple`) retain the legacy id-only path and still carry the latent stale-id limitation (tracked in issue #20558).
