...
- **`FlowRun` and `TaskRun` cannot be passed as raw task arguments.** `visit_collection` recurses into all Pydantic fields including `.state`, causing the embedded state to be misinterpreted as an upstream task dependency and producing a misleading `NotReady` or upstream-`None` error instead of a clear message. Wrap with `opaque(obj)` from `utilities/annotations.py` to skip traversal. This guard applies whether the object is passed directly, as a mapped argument, or returned from an upstream task.
