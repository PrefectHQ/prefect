...
class CensusSyncTask(Task):
    ...
    def run(
        self,
        api_trigger: str,
        poll_status_every_n_seconds: int = DEFAULT_WAIT_TIME
    ) -> dict:
        f"""
        ...
        if not api_trigger:
            raise ValueError("""Value for parameter `api_trigger` must be provided. See Census sync 
                                configuration page.""")
        ...
        if not confirmed_pattern:
            raise ValueError("""Invalid parameter for `api_trigger` please paste directly from the Census 
        ...
        self.logger.info(f"Started Census sync {sync_id}, sleep time set to {sleep_time} seconds.")
        ...
        while True:
            ...
            if status_response.status_code != 200 or 'data' not in response_dict.keys():
                raise ValueError(f"Getting status of sync failed, please visit Census Logs at {log_url} to see more.")
            result = response_dict['data']
            status = result['status']
            if status == 'working':
                self.logger.info(f"Sync {sync_id} still running after {round(time.time()-start_time, 2)} seconds.")
                continue
            break

        self.logger.info(f"Sync {sync_id} has finished running after {round(time.time()-start_time, 2)} seconds.")
