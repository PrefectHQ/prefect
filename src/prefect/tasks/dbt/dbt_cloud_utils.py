...
def list_run_artifacts(account_id: int, token: str, run_id: int) -> List[str]:
    """
    Lists the artifacts generated by the last step of a dbt Cloud run
    ...
    Returns:
        - List of dbt artifact names

    Raises
        - requests.HTTPError: if request to dbt API returns a 4xx or 5xx response
    """
    list_run_artifacts_response = requests.get(
    ...
    list_run_artifacts_response.raise_for_status()

    return list_run_artifacts_response.json()
