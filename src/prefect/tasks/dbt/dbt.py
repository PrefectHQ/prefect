...
class DbtCloudRunJob(Task):
    ...
    will contain links to the [manifest](https://docs.getdbt.com/reference/artifacts/manifest-json),
    [run results](https://docs.getdbt.com/reference/artifacts/run-results-json), and
    [catalog](https://docs.getdbt.com/reference/artifacts/catalog-json) artifacts generated by dbt Cloud.
    Please note that the catalog artifact will only be generated if dbt docs are generated as part of the
    dbt Cloud job run via `dbt docs generate`.
    ...
    def run(
        ...
        if wait_for_job_run_completion:
            ...
            artifact_links = create_run_artifact_links(
