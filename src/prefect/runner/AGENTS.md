...
**Automatic `uv` command selection:** When no explicit command is passed, `WorkspaceResolvingEngineCommandStarter` reads `pyproject.toml` once, requires an active `prefect` dependency, then chooses in priority order: a `UV_PROJECT_ENVIRONMENT` env var (relative paths resolved against `project_root`), an active `VIRTUAL_ENV` that lives under `project_root` (unrelated venvs are ignored), the current Python environment satisfying the active `project.dependencies` that Prefect can verify from installed distribution names and versions while being able to load the flow entrypoint, or an editable install whose `direct_url.json` (PEP 610) points at the exact `project_root`. Dependencies with extras, direct URLs, or `[tool.uv.sources]` entries are handled by `uv run` unless another pre-materialized environment signal applies. For `.py` file entrypoints, a local project package under `project_root` or `project_root/src` must be importable from the workspace path or installed before this shortcut is used. A project-root `.venv` is not treated as an automatic signal unless selected by `UV_PROJECT_ENVIRONMENT` or `VIRTUAL_ENV`. If a usable Python environment is found, the command is `<python> -m prefect.flow_engine <entrypoint>` — skipping `uv run` entirely. Otherwise, it falls back to `uv run --no-dev --project <project_root> -m prefect.flow_engine` when `uv` is found via the workspace's `PATH` env var (not the system PATH). If none of these conditions are met, the command falls back to `None`. Explicit commands always take precedence.
