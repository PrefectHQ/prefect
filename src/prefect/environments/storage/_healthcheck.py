    ...
    for flow in flows:
        ...
        for task in retry_tasks:
            if any(
                ...
                raise ValueError(
                    "Task {} has retry settings but some upstream dependencies do not have result handlers.".format(
                        task
                    )
                )
        ...
        for task in cached_tasks:
            if task.result_handler is None:
                raise ValueError(
                    "Task {} has cache settings but does not have a result handler.".format(
                        task
                    )
                )
            if any(
                ...
                raise ValueError(
                    "Task {} has cache settings but some upstream dependencies do not have result handlers.".format(
                        task
                    )
                )
