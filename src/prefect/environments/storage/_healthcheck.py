...
def result_handler_check(flows: list):
    for flow in flows:
        ...
        if any([(t.checkpoint and t.result_handler is None) for t in flow.tasks]):
            raise ValueError(
                "Some tasks request to be checkpointed but do not have a result handler. See https://docs.prefect.io/core/core_concepts/results.html for more details."
            )
        ...
        for task in retry_tasks:
            if any(
                ...
                raise ValueError(
                    "Task {} has retry settings but some upstream dependencies do not have result handlers. See https://docs.prefect.io/core/core_concepts/results.html for more details.".format(
                        task
                    )
                )
        ...
        for task in cached_tasks:
            if task.result_handler is None:
                raise ValueError(
                    "Task {} has cache settings but does not have a result handler. See https://docs.prefect.io/core/core_concepts/results.html for more details.".format(
                        task
                    )
                )
            if any(
                ...
                raise ValueError(
                    "Task {} has cache settings but some upstream dependencies do not have result handlers. See https://docs.prefect.io/core/core_concepts/results.html for more details.".format(
                        task
                    )
                )
