...
def set_(
    ...
    for item in settings:
        ...
        if setting not in valid_setting_names:
            _exit_with_error(f"Unknown setting name {setting!r}.")
...
def validate():
    ...
    _exit_with_success("Configuration valid!")
    ...
    *,
    ...
    for setting_name in setting_names:
        if setting_name not in valid_setting_names:
            _exit_with_error(f"Unknown setting name {setting_name!r}.")
    ...
    prefect.settings.save_profiles(profiles)
    _exit_with_success(f"Updated profile {profile.name!r}.")
...
def view(
    *,
    show_defaults: Annotated[
        bool,
        cyclopts.Parameter(
            "--show-defaults", negative="--hide-defaults", help="Show default values"
        ),
    ] = False,
    show_sources: Annotated[
        bool,
        cyclopts.Parameter(
            "--show-sources", negative="--hide-sources", help="Show value sources"
        ),
    ] = True,
    show_secrets: Annotated[
        bool,
        cyclopts.Parameter(
            "--show-secrets", negative="--hide-secrets", help="Show secret values"
        ),
    ] = False,
):
    """
    ...
    if ui_url := prefect.settings.PREFECT_UI_URL.value():
        console.print(f"Connected to: [green]{ui_url}[/green]", soft_wrap=True)
