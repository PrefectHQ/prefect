...
async def _diagnose_plugins() -> None:
    """Implementation of the diagnose command (also called from the deprecated alias)."""
    import importlib.metadata as md

    from prefect._internal.plugins.manager import ENTRYPOINTS_GROUP
    from prefect.settings import get_current_settings

    _cli.console.print("\n[bold]Prefect Plugin System Diagnostics[/bold]\n")
    ...
    if not enabled:
        ...
        _cli.console.print("Set [cyan]PREFECT_PLUGINS_ENABLED=1[/cyan] to enable.\n")
        return
