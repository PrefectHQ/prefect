...
storage_app: cyclopts.App = cyclopts.App(
    name="storage", help="Manage server default result storage."
)

_monotonic = time.monotonic
server_app.command(database_app)
server_app.command(services_app)
server_app.command(storage_app)
...
def _display_server_default_result_storage(
...
@storage_app.command(name="inspect")
@with_cli_exception_handling
async def storage_inspect(
    *,
    ...
    """Inspect the configured server default result storage."""
    from prefect.client.orchestration import get_client
    ...
    async with get_client() as client:
        ...
        if block_document_id is None:
            if output and output.lower() == "json":
                ...
                _cli.console.print("No server default result storage is configured.")
            return
        ...
        if output and output.lower() == "json":
            ...
            _cli.console.print(
                _display_server_default_result_storage(block_document_id, block_slug)
            )


@storage_app.command(name="set")
@with_cli_exception_handling
async def storage_set(
    ...
    *,
    ...
    exit_with_success("Set server default result storage.")


@storage_app.command(name="clear")
@with_cli_exception_handling
async def storage_clear():
    """Clear the configured server default result storage."""
    from prefect.client.orchestration import get_client

    async with get_client() as client:
        await client.clear_server_default_result_storage()

    exit_with_success("Cleared server default result storage.")
