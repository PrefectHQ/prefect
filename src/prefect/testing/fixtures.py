...
async def events_server(
    ...
    async def incoming_events(socket: ServerConnection):
        # 1. authentication (only when puppeteer.token is set, indicating auth is expected)
        # This mirrors the server behavior where auth is only required when
        # PREFECT_SERVER_API_AUTH_STRING is configured
        if puppeteer.token is not None:
