...
1. Before launching the child, the runner registers the flow run with its
   :class:`~prefect.runner._control_channel.ControlChannel` and injects
   `PREFECT__CONTROL_PORT` and `PREFECT__CONTROL_TOKEN` into the child env.
2. The child calls :func:`start` early at process entry. `start` connects
...
3. When the runner wants to act on the run, it writes the intent byte over
   ...
   :data:`Intent`, sets a process-level intent flag, sends `b'a'` as an
...
4. The signal handler raises `TerminationSignal`. The engine's
   `except TerminationSignal` block then reads :func:`get_intent` at
...
The set of valid intents lives in :data:`Intent` below. Only `"cancel"`
...
#: :data:`_INTENT_FOR_BYTE` below and in `_BYTE_FOR_INTENT` in
#: :mod:`prefect.runner._control_channel`, plus a new dispatch branch in
