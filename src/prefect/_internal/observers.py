...
class FlowRunSuspendingObserver:
    ...
    async def __aenter__(self):
        ...
        # queue overflows under load, when the subprocess is CPU-starved and
        # can't drain the socket fast enough, or when a downstream proxy
        # buffers messages. In those cases `_consume_events` waits forever on
        # `recv()` and the websocket-failure fallback never fires. Polling at
