...
@pytest.mark.usefixtures("ecs_mocks")
async def test_failed_to_retrieve_task_definition_for_cached_arn_logs_warnings(
    aws_credentials: AwsCredentials, flow_run: FlowRun, caplog
):
    configuration = await construct_configuration(
        aws_credentials=aws_credentials,
    ...
    _TASK_DEFINITION_CACHE[deployment_id] = fake_task_def_arn
    ...
    async with ECSWorker(work_pool_name="test") as worker:
        ...
        task_definition = worker._prepare_task_definition(
            configuration, region=region, flow_run=flow_run
        )

        caplog.clear()
        with caplog.at_level(logging.WARNING):
            task_def_arn, registered = worker._get_or_register_task_definition(
                logging.getLogger(),
                ecs_client,
                configuration,
                flow_run,
...
@pytest.mark.usefixtures("ecs_mocks")
async def test_failed_to_retrieve_task_definition_from_family_logs_warnings(
    aws_credentials: AwsCredentials, flow_run: FlowRun, caplog
):
    configuration = await construct_configuration(
        aws_credentials=aws_credentials,
    ...
    async with ECSWorker(work_pool_name="test") as worker:
        ...
        family_name = f"test-family-{flow_run.deployment_id}"
        task_definition["family"] = family_name
