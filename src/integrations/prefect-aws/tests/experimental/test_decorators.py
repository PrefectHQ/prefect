...
def mock_submit(monkeypatch: pytest.MonkeyPatch) -> Generator[AsyncMock, None, None]:
    ...
    monkeypatch.setattr(ECSWorker, "submit", mock)
    yield mock
...
def test_ecs_decorator_sync_flow(mock_submit: AsyncMock) -> None:
    """Test that a synchronous flow is correctly decorated and executed"""

    @ecs(work_pool="test-pool", memory=5120)
    @prefect.flow
    ...
    assert kwargs["job_variables"] == {"memory": 5120}
    assert result == "test_result"


async def test_ecs_decorator_async_flow(mock_submit: AsyncMock) -> None:
    """Test that an asynchronous flow is correctly decorated and executed"""

    @ecs(work_pool="test-pool", cpu=2048)
    @prefect.flow
...
@pytest.mark.usefixtures("mock_submit")
def test_ecs_decorator_preserves_flow_attributes() -> None:
    ...
    decorated_flow = ecs(work_pool="test-pool")(original_flow)
...
def test_runtime_error_when_work_pool_is_missing_storage_configuration(
    ...
    @ecs(work_pool=work_pool_without_storage_configuration.name)
    @prefect.flow
    def test_flow():
        return "test"
...
# Tests for include_files parameter


class TestEcsDecoratorIncludeFiles:
    """Tests for the include_files parameter of the @ecs decorator"""
    ...
    def test_include_files_none_passes_none_to_bound_flow(
        ...
        # Verify the include_files attribute is None
    ...
    def test_include_files_empty_list_passes_empty_list(
        ...
        @ecs(work_pool="test-pool", include_files=[])
        @prefect.flow
        def test_flow():
            return "test"
    ...
    def test_include_files_tuple_converted_to_list(
        ...
        @ecs(work_pool="test-pool", include_files=("a.txt", "b.txt"))
        @prefect.flow
        def test_flow():
            return "test"
    ...
    def test_include_files_non_string_item_raises_value_error(self) -> None:
        ...
        with pytest.raises(
            ...
            @ecs(work_pool="test-pool", include_files=["valid", 123])
            @prefect.flow
            def test_flow():
                return "test"
    ...
    def test_include_files_whitespace_only_raises_value_error(self) -> None:
        ...
        with pytest.raises(
            ...
            @ecs(work_pool="test-pool", include_files=["valid", "  "])
            @prefect.flow
            def test_flow():
                return "test"
    ...
    def test_include_files_none_type_item_raises_value_error(self) -> None:
        ...
        with pytest.raises(
            ...
            @ecs(work_pool="test-pool", include_files=[None])  # type: ignore[list-item]
            @prefect.flow
            def test_flow():
                return "test"
