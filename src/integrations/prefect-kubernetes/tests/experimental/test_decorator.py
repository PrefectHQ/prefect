...
class TestKubernetesDecoratorIncludeFiles:
    """Tests for the include_files parameter of the @kubernetes decorator"""
    ...
    def test_include_files_empty_list_passes_empty_list(
        ...
        @kubernetes(work_pool="test-pool", include_files=[])
        @prefect.flow
        def test_flow():
            return "test"
    ...
    def test_include_files_tuple_converted_to_list(
        ...
        @kubernetes(work_pool="test-pool", include_files=("a.txt", "b.txt"))
        @prefect.flow
        def test_flow():
            return "test"
    ...
    def test_include_files_non_string_item_raises_value_error(self) -> None:
        ...
        with pytest.raises(
            ...
            @kubernetes(work_pool="test-pool", include_files=["valid", 123])
            @prefect.flow
            def test_flow():
                return "test"
    ...
    def test_include_files_whitespace_only_raises_value_error(self) -> None:
        ...
        with pytest.raises(
            ...
            @kubernetes(work_pool="test-pool", include_files=["valid", "  "])
            @prefect.flow
            def test_flow():
                return "test"
    ...
    def test_include_files_none_type_item_raises_value_error(self) -> None:
        ...
        with pytest.raises(
            ...
            @kubernetes(work_pool="test-pool", include_files=[None])  # type: ignore[list-item]
            @prefect.flow
            def test_flow():
                return "test"
