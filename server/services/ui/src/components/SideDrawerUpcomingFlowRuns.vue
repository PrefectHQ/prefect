...
export default {
  ...
  apollo: {
    flowRuns: {
      query: require('@/graphql/Drawers/upcoming-flow-runs-drawer.gql'),
      variables() {
        return {
    ...
    flowRunsCount: {
      query: require('@/graphql/Drawers/upcoming-flow-runs-drawer-count.gql'),
      variables() {
        return {
          ...this.genericInput.queryOptions.variables
        }
...
<template>
  <v-container class="side-drawer-flow-runs mb-10 pb-10">
    <v-layout
      v-if="$apollo.queries.flowRuns && $apollo.queries.flowRuns.error"
      column
    ...
    <v-layout v-else-if="$apollo.loading" align-center justify-center>
