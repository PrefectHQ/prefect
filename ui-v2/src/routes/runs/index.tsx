...
export const Route = createFileRoute("/runs/")({
	...
	component: function RouteComponent() {
		...
		const flowRunsCriteriaSignature = useMemo(
			() =>
				JSON.stringify({
					flowRunSearch,
					hideSubflows,
					...
					flows: [...selectedFlows].sort(),
					deployments: [...selectedDeployments].sort(),
					tags: [...selectedTags].sort(),
					workPools: [...selectedWorkPools].sort(),
					dateRange,
				}),
			[
				dateRange,
		...
		const taskRunsCriteriaSignature = useMemo(
			() =>
				JSON.stringify({
					taskRunSearch,
					taskRunsSort,
		...
		const previousFlowRunsCriteriaRef = useRef<string | null>(null);
		const previousTaskRunsCriteriaRef = useRef<string | null>(null);

		useEffect(() => {
			if (previousFlowRunsCriteriaRef.current === null) {
				previousFlowRunsCriteriaRef.current = flowRunsCriteriaSignature;
				return;
			}
			...
			previousFlowRunsCriteriaRef.current = flowRunsCriteriaSignature;
		}, [flowRunsCriteriaSignature, navigate, pagination.page, tab]);

		useEffect(() => {
			if (previousTaskRunsCriteriaRef.current === null) {
				previousTaskRunsCriteriaRef.current = taskRunsCriteriaSignature;
				return;
			}
			...
			previousTaskRunsCriteriaRef.current = taskRunsCriteriaSignature;
		}, [navigate, tab, taskRunsCriteriaSignature, taskRunsPagination.page]);
