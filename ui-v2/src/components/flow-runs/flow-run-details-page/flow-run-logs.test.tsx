...
const MOCK_LOGS = [
	...
	createFakeLog({ level: 20, message: "Info: flow started" }),
...
describe("FlowRunLogs", () => {
	...
	it("filters logs by level", async () => {
		...
		await waitFor(() => {
			expect(screen.getByText("Critical error in flow")).toBeInTheDocument();
			expect(screen.getByText("Error processing data")).toBeInTheDocument();
			expect(
				screen.queryByText("Warning: slow performance"),
			).not.toBeInTheDocument();
			expect(screen.queryByText("Info: flow started")).not.toBeInTheDocument();
			expect(
	...
	it("handles empty logs response for scheduled flow runs", async () => {
	...
	it("handles empty logs response for running flow runs", async () => {
		server.use(
			http.post(buildApiUrl("/logs/filter"), () => {
				return HttpResponse.json([]);
			}),
	...
	it("handles empty logs response for completed flow runs", async () => {
		server.use(
			http.post(buildApiUrl("/logs/filter"), () => {
				return HttpResponse.json([]);
			}),
