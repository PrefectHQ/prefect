...
`Dialog`/`DialogContent` (`@/components/ui/dialog`) and `PopoverContent` (`@/components/ui/popover`) already carry `data-row-click-ignore="true"`, so opening one from a row action needs no extra stopPropagation. Other portal-rendered primitives (e.g. `DropdownMenuContent`) still need the manual exception above.
