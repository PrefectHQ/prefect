...
export const FlowLastRun = ({ row }: { row: { original: Flow } }) => {
    const { data } = useQuery(getLatestFlowRunsQueryParams(row.original.id || '', 16, { enabled: !!row.original.id }))
...
export const FlowNextRun = ({ row }: { row: { original: Flow } }) => {
    const { data } = useQuery(getNextFlowRunsQueryParams(row.original.id || '', 16, { enabled: !!row.original.id }))
...
export const FlowDeploymentCount = ({ row }: { row: { original: Flow } }) => {
    const { data } = useQuery(deploymentsCountQueryParams(row.original.id || '', { enabled: !!row.original.id }))
    if (!row.original.id) return null;
