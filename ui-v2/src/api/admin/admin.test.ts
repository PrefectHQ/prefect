...
test("buildSettingsQuery -- fetches version query from API", async () => {
	const MOCK_SETTINGS_RESPONSE = {
		home: "/my/path/.prefect",
		profiles_path: "/my/path/.prefect/profiles.toml",
		debug_mode: false,
		api: {
		...
		logging: {
			level: "INFO",
			config_path: "/my/path/.prefect/logging.yml",
			extra_loggers: [],
			log_prints: false,
			colors: true,
			markup: false,
		...
		results: {
			...
			local_storage_path: "/my/path/.prefect/storage",
		},
		runner: {
			process_limit: 5,
			poll_frequency: 10,
		...
		server: {
			...
			memo_store_path: "/my/path/.prefect/memo_store.toml",
