...
import { useListActiveTaskRuns } from "./use-list-active-task-runs";

const MOCK_TASK_RUN: components["schemas"]["TaskRun"] = {
...
const MOCK_FLOW_RUN: components["schemas"]["FlowRun"] = {
...
test("useListActiveTaskRuns() returns list of active task run info", async () => {
	server.use(
		http.get("http://localhost:4200/api/task_runs/:id", () => {
			return HttpResponse.json(MOCK_TASK_RUN);
		}),
		http.get("http://localhost:4200/api/flow_runs/:id", () => {
	...
	const MOCK_ACTIVE_SLOTS = ["652d0799-8e73-4b2d-8428-e60b070d0a97"];

	const { result } = renderHook(
		() => useListActiveTaskRuns(MOCK_ACTIVE_SLOTS),
		{ wrapper: createWrapper() },
	);

	await waitFor(() => expect(result.current[0].isSuccess).toBe(true));
	const RESULT_DATA = result.current[0].data;
	expect(RESULT_DATA?.flow).toMatchObject(MOCK_FLOW);
	expect(RESULT_DATA?.flowRun).toMatchObject(MOCK_FLOW_RUN);
	expect(RESULT_DATA?.taskRun).toMatchObject(MOCK_TASK_RUN);
});
