...
- **Pending placeholder nodes are not the same as unfinished nodes.** A `task-run` with `state_type === "PENDING"` and `end_time === null` is a downstream task that never ran (e.g., blocked by upstream failure), not an in-progress node. Use `isPendingPlaceholderNode()` (exported from `factories/nodeBar.ts`) when testing for this; these nodes must not tick, must render at minimum bar width, and must use the stable cache key `"pending-placeholder"`. The `!end_time` check alone misclassifies them as running.
