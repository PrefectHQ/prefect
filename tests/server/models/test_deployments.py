...
class TestScheduledRuns:
    async def test_schedule_runs_inserts_in_db(self, deployment, session):
        ...
        expected_times = {
            DateTime.now("UTC").start_of("day").add(days=i + 1)
    ...
    async def test_schedule_runs_with_end_time(self, flow, deployment, session):
        scheduled_runs = await models.deployments.schedule_runs(
            session,
            deployment_id=deployment.id,
            end_time=DateTime.now("UTC").add(days=17),
    ...
    async def test_schedule_runs_with_start_time(self, flow, deployment, session):
        scheduled_runs = await models.deployments.schedule_runs(
            session,
            deployment_id=deployment.id,
            start_time=DateTime.now("UTC").add(days=100),
            end_time=DateTime.now("UTC").add(days=110),
            # set min runs very high to ensure we keep generating runs until we hit the end time
            # note that end time has precedence over min runs
            min_runs=100,
        )
        ...
        expected_times = {
            DateTime.now("UTC").start_of("day").add(days=i + 1) for i in range(100, 110)
        }
        actual_times = set()
        for run_id in scheduled_runs:
            run = await models.flow_runs.read_flow_run(
    ...
    async def test_schedule_runs_with_times_and_max_number(
        ...
        scheduled_runs = await models.deployments.schedule_runs(
            ...
            deployment_id=deployment.id,
            start_time=DateTime.now("UTC").add(days=100),
            end_time=DateTime.now("UTC").add(days=150),
            max_runs=3,
        )
        assert len(scheduled_runs) == 3

        expected_times = {
            DateTime.now("UTC").start_of("day").add(days=i + 1) for i in range(100, 103)
        }
        actual_times = set()
        for run_id in scheduled_runs:
            run = await models.flow_runs.read_flow_run(
    ...
    async def test_backfill(self, flow, deployment, session):
        ...
        scheduled_runs = await models.deployments.schedule_runs(
            session,
            deployment_id=deployment.id,
            start_time=DateTime.now("UTC").subtract(days=1000),
            end_time=DateTime.now("UTC").subtract(days=990),
            # set min runs very high to ensure we keep generating runs until we hit the end time
            # note that end time has precedence over min runs
            min_runs=100,
        )
        ...
        expected_times = {
            DateTime.now("UTC").start_of("day").subtract(days=i)
