...
class TestJSON:
    ...
    async def test_json_contains_left_side_literal(
        self,
        session: AsyncSession,
        keys: list[str] | list[str | list[int] | int],
        ids: list[int],
    ):
        query = (
            sa.select(SQLJSONModel)
    ...
    async def test_json_contains_both_sides_columns(
        self,
        session: AsyncSession,
        id_for_keys: list[list[int] | int],
        ids_for_results: list[list[int] | int],
    ):
        query = (
            sa.select(SQLJSONModel)
            .where(
...
class TestCustomFunctions:
    def test_sqlite_now_compilation(self) -> None:
        ...
        assert compiled == "strftime('%Y-%m-%d %H:%M:%f000', 'now')"
...
class TestDateFunctions:
    ...
    async def test_date_diff_seconds_from_now_literal(self, session: AsyncSession):
        value = datetime.datetime.now(tz=datetime.UTC) - datetime.timedelta(seconds=17)
