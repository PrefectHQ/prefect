...
def test_validate_runtime_accepts_supported_prefect_versions(
    monkeypatch: pytest.MonkeyPatch, version: str
) -> None:
    monkeypatch.setattr(importlib.metadata, "version", lambda _package: version)

    _workspace_runtime_bootstrap._validate_runtime()


@pytest.mark.parametrize("version", ["2.20.0", "4.0.0"])
def test_validate_runtime_rejects_unsupported_prefect_versions(
    monkeypatch: pytest.MonkeyPatch, version: str
) -> None:
    monkeypatch.setattr(importlib.metadata, "version", lambda _package: version)
    ...
    with pytest.raises(
        ...
        match=(
            ...
            r"ProcessWorker workspace execution requires Prefect >=3\.7,<4"
        ),
    ):
        _workspace_runtime_bootstrap._validate_runtime()
...
def test_engine_bootstrap_runs_flow_engine_with_entrypoint(
    ...
    monkeypatch.setattr(importlib.metadata, "version", lambda _package: "3.7.0")
