...
class TestMakeFlowResolver:
    async def test_resolves_from_bundle_map(self, tmp_path: Path):
        ...
        result = await resolver(flow_run)
    ...
    async def test_cache_hit_skips_api_call(self, tmp_path: Path):
        ...
        first = await resolver(flow_run)
        second = await resolver(flow_run)

        assert first is expected_flow
        assert second is expected_flow
        assert load_flow.await_count == 1

    async def test_cache_is_closure_scoped(self, tmp_path: Path):
        deployment_id = uuid4()
        flow_run = _make_flow_run(deployment_id=deployment_id)
        flow_a = MagicMock()
        flow_b = MagicMock()
        ...
        result_a = await resolver_a(flow_run)
        result_b = await resolver_b(flow_run)
