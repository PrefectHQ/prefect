...
default_base_job_template = {
    ...
    "variables": {
        "description": (
            ...
            " It contains all the information necessary to execute the\nflow run as a"
            " Docker container.\n\nAttributes:\n    name: The name to give to created"
            " Docker containers.\n    command: The command executed in created Docker"
            " containers to kick off\n        flow run execution.\n    env: The"
            " environment variables to set in created Docker containers.\n    labels:"
            " The labels to set on created Docker containers.\n    image: The image"
            " reference of a container image to use for created jobs.\n        If not"
            " set, the latest Prefect image will be used.\n    image_pull_policy: The"
            " image pull policy to use when pulling images.\n    networks: Docker"
            " networks that created containers should be connected to.\n   "
            " network_mode: The network mode for the created containers (e.g. host,"
            ...
            " of created containers. Accepts a value\n        with a unit identifier"
            " (e.g. 100000b, 1000k, 128m, 1g.) If a value is\n        given without a"
            " unit, bytes are assumed.\n    memswap_limit: Total memory (memory +"
            " swap), -1 to disable swap. Should only be\n        set if `mem_limit` is"
            " also set. If `mem_limit` is set, this defaults to\n        allowing the"
            " container to use as much swap as memory. For example, if\n       "
            " `mem_limit` is 300m and `memswap_limit` is not set, containers can use\n "
            "       600m in total of memory and swap.\n    privileged: Give extended"
            " privileges to created containers."
        ),
        "type": "object",
        "properties": {
            "command": {
            ...
            "labels": {
                "title": "Labels",
                "description": (
                    "Labels applied to infrastructure created by the worker using this"
                    " job configuration."
                ),
                "type": "object",
                "additionalProperties": {"type": "string"},
            ...
            "name": {
                "title": "Name",
                "description": (
                    "Name given to infrastructure created by the worker using this job"
                    " configuration."
                ),
                "type": "string",
            },
            ...
            "mem_limit": {
                "title": "Memory Limit",
                "description": (
                    "Memory limit of created containers. Accepts a value with a unit"
                    " identifier (e.g. 100000b, 1000k, 128m, 1g.) If a value is given"
                    " without a unit, bytes are assumed."
                ),
                "type": "string",
            },
            "memswap_limit": {
                ...
                "description": (
                    ...
                    " set if `mem_limit` is also set. If `mem_limit` is set, this"
                    " defaults toallowing the container to use as much swap as memory."
                    " For example, if `mem_limit` is 300m and `memswap_limit` is not"
                    " set, containers can use 600m in total of memory and swap."
                ),
                "type": "string",
            },
            "privileged": {
...
base_job_template_with_defaults["variables"]["properties"]["networks"]["default"] = [
    "network1",
    "network2",
]
...
@pytest.mark.parametrize(
    ...
    [
        (DockerContainer(), default_base_job_template),
        (
            DockerContainer(
