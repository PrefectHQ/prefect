...
from prefect.tasks.airbyte.airbyte import AirbyteServerNotHealthyException, ConnectionNotFoundException, \
...
class TestAirbyte:
    ...
    def test_trigger_manual_sync_connection(self):
        airbyte_base_url = f"http://localhost:8000/api/v1"
        responses.add(responses.POST, airbyte_base_url + "/connections/sync/", json={"job": {"id": "1", "createdAt": "1234567890"}}, status=200)
        session = requests.Session()
        connection_id = "749c19dc-4f97-4f30-bb0f-126e53506960"
        task = AirbyteConnectionTask(connection_id)
        job_id, job_created_at = task.trigger_manual_sync_connection(session, airbyte_base_url, connection_id)
        assert job_id == "1"
    ...
    def test_trigger_manual_sync_connection_2(self):
        airbyte_base_url = f"http://localhost:8000/api/v1"
        responses.add(responses.POST, airbyte_base_url + "/connections/sync/", json={}, status=404)
        session = requests.Session()
        connection_id = "749c19dc-4f97-4f30-bb0f-126e53506960"
        task = AirbyteConnectionTask(connection_id)

        with pytest.raises(ConnectionNotFoundException):
            task.trigger_manual_sync_connection(session, airbyte_base_url, connection_id)

    @responses.activate
    def test_get_job_status(self):
        airbyte_base_url = f"http://localhost:8000/api/v1"
        responses.add(responses.POST, airbyte_base_url + "/jobs/get/", json={"job": {"status": "running",
                      "createdAt": "1234567890", "updatedAt": "1234567890"}}, status=200)
        session = requests.Session()
        connection_id = "749c19dc-4f97-4f30-bb0f-126e53506960"
        task = AirbyteConnectionTask(connection_id)
        job_id = "1"
        job_status, job_created_at, job_updated_at = task.get_job_status(session, airbyte_base_url, job_id)
    ...
    def test_get_job_status_2(self):
        airbyte_base_url = f"http://localhost:8000/api/v1"
        responses.add(responses.POST, airbyte_base_url + "/jobs/get/", json={}, status=404)
        session = requests.Session()
        connection_id = "749c19dc-4f97-4f30-bb0f-126e53506960"
        task = AirbyteConnectionTask(connection_id)
