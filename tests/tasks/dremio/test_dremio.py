...
class TestDremioFetch:
    def test_construction(self) -> None:
        task = DremioFetch(username="test", password="test", hostname="test")
        assert (task.username == "test") and (task.hostname == "test")
    ...
    def test_no_query_client_returns_proper_error(self) -> None:
        ...
        with pytest.raises(ValueError):
            task.run(username="test", password="test", hostname="test", tls=False)
