...
class TestCreateFlow:
    ...
    async def test_create_flow_returns_records_if_already_exists(self, database_session):
        # create a flow
        flow = await models.flows.create_flow(session=database_session, name="my-flow")
        assert flow.name == "my-flow"
        assert flow.id
        ...
        same_flow = await models.flows.create_flow(session=database_session, name="my-flow")
