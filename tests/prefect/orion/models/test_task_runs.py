...
class TestCreateTaskRun:
    async def test_create_task_run_succeeds(self, database_session):
        ...
        assert task_run.task_id == fake_task_run.task_id
        assert task_run.task_version == fake_task_run.task_version


class TestReadTaskRun:
    async def test_read_task_run(self, database_session):
...
class TestDeleteTaskRun:
    async def test_delete_task_run(self, database_session):
        # create a task run to delete
        fake_task_run = schemas.actions.TaskRunCreate(
            task_id=uuid4(), task_version="0.1"
        )
        task_run = await models.task_runs.create_task_run(
