...
def test_create_flow_run_with_input(patch_post, use_flow_id, use_extra_args):
    ...
    # Check the idempotency key which defaults to a UUID
    idempotency_key = variables["input"].pop("idempotency_key", None)
    expected_user_key = expected.pop("idempotency_key", None)
    assert idempotency_key is not None, "An idempotency key should always be provided"
    if expected_user_key is None:
        assert uuid.UUID(idempotency_key), "A UUID should be generated by the client"
    else:
        assert idempotency_key == expected_user_key
