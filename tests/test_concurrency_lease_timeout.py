...
async def test_concurrency_lease_survives_delay(monkeypatch):
    ...
    monkeypatch.setenv("PREFECT_SERVER_CONCURRENCY_LEASE_DURATION", "10")  # 10 seconds for renewals
    monkeypatch.setenv("PREFECT_SERVER_CONCURRENCY_INITIAL_LEASE_TIMEOUT", "10")  # 10 seconds for initial lease
...
async def test_concurrency_lease_timeout_with_short_duration(monkeypatch):
    ...
    monkeypatch.setenv("PREFECT_SERVER_CONCURRENCY_LEASE_DURATION", "1")  # 1 second for renewals - too short
    monkeypatch.setenv("PREFECT_SERVER_CONCURRENCY_INITIAL_LEASE_TIMEOUT", "1")  # 1 second for initial lease - too short
