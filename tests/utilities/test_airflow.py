...
def test_example_branch_operator():
    ...
    assert all([isinstance(s, Skipped) for t, s in follow_tasks if (branch.name not in t.name)])
    assert all([isinstance(s, Success) for t, s in follow_tasks if (branch.name in t.name)])
