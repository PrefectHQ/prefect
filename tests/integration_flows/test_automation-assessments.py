...
async def assess_compound_automation():
    ...
    async with create_or_replace_automation(
        ...
        async with get_events_client() as events:
            await events.emit(
                Event(event="integration.example.event.A", resource=expected_resource)
            )
            await events.emit(
                Event(event="integration.example.event.B", resource=expected_resource)
            )
        try:
            with anyio.fail_after(60):
                await listener
...
def test_automation_assessments():
    """Test for automation-assessments."""
