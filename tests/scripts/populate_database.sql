...
AS (
        VALUES ('SCHEDULED'::statetype, 'Scheduled', CURRENT_TIMESTAMP - INTERVAL '1 HOUR', ('{"scheduled_time":"' || CURRENT_TIMESTAMP - INTERVAL '1 HOUR' || '"}')::jsonb), ('PENDING', 'Pending', CURRENT_TIMESTAMP - INTERVAL '59 minutes', '{}'), ('RUNNING', 'Running', CURRENT_TIMESTAMP - INTERVAL '58 minutes', '{}'), ('COMPLETED', 'Completed', CURRENT_TIMESTAMP - INTERVAL '5 minutes', '{}'))
        SELECT
            flow_run.id,
            states.*
        FROM
...
WITH states (TYPE, name, timestamp, state_details)
AS (
        VALUES ('SCHEDULED'::statetype, 'Scheduled', CURRENT_TIMESTAMP - INTERVAL '1 HOUR', ('{"scheduled_time":"' || CURRENT_TIMESTAMP + INTERVAL '1 HOUR' || '"}')::jsonb))
        SELECT
            flow_run.id,
            states.*
        FROM
