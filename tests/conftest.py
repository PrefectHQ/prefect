...
def pytest_benchmark_compare_machine_info(
    config, benchmarksession, machine_info, compared_benchmark
):
    """
    Custom hook to handle machine info comparison for pytest-benchmark.

    This prevents INTERNAL ERRORs in CI environments where different runners
    may have different CPU frequencies, which would otherwise cause benchmark
    comparisons to fail with machine info warnings that escalate to internal errors.

    We ignore CPU frequency differences (hz_actual, hz_actual_friendly) since
    these can vary between CI runners but don't affect benchmark comparisons.
    """

    def deep_copy_without_cpu_hz(obj):
        """Recursively copy an object, excluding CPU frequency fields."""
        if isinstance(obj, dict):
            result = {}
            for key, value in obj.items():
                if key in ("hz_actual", "hz_actual_friendly"):
                    # Skip CPU frequency fields that vary between CI runners
                    continue
                result[key] = deep_copy_without_cpu_hz(value)
            return result
        elif isinstance(obj, (list, tuple)):
            return type(obj)(deep_copy_without_cpu_hz(item) for item in obj)
        else:
            return obj

    # Create copies of machine info without CPU frequency fields
    current_info = deep_copy_without_cpu_hz(machine_info)
    saved_info = deep_copy_without_cpu_hz(compared_benchmark["machine_info"])

    # Only warn if there are meaningful differences (not just CPU frequency)
    if current_info != saved_info:
        benchmarksession.logger.warning(
            "Benchmark machine_info is different (excluding CPU frequency). "
            f"Current: {current_info} VS saved: {saved_info}."
        )
