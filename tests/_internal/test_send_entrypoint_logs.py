...
class TestSend:
    async def test_creates_error_log_with_flow_run_id(self):
        ...
        with patch(
            ...
            await _send("test message", flow_run_id)
    ...
    async def test_works_without_flow_run_id(self):
        mock_client = AsyncMock()
...
class TestMain:
    def test_reads_from_file(self, tmp_path):
        ...
        with (
            ...
            patch("prefect._internal.send_entrypoint_logs.asyncio.run") as mock_run,
        ):
            main()
    ...
    def test_silently_swallows_exceptions(self, tmp_path):
        ...
        with (
            patch("sys.argv", ["send_entrypoint_logs", str(log_file)]),
            patch(
                "prefect._internal.send_entrypoint_logs.asyncio.run",
