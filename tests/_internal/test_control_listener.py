...
class TestControlListener:
    ...
    async def test_intent_set_on_cancel_byte(
        ...
        try:
            monkeypatch.setattr(
                control_listener, "_can_ack_control_intent", lambda: True
            )
            control_listener.start()
    ...
    def test_acknowledge_intent_windows_acks_before_interrupt(
        ...
        monkeypatch.setattr(control_listener, "_can_ack_control_intent", lambda: True)
        monkeypatch.setattr(
    ...
    async def test_listener_does_not_ack_when_bridge_is_unavailable(
        ...
        monkeypatch.setattr(control_listener, "_can_ack_control_intent", lambda: False)
        control_listener.start()
