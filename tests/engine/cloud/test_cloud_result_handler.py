...
from prefect.engine.cloud import CloudResultSerializer
...
class TestCloudHandler:
    def test_cloud_handler_initializes_with_no_args(self):
        ...
        assert handler.result_serializer_service is None
    ...
    def test_cloud_handler_pulls_settings_from_config_after_first_method_call(
        ...
        monkeypatch.setattr(
            "prefect.engine.cloud.result_serializer.Client",
            MagicMock(return_value=client),
        )
        with set_temporary_config({"cloud.result_serializer": "http://foo.bar:4204"}):
            handler = CloudResultSerializer()
            handler.serialize("random string")
        assert handler.result_serializer_service == "http://foo.bar:4204"
