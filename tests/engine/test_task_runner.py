...
class TestTaskRunner_get_pre_run_state:
    ...
    def test_returns_running_if_cached_state_with_expired_cache(self):
        runner = TaskRunner(SuccessTask(cache_validator=duration_only, cache_for=datetime.timedelta(days=1)))
    ...
    def test_returns_successful_if_cached_state_is_validated(self, validator):
        runner = TaskRunner(SuccessTask(cache_validator=validator, cache_for=datetime.timedelta(days=1)))
        expiration = datetime.datetime.utcnow() + datetime.timedelta(days=1)
        inputs = dict(x=2, y=1)
