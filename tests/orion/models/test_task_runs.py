...
class TestCreateTaskRun:
    ...
    async def test_create_task_run_with_state(self, flow_run, session, db_interface):
        state_id = uuid4()
        task_run = await models.task_runs.create_task_run(
            session=session,
            task_run=schemas.core.TaskRun(
        ...
        query = await session.execute(
            sa.select(db_interface.TaskRunState).filter_by(id=state_id)
        )
        result = query.scalar()
    ...
    async def test_create_task_run_with_state_and_dynamic_key(
        ...
        query = await session.execute(
            sa.select(db_interface.TaskRunState).filter_by(id=scheduled_state_id)
        )
        result = query.scalar()
