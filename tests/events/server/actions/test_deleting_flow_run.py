...
def patch_orchestration_delete(monkeypatch, session: AsyncSession):
    """
    Bypasses the test server's missing `docket` dependency on the DELETE route
    by mocking the client call to directly delete the flow run in the test database.
    """
    from httpx import Response

    from prefect.server.api.clients import OrchestrationClient

    async def mock_delete_flow_run(client_instance, flow_run_id):
        await flow_runs.delete_flow_run(session, flow_run_id)
        await session.commit()
        return Response(status_code=204)

    monkeypatch.setattr(OrchestrationClient, "delete_flow_run", mock_delete_flow_run)
...
async def test_deleting_long_exposure(
    ...
    super_long_exposure: FlowRun,
    session: AsyncSession,
    asserting_events_worker: AssertingEventsClient,
    client,
):
...
async def test_deleting_weird_exposure(
...
async def test_success_event(
    ...
    assert delete_that_weird_exposure.triggering_event is not None
