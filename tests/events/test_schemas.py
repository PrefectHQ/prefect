...
from prefect.events.schemas import RelatedResource, Resource
...
@pytest.mark.parametrize("resource_class", [Resource, RelatedResource])
def test_resources_support_getting(resource_class: Type[Resource]) -> None:
    resource = resource_class(
        __root__={
            "prefect.resource.id": "my.unique.resource",
            "prefect.resource.role": "any-role",
    ...
    assert resource.get("this.thing") == "hello"
    assert resource.get("nope") is None
    assert resource.get("nope", None) is None
    assert resource.get("nope", "42") == "42"
