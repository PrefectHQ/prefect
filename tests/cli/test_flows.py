...
def run_with_registry(cli_command):
    """
...
def test_flows_keys():
    ...
    output = json.loads(run_with_registry("prefect flows keys").decode())
    keys = ["project", "name", "version"]
    assert output == [dict(zip(keys, f.id())) for f in [f1, f2, f3]]
...
def test_flows_runs():
    ...
    with tempfile.NamedTemporaryFile() as tmp:
        ...
        run_with_registry("prefect flows run Flows Flow 1")
