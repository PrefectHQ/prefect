...
def test_orion_status_healthy():
    invoke_and_assert(["orion", "status"], expected_output_contains="Orion Healthy")
...
def test_orion_status_unhealthy(monkeypatch):
    ...
    invoke_and_assert(
        ["orion", "status"], expected_output_contains="All connection attempts failed"
    )
...
def test_orion_status_wait_arg(monkeypatch):
    ...
    invoke_and_assert(
        ["orion", "status", "--wait"], expected_output_contains="Orion Healthy"
    )
...
def test_orion_status_timeout(monkeypatch):
    ...
    invoke_and_assert(
        ...
        expected_output_contains="Timeout exceeded, All connection attempts failed",
    )
