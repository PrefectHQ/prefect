...
def test_login_with_invalid_key(key, expected_output):
    with respx.mock(
...
def test_login_with_prefect_api_key_env_var_equal_to_invalid_key_exits_with_error(
    key, expected_output, env_var_api_key
):
    with respx.mock(
        base_url=PREFECT_CLOUD_API_URL.value(), using="httpx"
    ) as respx_mock:
...
def test_login_with_key_and_missing_workspace():
    foo_workspace = gen_test_workspace(account_handle="test", workspace_handle="foo")
...
def test_login_with_key_and_workspace_with_no_workspaces():
    with respx.mock(
        base_url=PREFECT_CLOUD_API_URL.value(), using="httpx"
    ) as respx_mock:
        respx_mock.get("/me/workspaces").mock(
...
def test_login_with_key_and_workspace():
    foo_workspace = gen_test_workspace(account_handle="test", workspace_handle="foo")
    bar_workspace = gen_test_workspace(account_handle="test", workspace_handle="bar")
...
def test_login_with_key_and_no_workspaces():
    with respx.mock(
        base_url=PREFECT_CLOUD_API_URL.value(), using="httpx"
    ) as respx_mock:
        respx_mock.get("/me/workspaces").mock(
...
@pytest.mark.usefixtures("interactive_console")
def test_login_with_key_and_select_first_workspace():
    foo_workspace = gen_test_workspace(account_handle="test", workspace_handle="foo")
    bar_workspace = gen_test_workspace(account_handle="test", workspace_handle="bar")
...
@pytest.mark.usefixtures("interactive_console")
def test_login_with_key_and_select_second_workspace():
    foo_workspace = gen_test_workspace(account_handle="test", workspace_handle="foo")
    bar_workspace = gen_test_workspace(account_handle="test", workspace_handle="bar")
...
@pytest.mark.usefixtures("interactive_console")
def test_login_with_interactive_key_single_workspace():
    foo_workspace = gen_test_workspace(account_handle="test", workspace_handle="foo")
...
@pytest.mark.usefixtures("interactive_console")
def test_login_with_interactive_key_multiple_workspaces():
    foo_workspace = gen_test_workspace(account_handle="test", workspace_handle="foo")
    bar_workspace = gen_test_workspace(account_handle="test", workspace_handle="bar")
...
@pytest.mark.xfail(reason="This test needs to be rewritten for new respx versions")
@pytest.mark.usefixtures("interactive_console")
...
@pytest.mark.usefixtures("interactive_console")
def test_login_already_logged_in_to_another_profile():
    foo_workspace = gen_test_workspace(account_handle="test", workspace_handle="foo")
...
@pytest.mark.usefixtures("interactive_console")
def test_set_workspace_with_less_than_10_workspaces():
    foo_workspace = gen_test_workspace(account_handle="test1", workspace_handle="foo")
    bar_workspace = gen_test_workspace(account_handle="test2", workspace_handle="bar")
...
def test_list_webhooks():
    ...
    with respx.mock(base_url=foo_workspace.api_url(), using="httpx") as respx_mock:
        respx_mock.post("/webhooks/filter").mock(
            return_value=httpx.Response(
                status.HTTP_200_OK,
...
def test_rotate_webhook():
    ...
    with respx.mock(base_url=foo_workspace.api_url(), using="httpx") as respx_mock:
        respx_mock.post(f"/webhooks/{webhook_id}/rotate").mock(
            return_value=httpx.Response(
                status.HTTP_200_OK,
...
def test_toggle_webhook():
    ...
    webhook_id = str(uuid.uuid4())
    with respx.mock(base_url=foo_workspace.api_url(), using="httpx") as respx_mock:
        respx_mock.get(f"/webhooks/{webhook_id}").mock(
            return_value=httpx.Response(
                status.HTTP_200_OK,
                json={"enabled": True},
            )
        ...
        respx_mock.patch(f"/webhooks/{webhook_id}").mock(
            return_value=httpx.Response(
...
def test_update_webhook():
    ...
    with respx.mock(base_url=foo_workspace.api_url(), using="httpx") as respx_mock:
        ...
        respx_mock.put(f"/webhooks/{webhook_id}", json=request_body).mock(
            return_value=httpx.Response(
                status.HTTP_204_NO_CONTENT,
            )
        )
...
def test_delete_webhook():
    ...
    with respx.mock(base_url=foo_workspace.api_url(), using="httpx") as respx_mock:
        respx_mock.delete(f"/webhooks/{webhook_id}").mock(
            return_value=httpx.Response(
                status.HTTP_204_NO_CONTENT,
            )
        )
...
def test_open_current_workspace_in_browser_success(mock_webbrowser):
    foo_workspace = gen_test_workspace(account_handle="test", workspace_handle="foo")
