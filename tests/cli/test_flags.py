...
def disabled_flag():
    with temporary_settings(updates={PREFECT_FEATURE_FLAGGING_ENABLED: "true"}):
        yield feature_flags.create_if_missing(ENABLE_NEW_PROFILES, is_enabled=False)


@pytest.fixture
def enabled_flag():
    with temporary_settings(updates={PREFECT_FEATURE_FLAGGING_ENABLED: "true"}):
        yield feature_flags.create_if_missing(ENABLE_FANCY_DEPLOYMENTS, is_enabled=True)
