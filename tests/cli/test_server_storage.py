...
async def test_server_storage_cli_e2e(
    ...
    await run_sync_in_worker_thread(
        invoke_and_assert,
        ["server", "storage", "inspect"],
        expected_code=0,
        expected_output_contains="No server default result storage is configured.",
    )

    await run_sync_in_worker_thread(
        invoke_and_assert,
        ["server", "storage", "inspect", "--output", "yaml"],
        expected_code=1,
    ...
    await run_sync_in_worker_thread(
        invoke_and_assert,
        ["server", "storage", "set", "server-default-results"],
        expected_code=1,
    ...
    try:
        await run_sync_in_worker_thread(
            invoke_and_assert,
            ["server", "storage", "set", f"local-file-system/{block_name}"],
            expected_code=0,
            expected_output_contains="Set server default result storage.",
        )
        ...
        result = await run_sync_in_worker_thread(
            invoke_and_assert,
            ["server", "storage", "inspect", "--output", "json"],
            expected_code=0,
        )
        ...
        await run_sync_in_worker_thread(
            ...
            expected_code=0,
            expected_output_contains="Cleared server default result storage.",
        )
        ...
        await run_sync_in_worker_thread(
            invoke_and_assert,
            ["server", "storage", "set", "--id", str(block_document_id)],
            expected_code=0,
            expected_output_contains="Set server default result storage.",
        )
