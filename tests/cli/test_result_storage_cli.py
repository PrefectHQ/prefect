...
async def test_result_storage_cli_e2e(
    ...
    await run_sync_in_worker_thread(
        invoke_and_assert,
        ["experimental", "result-storage", "inspect"],
        expected_code=0,
    ...
    await run_sync_in_worker_thread(
        invoke_and_assert,
        ["experimental", "result-storage", "inspect", "--output", "yaml"],
        expected_code=1,
    ...
    await run_sync_in_worker_thread(
        invoke_and_assert,
        ["experimental", "result-storage", "set", "server-default-results"],
        expected_code=1,
    ...
    try:
        ...
        result = await run_sync_in_worker_thread(
            invoke_and_assert,
            ["experimental", "result-storage", "inspect", "--output", "json"],
            expected_code=0,
        )
        ...
        await run_sync_in_worker_thread(
            invoke_and_assert,
            ["experimental", "result-storage", "clear"],
            expected_code=0,
        ...
        await run_sync_in_worker_thread(
            invoke_and_assert,
            ["experimental", "result-storage", "set", "--id", str(block_document_id)],
            expected_code=0,
            expected_output_contains="Set default result storage.",
        )
