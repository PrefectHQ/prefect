...
def test_not_all_skipped_with_some_skipped_and_failed():
    assert triggers.not_all_skipped(generate_states(failed=1, skipped=3)) is True


def test_not_all_skipped_with_some_skipped():
    assert triggers.not_all_skipped(generate_states(success=1, skipped=3)) is True


def test_not_all_skipped_with_all_successful():
    assert triggers.not_all_skipped(generate_states(success=3)) is True


def test_not_all_skipped_with_all_skipped():
    with pytest.raises(signals.SKIP):
        triggers.not_all_skipped(generate_states(skipped=3))
