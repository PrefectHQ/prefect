...
from prefect.concurrency.asyncio import acquire_concurrency_slots
...
async def test_calls_increment_client_method():
    ...
    with mock.patch(
        ...
        await acquire_concurrency_slots(
...
async def test_returns_minimal_concurrency_limit():
    ...
    with mock.patch(
        ...
        result = await acquire_concurrency_slots(["test-1", "test-2"], 1)
