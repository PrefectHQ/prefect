---
title: Logging
description: Prefect enables you to log a variety of useful information about your flow and task runs, capturing information about your workflows for purposes such as monitoring, troubleshooting, and auditing.
---
...
You can view and filter logs in the [Prefect UI](https://docs.prefect.io/ui/flow-runs/#inspect-a-flow-run) or Prefect Cloud, or access log records via the API.
...
You can see logs for a flow run in the Prefect UI by navigating to the [**Flow Runs**](https://docs.prefect.io/ui/flow-runs/#inspect-a-flow-run) page and selecting a specific flow run to inspect.

![Viewing logs for a flow run in the Prefect UI](/images/logging1.png)
...
Prefect provides several settings for configuring [logging level and loggers](https://docs.prefect.io/concepts/logs/).
...
You can override any logging configuration by setting an environment variable or [Prefect Profile](https://docs.prefect.io/concepts/settings/) setting using the syntax `PREFECT_LOGGING_[PATH]_[TO]_[KEY]`, with `[PATH]_[TO]_[KEY]` corresponding to the nested address of any setting.
...
By default, tasks and nested flows will inherit the `log_prints` setting from their parent flow, unless opted out with their own explicit `log_prints` setting.
...
You can also configure this behavior globally for all Prefect flows, tasks, and nested flows.
...
1.  Update `/.prefect/logging.yml` to use `my_package_or_module.CustomConsoleHandler` and additionally reference the base\_style and named expression: `log.email`.
...
For more information, see [Log schema](https://docs.prefect.io/api-ref/server/schemas/core/#prefect.server.schemas.core.Log) in the API documentation.
...
You can set this setting as an environment variable or in a profile. See [Settings](https://docs.prefect.io/concepts/settings/) for more details about how to use settings.