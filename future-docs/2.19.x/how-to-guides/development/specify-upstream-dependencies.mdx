...
Results from a task can be provided to other tasks or flows as upstream dependencies. Prefect uses upstream dependencies in two ways:

1.  To populate dependency arrows in the flow run graph
2.  To determine execution order for [concurrently submitted](https://docs.prefect.io/concepts/task-runners) units of work that depend on each other

<Note>
**Tasks vs. other functions**

**Only results from tasks** inform Prefect's ability to determine dependencies. Return values from functions without task decorators, including nested flow runs, do not carry the same information about their origin as task results.
</Note>
...
A run's upstream dependencies can be inferred automatically via its inputs, or stated explicitly via the `wait_for` parameter.
...
This applies to every way you can run tasks with Prefect, whether you're calling the task function directly, calling [`.submit()`](https://docs.prefect.io/api-ref/prefect/tasks/#prefect.tasks.Task.submit), or calling [`.map()`](https://docs.prefect.io/api-ref/prefect/tasks/#prefect.tasks.Task.map). 
Nested flow runs also recognize also task results as upstream dependencies.
...
<Frame caption="Flow run graph displaying manual dependencies with the 'Dependency grid' layout selected">![Flow run graph for manual task dependencies](/images/upstream2.png)</Frame>


Deployments as dependencies
-----------------------------------------------------------------------------

For more complex workflows, parts of your logic may require additional resources, different infrastructure, or independent parallel execution. A typical approach for addressing these needs is to execute that logic as separate [deployment](https://docs.prefect.io/concepts/deployments) runs from within a flow.

Composing deployment runs into a flow so that they can be treated as upstream dependencies is as simple as calling [`run_deployment`](https://docs.prefect.io/api-ref/prefect/deployments/deployments/#prefect.deployments.deployments.run_deployment) from within a task.
...
By default, deployments started from `run_deployment` will also appear as nested flow runs for tracking purposes. 
This behavior can be disabled by setting the `as_subflow` parameter for `run_deployment` to `False`.

<Frame caption="Flow run graph displaying deployments as dependencies with the 'Dependency grid' layout selected">![Flow of deployments](/images/upstream3.png)</Frame>
