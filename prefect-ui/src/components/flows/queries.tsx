...
import { QueryFunction, QueryKey,QueryObserverOptions, MutationFunction } from '@tanstack/react-query';
import { components } from '@/api/prefect';
import { format } from 'date-fns';

export const flowQueryParams = (flowId: string, queryParams: Partial<QueryObserverOptions> = {}): {
  queryKey: QueryKey;
...
export const getLatestFlowRunsQueryParams = (
    ...
    ...queryParams,
  queryKey: ['flowRun', JSON.stringify({ 'flowId': id, 'offset': 0, 'limit': n, 'sort': 'START_TIME_DESC' })] as const,
  queryFn: async () => {
    const response = await QueryService.POST('/flow_runs/filter', {
      body: {
...
export const getNextFlowRunsQueryParams = (
    ...
    ...queryParams,
  queryKey: ['flowRun', JSON.stringify({ 'flowId': id, 'offset': 0, 'limit': n, 'sort': 'EXPECTED_START_TIME_ASC' })] as const,
  queryFn: async () => {
    const response = await QueryService.POST('/flow_runs/filter', {
      body: {
        flows: { operator: 'and_' as const, id: { any_: [id] } },
        flow_runs: { operator: 'and_' as const, 'expected_start_time': { 'after_': format(new Date(), "yyyy-MM-dd'T'HH:mm:ss'Z'") } },
        offset: 0,
        limit: n,
  ...
  queryFn: async () => {
    const response = await QueryService.POST('/flow_runs/count', {
      body: {
        ...body,
        flows: { ...body?.flows, operator: 'and_' as const, id: { any_: [id] } },
        flow_runs: { operator: 'and_' as const, 'expected_start_time': { 'before_': format(new Date(), "yyyy-MM-dd'T'HH:mm:ss'Z'")}, ...body?.flow_runs }
      },
    }).then((response) => response.data);
...
export class FlowQuery {
  ...
  public getLatestFlowRunsQueryParams(
    ...
    queryFn: () => Promise<components['schemas']['FlowRunResponse'][]>;
        ...
        return flowRunsQueryParams(this.flowId, { offset: 0, limit: n, sort: 'START_TIME_DESC' }, queryParams);
    }
  ...
  public getDeploymentsCountQueryParams(queryParams: Partial<QueryObserverOptions> = {}): {
    queryKey: readonly ['deploymentsCount', string];
    queryFn: () => Promise<number>;
  } {
