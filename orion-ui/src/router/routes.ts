...
export const routes = {
  ...
  queue: (id: string) => ({ name: 'work-queue', params: { id } }) as const,
  queueEdit: (id: string) => ({ name: 'edit-work-queue', params: { id } }) as const,
  queueCreate: () => ({ name: 'create-work-queue' }) as const,
  queues: () => ({ name: 'work-queues' }) as const,
