  ...
  >
    ...
    <div ref="scrollContainer" class="scroll-container" @scroll="handleScroll">
      <div class="node-container" :style="nodeContainerStyle">
        ...
        >
          <svg :id="`${id }-axis`" ref="chart-axis" />
        </div>

        <svg :id="id" class="timeline-background" />
      </div>
    </div>
  ...
  export default class Timeline extends mixins(D3Base).with(Props) {
    ...
    get interval(): string {
      return Object.entries(intervals)
        .sort(([, a], [, b]) => b - a)
        .reduce(([key, val], [_key, _val]) => this.totalSeconds < _val ? [_key, _val] : [key, val],
        )[0]
    }
    ...
    get end(): Date {
      return new Date(
        this.maxEndTime
          ...
          : new Date(
            Math.max(
              ...this.sortedItems.map((item) => item.end_time ? new Date(item.end_time).getTime() : Date.now(),
              ),
            ),
          ).getTime(),
      )
    ...
    updated(): void {
      if (!this.svg || !this.gridSelection) {
        this.createChart()
      }
    }
