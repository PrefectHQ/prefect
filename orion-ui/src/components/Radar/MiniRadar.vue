...
<script lang="ts" setup>
  ...
  const visibleNodes = computed<RadarNodes>(() => {
    const collapsed = [...props.collapsedTrees?.entries() || new Map().entries()]
    return new Map(
  ...
  const updateRings = (): void => {
    ...
    ringContainer.value
      ...
      .join(
        // enter
        (selection: any) => selection.append('g').attr('class', 'radar__arc-segment-group'),
        // update
  ...
  const dragViewport = (e: MouseEvent): void => {
    if (!dragging.value || props.disableInteractions) {
      return
    }
