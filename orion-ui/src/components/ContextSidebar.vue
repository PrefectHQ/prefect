<template>
  <p-context-sidebar class="context-sidebar">
    ...
    <p-context-nav-item v-if="canSeeWorkers" title="Workers" icon="DatabaseIcon" :to="routes.workerPools()" />
...
<script lang="ts" setup>
  ...
  const can = useCan()
  const canSeeWorkers = computed(() => can.access.workers && can.read.worker_pool)
</script>