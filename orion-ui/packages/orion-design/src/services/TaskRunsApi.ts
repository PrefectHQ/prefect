...
export class TaskRunsApi extends Api {
  ...
  public getTaskRun(id: string): Promise<TaskRun> {
    return this.get(`/${id}`).then(taskRunResponseMapper)
  }
...
function taskRunMapper(taskRun: ITaskRunResponse): TaskRun {
  return new TaskRun({
    id: taskRun.id,
...
function taskRunResponseMapper({ data }: AxiosResponse<ITaskRunResponse>): TaskRun {
  return taskRunMapper(data)
}

function taskRunsResponseMapper({ data }: AxiosResponse<ITaskRunResponse[]>): TaskRun[] {
  return data.map(task => taskRunMapper(task))
}
