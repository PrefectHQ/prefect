...
Flows can include calls to [tasks](/2.18/concepts/tasks/) as well as to other flows, which Prefect calls ["subflows"](#composing-flows) in this context.
Flows may be defined within modules and imported for use as subflows in your flow definitions.

[Deployments](/2.18/concepts/deployments/) elevate individual workflows from functions that you call manually to API-managed entities.
...
- Creating a [deployment](/2.18/concepts/deployments/) on Prefect Cloud or a locally run Prefect server.
...
![Prefect UI](/2.18/img/ui/timeline-flows.png)
...
| `task_runner`                                      | An optional [task runner](/2.18/concepts/task-runners/) to use for task execution within the flow when you `.submit()` tasks. If not provided and you `.submit()` tasks, the `ConcurrentTaskRunner` will be used.         |
...
![A simple flow visualized with the .visualize() method](/2.18/img/orchestration/hello-flow-viz.png)
...
![A flow with return values visualized with the .visualize() method](/2.18/img/orchestration/viz-return-value-tracked.png)
...
When a subflow starts, it will create a new [task runner](/2.18/concepts/task-runners/) for tasks within the subflow.
...
Whether running locally or via a [deployment](/2.18/concepts/deployments/), you must indicate which flow is the entrypoint for a flow run.
...
**Cancelling subflow runs**
    ...
    If you may need to cancel a subflow run independent of its parent flow run, we recommend deploying it separately and starting it using the [run_deployment](/2.18/api-ref/prefect/deployments/deployments/#prefect.deployments.deployments.run_deployment) function.
...
Note that parameter values can be provided to a flow via API using a [deployment](/2.18/concepts/deployments/).
...
**Prerequisite**
    ...
    Read the documentation about [states](/2.18/concepts/states) before proceeding with this section.
...
```python pythonfrom prefect import task, flow
...
The simplest way to create a [deployment](/2.18/concepts/deployments/) for your flow is by calling its [`serve` method](/2.18/api-ref/prefect/flows/#prefect.flows.Flow.serve).
...
You can take this further and serve multiple flows with the same process using the [`serve`](/2.18/api-ref/prefect/runner/#prefect.runner.serve) utility along with the `to_deployment` method of flows:
...
Flows can be retrieved from remote storage using the [`flow.from_source`](/2.18/api-ref/prefect/flows/#prefect.flows.Flow.from_source) method.
...
If you need additional configuration, such as specifying a private repository, you can provide a [`GitRepository`](/2.18/api-ref/prefect/flows/#prefect.runner.storage.GitRepository) instead of URL:
...
Prefect exposes this functionality via the [`pause_flow_run`](/2.18/api-ref/prefect/engine/#prefect.engine.pause_flow_run) and [`resume_flow_run`](/2.18/api-ref/prefect/engine/#prefect.engine.resume_flow_run) functions.
...
**The difference between pausing and suspending a flow run**
    ...
    However, when the flow run resumes, the flow code will execute again from the beginning of the flow, so you should use [tasks](/2.18/concepts/tasks/) and [task caching](/2.18/concepts/tasks/#caching) to avoid recomputing expensive operations.

</Note>

Prefect exposes this functionality via the [`suspend_flow_run`](/2.18/api-ref/prefect/engine/#prefect.engine.suspend_flow_run) and [`resume_flow_run`](/2.18/api-ref/prefect/engine/#prefect.engine.resume_flow_run) functions, as well as the Prefect UI.
...
For more in-depth information on receiving input from users when pausing and suspending flow runs, see the [Creating interactive workflows](/2.18/guides/creating-interactive-workflows/) guide.
...
![Prefect UI](/2.18/img/ui/flow-run-cancellation-ui.png)
