...
1. Use a [task runner](/2.18/concepts/task-runners/) for parallelizable operations to save time.
...
Learn more about blocks [here](/2.18/concepts/blocks/).
...
The [moving data guide](/2.18/guides/moving-data/) has step-by-step examples for each cloud provider.
...
Caching is discussed in detail in the [tasks concept page](/2.18/concepts/tasks.md/#caching).
...
Read about [compressing results with Prefect](/2.18/concepts/results/) for more details.
...
See the [guide to working with Dask and Ray Task Runners](/2.18/guides/dask-ray-task-runners/) for details.
