...
Prefect Cloud can also schedule flow runs through event-driven [automations](/2.17/concepts/automations/).

Schedules tell the Prefect API how to create new flow runs for you automatically on a specified cadence.

You can add a schedule to any [deployment](/2.17/concepts/deployments/).
...
!!! tip "Support for multiple schedules"
    We are currently rolling out support for multiple schedules per deployment. You can now assign multiple schedules to deployments in the Prefect UI, the CLI via `prefect deployment schedule` commands, the `Deployment` class, and in [block-based deployment](/2.17/concepts/deployments/#block-based-deployments) YAML files.

    Support for multiple schedules in `flow.serve`, `flow.deploy`, `serve`, and [worker-based deployments](/2.17/concepts/work-pools/) with `prefect deploy` will arrive soon.
...
An `Interval` schedule creates new flow runs on a regular interval measured in seconds. Intervals are computed using an optional `anchor_date`. For example, here's how you can create a schedule for every 10 minutes in a [block-based deployment](/2.17/concepts/deployments/#block-based-deployments) YAML file:
...
For example, the following RRule schedule in a [block-based deployment](/2.17/concepts/deployments/#block-based-deployments) YAML file creates flow runs on Monday, Wednesday, and Friday until July 30, 2024.
...
- Via the `cron`, `interval`, or `rrule` parameters if building your deployment via the [`serve` method](/2.17/concepts/flows/#serving-a-flow) of the `Flow` object or [the `serve` utility](/2.17/concepts/flows/#serving-multiple-flows-at-once) for managing multiple flows simultaneously
- If using [worker-based deployments](/2.17/concepts/work-pools/)
...
- If using [block-based deployments](/2.17/concepts/deployments/#block-based-deployments) - Deprecated 
...
![Prefect UI with Interval button selected](/2.17/img/ui/interval-schedule.png)
...
If you are using [worker-based deployments](/2.17/concepts/work-pools/), you can create a schedule through the interactive `prefect deploy` command.
...
See the [Settings docs](/2.17/concepts/settings/) for more information on altering your settings.
