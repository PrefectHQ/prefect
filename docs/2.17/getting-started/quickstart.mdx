...
See the [install guide](/2.17/getting-started/installation/) for more detailed installation instructions, if needed.
...
1. Use the `prefect cloud login` CLI command to [log in to Prefect Cloud](/2.17/cloud/users/api-keys) from your environment.
...
!!! note "Self-hosted Prefect server instance"
    If you would like to host a Prefect server instance on your own infrastructure, see the [tutorial](/2.17/tutorial/) and select the "Self-hosted" tab.
...
[Flows](/2.17/concepts/flows/) are the core observable, deployable units in Prefect and are the primary entrypoint to orchestrated work.
...
We can tell Prefect where we want to run our workflow by creating a [work pool](/2.17/concepts/work-pools/).

We can have Prefect Cloud run our flow code for us with a Prefect Managed work pool.

Let's create a [Prefect Managed work pool](/2.17/guides/managed-execution/) so that Prefect can run our flows for us.
...
Let's package both of these things, along with the location for where to find our flow code, into a [deployment](/2.17/concepts/deployments/) so that we can schedule our workflow to run remotely.
...
!!! note "Code storage options"
    ...
    Read more [here](/2.17/guides/prefect-deploy/#creating-work-pool-based-deployments).
...
![Managed flow run graph and logs](/2.17/img/ui/qs-flow-run.png)
...
To learn how to run flows on your own infrastructure, see how to customize the Docker image where your flow runs, and see how to gain lots of orchestration and observation benefits check out the [tutorial](/2.17/tutorial/).
