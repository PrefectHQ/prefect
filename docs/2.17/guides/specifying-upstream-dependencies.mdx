...
2. To determine execution order for [concurrently submitted](/2.17/concepts/task-runners) units of work that depend on each other
...
<figure markdown="span">
  ![Flow run graph for automatic task dependencies](/2.17/img/guides/automatic-task-dependencies.png)
...
<figure markdown="span">
  ![Flow run graph for manual task dependencies](/2.17/img/guides/manual-task-dependencies.png)
...
For more complex workflows, parts of your logic may require additional resources, different infrastructure, or independent parallel execution. A typical approach for addressing these needs is to execute that logic as separate [deployment](/2.17/concepts/deployments) runs from within a flow.

Composing deployment runs into a flow so that they can be treated as upstream dependencies is as simple as calling [`run_deployment`](/2.17/api-ref/prefect/deployments/deployments/#prefect.deployments.deployments.run_deployment) from within a task.
...
<figure markdown="span">
  ![Flow of deployments](/2.17/img/guides/flow-of-deployments.png)
