...
<Warning>
If you're using Python, it is recommended to use `pip` or [`uv`](https://docs.astral.sh/uv/) to install and use the `prefect` SDK's client methods to interact with the API.
...
```
Don't panic - run a server in the background 🙂
</Accordion>
...
print(f"🔑 Connecting to {settings.api_url}")
client = httpx.Client(
...
class PrefectSettings(BaseSettings):
    ...
    # if these are not set, we'll use localhost
    PREFECT_API_KEY: Optional[str] = Field(default=None)
    PREFECT_ACCOUNT_ID: Optional[str] = Field(default=None)
    PREFECT_WORKSPACE_ID: Optional[str] = Field(default=None)
...
if __name__ == "__main__":
    ...
    print("🏗️  Creating AWS credentials block...")
    aws_creds = get_or_create_block(
        client,
        name="demo-aws-creds",
        block_type_id=client.get("/block_types/slug/aws-credentials")
    ...
    print("📦 Creating S3 bucket block...")
    bucket_block = get_or_create_block(
        client,
        name="demo-s3-bucket",
        block_type_id=client.get("/block_types/slug/s3-bucket")
    ...
    print(f"🎉 All done! check out your block with `uvx prefect block inspect s3-bucket/{bucket_block['name']}`")
```

```bash output
» pbpaste | uv run -

🔑 Connecting to http://127.0.0.1:4200/api
🏗️  Creating AWS credentials block...
📦 Creating S3 bucket block...
✅ Verifying block structure...
🎉 All done! check out your block with `uvx prefect block inspect s3-bucket/demo-s3-bucket`
```
