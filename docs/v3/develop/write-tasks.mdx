...
Though it is typically recommended to use `@task` decorators to define tasks, if needed you can use the `task` decorator inline with any callable to create tasks on the fly:
```python
from prefect import flow, task
