...
- Use automatic [logging](develop/logging/) to capture runtime details, tags, 
and final state
...
(see all [task settings](develop/write-tasks/#task-configuration) and 
[flow settings](develop/write-flows/#flow-settings))
...
<Tip>
Tasks are always executed in the main thread by default, unless a specific [task runner](develop/task-runners) is used to execute them on different threads, processes, or infrastructure. This facilitates native Python debugging and profiling.
</Tip>
...
- Filter task runs by tag in the UI and through the [Prefect REST API](api-ref/rest-api/#filtering).
