...
| `cache_policy`        | An optional policy that determines what information is used to generate cache keys. Available policies include `INPUTS` (default), `TASK_SOURCE`, `RUN_ID`, `FLOW_PARAMETERS`, and `NONE`. Can be combined using the + operator. |
...
Prefect includes an [`exponential_backoff`](https://prefect-python-sdk-docs.netlify.app/prefect/tasks/#prefect.tasks.exponential_backoff) utility that will automatically generate a list of retry delays that correspond to an exponential backoff retry strategy. 
...
Caching saves you time and compute by allowing you to avoid re-running tasks unnecessarily. Prefect provides several built-in cache policies that can be combined to control how tasks are cached:

### Cache Policies

Cache policies are composable "strategies" that determine how a task's cache key is computed.

| Cache Policy | Description |
|-------------|-------------|
| `INPUTS` | Cache based on task input parameters (like `task_input_hash`) |
| `TASK_SOURCE` | Cache based on the task's source code |
| `RUN_ID` | Cache based on the flow run ID or task run ID |
| `FLOW_PARAMETERS` | Cache based on flow-level parameters |
| `NONE` | Disable caching |
| `CacheKeyFn` | Cache based on a custom function |

You can use these policies individually or combine them with the `+` or `-` operators:

```python
from prefect import task
from prefect.cache_policies import INPUTS, TASK_SOURCE, NONE, CacheKeyFnPolicy

# Cache based only on inputs
@task(cache_policy=INPUTS)
def task1(): ...

# Cache based on inputs AND source code
@task(cache_policy=INPUTS + TASK_SOURCE)
def task2(): ...

# Cache based on a custom function
@task(cache_policy=CacheKeyFnPolicy(lambda *args, **kwargs: "my-custom-key"))
def task3(): ...
...
When working with cached tasks, you might need to pass objects that can't (or shouldn't) be serialized as part of the cache key. There are two main approaches to handle this:
