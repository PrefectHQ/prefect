...
You can see supported environment variables for each setting in the [settings reference documentation](develop/settings-ref).
...
View supported environment variables for each setting in the [settings reference documentation](develop/settings-ref).
...
You can see the `prefect.toml` path for each setting in the [settings reference documentation](develop/settings-ref).
...
- [`api.url`](develop/settings-ref/#url): this setting specifies the API endpoint of your 
Prefect Cloud workspace or a self-hosted Prefect server instance.
- [`api.key`](develop/settings-ref/#key): this setting specifies the 
[API key](manage/cloud/manage-users/api-keys/) used to authenticate with Prefect Cloud.
- [`home`](develop/settings-ref/#home): the `home` value specifies the local Prefect directory for configuration files,
...
Read more about [managing API keys](manage/cloud/manage-users/api-keys/). 
</Tip>

## Common server settings


- [`server.database.connection_url`](http://localhost:3000develop/settings-ref#connection-url): the database connection URL for a self-hosted Prefect server instance.
...
The [`ui.api_url`](develop/settings-ref/#api_url) setting should be set to the external proxy URL.
...
- [`server.api.csrf_protection_enabled`](develop/settings-ref/#csrf-protection-enabled): activates CSRF protection on the server, 
requiring valid CSRF tokens for applicable requests. Recommended for production to prevent CSRF attacks. 
Defaults to `False`.
- [`server.api.csrf_token_expiration`](develop/settings-ref/#csrf-token-expiration): sets the expiration duration for server-issued CSRF tokens, 
influencing how often tokens need to be refreshed. The default is 1 hour.
- [`client.csrf_support_enabled`](develop/settings-ref/#csrf-support-enabled): enables or disables CSRF token handling in the Prefect client. 
...
- [`server.api.cors_allowed_origins`](develop/settings-ref/#cors-allowed-origins): a list of origins that are allowed to make cross-origin requests.
- [`server.api.cors_allowed_methods`](develop/settings-ref/#cors-allowed-methods): a list of HTTP methods that are allowed to be used during cross-origin requests.
- [`server.api.cors_allowed_headers`](develop/settings-ref/#cors-allowed-headers): a list of headers that are allowed to be used during cross-origin requests.
