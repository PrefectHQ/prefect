...
<Tip>
Tag-based task concurrency is different from [Global concurrency limits](develop/global-concurrency-limits), though they can be used to achieve similar outcomes. Global concurrency limits are a more general way to control concurrency for any Python-based operation, whereas tag-based concurrency limits are specific to Prefect tasks.
</Tip>
...
Task tag limits are checked whenever a task run attempts to enter a [`Running` state](develop/manage-states/).
...
While task run concurrency limits are configured through tags (as shown below), [flow run concurrency limits](develop/global-concurrency-limits) are configured through work pools and/or work queues.
</Tip>
