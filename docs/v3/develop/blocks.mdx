...
For configuration that will change between flow runs, we recommend using [parameters](develop/write-flows/#parameters).
</Tip>
...
These types separate blocks from [Prefect variables](develop/variables/), which are unstructured JSON documents.
...
The JSON, DateTime, and String blocks are deprecated in favor of [Variables](develop/variables/).
</Warning>
