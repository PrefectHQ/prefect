...
- Each [state](develop/manage-states/) the flow enters is recorded. This
allows you to observe and [act upon each transition](develop/manage-states#execute-code-on-state-changes) in the flow's execution.
...
- A flow can be [deployed](deploy/infrastructure-examples/docker/), which exposes an API for interacting with it remotely.
...
- Triggering a [deployment](deploy/infrastructure-examples/docker/) of that flow in Prefect Cloud or self-hosted Prefect server
...
You can log a [variety of metadata](develop/logging) about flow runs for monitoring, troubleshooting, and auditing purposes.
...
These arguments defined on your flow function are called [parameters](develop/write-flows/#parameters).
...
Note that you can provide parameter values to a flow through the API using a [deployment](deploy/).
...
Flows can call [tasks](develop/write-tasks), the most granular units of orchestrated work in Prefect workflows:
...
When a nested flow run starts, it creates a new [task runner](develop/task-runners/) for any tasks it contains.
...
Whether running locally or through a [deployment](deploy/infrastructure-examples/docker/), you must indicate which flow is the entrypoint for a flow run.
...
Most users run flows by creating a [deployment](deploy/) on Prefect Cloud or Prefect server and then scheduling a flow run for the deployment through a schedule, the Prefect UI, or the Prefect API.
...
| `task_runner`                                      | An optional [task runner](develop/task-runners/) to use for task execution within the flow when you `.submit()` tasks. If not provided and you `.submit()` tasks, the `ThreadPoolTaskRunner` is used.         |
...
See the [manage states](develop/manage-states) page for more information.
...
Check out [Transactions](develop/transactions/) to make your flows even more resilient and rollback actions when desired.

## See also

- Store and reuse non-sensitive bits of data, such as configuration information, by using [variables](develop/variables).
- Make your flow more manageable, performant, and observable by breaking it into discrete units of orchestrated work with [tasks](develop/write-tasks/).
