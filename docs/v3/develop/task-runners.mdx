...
order for tasks that do not directly exchange data.
...
Similar to the `.submit()` method, the `.map()` method accepts a `wait_for` argument to establish state dependencies between mapped tasks that do not have data dependencies.
