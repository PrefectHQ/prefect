...
This method returns a [`PrefectFuture`](https://reference.prefect.io/prefect/futures/#prefect.futures.PrefectFuture), which is a Prefect object that contains:
- a reference to the payload returned by the task;
- and a [`State`](https://reference.prefect.io/prefect/server/schemas/states/), which is a Prefect object indicating the state of the task run.
