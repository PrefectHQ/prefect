...
Results are the bedrock of many Prefect features - most notably [transactions](develop/transactions)
and [caching](develop/task-caching) - and are foundational to the resilient execution paradigm that Prefect enables.
...
See [settings](manage/settings-and-profiles) for more information on how settings are managed.
</Tip>

## Configuring result persistence

There are four categories of configuration for result persistence:
- [whether to persist results at all](develop/results#enabling-result-persistence): this is configured through
various keyword arguments and the `PREFECT_RESULTS_PERSIST_BY_DEFAULT` setting.
- [what filesystem to persist results to](develop/results#result-storage): this is configured through the `result_storage`
keyword and the `PREFECT_DEFAULT_RESULT_STORAGE_BLOCK` setting.
- [how to serialize and deserialize results](develop/results#result-serialization): this is configured through
the `result_serializer` keyword and the `PREFECT_RESULTS_DEFAULT_SERIALIZER` setting.
- [what filename to use](develop/results#result-filenames): this is configured through one of
...
through any of the mechanisms [described below](develop/results#enabling-result-persistence) - Prefect's default
...
- `cache_policy`: a [cache policy](develop/task-caching#cache-policies) specifying the behavior of the task's cache.
- `cache_key_fn`: [a function](develop/task-caching#cache-key-functions) that configures a custom cache policy.
...
This keyword accepts an instantiated [filesystem block](develop/blocks/), or a block slug. Find your blocks' slugs with `prefect block ls`.
...
a string. See [task caching documentation](develop/task-caching#cache-key-functions) for more information.
...
To learn more about caching mechanics in Prefect, see the [caching documentation](develop/task-caching).
