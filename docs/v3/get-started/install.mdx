...
<Summary>Commands for power users</Summary>
...
```bash
uvx -p 3.12 --with prefect ipython
```

to install prefect into a `uv` virtual environment:
...
to add prefect to an active project:

```bash
uv add prefect
```

to run a prefect server in an ephemeral python environment with `uvx`:
...
to run a prefect server in a container port-forwarded to your local machine's 4200 port:
