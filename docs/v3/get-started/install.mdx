...
<Note>
If you run Prefect against a separate server (self-hosted or Prefect Cloud), the client and server versions must be compatible. See [Versioning and compatibility](/v3/release-notes/versioning) for the rules.
</Note>
...
<Note>
Prefect Cloud is compatible with all client versions. Against a self-hosted server, the client version must be compatible with the server. See [Versioning and compatibility](/v3/release-notes/versioning).
</Note>
