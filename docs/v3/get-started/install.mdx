...
<Note>
When installing a client against a self-hosted server, see [Versioning and compatibility](/v3/get-started/versioning) for which client versions work with a given server. Prefect Cloud is compatible with all client versions.
</Note>
