...
<Tip>
See [Install Prefect](get-started/install/) for more details on installation.
</Tip>
...
If you have any issues with browser-based authentication, you can [authenticate with a manually created API key](manage/cloud/manage-users/api-keys/) instead.
  </Tab>
</Tabs>
...
This will create a corresponding [flow](develop/write-flows/). 

Adding `@task` decorators to any functions called by the flow converts them to [tasks](develop/write-tasks/). 
...
A [work pool](deploy/infrastructure-concepts/work-pools/) is the most common interface for deploying flows to remote infrastructure.
...
1. Create a [managed work pool](deploy/infrastructure-concepts/work-pools):
...
A [deployment](deploy/infrastructure-examples/docker/) is used to determine when, where, and how a flow should run.
...
1. Create a deployment in code:
   ...
   See [Where to store your flow code](deploy/infrastructure-concepts/store-flow-code) for more details.
   </Tip>
...
1. Schedule a run for the deployment:
   ...
   ![Flow run graph and logs](img/ui/qs-flow-run.png)
...
* [Write flows](develop/write-flows)
* [Write tasks](develop/write-tasks)
* [Cloud and server](manage)
* [Manage infrastructure with work pools](deploy/infrastructure-concepts/work-pools) to learn about running workflows on Kubernetes, Docker, and serverless infrastructure.
