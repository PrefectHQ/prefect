...
This will create a corresponding [flow](/v3/develop/write-flows/). 

Adding `@task` decorators to any functions called by the flow converts them to [tasks](/v3/develop/write-tasks/). 
...
A [deployment](/v3/deploy/) is used to determine when, where, and how a flow should run.
...
* [Write tasks](/v3/develop/write-tasks)
* [Cloud and server](/v3/manage)
* [Manage infrastructure with work pools](/v3/deploy/infrastructure-concepts/work-pools) to learn about running workflows on Kubernetes, Docker, and serverless infrastructure.
