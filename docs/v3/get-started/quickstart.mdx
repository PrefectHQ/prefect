...
Prefect is a workflow orchestration tool that helps you build, run, and monitor data pipelines. It makes complex workflows reliable by tracking dependencies and handling failures gracefully. In this tutorial, you'll deploy and run your first workflow by specifying what code should run, where it should run, and when it should run.

<Steps>
  <Step title="Create and login to your Prefect Cloud account.">
    For the ease of getting started, we'll use a hosted Prefect server on Prefect Cloud. To create an account, we need to install `prefect-cloud` and will leverages `uv`, a modern Python package manager, to do this. Run the following commands in your terminal, after which you'll be prompted in your browser to create or login to your free Prefect Cloud account.
  ...
  <Step title="What code should run?">
   ...
   ```python 01_getting_started.py [expandable]
    ...
    ...Ommitted for brevity in this tutorial
    00:30:54.553 | INFO    | Task run 'process_customer-4af' - Finished in state Completed()
    00:30:54.555 | INFO    | Task run 'process_customer-e66' - Finished in state Completed()
    00:30:54.556 | INFO    | Task run 'process_customer-e7e' - Finished in state Completed()
    00:30:54.556 | INFO    | Task run 'process_customer-5bd' - Finished in state Completed()
    00:30:54.557 | INFO    | Task run 'process_customer-05d' - Finished in state Completed()
    00:30:54.558 | INFO    | Task run 'process_customer-b60' - Finished in state Completed()
    00:30:54.701 | INFO    | Flow run 'airborne-ringtail' - Finished in state Completed('All states completed.')
    ```
    </Step>
    ...
    Since we know what code to run and where to run it, this can now be run remotely. 
    To do that, run the following command and visit the returned link to see the run's status in the Prefect UI:
    ```python 
    uvx prefect-cloud run main/main
    ```
  ...
  <Step title="When should it run?">
    ...
    Let's schedule our workflow to run every day at 8:00 AM:
    
    ```python
    uvx prefect-cloud schedule main/main "0 8 * * *"
    ```
    
    The schedule is specified using cron syntax:
    - `0` - At minute 0
    - `8` - At 8:00 AM
    - `* * *` - Every day, every month, every day of the week
    
    You can use different cron expressions to create various scheduling patterns:
    
    - Every hour: `0 * * * *`
    - Every Monday at noon: `0 12 * * 1`
    - Every 15 minutes: `*/15 * * * *`
    
    After running the command, your workflow will automatically execute according to the schedule you've set. You can view and manage all your scheduled runs in the Prefect UI.
  </Step>
