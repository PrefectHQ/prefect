...
---
## What is Prefect?
Prefect is an open-source orchestration engine that turns your Python functions into production-grade data pipelines with minimal friction. Build and schedule workflows in pure Python—no DSLs, no YAML, no headaches—and run them anywhere. Prefect handles the hard stuff out of the box: state tracking, failure recovery, real-time monitoring, and more.

Under the hood, Prefect takes care of scheduling and execution so you can focus on clean, maintainable Python code instead of battling infrastructure. Whether you’re a data engineer or part of a broader team, Prefect makes it easy to go from idea to production-grade workflows—fast.
...
| **Actually open source** | Prefect is Apache 2.0 and is deployed in production by some of the most demanding enterprises in the world. Host the Prefect orchestration server yourself using containers, Kubernetes, or infrastructure-as-code. Keep your workflow infrastructure wherever it belongs while choosing the orchestration model that fits your needs. |
</div>

## Why is Prefect?
In 2018, we set out to solve a problem many of you know too well: orchestrating data workflows shouldn’t feel like pulling teeth—or writing DAGs in YAML. Prefect was designed to handle the challenges that tools like Airflow struggled with: dynamic workflows, modern infrastructure, and the complexity of today’s data pipelines. Since then, it’s become a go-to tool for everyone from startups to Fortune 100 companies. Prefect helps you ship production-ready workflows without the headaches, so you can spend more time building and less time debugging.
