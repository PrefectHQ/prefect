...
The `.serve` method allows you to easily elevate a flow to a deployment, listening for scheduled work to execute [as a local process](deploy/run-flows-in-local-processes).
