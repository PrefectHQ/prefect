...
While the majority of deployment properties are included in a deployment's version, some properties cannot be versioned to prevent unexpected execution behavior when rolling back or promoting versions.
...
Additionally, Prefect automatically collects your **repository name**, **repository URL**, your currently checked out **branch**, the **commit SHA**, and the **first line of your commit message** from your environment if you're deploying from a supported version control platform or from inside a git repository.
This information is used to help create a link between deployment versions and code versions.

<Note>
Prefect's automatic version information collection currently supports Github Actions, Gitlab CI, Bitbucket Pipelines, and Azure Pipelines on Azure DevOps.
</Note>