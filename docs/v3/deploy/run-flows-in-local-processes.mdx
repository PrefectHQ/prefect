...
The simplest way to create a [deployment](deploy/infrastructure-examples/docker/) for your flow is by calling its `serve` method.
...
[schedules](automate/add-schedules/) for more advanced scheduling options
...
For more ways to store and access flow code, see the [Retrieve code from storage page](deploy/infrastructure-concepts/store-flow-code).
...
Learn more about work-pool based deployments on the [Configure dynamic infrastructure with work pools page](deploy/infrastructure-concepts/work-pools)