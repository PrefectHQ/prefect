...
1.  **Include source code directly in your runtime:** Often, this means [building your code into a Docker image](/v3/deploy/infrastructure-examples/docker/).
2.  **Retrieve code from storage at runtime:** The worker pulls code from a specified location before starting the flow run.

This page focuses on the second approach: retrieving code from storage. You have several options for where your code can be stored and pulled from:
...
In the examples below, we show how to create a deployment configured to run on [dynamic infrastructure](/v3/deploy/infrastructure-concepts/work-pools) for each of these storage options. 
...
- [Python code with the `flow.deploy` method](/v3/deploy/infrastructure-concepts/deploy-via-python)
...
- [YAML specification defined in a `prefect.yaml` file](/v3/deploy/infrastructure-concepts/prefect-yaml)
...
By default, deployment flow runs associated with a Docker work pool will use the base Prefect image when executing.
Alternatively, you can create a custom Docker image outside of Prefect by running `docker build` && `docker push` elsewhere (e.g. in your CI/CD pipeline) and then reference the `image` in the `job_variables` section of your deployment definition, or set the `image` as a default directly on the work pool.
