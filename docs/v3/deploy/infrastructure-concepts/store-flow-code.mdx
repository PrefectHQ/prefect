...
You can create a deployment through [Python code with the `flow.deploy` method](deploy/infrastructure-concepts/deploy-via-python) or through a [YAML specification defined in a `prefect.yaml` file](deploy/infrastructure-concepts/prefect-yaml).
...
For more information, see [this discussion of custom Docker images](deploy/infrastructure-examples/docker/#automatically-build-a-custom-docker-image-with-a-local-dockerfile).
...
Another authentication option is to give the [worker](deploy/infrastructure-concepts/workers/) access to the storage location at runtime through SSH keys.
...
Read more about when to consider using `serve` [here](deploy/infrastructure-concepts/deploy-via-python#when-to-consider-flow-deploy-over-flow-serve).
