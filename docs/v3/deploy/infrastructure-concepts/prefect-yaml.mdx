...
As discussed above, if you use [blocks](develop/blocks/), you can template the credentials section with 
...
- **blocks**: you can reference [Prefect blocks](develop/blocks) with the  
...
- **variables**: you can reference [Prefect variables](develop/variables) with the  
...
| `schedule`                                 | An optional [schedule](automate/add-schedules) to assign to the deployment. Fields for this section are documented in the [Schedule Fields](#schedule-fields) section.                                                                                                                      |
| `concurrency_limit`                        | An optional [deployment concurrency limit](deploy/index#concurrency-limiting). Fields for this section are documented in the [Concurrency Limit Fields](#concurrency-limit-fields) section.                                                                                                                      |
| `triggers`                                  | An optional array of [triggers](automate/events/automations-triggers/#custom-triggers) to assign to the deployment |
...
| `job_variables`                                | Values used to override the default values in the specified work pool's [base job template](deploy/infrastructure-concepts/work-pools/#base-job-template). Maps directly to a created deployments `infra_overrides` attribute. |
...
- [Managed work pools](deploy/infrastructure-examples/managed/)
- [Push work pools](deploy/infrastructure-examples/serverless/)
- [Kubernetes work pools](deploy/infrastructure-examples/kubernetes/)
- [Serverless hybrid work pools](deploy/infrastructure-examples/docker/)
