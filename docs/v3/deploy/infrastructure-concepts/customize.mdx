...
There are two ways to deploy flows to work pools: with a [`prefect.yaml` file](deploy/infrastructure-concepts/prefect-yaml) or using the [Python `deploy` method](deploy/infrastructure-concepts/deploy-via-python). 
...
![Job variables through custom run](img/ui/deployment-job-variables.png)
...
![Job variables through automation action](img/ui/automations-action-job-variable.png)
