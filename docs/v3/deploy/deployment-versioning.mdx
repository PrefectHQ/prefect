...
- prefect.deployments.steps.git_clone:
    ...
    commit_sha: "{{ $GITHUB_SHA }}" # replace with the relevant env var from the table above
...
if __name__ == "__main__":
    my_flow.from_source(
        source=GitRepository(
            ...
            commit_sha=os.getenv("GITHUB_SHA"), # replace with the relevant env var from the table above
        )
    ).deploy(
        name="my-deployment",
        version="0.0.1",
