---
title: Run flows on cloud with Coiled
...
-  [Serverless push work pools](serverless), which remove the need to
   host any always-on Prefect worker
-  [Coiled](https://coiled.io), which manages cloud VMs in your account like
   ECS/Cloud Run/ACI, but in a way that is more ergonomic and accessible.

We like this combination because it combines the cost efficiency and security
of a cloud VM approach with an easy setup and zero-maintenance infrastructure.
...
walkthrough shows the steps of this article:
...
If you haven't used Prefect Cloud before, you'll need to [create an account](https://app.prefect.cloud) and log in:

```
prefect cloud login
```

If you haven't used Coiled before, you'll need to [create a Coiled account](https://cloud.coiled.io/signup) and log in:
...
Because of this, Coiled also provides a Dockerless execution process that we call [Package Sync](#how-environment-synchronization).  Coiled can automatically inspect your local environment and build a remote cloud environment on the fly.  We find that this is more ergonomic and accessible to everyone on the team.

To invoke this feature today you need to have both a Python file `example_flow.py` with your flow, and a configuration file `prefect.yaml`.
...
For a full list of possible configuration options see [Coiled API Documentation](https://docs.coiled.io/user_guide/api.html#coiled-batch-run)

## Extra: Scale Out

Coiled makes it easy to parallelize individual flows on distributed cloud hardware.  The easiest way to do this is to annotate your Prefect tasks with the `@coiled.function` decorator.
...
See [Coiled Documentation](https://docs.coiled.io/user_guide/why.html) to learn more about Coiled architecture and design decisions.
