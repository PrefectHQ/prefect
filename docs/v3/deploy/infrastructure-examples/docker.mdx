...
- a Docker [**work pool**](deploy/infrastructure-concepts/work-pools/): stores the infrastructure configuration for your deployment
- a Docker [**worker**](deploy/infrastructure-concepts/workers/): process that polls the Prefect API for flow runs to execute as Docker containers
- a [**deployment**](deploy/index/): a flow that should run according to the configuration on your Docker work pool
...
To instead learn how to run flows in a static, long-lived container, see [this](deploy/static-infrastructure-examples/docker/) guide.
...
In a production setting this worker should run as a [daemonized or managed process](resources/daemonize-processes/).
...
Note that you used a [Prefect Secret block](develop/blocks/) to load the URL configuration for
...
See the [Store flow code](deploy/infrastructure-concepts/store-flow-code/) guide for more flow code storage options.
...
[base job template](deploy/infrastructure-concepts/work-pools/#base-job-template).
...
See [Override work pool job variables](deploy/infrastructure-concepts/customize) for more information about how to customize these variables.
...
- [Deploy flows on Kubernetes](deploy/infrastructure-examples/kubernetes/)
- [Deploy flows on serverless infrastructure](deploy/infrastructure-examples/serverless/)
- [Daemonize workers](resources/daemonize-processes/)
