...
- **Orphan reconciliation** (`orphans`): Finds and removes orphaned logs and artifacts, then reconciles stale artifact collections. Automatically enabled with `flow_runs`, but runs less frequently because each pass can scan the full child tables. You can also enable it independently.
...
<Note>
Orphan reconciliation uses anti-joins that can scan the full `log` and `artifact` tables. It runs once every 24 hours by default instead of on the normal hourly vacuum loop. Increase `PREFECT_SERVER_SERVICES_DB_VACUUM_ORPHAN_CLEANUP_LOOP_SECONDS` if those scans affect database performance. You do not need to add `orphans` when `flow_runs` is enabled; the separate value is for running orphan cleanup without deleting old flow runs.
</Note>
