...
- Orphaned artifacts left by other flow run deletion paths
...
- Optionally, orphaned logs and artifacts left by interrupted cleanup, late writes, direct SQL deletion, or older Prefect versions
...
The vacuum service has three independent components controlled by `PREFECT_SERVER_SERVICES_DB_VACUUM_ENABLED`:

- **Event vacuum** (`events`): Cleans up old events and event resources. Enabled by default.
- **Flow run vacuum** (`flow_runs`): Cleans up old flow runs with their logs and artifacts, removes orphaned artifacts, and reconciles stale artifact collections. Disabled by default.
- **Orphan backfill** (`orphans`): Finds and removes orphaned logs and artifacts. Disabled by default because each pass can scan the full child tables.
...
2. **Delete orphaned artifacts**: Removes artifacts whose flow run no longer exists, including artifacts left by other flow run deletion paths.
...
### Orphan backfill (when `orphans` is enabled)
...
Flow run selection and event cleanup use transaction batches controlled by `PREFECT_SERVER_SERVICES_DB_VACUUM_BATCH_SIZE`. Once a locked flow run batch is deleted, its logs and artifacts are deleted in their own batches of the same size, so a run with many child rows does not create one large transaction.

<Warning>
The orphan backfill uses anti-joins that can scan the full `log` and `artifact` tables. Enable `orphans` only to drain a known backlog, then disable it. It is not part of steady-state flow run cleanup.
</Warning>
...
<Tip>
After direct SQL deletion, temporarily enable the orphan backfill with `PREFECT_SERVER_SERVICES_DB_VACUUM_ENABLED="events,orphans"` to remove child rows that no longer reference an existing flow run.
</Tip>
