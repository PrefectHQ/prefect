...
- Old top-level flow runs that have reached a terminal state (completed, failed, cancelled, or crashed), together with their logs and artifacts
- Orphaned logs and artifacts left by interrupted cleanup, late writes, direct SQL deletion, or older Prefect versions
...
The vacuum service has three independently scheduled components controlled by `PREFECT_SERVER_SERVICES_DB_VACUUM_ENABLED`:

- **Event vacuum** (`events`): Cleans up old events and event resources. Enabled by default.
- **Flow run vacuum** (`flow_runs`): Cleans up old flow runs with their logs and artifacts. Disabled by default.
- **Orphan reconciliation** (`orphans`): Finds and removes orphaned logs and artifacts, then reconciles stale artifact collections. Automatically enabled with `flow_runs`, but runs less frequently because each pass walks every `flow_run_id` in the child tables. You can also enable it independently.
...
|---------|---------|-------------|
| `PREFECT_SERVER_SERVICES_DB_VACUUM_ENABLED` | `events` | Comma-separated set of vacuum types to enable. Valid values: `events`, `flow_runs`, `orphans`. |
| `PREFECT_SERVER_SERVICES_DB_VACUUM_LOOP_SECONDS` | `3600` (1 hour) | How often the flow run and event vacuum cycles run, in seconds. |
| `PREFECT_SERVER_SERVICES_DB_VACUUM_ORPHAN_CLEANUP_LOOP_SECONDS` | `86400` (24 hours) | How often orphan reconciliation runs when enabled. |
...
| `PREFECT_SERVER_SERVICES_DB_VACUUM_EVENT_RETENTION_OVERRIDES` | `{"prefect.flow-run.heartbeat": 604800}` | Per-event-type retention period overrides in seconds. Event types not listed fall back to `PREFECT_SERVER_EVENTS_RETENTION_PERIOD`. Each override is capped by the global events retention period. |
...
The vacuum services schedule independent cleanup tasks:

<Steps>
### Flow run vacuum (when `flow_runs` is enabled)

1. **Delete old flow runs**: Selects and deletes a locked batch of top-level terminal flow runs whose end time is older than the configured retention period. It then deletes logs and artifacts by `flow_run_id`, without scanning the log table. Subflows are cleaned up independently by later batches and cycles once they meet the same criteria.

### Orphan reconciliation (when `flow_runs` or `orphans` is enabled)

1. **Delete orphaned logs**: Removes logs whose flow run no longer exists.
2. **Delete orphaned artifacts**: Removes artifacts whose flow run no longer exists.
3. **Reconcile stale artifact collections**: Repairs collections affected by artifact cleanup.
...
2. **Delete old events**: Removes all events and event resources older than `PREFECT_SERVER_EVENTS_RETENTION_PERIOD`.
</Steps>
...
Flow run selection, orphan reconciliation, and event cleanup use transaction batches controlled by `PREFECT_SERVER_SERVICES_DB_VACUUM_BATCH_SIZE`. Once a locked flow run batch is deleted, its logs and artifacts are deleted in their own batches of the same size, so a run with many child rows does not create one large transaction.

<Note>
Orphan reconciliation walks the distinct `flow_run_id` values in the `log` and `artifact` tables in index order, in pages of `PREFECT_SERVER_SERVICES_DB_VACUUM_BATCH_SIZE`, and deletes rows for flow runs that no longer exist. Each query reads one slice of the `flow_run_id` index instead of scanning the full table. It runs once every 24 hours by default instead of on the normal hourly vacuum loop. Increase `PREFECT_SERVER_SERVICES_DB_VACUUM_ORPHAN_CLEANUP_LOOP_SECONDS` if that pass affects database performance. You do not need to add `orphans` when `flow_runs` is enabled; the separate value is for running orphan cleanup without deleting old flow runs.
</Note>
...
- **Large batch size**: Increasing `PREFECT_SERVER_SERVICES_DB_VACUUM_BATCH_SIZE` can speed up cleanup but may make transactions larger and hold database locks longer. Decrease the batch size if you observe performance impacts during vacuum cycles.
...
<Tip>
After direct SQL deletion, enable orphan reconciliation with `PREFECT_SERVER_SERVICES_DB_VACUUM_ENABLED="events,orphans"` to remove child rows that no longer reference an existing flow run.
</Tip>
...
prefect config set PREFECT_SERVER_EVENTS_RETENTION_PERIOD="2d"
```
