...
- Optionally, orphaned logs and artifacts (rows that reference flow runs which no longer exist)
...
- **Flow run vacuum** (`flow_runs`): Cleans up old flow runs with their logs and artifacts, and reconciles stale artifact collections. Disabled by default.
- **Orphan backfill** (`orphans`): Cleans up logs and artifacts that reference flow runs which no longer exist. Disabled by default.
...
1. **Delete old flow runs**: Removes top-level flow runs (not subflows) that are in a terminal state and whose end time is older than the configured retention period. Logs and artifacts have no cascade from `flow_run`, so each batch deletes them by `flow_run_id` (an indexed lookup) before it deletes the runs. Subflows are not cascade-deleted when their parent is removed; they are cleaned up independently by later batches and cycles once they also meet the top-level, terminal state, and age criteria.
2. **Reconcile stale artifact collections**: For collections pointing to a deleted artifact, re-points to the next most recent artifact version. If no versions remain, deletes the collection.
...
3. **Reconcile stale artifact collections**: Same reconciliation as above, for the collections these deletions leave stale.
...
<Warning>
Each orphan backfill batch scans the full `log` and `artifact` table, which is expensive on large tables. Enable `orphans` only to clear a known backlog of orphans (for example, after direct SQL deletes or an upgrade from a version that left orphans behind), and disable it again when the backlog is drained. The flow run vacuum does not create orphans.
</Warning>
...
<Tip>
If you use direct SQL for flow run deletion, enable the orphan backfill with `PREFECT_SERVER_SERVICES_DB_VACUUM_ENABLED="events,orphans"` to clean up any orphaned logs and artifacts left behind.
</Tip>
