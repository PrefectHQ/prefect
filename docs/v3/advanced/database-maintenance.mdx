...
1. **Delete old flow runs**: Selects top-level terminal flow runs whose end time is older than the configured retention period. For each selected batch, the service deletes logs and artifacts by `flow_run_id` before deleting the runs. It does not scan the log table for orphaned logs. Subflows are cleaned up independently by later batches and cycles once they meet the same criteria.
...
Flow run selection and event cleanup use transaction batches controlled by `PREFECT_SERVER_SERVICES_DB_VACUUM_BATCH_SIZE`. The logs and artifacts of the selected runs are deleted in their own batches of the same size, so a run with many child rows does not create one large transaction. Each run is deleted after its logs and artifacts, which means an interrupted vacuum cycle leaves the run in place and cleans it up on the next cycle.
