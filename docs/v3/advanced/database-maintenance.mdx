...
Flow run selection and event cleanup use transaction batches controlled by `PREFECT_SERVER_SERVICES_DB_VACUUM_BATCH_SIZE`. Large selected-ID lists are split across multiple delete statements to stay within database parameter limits, but the statements still share one transaction. Related logs and artifacts are not limited by row count, so a selected run with many child rows can still create a large transaction.
