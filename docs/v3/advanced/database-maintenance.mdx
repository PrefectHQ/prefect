...
| `PREFECT_SERVER_SERVICES_DB_VACUUM_MAINTENANCE_POOL_SIZE` | `1` | Size of the dedicated connection pool used for vacuum maintenance queries. Increase this when event and flow-run vacuum tasks contend on high-volume deployments. |
| `PREFECT_SERVER_SERVICES_DB_VACUUM_MAINTENANCE_MAX_OVERFLOW` | `0` | Number of overflow connections allowed for the vacuum maintenance connection pool. |
...
<Warning>
The vacuum service issues SQL `DELETE` statements. On PostgreSQL heap tables, disk usage does not drop immediately: dead tuples become reusable only after `VACUUM` or autovacuum, and relation files shrink only with `VACUUM FULL`, `pg_repack`, or `pg_squeeze`. See [PostgreSQL VACUUM](#postgresql-vacuum) and [Tune autovacuum for Prefect workloads](#tune-autovacuum-for-prefect-workloads). Monitor the `event`, `event_resource`, `flow_run`, and `log` tables for bloat.
</Warning>
...
- **High-volume PostgreSQL deployments**: If you run both event and flow-run vacuum types, consider increasing `PREFECT_SERVER_SERVICES_DB_VACUUM_MAINTENANCE_POOL_SIZE` to reduce connection contention between vacuum tasks.
