...
Flow run selection and event cleanup use batches controlled by `PREFECT_SERVER_SERVICES_DB_VACUUM_BATCH_SIZE`. A selected flow run's logs and artifacts are deleted in the same transaction, so a run with many related rows can still create a large transaction.
