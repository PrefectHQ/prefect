...
flowchart TB
    ...
    subgraph servers[Prefect Server Components]
        ...
        subgraph bg[Background Services - Horizontal Scaling]
            direction LR
            BG1[Background 1<br/>services start]:::bgClass
            BG2[Background 2<br/>services start]:::bgClass
            BG3[Background N...<br/>services start]:::bgClass
        end
    end
    ...
    subgraph data[Data Layer]
        ...
        Redis[(Redis<br/>• Event Streams<br/>• Consumer Groups<br/>• Task Coordination)]:::dataClass
    end
...
**Background services** (can be horizontally scaled):
```bash
prefect server services start
```

<Note>
Background services are designed for horizontal scaling. They use Redis consumer groups to coordinate work distribution, so multiple instances safely process events without duplication. Periodic tasks (like the scheduler) use distributed coordination via Redis to ensure only one instance runs each task at a time.
</Note>
...
<Accordion title="3 API servers, 1 background service">
...
<Accordion title="3 API servers, 3 background services (horizontally scaled)">

For high-volume deployments, scale background services to handle increased event processing load:
...
services:
  ...
  # Background services can be horizontally scaled - Redis consumer groups
  # ensure events are processed exactly once across all instances
  prefect-background:
    image: prefecthq/prefect:3-latest
    depends_on:
      migrate:
