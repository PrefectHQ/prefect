...
- Redis for event messaging and background task coordination
...
- **Multiple API server instances** - Handle UI and API requests, each with an embedded docket worker for background tasks
- **[PostgreSQL](https://www.postgresql.org/) database** - Stores all persistent data and synchronizes state across servers
- **[Redis](https://redis.io/)** - Coordinates background task execution across servers and distributes events between services
...
<Note>
Prefect uses [docket](https://github.com/prefecthq/docket), a Redis-based task queue, to coordinate background services across multiple API servers. Each API server runs an embedded docket worker that picks up tasks from Redis, ensuring background services like the scheduler and automation triggers run exactly once across your deployment—even with multiple API replicas.
</Note>
...
flowchart TB
    ...
    subgraph servers[Prefect Server Components]
        ...
        subgraph api[API Servers - Horizontal Scaling]
            direction LR
            API1[API Server 1<br/>+ Docket Worker]:::apiClass
            API2[API Server 2<br/>+ Docket Worker]:::apiClass
            API3[API Server N...<br/>+ Docket Worker]:::apiClass
        end
    end
    ...
    subgraph data[Data Layer]
        ...
        Redis[(Redis<br/>• Docket Task Queue<br/>• Events<br/>• Automations)]:::dataClass
    end
...
Configure Redis as your server's message broker and task coordinator:
...
<Note>
Redis is used for two purposes: (1) event messaging between services via `prefect-redis`, and (2) coordinating background task execution via docket's Redis streams. Both use the same Redis connection configured above.
</Note>
...
Each API server includes an embedded docket worker that processes background tasks. Simply run multiple instances:

```bash
prefect server start --host 0.0.0.0 --port 4200
```

Background services (scheduler, automation triggers, etc.) are automatically coordinated across all API server instances via Redis. Docket ensures each task runs exactly once, even with multiple replicas.
...
<Accordion title="3 API servers with embedded background services, postgres, redis">
...
Each API server replica includes an embedded docket worker. Background services like the scheduler coordinate across all replicas via Redis, ensuring tasks run exactly once.
</Accordion>
