...
<Note>
Prefect server PostgreSQL connections always execute transactions at `READ COMMITTED`, even when the database or role default is stronger. This guarantees that conditional concurrency-counter updates wait and re-evaluate their predicates against the latest committed row.
</Note>
