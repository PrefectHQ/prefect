...
4. **Configure your connection URL**:
   ```bash
   export PREFECT_SERVER_DATABASE_CONNECTION_URL="postgresql+asyncpg://iam_user@host:5432/prefect"
   ```
...
3. **Enable plugins and managed identity authentication**:
   ...
   # Optional: export PREFECT_INTEGRATIONS_AZURE_POSTGRES_MANAGED_IDENTITY_CLIENT_ID=<client-id>
   ```
...
```bash
export PREFECT_SERVER_EVENTS_MESSAGING_BROKER="prefect_redis.messaging"
export PREFECT_SERVER_EVENTS_MESSAGING_CACHE="prefect_redis.messaging"
...
Prefect uses [Docket](https://github.com/chrisguidry/docket) to coordinate background services like the scheduler, late run detection, and automation triggers. Docket also schedules deferred task runs and provides the Redis transport for task-keyed delivery to background task workers. By default, Docket uses in-memory storage (`memory://`), which only works for single-server deployments.
...
With shared Redis, a task worker only claims runs for the task keys it serves. A delivery remains pending until the task worker acknowledges it; if the worker or its API replica disconnects, another subscription for the same task key can reclaim it after `server.tasks.scheduling.delivery_visibility_timeout`.

Point background task workers at the load-balanced API URL so they reconnect through a healthy replica. Docket coordinates delivery, while task parameters and return values continue to use Prefect result storage; use shared result storage when submitters and task workers do not share a filesystem.
...
```bash
export PREFECT_SERVER_DATABASE_MIGRATE_ON_START="false"
```
...
- `PREFECT_SERVER_UI_API_URL`: Connection URL from UI to API
- `PREFECT_SERVER_UI_SERVE_BASE`: Base URL path to serve the UI
...
When running migrations on large database instances (especially where tables like `events`, `flow_runs`, or `task_runs` can reach millions of rows), creating indexes can take a long time. On PostgreSQL, migrations use a dedicated statement timeout (`PREFECT_SERVER_DATABASE_MIGRATION_TIMEOUT`) instead of the general database timeout (`PREFECT_SERVER_DATABASE_TIMEOUT`), and have no timeout by default.

If you want to bound how long a migration can run, set a migration timeout:

```bash
# Set timeout to 10 minutes (adjust based on your database size)
export PREFECT_SERVER_DATABASE_MIGRATION_TIMEOUT=600
...
```bash
docker run -e PREFECT_SERVER_DATABASE_MIGRATION_TIMEOUT=600 prefecthq/prefect:latest prefect server database upgrade -y
```
