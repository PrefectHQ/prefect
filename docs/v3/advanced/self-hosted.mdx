...
Use the `rediss+sentinel://` scheme for SSL/TLS; a private CA can be supplied with the
`ssl_ca_certs` query parameter and applies to both the Sentinel daemons and the data
nodes. This requires `pydocket>=0.23.1`.
