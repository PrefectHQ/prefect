...
Prefect uses [Docket](https://github.com/chrisguidry/docket) to coordinate background services like the scheduler, late run detection, and automation triggers. Prefect also uses the same Docket connection for durable, task-keyed background task delivery. By default, Docket uses in-memory storage (`memory://`), which only works for single-server deployments.
