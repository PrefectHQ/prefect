---
title: Experimental Plugin System
sidebarTitle: Plugins (Experimental)
---

<Warning>
**Experimental Feature**

The plugin system is an **experimental feature** under active development. The API is subject to change without notice, and features may be modified or removed in future releases. Use in production environments at your own risk.

This feature is explicitly namespaced under `prefect._experimental` to indicate its unstable status.
</Warning>

The Prefect experimental plugin system allows third-party packages to run startup hooks before Prefect CLI commands, workers, or agents execute. This enables plugins to configure environment variables, authenticate with external services, or perform other initialization tasks automatically.
...
Once enabled, plugins will automatically run before every Prefect CLI command and worker/agent startup.
...
from prefect._experimental.plugins import hookimpl, HookContext, SetupResult
...
**Q: Can I use plugins without the `@hookimpl` decorator?**

A: The `@hookimpl` decorator is recommended but not strictly required for the plugin system to discover your hooks.
