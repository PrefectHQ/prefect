...
- Flow run logs, task run logs, and logs from [extra loggers](#include-logs-from-other-libraries) are not sent to the API. Prefect Cloud worker logs are not sent either.
