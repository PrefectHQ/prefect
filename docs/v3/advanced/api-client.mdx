...
If you pass custom authentication, timeout, or transport objects through `httpx_settings`, use types from `httpx2`.
Use `httpx2` exception classes when catching HTTP errors from the Prefect client.
