...
To opt into HTTPX2, install `prefect[httpx2]` (or `prefect-client[httpx2]`) and set this environment variable
before starting Python or a worker:
...
Each process selects its backend once, and flow-run subprocesses inherit the selection.
Installing the extra alone does not change the backend.
