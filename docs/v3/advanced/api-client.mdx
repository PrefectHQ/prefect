...
keywords: ["API client", "REST API", "PrefectClient", "HTTP", "programmatic access"]
---
...
Prefect continues to use HTTPX by default, including its authentication, transport, response, and exception types.
The HTTPX backend is deprecated. It remains available for at least six months and three minor version increases
after the first release announcing its deprecation, following the [versioning policy](/v3/release-notes/versioning#deprecation).

To opt into HTTPX2 with either `prefect` or `prefect-client`, add `httpx2[http2]>=2.13.0,<3.0.0`
to your environment's dependencies and set this environment variable before starting Python:

{/* pmd-metadata: notest */}
```bash
export PREFECT_CLIENT_HTTP_BACKEND=httpx2
```

Each process selects its backend once. Local flow-run subprocesses inherit the selection.
Docker, Kubernetes, and other remote work-pool jobs use their own environments: install HTTPX2
in the job's runtime image and set `PREFECT_CLIENT_HTTP_BACKEND=httpx2` in the work pool or deployment's
job variables (`env`). Configuring HTTPX2 on the worker does not configure its remote jobs.
Installing HTTPX2 alone does not change the backend.
When using HTTPX2, pass `httpx2` authentication, timeout, and transport objects through `httpx_settings`,
and catch `httpx2` exceptions instead of `httpx` exceptions.
...
When `limit` is `None` (the default), the server applies `PREFECT_SERVER_API_DEFAULT_LIMIT`, which defaults
