...
The client also sends this authentication string as a `Basic` `Authorization` header when opening the WebSocket connection used for [events](/v3/concepts/events), so servers or proxies that require basic authentication before allowing the connection upgrade authenticate correctly.
