...
keywords: ["security", "CSRF", "CORS", "content security policy", "hardening"]
---
...
<Note>
Prefect client 3.6.14 or later is required for event streaming when the server has `PREFECT_SERVER_API_AUTH_STRING` configured. Older clients cannot complete the authenticated WebSocket handshake and will not emit run events. If you see a warning in your logs that says "Unable to connect to the events WebSocket", upgrade the Prefect client to 3.6.14 or later.
</Note>
