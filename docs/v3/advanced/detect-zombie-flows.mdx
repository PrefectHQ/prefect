...
Heartbeats for async flows are emitted by an `asyncio.Task` that runs alongside your flow code. If something blocks
...
The most common cause is calling a blocking sync function from an async flow:
...
To avoid this, use `asyncio.to_thread()` to run blocking sync calls without starving the event loop:

{/* pmd-metadata: notest */}
```python
@flow
async def pipeline():
    await asyncio.to_thread(sync_etl)  # runs in a thread, event loop stays free
```

<Warning>
If you see `The event loop was blocked for N seconds, preventing heartbeat emission` in your
logs, your zombie detection automation may produce false positives. You can also increase the
`within` window in your automation trigger to account for long-running sync calls.
</Warning>