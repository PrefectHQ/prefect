...
keywords: ["infrastructure", "submit", "work pool", "decorator", "remote execution", "include files", "bundle"]
---
...
Files that match a `.prefectignore` pattern are excluded from the bundle even if they match an `include_files` pattern. If this happens, Prefect logs a warning.
...
<Warning>
Prefect warns if bundled files match sensitive patterns such as `.env*`, `*.pem`, `*.key`, or `credentials.*`. Review these warnings and add sensitive files to `.prefectignore` to prevent accidental inclusion.
</Warning>

### Using `include_files` across infrastructure types

The `include_files` parameter is available on every infrastructure decorator:
...
@docker(work_pool="my-pool", include_files=["config.yaml"])
@flow
def my_flow():
    ...
```
...
@kubernetes(work_pool="my-pool", include_files=["config.yaml"])
@flow
def my_flow():
    ...
```
...
@cloud_run(work_pool="my-pool", include_files=["config.yaml"])
@flow
def my_flow():
    ...
```

```python Azure Container Instances
from prefect import flow
from prefect_azure.experimental import azure_container_instance


@azure_container_instance(work_pool="my-pool", include_files=["config.yaml"])
@flow
def my_flow():
    ...
```
