...
Any kwargs passed to the infrastructure decorator will override the corresponding default value in the [base job template](/v3/advanced/customize-base-job-templates) for the specified work pool.

### Running a flow with custom dependencies

By default, infrastructure-bound flows run in the standard Prefect image, which only includes `prefect` and its dependencies. If your flow needs additional Python packages, you provide a custom Docker image with those packages installed and pass it via the `image` job variable.

First, build and push a Docker image that includes your dependencies:
...
Then pass that image in the decorator so the Kubernetes job runs your custom image instead of the default:
...
The `image` kwarg overrides the `{{ image }}` placeholder in the work pool's [base job template](/v3/advanced/customize-base-job-templates), so the pod runs your image with PyTorch & friends installed. `ml_pipeline` runs locally, and `train_model` runs in a Kubernetes job.

<Tip>
You can also set a default `image` directly on the work pool (via the UI or CLI) so that all flows submitted to that pool use your custom image automatically.
</Tip>
