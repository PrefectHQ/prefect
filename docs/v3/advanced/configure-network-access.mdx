...
The Prefect client uses [HTTPX2](https://pydantic.dev/docs/httpx2/) for HTTP requests.
HTTPX2 respects standard proxy environment variables, so you can route Prefect traffic through a
corporate proxy:

{/* pmd-metadata: notest */}
```bash
export HTTPS_PROXY=http://proxy.example.com:8080
export PREFECT_API_SSL_CERT_FILE=/path/to/corporate-ca-bundle.crt
export SSL_CERT_FILE=/path/to/corporate-ca-bundle.crt
```

Use `PREFECT_API_SSL_CERT_FILE` for orchestration requests and `SSL_CERT_FILE` for other HTTPX2
requests, such as Cloud account operations. See the [HTTPX2 migration guide](/v3/how-to-guides/migrate/migrate-to-httpx2)
for certificate defaults and custom client settings.
