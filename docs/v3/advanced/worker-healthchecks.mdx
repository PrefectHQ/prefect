...
keywords: ["health check", "worker", "monitoring", "liveness", "readiness"]
---
...
When enabled, the worker binds to `0.0.0.0:8080` by default (all interfaces). From the same machine, you can reach it at:
...
and its health endpoint will return 503 (Service Unavailable). With default settings, a worker is unhealthy if it hasn't polled in 300 seconds (5 minutes).
...
- `PREFECT_WORKER_QUERY_SECONDS`: How often workers poll for new flow runs (default: 10)
- `PREFECT_WORKER_PREFETCH_SECONDS`: How far in advance to submit flow runs (default: 10)
- `PREFECT_WORKER_WEBSERVER_HOST`: Health server host (default: 0.0.0.0)
