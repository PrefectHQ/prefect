...
If you enable result persistence and don't specify a filesystem block, Prefect resolves the default
storage location in this order:

1. The `PREFECT_DEFAULT_RESULT_STORAGE_BLOCK` setting, if configured
2. A server-level default result storage block, if one is configured on your Prefect server
3. Local filesystem at `~/.prefect/storage/` (fallback)

When a server-level default result storage block is configured on the Prefect server, Prefect also
automatically enables result persistence — you don't need to set `PREFECT_RESULTS_PERSIST_BY_DEFAULT`
or `persist_result=True` explicitly.

You can configure the local fallback path through the `PREFECT_LOCAL_STORAGE_PATH` setting.
...
You can specify a default filesystem using the `PREFECT_DEFAULT_RESULT_STORAGE_BLOCK` setting.
Provide a block document slug to use that filesystem as the default for all flows and tasks.

```bash
prefect config set PREFECT_DEFAULT_RESULT_STORAGE_BLOCK='s3-bucket/my-prod-block'
```

Alternatively, a Prefect server administrator can configure a server-level default result storage block
via the admin API. Prefect uses this server default when no settings-based block is configured and the
current storage is local. See the [resolution order above](#default-persistence-configuration) for
the full priority sequence.

<Info>
Any explicit configuration of `result_storage` on a flow or task overrides both the settings-based
default and the server-level default.
</Info>
