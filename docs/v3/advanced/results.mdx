...
If you delete the block document, block schema, or block type that is configured as the server-side
default result storage, Prefect automatically clears the default. Subsequent flow runs that relied on
the default will no longer persist results unless you configure a new default or set `result_storage`
explicitly.
