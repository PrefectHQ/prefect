...
These requests are made with the [HTTPX2](https://pydantic.dev/docs/httpx2/) Python library.
HTTPX2 respects accepted proxy environment variables, so the Prefect client can communicate through proxies.

To enable communication through proxies, set `HTTPS_PROXY` in your execution environment.
If the proxy requires a custom CA bundle, set `PREFECT_API_SSL_CERT_FILE` for orchestration requests
and `SSL_CERT_FILE` for other HTTPX2 requests, such as Cloud account operations.
See [configure network access](/v3/advanced/configure-network-access) for an example.
