...
keywords: ["API key", "authentication", "token", "Prefect Cloud", "access"]
---
...
In an interactive terminal you can run `prefect cloud login` and paste the key when prompted.

In non-interactive environments (CI, containers, startup scripts), supply both `--key` and `--workspace`:

```bash
prefect cloud login --key '<my-api-key>' --workspace '<account-handle>/<workspace-handle>'
```
