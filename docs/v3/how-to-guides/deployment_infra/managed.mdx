...
keywords: ["managed infrastructure", "Prefect Cloud", "serverless", "hosted execution"]
---
...
## Access cloud providers without static credentials

Prefect Managed work pools support cloud workload identity federation to inject short-lived credentials into each run.
This lets pull steps and flow code access cloud services without storing static keys in Prefect Cloud.

See [Using cloud workload identity](/v3/how-to-guides/deployment_infra/managed-federated-identity) for setup instructions covering AWS and GCP.
