...
keywords: ["Kubernetes", "k8s", "container", "pod", "cluster", "helm"]
---
...
Number of times Kubernetes retries a job after its pod is terminated. Defaults to 0.

When set to 0, Kubernetes does not retry the job. Instead, the worker sets
`PREFECT_FLOW_RUN_EXECUTE_SIGTERM_BEHAVIOR=reschedule` in the pod's environment, so a
flow run whose pod receives a `SIGTERM` (for example, from an eviction, node drain, or
spot instance reclaim) moves to `AwaitingRetry` and starts again in a new pod.
An abrupt termination such as `OOMKilled` never reaches the `SIGTERM` handler, so with a
backoff limit of 0 the run ends as `Crashed` regardless of the variable's value.
Kubernetes still retries the job when the backoff limit is greater than 0.

`PREFECT_FLOW_RUN_EXECUTE_SIGTERM_BEHAVIOR` accepts the following values:

| Value | Behavior on `SIGTERM` |
| --- | --- |
| `reschedule` | The run is rescheduled and the worker starts a new pod. |
| `relinquish` | Prefect leaves the run state unchanged so a Kubernetes-retried pod can resume it. Requires Prefect 3.8.1 or later. |
| `crash` (or any other value) | The run ends as `Crashed` and no new pod starts. |

To keep the backoff limit at 0 but stop Prefect from rescheduling terminated runs, set
the variable yourself in **Environment Variables**:

```json
{"PREFECT_FLOW_RUN_EXECUTE_SIGTERM_BEHAVIOR": "crash"}
```

The worker only injects its own value when the variable is unset.
You can override this on each deployment through `job_variables`.

When set to a value greater than 0, Kubernetes owns retries. The worker sets the
variable to `relinquish` and Prefect leaves the run state unchanged so the retried pod
can pick it up.

<Note>
The worker began injecting `reschedule` in prefect-kubernetes 0.5.9 and started
respecting a preset `PREFECT_FLOW_RUN_EXECUTE_SIGTERM_BEHAVIOR` in prefect-kubernetes
0.6.6. The `relinquish` value and its use when the backoff limit is greater than 0
require Prefect 3.8.1 and prefect-kubernetes 0.7.12 or later.
</Note>
...
For comprehensive examples of customizing the base job template, see [Customizing Base Job Templates](/v3/advanced/customize-base-job-templates).
...
The Kubernetes worker's service account requires specific RBAC permissions to create
and manage flow run jobs. If you deployed the worker with the
[Prefect Helm chart](https://github.com/PrefectHQ/prefect-helm/tree/main/charts/prefect-worker),
these permissions are configured automatically for the worker's namespace. If the
worker submits jobs to a different namespace, create the Role and RoleBinding in each
target namespace and add those namespaces to
`PREFECT_INTEGRATIONS_KUBERNETES_OBSERVER_NAMESPACES` so the observer can detect
crashed flow runs there. If you manage RBAC entirely outside of Helm, apply the
permissions described below.

### Required permissions

At minimum, the worker needs a Role in each namespace where it creates jobs:
...
metadata:
  ...
  namespace: prefect
rules:
  # Monitor pod status and stream flow run logs
  - apiGroups: [""]
    resources: ["pods"]
  ...
  - apiGroups: [""]
    resources: ["pods/log"]
    verbs: ["get"]
  # Create, monitor, and clean up flow run jobs
  - apiGroups: ["batch"]
    resources: ["jobs"]
    verbs: ["get", "list", "watch", "create", "delete"]
```

Bind this Role to the worker's service account with a RoleBinding:
...
If you enable the worker to store the Prefect API key in a Kubernetes secret
(by setting `PREFECT_INTEGRATIONS_KUBERNETES_WORKER_CREATE_SECRET_FOR_API_KEY=true`),
add secret permissions to the Role:
...
If you enable the worker to store generated flow-run environment variables in a
per-job Kubernetes secret instead of embedding them directly in the Job manifest
(by setting `PREFECT_INTEGRATIONS_KUBERNETES_WORKER_STORE_ENV_AS_SECRET=true`),
add `create`, `patch`, and `delete` permissions on secrets to the Role:
...
The Kubernetes worker includes an observer that watches for pod and job events to
detect crashed flow runs and emit Prefect events. The observer is enabled by default.

By default, the Prefect Helm chart configures the observer to watch only the worker's
namespace. If you set `PREFECT_INTEGRATIONS_KUBERNETES_OBSERVER_NAMESPACES` to a
comma-separated list of namespaces, the observer watches those namespaces and
namespaced Roles and RoleBindings are sufficient.

To enable **cluster-wide mode** (by leaving
`PREFECT_INTEGRATIONS_KUBERNETES_OBSERVER_NAMESPACES` unset and not using the Helm
chart defaults), the observer requires a ClusterRole with additional permissions:
...
kind: ClusterRole
metadata:
  name: prefect-worker-observer
rules:
  # Watch jobs and pods across all namespaces
  - apiGroups: ["batch"]
    resources: ["jobs"]
...
Bind this ClusterRole with a ClusterRoleBinding:
...
To disable the observer entirely, set
`PREFECT_INTEGRATIONS_KUBERNETES_OBSERVER_ENABLED=false`. The worker still creates
and manages jobs, but it does not watch for pod events or automatically detect crashed
flow runs.

### Troubleshooting permission errors

If the worker's service account lacks the required permissions, you may see errors
such as:

{/* pmd-metadata: notest */}
```
403 Forbidden: User "system:serviceaccount:prefect:prefect-worker"
cannot create resource "jobs" in API group "batch" in the namespace "prefect"
```

To resolve this, verify that the Role and RoleBinding (or ClusterRole and
ClusterRoleBinding) are applied correctly and that the service account name matches
the one used by the worker pod. Check the current permissions with:
