...
these permissions are configured automatically. If you manage RBAC manually, apply the
...
When the observer runs in **cluster-wide mode** (the default when
`PREFECT_INTEGRATIONS_KUBERNETES_OBSERVER_NAMESPACES` is not set), it requires a
ClusterRole with additional permissions:
...
To reduce the scope of cluster-wide permissions, restrict the observer to specific
namespaces by setting
`PREFECT_INTEGRATIONS_KUBERNETES_OBSERVER_NAMESPACES=namespace1,namespace2`.
When namespaces are specified, namespaced Roles and RoleBindings in those namespaces
are sufficient and the ClusterRole is not needed.
