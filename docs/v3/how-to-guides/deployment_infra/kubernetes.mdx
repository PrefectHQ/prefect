...
## Base Job Template Examples

This section provides comprehensive examples for customizing the Kubernetes base job template. These examples demonstrate common configuration patterns for environment variables, secrets, resource limits, and image pull secrets.
...
For the complete default base job template structure, run:
```bash
