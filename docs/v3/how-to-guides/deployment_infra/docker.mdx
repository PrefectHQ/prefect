...
keywords: ["Docker", "container", "Docker work pool", "image", "Dockerfile"]
---
...
Since you are running this worker locally, the worker will stop if you close the terminal.
...
<Note>
The generated Dockerfile installs dependencies from `requirements.txt`, but not dependencies declared only in
`pyproject.toml`. If your project uses `pyproject.toml`, either install those dependencies in a custom Dockerfile,
generate a `requirements.txt` file for the generated Dockerfile, or enable Prefect's
[automatic dependency installation](/v3/how-to-guides/deployments/store-flow-code#python-project-dependencies-in-custom-images)
when code is pulled at runtime.
</Note>
...
**Default Docker namespace**
    ...
    You can set the `PREFECT_DEPLOYMENTS_DEFAULT_DOCKER_BUILD_NAMESPACE` setting to append a default Docker namespace to all images
    ...
    ```bash
    prefect config set PREFECT_DEPLOYMENTS_DEFAULT_DOCKER_BUILD_NAMESPACE=<docker-registry-url>/<organization-or-username>
    ```
    ...
    when `PREFECT_DEPLOYMENTS_DEFAULT_DOCKER_BUILD_NAMESPACE` is set.
</Tip>
...
You can override job variables, such as `image_pull_policy` and `image`, for a specific deployment with the `job_variables`
argument. Valid values for `image_pull_policy` are `IfNotPresent`, `Always`, `IfPossible`, and `Never`. `IfPossible` always attempts to
pull the image, but falls back to a local copy if the pull fails instead of raising an error.
