...
You can serve flow methods that are part of a class instance. This is useful when your flow needs to maintain state or access instance-specific configuration across multiple runs.

Here's an example of a rate limiter that tracks request counts and changes behavior based on accumulated state:
...
class RateLimiter:
    """Tracks and enforces rate limits across flow runs."""
    ...
    def process_request(self, request_id: str):
        """Process a request if within rate limits."""
        current_time = time.time()

        # Reset window if expired
        if current_time - self.window_start >= self.window_seconds:
            print(f"Rate limit window reset after {self.window_seconds}s")
            self.request_count = 0
            self.window_start = current_time

        # Check rate limit
        self.request_count += 1
        print(f"Request count: {self.request_count}/{self.max_requests}")

        if self.request_count <= self.max_requests:
            print(f"✓ Processing request {request_id}")
            return {"status": "success", "request_id": request_id}
        else:
            print(f"✗ Rate limit exceeded for request {request_id}")
            return {"status": "rate_limited", "request_id": request_id}


if __name__ == "__main__":
    limiter = RateLimiter(max_requests=100, window_seconds=60)
    limiter.process_request.serve(name="rate-limited-processor")
```

When serving an instance method, the flow maintains access to the instance's state and attributes during execution. In this example, the `request_count` and `window_start` attributes persist across flow runs, allowing the rate limiter to track requests and change behavior (accepting or rejecting requests) based on the accumulated state.
