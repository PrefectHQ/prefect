...
You can serve flow methods that are part of a class instance. This is useful when your flow needs to maintain state or access instance-specific configuration:
...
class MyFlowManager:
    ...
    @flow(log_prints=True)
    def my_flow(self, override_sleep_time: int | None = None):
        sleep_duration = override_sleep_time or self.sleep_time
        print(f"sleeping for {sleep_duration} seconds")
        # flow implementation here


if __name__ == "__main__":
    manager = MyFlowManager(sleep_time=30)
    manager.my_flow.serve(name="my-instance-flow")
```

When serving an instance method, the flow maintains access to the instance's state and attributes during execution.
