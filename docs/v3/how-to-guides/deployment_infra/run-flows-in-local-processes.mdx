...
Here's an example of a counter that tracks how many times it has been run:
...
class RunCounter:
    """Tracks state across flow runs."""

    def __init__(self):
        self.count = 0

    @flow(log_prints=True)
    def my_flow(self, name: str = "world"):
        """A flow that tracks how many times it has run."""
        self.count += 1
        print(f"Hello {name}!")
        print(f"This flow has run {self.count} time(s)")

        # Behavior changes based on state
        if self.count % 10 == 0:
            print(f"Milestone: {self.count} runs! 🎉")


if __name__ == "__main__":
    counter = RunCounter()
    counter.my_flow.serve(name="tracked-flow")
```

When serving an instance method, the flow maintains access to the instance's state during execution. The `count` attribute persists and updates across flow runs, demonstrating how instance methods can track state and change behavior over time.
