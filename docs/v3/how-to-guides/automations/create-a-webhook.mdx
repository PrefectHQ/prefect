---
title: Setup a webhook in the UI
description: Learn how to setup a webhook in the UI to trigger automations from external events.
---

Prefect Cloud webhooks allow you to trigger Prefect automations from any external system capable of sending an HTTP request. When an external system invokes your webhook's unique URL, Prefect transforms the incoming request into a Prefect [event](/v3/automate/events/) using a configurable template. This event can then trigger [automations](/v3/automate/events/automations-triggers/) to run deployments, send notifications, or perform other actions.

For a detailed explanation of webhooks, including advanced template configuration, authentication, and how Prefect processes incoming requests, please see the [Webhooks Concepts](/v3/concepts/webhooks) page.
