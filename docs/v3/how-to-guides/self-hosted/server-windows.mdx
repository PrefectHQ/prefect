...
keywords: ["Windows", "PowerShell", "self-hosted", "server", "worker"]
---
...
$env:PREFECT_SERVER_DATABASE_TIMEOUT = "600"

# Set permanently for user
[System.Environment]::SetEnvironmentVariable("PREFECT_SERVER_DATABASE_TIMEOUT", "600", "User")
```

<Note>
The `PREFECT_API_DATABASE_*` environment variable names (for example, `PREFECT_API_DATABASE_TIMEOUT`) are still accepted as legacy aliases, but the canonical names use the `PREFECT_SERVER_DATABASE_` prefix.
</Note>
...
```powershell
prefect config set PREFECT_SERVER_DATABASE_CONNECTION_URL="postgresql+asyncpg://postgres:password@localhost:5432/prefect"
```
