---
title: Run Prefect Server with Docker Compose
sidebarTitle: Prefect Server with Docker Compose
description: Deploy a self-hosted Prefect server environment using Docker Compose, including PostgreSQL, Redis, server, services, and worker.
---
...
services:
  ...
  prefect-worker:
    ...
    environment:
      PREFECT_API_URL: http://prefect-server:4200/api
    command: >
      bash -c "prefect worker start --pool local-pool"
...
* Redis is a required service that supports event messaging and worker communication. Messaging environment variables are omitted here for simplicity.
...
* Use `docker compose logs -f` to view live logs of all services.
