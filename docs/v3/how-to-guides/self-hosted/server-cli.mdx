...
keywords: ["self-hosted", "server", "CLI", "local", "prefect server start"]
---
...
A fresh browser opens the V2 UI by default. You can switch to the V1 UI from the sidebar in V2, and the V1 dashboard includes a banner to switch back. To restore V1 as the default for browsers without a saved preference, set `PREFECT_SERVER_UI_V2_ENABLED=false`.
...
```bash
prefect config set PREFECT_SERVER_DATABASE_CONNECTION_URL="postgresql+asyncpg://postgres:yourTopSecretPassword@localhost:5432/prefect"
```
...
2. **Redis for messaging, event ordering, and concurrency lease storage** — the in-memory defaults do not work across processes
...
# Configure Redis-backed messaging, event ordering, and lease storage
...
background services, see [How to scale self-hosted Prefect](/v3/advanced/self-hosted).
