...
When retries are configured, `on_running` hooks fire on **each retry attempt**, including the initial run. This matches flow retry behavior. For example, a task configured with `retries=2` will trigger its `on_running` hooks up to three times: once on the initial run and once for each retry attempt.
