...
You can control iframe permissions with `sandbox`. Prefect always applies a
fixed restrictive Content Security Policy (CSP) derived from the sanitized
`sandbox` settings:
...
Prefect applies a restrictive policy that:

- allows inline styles
- allows inline scripts only when `allow-scripts` is enabled
- allows images, fonts, and media from `data:` only
