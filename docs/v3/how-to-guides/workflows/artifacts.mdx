...
keywords: ["create_rich_artifact", "rich artifact", "html artifact", "iframe", "artifact lineage"]
---
...
### Create rich artifacts

Rich artifacts render trusted HTML content in a sandboxed iframe in the Prefect UI.
To create one, call `create_rich_artifact()`.
...
def create_validation_report():
    create_rich_artifact(
        key="daily-validation-report",
        html="""
<h2>Validation report</h2>
<p>Rows checked: <strong>12,540</strong></p>
<p>Failed checks: <strong>2</strong></p>
""",
        description="HTML summary of the latest data-quality checks.",
    )
...
You can also control iframe permissions with `sandbox` and apply a Content Security Policy (CSP) with the `csp` parameter:
...
@flow
def create_strict_rich_artifact():
    create_rich_artifact(
        key="strict-html-report",
        html="<h2>Strict report</h2><p>Inline script blocked by CSP.</p>",
        sandbox=[],
        csp="default-src 'none'; style-src 'unsafe-inline'; img-src https:",
    )


if __name__ == "__main__":
    create_strict_rich_artifact()
```

If you omit `sandbox`, Prefect uses `['allow-scripts']` by default.
Prefect only applies the `allow-scripts` and `allow-same-origin` sandbox tokens,
and it removes `allow-same-origin` when scripts are enabled.
If you omit `csp`, Prefect applies a restrictive default policy that:

- allows inline styles
- allows inline scripts only when `allow-scripts` is enabled
- allows images, fonts, and media from `https:` and `data:`
- blocks outbound connections, embedded frames, plugins, workers, base URI changes, and form submission
