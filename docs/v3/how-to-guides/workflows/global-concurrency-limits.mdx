...
If the limit does not exist or is inactive, non-strict mode (the default) logs a warning and execution continues without blocking. Use `strict=True` when you need concurrency enforcement to fail fast instead of silently proceeding.
</Note>
