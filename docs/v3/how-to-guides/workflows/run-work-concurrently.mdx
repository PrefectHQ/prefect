...
When running tasks concurrently, sometimes you might want all concurrent work to complete, even if some tasks fail.

Here are different approaches to run concurrent work to completion and handle failures:

<CodeGroup>
```python Using .map() with return_state
from prefect import flow, task
...
def batch_processing():
    ...
    # Get states instead of results
    states = process_data.map(items, return_state=True)
    ...
    for i, state in enumerate(states):
        if state.is_completed():
            successful.append(state.result())
        else:
            failed.append((items[i], state))
...
```python Using .submit() with return_state
from prefect import flow, task
...
def batch_processing():
    ...
    # Submit individually for more control
    states = []
    for item in items:
        state = process_data.submit(item, return_state=True)
...
```python Using allow_failure for dependent tasks
from prefect import flow, task
...
@task
def save_result(result_or_error) -> str:
    # With allow_failure, failed tasks pass their exception
    if isinstance(result_or_error, Exception):
        return f"Logged failure: {type(result_or_error).__name__}"
    return f"Saved: {result_or_error}"
...
def batch_processing():
    ...
    for item in items:
        # Submit processing task (might fail)
        future = process_data.submit(item)
        # Save result regardless of success/failure
        saved = save_result.submit(allow_failure(future))
...
<Note>
Use `.map()` for concurrent batch processing, `.submit()` for individual control, or `allow_failure` when downstream tasks must run regardless of upstream failures.
</Note>
