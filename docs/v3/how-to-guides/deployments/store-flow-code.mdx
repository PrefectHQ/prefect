...
Process workers detect the project root after deployment pull steps finish. Set `working_dir` only when the project
code already exists on the worker and no pull step selects its directory.
