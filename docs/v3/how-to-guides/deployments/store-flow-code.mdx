...
- The project is already present in the execution environment, such as code baked into a custom Docker image and selected with `set_working_directory`.
...
- The current Python environment already satisfies the active `[project]` dependencies that Prefect can verify from installed distribution names and versions. Dependencies with extras or direct URLs are handled by `uv run` unless another pre-materialized environment signal applies.
...
contains a project virtual environment, set `UV_PROJECT_ENVIRONMENT` or `VIRTUAL_ENV` in the image or work pool job
variables so Prefect can use that environment directly.
