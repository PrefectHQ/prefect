...
Process workers follow the same rules. The worker prepares the flow run's working directory first, so it detects a
`pyproject.toml` that a pull step retrieves at run time as well as one that is already present in the `working_dir`
job variable.
