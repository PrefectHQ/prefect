...
If the `pyproject.toml` declares `prefect` in its `[project]` dependencies, Prefect starts the flow run with
`uv run --no-dev --project <project-root>` so runtime dependencies are installed from the project environment.
Development dependency groups are not installed for flow runs.

If your custom image already contains the required project code and dependencies, disable automatic runtime dependency
installation by setting `PREFECT_RUNNER_AUTO_INSTALL_DEPENDENCIES=false` in the image or work pool job variables. With
this setting disabled, Prefect starts the flow run with the current Python environment instead of preparing dependencies
from `pyproject.toml`.
