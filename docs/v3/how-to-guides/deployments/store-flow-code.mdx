...
Process workers follow the same rules and detect the project root after deployment pull steps finish. This includes
projects retrieved at runtime by steps such as `git_clone`. When the project already exists on the worker, set the
`working_dir` job variable to the directory that contains your `pyproject.toml`.
