...
If you want Prefect to prepare the project's runtime dependencies before starting the flow run, set
...
1. `PREFECT_RUNNER_AUTO_INSTALL_DEPENDENCIES` is set to `true`.
2. A `pyproject.toml` exists at the detected project root.
3. The `dependencies` list under `[project]` in that `pyproject.toml` includes `prefect`.
4. `uv` is available on the runtime environment's `PATH`.
...
`PREFECT_RUNNER_AUTO_INSTALL_DEPENDENCIES` unset or set it to `false`.

Process workers follow the same rules. Project detection runs after any deployment pull steps complete, so a
`pyproject.toml` added by a pull step (for example, `git_clone`) is still found. If you don't use pull steps, set the
`working_dir` job variable to the directory that contains your `pyproject.toml` so the worker can detect the project.
