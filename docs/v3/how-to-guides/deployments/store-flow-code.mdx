...
When a deployment pulls flow code into an execution environment and no explicit flow run command is configured, Prefect looks for a `pyproject.toml` in the prepared workspace.
If the `pyproject.toml` declares `prefect` in its `[project]` dependencies, Prefect can start the flow run with `uv run --project <project-root>` so dependencies are available from the project environment.

Prefect skips `uv run` when it detects an explicitly pre-built Python environment for the prepared workspace:
