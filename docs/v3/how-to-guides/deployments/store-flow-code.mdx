...
This approach is recommended for organizations that use **GitHub Enterprise with SAML/SSO**,
because it removes the need for Personal Access Tokens (PATs) that must be individually
...
| **Credential storage** | No long-lived credentials are stored in Prefect Cloud or in your deployment configuration |
