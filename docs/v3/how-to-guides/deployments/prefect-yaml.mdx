...
keywords: ["prefect.yaml", "YAML", "deployment definition", "configuration file"]
---
...
the provided name and tag. To publish the image to a registry, use a separate
`prefect_docker.deployments.steps.push_docker_image` step.
...
<Note>
**Omitted `pull` versus empty `pull`**

    An omitted or `null` `pull` section causes `prefect deploy` to auto-generate a default pull
    action (a `set_working_directory` step pointing at the deploying machine's current working
    directory). If your worker prepares the flow's code itself (for example, a custom job command
    that clones code at runtime), set `pull: []` to explicitly disable pull steps so no
    `set_working_directory` step is generated.
</Note>
...
Block references can also be embedded inline within a larger string value, for example:
`"{{ prefect.blocks.secret.my-registry }}/my-image"`. The resolved block value is coerced to a string;
non-scalar values such as dicts and lists cannot be used for inline substitution
...
Pull steps run before Prefect imports the module, so you can use pull steps such as `git_clone`
to clone your repository and make the package importable. This is also useful when your flow is
part of a package pre-installed in the execution environment.
...
<Note>
    **CLI options when deploying multiple deployments**
    ...
    `PREFECT_DEPLOYMENTS_DEFAULT_WORK_POOL_NAME`](/v3/api-ref/settings-ref#default_work_pool_name), is still applied to deployments whose `prefect.yaml` entry
...
In the above example, YAML aliases reuse work pool, schedules, and build configuration across multiple deployments:

- `deployment-1` and `deployment-2` use the same work pool configuration
- `deployment-1` and `deployment-3` use the same schedules
...
| `schedules`                                | An optional list of [schedules](/v3/how-to-guides/deployments/create-schedules) to assign to the deployment. Fields for each schedule are documented in the [Schedule Fields](#schedule-fields) section.                                                                                                                      |
...
These are fields you can add to each entry in a deployment declaration's `schedules` list.
...
<Note>
**Removing schedules from a deployment**

Removing the `schedules` key from your `prefect.yaml` file, or setting `schedules: null` or `schedules: []`, does **not** remove existing schedules from a deployment when you run `prefect deploy`. Prefect treats the absence of schedule configuration as "no change" rather than "delete all schedules."

To explicitly remove schedules from an existing deployment, use the CLI:
...
prefect deployment schedule delete <flow-name>/<deployment-name> <schedule-id> -y
```
</Note>
...
| `collision_strategy`                       | Configure the behavior for runs once the [concurrency limit](/v3/concepts/deployments#concurrency-limiting) is reached. Options are `ENQUEUE` and `CANCEL_NEW`. Defaults to `ENQUEUE`.                               |
