...
keywords: ["deploy", "Python", "SDK", "work pool", "programmatic"]
---
...
The `entrypoint` parameter is either the path to the flow function within your repository combined with the name of the flow function (for example, `path/to/flow.py:my_func`), or a Python module path (for example, `my_module.my_flow.my_func`).
