---
title: Manage schedules in bulk
sidebarTitle: Bulk Schedule Management
description: Learn how to pause and resume deployment schedules in bulk using the Prefect CLI.
---

When managing multiple deployments with active schedules, you may need to pause or resume all schedules at once. 
...
- An existing [Prefect server or Prefect Cloud](/v3/deploy/infrastructure-concepts/deploy-to-cloud) workspace
...
- The [Prefect CLI](/v3/install-prefect) installed
...
This command will:
1. Query all deployments in your workspace
2. Count the number of active schedules
3. Prompt for confirmation in interactive mode (bypass with `prefect --no-prompt`)
4. Pause each active schedule
5. Display progress as it pauses each deployment's schedules
...
This command will:
1. Query all deployments in your workspace
2. Count the number of inactive schedules
3. Prompt for confirmation in interactive mode
4. Resume each paused schedule
5. Display progress as it resumes each deployment's schedules
...
Use in automation scripts with the `--no-prompt` flag:

```bash
#!/bin/bash
# pause_all_schedules.sh

echo "Pausing all deployment schedules..."
prefect --no-prompt deployment schedule pause --all

if [ $? -eq 0 ]; then
    echo "Successfully paused all schedules"
else
    echo "Failed to pause schedules"
    exit 1
fi
```
...
You can still manage individual deployment schedules as before:
...
For workspaces with hundreds of deployments, bulk operations may take some time to complete.
The CLI displays progress as it processes each deployment to keep you informed.
</Note>
