...
description: Override job variables on a deployment or flow run to set environment variables, specify a Docker image, allocate resources, and more.
---
...
Individual deployments can override the shared command. For example, to pin a different
Python version when a dependency doesn't yet support the latest release:
...
Here `my-legacy-flow` runs on Python 3.13 (downloaded on-demand by `uv`) while
other deployments use the image's default.
