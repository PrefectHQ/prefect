...
keywords: ["Airflow", "migration", "DAG", "migrate from Airflow", "comparison"]
---
...
3. Run `prefect deploy -n <name>` to create or update the deployment, or use `prefect deploy --all` to deploy every entry in `prefect.yaml` at once.
