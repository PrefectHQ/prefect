...
Data engineers want to separate **what** a pipeline computes from **where** it stores results. This separation enables:

- Swapping storage backends between environments (local files in dev, S3 in prod)
- Consistent serialization without boilerplate in every function
- Easier testing by mocking storage

Dagster solves this with I/O Managers that automatically handle reads and writes. Prefect solves it with **result persistence**—configuring where and how task/flow outputs are stored.
...
def cleaned_orders(raw_orders):
    # I/O manager handles reading raw_orders and writing output
...
@task(
    persist_result=True,
    result_storage="s3-bucket/prod-results",  # Block reference
    result_serializer="json",
)
def cleaned_orders(raw_orders: list[dict]) -> list[dict]:
    return [o for o in raw_orders if o["amount"] > 0]

@flow(result_storage="s3-bucket/prod-results")
def pipeline():
...
Set a default storage backend for all flows:

```bash
prefect config set PREFECT_DEFAULT_RESULT_STORAGE_BLOCK='s3-bucket/prod-results'
```

The key difference: Dagster I/O managers handle both reads and writes automatically. Prefect result persistence handles output storage—you still read inputs explicitly.

See [result persistence](/v3/advanced/results) for configuration options.

## Partitions

Data engineers partition data by time, region, or other dimensions to process incrementally and track what's been computed. Dagster has first-class partition support with state tracking. Prefect uses parameters and `.map()` for similar patterns.
...
@task
def process_orders_for_date(partition_date: str) -> dict:
    return {"date": partition_date, "order_count": 42}

@flow
def daily_orders(partition_date: str):
    return process_orders_for_date(partition_date)

@flow
def backfill_orders(start: str, end: str):
    ...
    return process_orders_for_date.map(dates)

if __name__ == "__main__":
    daily_orders("2024-06-15")
    backfill_orders("2024-01-01", "2024-01-07")
```
</CodeGroup>
