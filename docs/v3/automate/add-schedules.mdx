...
You can add one or more schedules to any [served or deployed](deploy/run-flows-in-local-processes) flow.
...
[`serve` method](develop/write-flows/#serving-a-flow) of the `Flow` object or
[the `serve` utility](develop/write-flows/#serving-multiple-flows-at-once) for managing multiple flows simultaneously
- If using [worker-based deployments](deploy/infrastructure-concepts/workers/)
...
See the [Settings docs](manage/settings-and-profiles/) for more information on altering your settings.
