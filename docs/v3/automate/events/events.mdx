...
In Prefect Cloud, events power [audit logs](manage/cloud/manage-users/audit-logs/).

Events enable observability into your data stack through the [event feed in the UI](#events-in-the-cloud-ui) and the 
configuration of Prefect's reactivity through [automations](automate/events/automations-triggers/).
...
Prefect Cloud offers [programmable webhooks](automate/events/webhook-triggers/) to receive HTTP requests 
from other systems and translate them into events within your workspace. 
Webhooks can emit [pre-defined static events](automate/events/webhook-triggers/#static-webhook-events), 
dynamic events that [use portions of the incoming HTTP request](automate/events/webhook-triggers/#dynamic-webhook-events), or events derived from [CloudEvents](automate/events/webhook-triggers/#accepting-cloudevents).
...
From an event page, you can configure an [automation](automate/events/automations-triggers/) to trigger on 
...
Advanced configuration is possible through [custom triggers](automate/events/custom-triggers/).
