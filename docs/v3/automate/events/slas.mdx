...
A Time to Completion SLA is triggered when a flow run takes longer than the specified duration to complete. This SLA is useful for monitoring long-running flows and ensuring they complete within an expected timeframe.

The SLA requires one unique parameter:
- `duration`: The maximum allowed duration in seconds before the SLA is violated

For example, if you set a Time to Completion SLA on your deployment with a `duration` of 600 seconds (10 minutes) the backend will emit an `prefect.sla.violation` event if a flow run takes longer than 10 minutes to complete.

### Frequency SLA

A Frequency SLA helps ensure that your deployments run at expected intervals.

The SLA requires one unique parameter:
- `stale_after`: The maximum allowed time between flow runs (e.g. 1 hour for hourly jobs)

For example, if you have a deployment that should run every hour, you can set up a Frequency SLA with a `stale_after` of 1 hour - this will trigger if more than an hour passes between flow runs.

### Lateness SLA

A Lateness SLA is triggered when a flow run is not started when there are flow runs scheduled.
