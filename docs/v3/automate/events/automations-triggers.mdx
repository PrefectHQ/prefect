...
You can define your own custom trigger to fire based on a custom [event](automate/events/custom-triggers/) defined in Python code.
With Prefect Cloud you can even create [webhooks](automate/events/webhook-triggers/) that can receive data for use in actions.
...
[grammar](automate/events/events/#event-grammar). Each trigger definition may include a [jinja template](https://en.wikipedia.org/wiki/Jinja_(template_engine))
...
- To learn more about Prefect events, which can trigger automations, see the [events docs](automate/events/events/).
- See the [webhooks guide](automate/events/webhook-triggers/)
