...
```python
collect_task_run_inputs_sync(expr: Any, future_cls: Any = PrefectFuture, max_depth: int = -1) -> set[TaskRunInput]
```
...
```python
propose_state_sync(client: 'SyncPrefectClient', state: State[Any], force: bool = False, task_run_id: Optional[UUID] = None, flow_run_id: Optional[UUID] = None) -> State[Any]
```
...
```python
get_state_for_result(obj: Any) -> Optional[State]
```
...
```python
link_state_to_result(state: State, result: Any) -> None
```
