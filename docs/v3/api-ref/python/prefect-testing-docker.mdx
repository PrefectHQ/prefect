---
title: docker
sidebarTitle: docker
---

# `prefect.testing.docker`

## Functions

### `capture_builders` <sup><a href="https://github.com/prefecthq/prefect/blob/main/src/prefect/testing/docker.py#L9" target="_blank"><Icon icon="github" size="14" /></a></sup>

```python
capture_builders() -> Generator[list[ImageBuilder], None, None]
```


Captures any instances of ImageBuilder created while this context is active

