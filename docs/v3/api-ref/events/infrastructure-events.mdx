...
| `kubernetes.diagnosis` | Stable, machine-matchable failure category (only when the pod's status matches a known failure condition) |

`kubernetes.diagnosis` is one of: `ImagePullError`, `CrashLoopBackOff`, `OOMKilled`,
`Evicted`, `Unschedulable`, `Unschedulable.InsufficientResources`,
`Unschedulable.NodeAffinity`, or `Unschedulable.Taint`. Match on this label in an
automation trigger to react to a specific failure mode rather than parsing the
pod phase or event payload.
