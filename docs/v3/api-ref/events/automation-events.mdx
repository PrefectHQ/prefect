...
description: Events emitted when automations are created, updated, deleted, trigger, resolve, and execute actions.
keywords: ["reactive", "proactive", "action lifecycle", "trigger state", "lifecycle", "CRUD"]
---

Automation events cover two categories: CRUD lifecycle events emitted when
automations are created, updated, or deleted, and trigger lifecycle events
emitted when an automation fires and executes actions. For more on automations,
see [Automations](/v3/concepts/automations).
...
The full automation serialized as JSON, including `id`, `name`, `description`,
`enabled`, `trigger`, `actions`, `actions_on_trigger`, and
`actions_on_resolve`.
...
Same as [`prefect.automation.created`](#prefectautomationcreated).
