...
keywords: ["flow", "workflow", "decorator", "@flow", "orchestration", "subflow"]
---
...
<Note>
Flow run parameters cannot exceed 512 KB in size by default. You can adjust this limit with the `PREFECT_SERVER_API_MAX_PARAMETER_SIZE` setting (in bytes). Set it to `0` to disable the limit.
...
These containers may be nested, for example, a list of the results of several `.map()`
calls. All nested states are considered when determining the final state.
