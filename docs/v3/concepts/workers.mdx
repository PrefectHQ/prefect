...
keywords: ["worker", "work pool", "polling", "execution", "infrastructure", "agent"]
---
...
To run a worker without occupying your terminal, connect to Prefect Cloud or set `PREFECT_API_URL`, then start it in the background:

```bash
prefect worker start --pool "my-pool" --background
```

Output is written to `worker.log` in your Prefect home.

For long-running workers, use a [process manager](/v3/advanced/daemonize-processes/).
...
scheduled start time. While it waits, the run holds any
[deployment concurrency slot](/v3/concepts/deployments#concurrency-limiting) it acquired at submission, as long as it
reaches `Running` within the deployment's `grace_period_seconds`.
