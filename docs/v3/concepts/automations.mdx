...
If you want Telegram notifications, you can use the **Custom Webhook** notification block. This avoids
adding a dedicated Telegram block and works with the standard Telegram Bot API.
...
- `parse_mode` is optional; remove it if you don’t want Markdown formatting.

Alternatively, you can create the block in Python:
...
Once saved, this block will appear in the UI notification options. When the automation
fires, Prefect will populate `{{ subject }}` and `{{ body }}` with the notification details.
