...
keywords: ["artifact", "markdown", "table", "result", "UI", "report"]
---
...
- come in standardized formats such as tables, progress indicators, images, Markdown, links, and rich HTML content
...
- images
- tables
- rich HTML (rendered in a sandboxed iframe)

Rich HTML artifacts are sandboxed executable content. They are intended for
trusted HTML, CSS, and JavaScript that you want to preview in the Prefect UI.

<Note> 
Each artifact created within a task is displayed individually in the Prefect UI.
This means that each call to helpers such as `create_link_artifact()`, `create_markdown_artifact()`, or `create_rich_artifact()` generates a distinct artifact.
...
To create artifacts such as reports or summaries using `create_markdown_artifact()` or `create_rich_artifact()`,
define your content string and pass it to the corresponding helper.
</Note>
...
For rich HTML examples, see [Create rich artifacts](/v3/how-to-guides/workflows/artifacts/#create-rich-artifacts).
