...
keywords: ["settings", "profile", "configuration", "environment variable", "PREFECT_API_URL"]
---
...
Immediately after installation, the `ephemeral` profile will be used, which only has `PREFECT_SERVER_EPHEMERAL_ENABLED` configured:

<Tip>
**What is `PREFECT_SERVER_EPHEMERAL_ENABLED`?**
