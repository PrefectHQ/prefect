...
keywords: ["state", "status", "pending", "running", "completed", "failed", "cancelled", "crashed"]
---
...
| `Submitting` | `PENDING` | No | A worker or runner is submitting the run to its infrastructure. |
| <span class="no-wrap">`InfrastructurePending`</span> | `PENDING` | No | The run's infrastructure has been created but is not running yet (for example, a Kubernetes pod that is still pending scheduling). |
...
| ✅ | Pending | Submitting | Worker or runner | The run is being submitted to its infrastructure. |
| ✅ | Submitting | InfrastructurePending | Worker | The infrastructure was created and is waiting to start (for example, a pending Kubernetes pod). |
...
flowchart TD
    %% Style definitions
    classDef scheduled fill:#fcd14edb,stroke:#fcd14edb,color:#000
    ...
    subgraph pending_type[Pending]
    Submitting[Submitting]:::pending
    InfrastructurePending[InfrastructurePending]:::pending
    end
    ...
    style scheduled_type fill:none
    style scheduled_type2 fill:none
    style running_type fill:none
    ...
    Pending --> |Submitting run to infrastructure| Submitting
    Submitting --> |Infrastructure created, not yet running| InfrastructurePending
    ...
    Submitting --> |Preconditions met| Running
    InfrastructurePending --> |Infrastructure starts| Running
