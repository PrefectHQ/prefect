...
Because workers submit runs ahead of their scheduled start (see [prefetch](/v3/concepts/workers#configure-prefetch)),
a slot can be held for the prefetch window plus the run's duration. With `CANCEL_NEW`, a run whose deployment slot is
held when the worker submits it is cancelled before it starts, so the schedule interval should be longer than the
prefetch window plus the expected run time. The slot is only guaranteed to be held until the run starts if the prefetch
window is shorter than `grace_period_seconds`.
