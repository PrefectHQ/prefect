...
keywords: ["deployment", "remote execution", "schedule", "trigger", "serve", "deploy", "naming"]
---
...
- **`entrypoint`**: the entrypoint of a deployment is a reference to a function decorated as a
flow that exists on some filesystem. Entrypoints support two formats:
    - **File path format**: a path to the file and function name separated by a colon, relative to the `path`
    (for example, `path/to/file.py:function_name`).
    - **Module path format**: a fully-qualified Python module path to the flow function
    (for example, `my_module.my_flow.my_func`). When using module paths, the module must be importable
...
Flow names describe **what** a workflow does; deployment names describe **how, where, or when** it runs.
Because every deployment is referenced as `{FLOW_NAME}/{DEPLOYMENT_NAME}`, choose deployment names that
add context beyond what the flow name already provides.

#### Encode what varies between deployments

If you deploy the same flow to multiple environments, regions, or schedules, reflect that difference in the deployment name:

{/* pmd-metadata: notest */}
```python
# Environment-based names
my_flow.serve(name="staging")
my_flow.serve(name="production")

# Schedule-based names
my_flow.deploy(name="hourly-refresh", work_pool_name="my-pool")
my_flow.deploy(name="nightly-full-sync", work_pool_name="my-pool")

# Region or team-based names
my_flow.deploy(name="us-east-analytics", work_pool_name="my-pool")
my_flow.deploy(name="eu-west-compliance", work_pool_name="my-pool")
```

These names produce clear, scannable entries in the CLI and UI:

{/* pmd-metadata: notest */}
```text
customer-data-pipeline/hourly-refresh
customer-data-pipeline/nightly-full-sync
invoice-processor/us-east-analytics
invoice-processor/eu-west-compliance
```

#### Avoid generic names

Names like `prod`, `default`, or `main` become hard to distinguish when you have many flows.
A deployment view filled with identically named entries provides no useful context at a glance:

{/* pmd-metadata: notest */}
```text
# Hard to scan — every deployment has the same name
customer-data-pipeline/prod
invoice-processor/prod
risk-model-training/prod
```

Instead, use names that describe the deployment's purpose or configuration:

{/* pmd-metadata: notest */}
```text
customer-data-pipeline/daily-sync
invoice-processor/us-east
risk-model-training/weekly-retrain
```

<Tip>
If you only have one deployment per flow today, choose a name that describes its schedule or
target environment rather than a label like `default`. As your project grows, descriptive names
prevent confusion when more deployments are added.
</Tip>

#### Naming conventions for teams

When multiple teams share a Prefect workspace, establish a consistent naming convention.
Common patterns include:

- **`{environment}`** — `staging`, `production`
- **`{environment}-{variant}`** — `production-hourly`, `staging-backfill`
- **`{region}-{environment}`** — `us-east-production`, `eu-west-staging`

Pick one pattern and apply it across your organization so that deployment names remain predictable
and filterable.
...
A flow run takes a concurrency slot when it transitions to `Pending`, which is when a worker submits it, and releases
the slot when it leaves `Pending`, `Running`, or `Cancelling` for any other state, for example when it completes,
fails, or is cancelled. A run that retries in-process keeps its slot during the retry delay.

A slot is therefore held from submission until the run finishes, not only while the run is `Running`. Workers submit
runs ahead of their scheduled start (see [prefetch](/v3/concepts/workers#configure-prefetch)), and infrastructure
can take time to start, so a slot can be held for the prefetch window (or the infrastructure startup time, whichever
is longer) plus the run's duration. With `CANCEL_NEW` and a `concurrency_limit` of 1, a run whose deployment slot is
held when it is submitted is cancelled before it starts, so the schedule interval should be longer than this whole
period. A run must reach `Running` within `grace_period_seconds` of acquiring its slot; otherwise the slot can be
taken by another run. Deployments served with `flow.serve()` do not use the worker setting; the runner submits runs 10
seconds before their scheduled start.
