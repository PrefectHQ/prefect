...
<Note>
In `prefect.yaml`, `collision_strategy` and `grace_period_seconds` are nested under the `concurrency_limit`
field alongside `limit`. The CLI decomposes these into the separate `concurrency_limit` and
`concurrency_options` fields shown above when sending the deployment to the API. See the
[`prefect.yaml` concurrency limit fields](/v3/how-to-guides/deployments/prefect-yaml#concurrency-limit-fields)
