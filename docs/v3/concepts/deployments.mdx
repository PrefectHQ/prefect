...
A flow run acquires a deployment concurrency slot when it enters `Pending`, before its infrastructure starts.
It keeps that slot while `Pending`, `Running`, or `Cancelling`, and releases it when it transitions outside those
three states, for example when it completes, fails, or is cancelled.

Workers [prefetch runs](/v3/concepts/workers#configure-prefetch) ahead of their scheduled start, so even a short flow
can occupy a slot long enough to overlap the next submission. Allow for the prefetch window or infrastructure
startup time, whichever is longer, plus the run's duration. With `CANCEL_NEW` and a `concurrency_limit` of 1,
the schedule interval should exceed this whole period to avoid cancelling the next run before it starts.

<Note>
In-process retries keep the slot during the retry delay.

A run must reach `Running` within `grace_period_seconds` of acquiring its slot to keep it reserved; otherwise,
another run can take the slot.
</Note>

Deployments served with `flow.serve()` use a runner that prefetches runs up to 10 seconds ahead of their scheduled
start. The worker's `PREFECT_WORKER_PREFETCH_SECONDS` setting does not configure this runner.
