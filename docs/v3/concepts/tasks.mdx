...
which means methods like `__post_init__` or `__init__` run again on the
reconstructed copy.

This can produce unexpected behavior if an initializer has side effects or
...
However, if `__post_init__` unconditionally mutates state or has other side
effects, the second invocation can produce incorrect results.
