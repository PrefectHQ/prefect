...
- `TASK_SOURCE`: this cache policy uses the task's raw source plus a definition-time snapshot of hashable captured closure values and referenced non-callable module globals. Later mutation does not change task identity. Nested/helper source, modules, callables, unhashable resources, and masked secrets are not reliable distinguishing inputs.
