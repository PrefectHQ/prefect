...
- `TASK_SOURCE`: this cache policy uses raw lines of task code plus a definition-time snapshot of hashable closure values and referenced non-callable module globals. Later mutation does not change the task's identity. Referenced helpers and modules are excluded, so changes to nested source do not invalidate the key. Unhashable resources and masked secrets may not distinguish otherwise identical tasks.
