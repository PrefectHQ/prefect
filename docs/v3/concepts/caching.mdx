---
title: Caching
keywords: ["cache", "caching", "cache key", "result caching", "memoization", "cache policy"]
---
...
- `TASK_SOURCE`: this cache policy uses raw lines of task code plus a definition-time snapshot of hashable closure values and referenced non-callable module globals. Later mutation does not change the task's identity, and names unresolved at definition time are ignored. Referenced helpers and modules are excluded, so changes to nested source do not invalidate the key. Opaque objects, unhashable resources, and masked secrets may not distinguish otherwise identical tasks.
