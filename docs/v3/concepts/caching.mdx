...
Values captured by the task function's closure are also included, so tasks created by a factory with identical source but different captured values do not share a cache entry.
