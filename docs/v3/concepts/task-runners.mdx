...
For CPU-intensive tasks that benefit from process-isolated execution, use the [`ProcessPoolTaskRunner`](/v3/api-ref/python/prefect-task_runners#processpooltaskrunner),
...
```python {12} title="use processes to run tasks in isolation"
from prefect import flow, task
from prefect.futures import wait
