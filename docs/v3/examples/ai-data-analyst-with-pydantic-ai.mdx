...
<Accordion title="Logging Tool Calls to Prefect Logs">

If you want tool call arguments and results to appear in Prefect's native flow logs, you can use an `event_stream_handler` to capture tool events and log them with Prefect's logger:
...
async def log_tool_calls(
    ...
    """Event stream handler that logs tool calls to Prefect."""
    ...
    async for event in events:
        if isinstance(event, FunctionToolCallEvent):
            logger.info(
                ...
                f"with args: {event.part.args_as_json_str()}"
            )
...
This produces Prefect logs like:
...
- `FunctionToolCallEvent`: Fired when a tool is about to be called. Access `event.part.tool_name` and `event.part.args_as_json_str()` for tool details.
- `FunctionToolResultEvent`: Fired when a tool returns. Access `event.result.tool_name` and `event.result.content` for the result.
