...
In many cases, there is an informative stack trace in Prefect's [logs](develop/logging/). 
...
[Service accounts](manage/cloud/manage-users/service-accounts/) (sometimes referred to as bots),  
...
Actors can be members of [workspaces](manage/cloud/workspaces/). An actor attempting an action in a 
...
an actor has a [role](manage/cloud/manage-users/manage-roles/) which grants them certain permissions. 
...
through a [deployment](deploy/infrastructure-examples/docker/) (optionally on different infrastructure).
