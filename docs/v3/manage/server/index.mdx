...
If your PostgreSQL enabled [Certificate Authentication](https://www.postgresql.org/docs/current/auth-cert.html):

1. Update `PREFECT_API_DATABASE_CONNECTION_URL` without password, for example

```bash
prefect config set PREFECT_API_DATABASE_CONNECTION_URL="postgresql+asyncpg://postgres@localhost:5432/prefect"
```

2. Enable TLS and specify certification path information by
