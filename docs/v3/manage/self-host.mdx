...
either [Prefect Cloud](manage/cloud/) or a self-hosted Prefect server, backed by a database and a UI.
...
See the [Prefect Cloud overview](manage/cloud/) for more information.
...
You can save the API server address in a [Prefect profile](manage/settings-and-profiles/). Whenever that profile is 
active, the API endpoint is at that address.

See [Profiles and configuration](manage/settings-and-profiles/) for more information on profiles and configurable Prefect settings.
