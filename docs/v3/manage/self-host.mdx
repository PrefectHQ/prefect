...
### Running a server in a Docker container

If you're running a server in a Docker container, you will need to:
...
```bash
docker run -p 4200:4200 --rm -d prefecthq/prefect:3-latest -- prefect server start --host 0.0.0.0
```

<Tip>
The `--rm` flag will remove the container when it is stopped and the `-d` flag will run the container in detached mode (i.e. in the background).
</Tip>

Read more about Docker networking [here](https://docs.docker.com/network/network-tutorial-host/).
