---
title: Resilient pipelines and serverless infrastructure
...
Here you'll experiment with adding failure handling and data quality checks to the flow, and deploy it with Prefect Cloud's [Managed Execution work pool](https://docs.prefect.io/v3/deploy/infrastructure-examples/managed#run-flows-on-prefect-managed-infrastructure).
...
In order to run the pipeline, you'll need:
...
If you're missing any of the AWS and MotherDuck resources, you can create them by following the steps in the [S3 and MotherDuck tutorial](/docs/v3/tutorials/s3-motherduck).

### Setup the environment

Complete code examples can be found in [this GitHub repository](https://github.com/PrefectHQ/dev-day-zoom-out).
...
Prefect offers a variety of ways to handle failures in your pipelines. In this section, you'll have the opportunity to explore a few different retry strategies.
...
This example shows how to use the `@task` decorator to apply a simple retry strategy to the `get_recent_games` task.

```python
#Replace the existing get_recent_games task with this one.
...
All that's needed is an additional `retry_delay_seconds` utility in the `@task` decorator to implement this retry strategy.
...
This means that each subsequent retry attempt waits longer than the previous one. In Prefect, this can be implemented by adding the `exponential_backoff` utility to the `@task` decorator.
...
This function will take three arguments: `task`, `task_run`, and `state`. It will return a boolean value indicating whether to retry the task.


```python
#Add this code snippet to the mlb_flow.py file.
...
Afer defining the handler, you can add it to the `@task` decorator using the `retry_condition_fn` parameter.
...
It contributes to improved runtime performance and autonomous task execution. Transactions also allow you to leverage on-commit and on-rollback hooks to handle side effects and failures.
...
```python
# Replace the existing mlb_flow function with this one.
@flow
...
In this section, you'll deploy the MLB flow to serverless infrastructure. That way you don't have to keep your laptop on 24/7 if you'd like to run this flow on a schedule.
...
Alternatively, you can create a new work pool in the Prefect Cloud UI by navigating to the Work Pools page.
...
The `.deploy()` method enables you to define a deployment programatically in Python code.
...
You now have a new deployment in Prefect Cloud! You can find it by navigating to the Deployments page in the Prefect Cloud UI.
...
One of the most commonly used types of schedules are cron schedules, which is used for scheduling workflows at specific times, days, or months.
...
It's worth noting that cron isn't the only schedule type available in Prefect. You can use `interval` and `rrule` schedules as well.

Interval schedules are useful for running flows at specific intervals, such as every 10 minutes.
```python
# Run every 10 minutes.
interval=timedelta(minutes=10)
```

For more complex recurrence patterns, Prefect supports RRule schedules. 
This enables you to run workflows on more intricate schedules. For example, running workflows every Monday, Wednesday, and Friday of the month at 4:00 PM.
```python
...
If you'd like to add a schedule to a deployment using the Prefect Cloud UI, you can navigate to the Deployment page and click the "Add Schedule" button.

<iframe 
width="560" 
height="315" 
src="https://demo.arcade.software/XYwssAx9RdPJ1EcPcsCd?embed&embed_mobile=tab&embed_desktop=inline&show_copy_link=true" 
title="Arcade player" 
frameborder="0" 
allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" 
referrerpolicy="strict-origin-when-cross-origin" 
...
You can continue your exploration of Prefect by checking out the different [work pools](/v3/deploy/infrastructure-concepts/work-pools) to deploy your flows to Docker, Kubernetes, or your favorite cloud provider.
If you're interested in building more workflows, check out the [machine learning tutorial](/v3/tutorials/ml#provision-cloud-resources) to learn how to build a machine learning pipeline!
