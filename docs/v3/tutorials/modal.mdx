...
You'll only pay for only the CPU time you use. 
...
Create a GitHub repo containing your Prefect flows.
...
1. Initialize the Prefect environment:
   ...
   On the first question, select `git` as the source control system.

   ![image.png](/v3/img/tutorials/modal/modal%201.png)

1. Because this is meant to be a scalable repo, create a submodule just for your workflows.
...
The important sections here are `pull`, which defines the repo and branch to pull from, as well as `deployments`, which define the `flows` that will be `run` at some `schedule` on a `work_pool`.
...
- Change the repository name to yours.
...
<Info>
Typically, `uv sync` creates a new virtual environment, but Prefect requires you to use root Python.
...
The flow will run automatically at 12 PM, but you can run it manually.
