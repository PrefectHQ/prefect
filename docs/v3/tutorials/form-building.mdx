...
Parameterizing workflows is a critical part of orchestration. It allows you create contracts between modular workflows in your organization and empower less-technical users to interact with your workflows intuitively.
...
This is good enough for many cases, but consider the additional practical constraints:
...
- you have spin up the infrastructure associated with your flow in order to check the constraints, which is wasteful if it turns out that bad parameters were provided
- this might get duplicative, especially if you have similarly constrained parameters for different workflows

To improve on this, we will use `pydantic` to build a convenient, self-documenting, and reusable schema that the Prefect UI can build a form from.
