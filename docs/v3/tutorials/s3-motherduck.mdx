...
The code example for the tutorial can be found in [this GitHub repository](https://github.com/PrefectHQ/dev-day-zoom-out).
...
## Setting up S3 and MotherDuck resources
Before building your pipeline, you'll need to set up the necessary resources in AWS, MotherDuck, and Prefect Cloud.
If you're a seasoned AWS user and already have an IAM User and S3 bucket ready to go, you can skip the `AWS IAM User Setup` and `AWS S3 Bucket Setup` sections.
...
Sign in to the [AWS Console](https://aws.amazon.com/console/) to create an AWS IAM User and an AWS S3 bucket, and [Prefect Cloud](https://app.prefect.cloud/) to create a Prefect AWS credentials block and an S3 bucket block.
...
2. Click "Create user"
3. Enter a username (for example, "prefect-s3")
4. Select "Attach policies directly" from the Permissions options, and choose "AmazonS3FullAccess" from the Permissions policy list
5. Create the user
6. Once the user is created, click the new user record
7. Click "Create access key"
8. From the "Access key best practices & alternatives" menu, select "Local code"
9. Click "Next"
10. Set a description tag for the access key (for example, "Prefect S3 Access Key")
11. Click "Create access key"

<Tip>
Important: Save the Access Key ID and Secret Access Key - you'll need these to create a Prefect AWS credentials block.
</Tip>

#### Create an AWS S3 bucket
To create an AWS S3 bucket, complete the following steps in the AWS Console:

1. Navigate to S3 service
2. Click "Create bucket"
3. Enter a unique bucket name (for example, "mlb-game-data")
...
<Expandable title="Python SDK method">
...
<Expandable title="Prefect Cloud UI method">
<iframe 
width="560" 
height="315" 
...
S3 bucket blocks are used to store your S3 bucket configuration. It allows your flows and tasks to connect to your S3 bucket.
Similarly to the AWS credentials block, you can create this block by using the Python SDK, or through the Prefect Cloud UI.
...
Login to your [MotherDuck Console](https://motherduck.com/) and complete the following steps to create the access token:

<iframe 
width="560" 
height="315" 
src="https://demo.arcade.software/7Qag37hD0ttC92p7SjTh?embed&embed_mobile=tab&embed_desktop=inline&show_copy_link=true"
title="Arcade player" 
frameborder="0" 
allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" 
referrerpolicy="strict-origin-when-cross-origin" 
...
You can create a secret block by using the Python SDK, or through the Prefect Cloud UI.
...
Here are some commands you can use to create the project directory and files:
```bash
...
#### Fetch the game data
Let's start by importing the necessary packages, and creating three tasks that fetch game data by using the [statsapi package](https://github.com/toddrob99/MLB-StatsAPI/tree/master?tab=readme-ov-file).
...
If you run this flow as-is, you'll see the raw data saved to the `raw_data` folder in your project directory.

#### Loading data to S3
For the next two tasks, you'll upload the raw data file to s3, and pull it back down for further processing. You'll need to add these new tasks to your flow function as well.

<Expandable title="S3 tasks">
{/* pmd-metadata: notest */}
```python
#Copy and paste this code in your mlb_flow.py file.
...
Let's create a set of functions that clean up the raw data, run some basic statistics, and save the results to a MotherDuck database.
...
def clean_time_value(data_file_path):
    ...
    for game_data in game_data_list:
        ...
        # Remove any non-d igit, non-colon characters.
...
As a last step, let's add some visibility into the results of your flow. You can add a task that creates a markdown artifact to view the results in the Prefect Cloud UI.
...
Jump to the next tutorial to learn how you can use Prefect to do just that: [Resilient Pipelines and Serverless Infrastructure](/v3/tutorials/resilience-and-deployment)