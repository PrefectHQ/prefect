...
1. [**Install Prefect**](https://chatgpt.com/v3/get-started/install) (`pip install prefect`).
...
3. **Run initial flows without infrastructure setup**: Run flows locally or using Prefect's managed execution allows you to test without configuring work pools or Kubernetes.
...
```
This will allo your flow to run remotely without provisioning workers, setting up Kubernetes, or maintaining cloud infrastructure.

**When to Use Prefect Managed Execution**

✅ Best for:
- Quickly testing and running flows without infrastructure setup.
- Production use cases where infrastructure management is not required.
- Teams wanting fully managed execution without needing a cloud provider.

❌ Consider self-hosted execution if you need:
- Custom container images or dependency-heavy environments.
- Private networking beyond static outbound IPs.
- Higher concurrency limits beyond what's available in the free or paid tiers.
...
For full details on Prefect Managed Execution, refer to the [Managed Execution documentation](/v3/deploy/infrastructure-examples/managed).
