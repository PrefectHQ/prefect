...
- [Install Prefect](/v3/get-started/install) (`pip install prefect`) and launch a [**Prefect Server**](/v3/manage/self-host) or sign up for [**Prefect Cloud**](https://app.prefect.cloud/)
...
Many legacy Airflow DAGs include branches (using `BranchPythonOperator`) or conditionally skipping tasks (`ShortCircuitOperator`, trigger rules, etc.). In Prefect, you implement branching with normal Python `if/else` or loops inside the flow function. For example, suppose after extraction you choose one of two paths to run (Branch A or Branch B), and later join. In legacy Airflow you'd use a BranchPythonOperator and perhaps DummyOperators to join branches. In Prefect, you can do:
...
For each task and special case, including [subDAGs](/docs/apache-airflow/stable/core-concepts/dags.html#concepts-subdags) and [TaskGroups](/docs/apache-airflow/stable/core-concepts/dags.html#taskgroups), implement them as subflows or Python functions in Prefect. When transitioning from Airflow's TaskFlow API, keep in mind that Prefect's `@task` decorator serves a similar purpose but does not rely on XComs.
...
In Airflow, the **KubernetesExecutor** launches **each task** in its own Kubernetes pod. Prefect, instead, uses a **Kubernetes Work Pool**, where each **flow run** is executed in a Kubernetes pod, rather than launching separate pods for each task. This approach reduces fragmentation and simplifies configuration while maintaining scalability and isolation.

**Configuring a Kubernetes Work Pool**
For detailed instructions, see [Prefect's Kubernetes Work Pool documentation](/v3/deploy/infrastructure-examples/kubernetes). But the general steps to take are:
...
Alternatively, you can use a [prefect.yaml](/v3/deploy/infrastructure-examples/kubernetes#define-a-prefect-deployment) file to deploy your flow to the Kubernetes work pool.

3. [**Run a Kubernetes worker in-cluster**](/v3/deploy/infrastructure-examples/kubernetes#deploy-a-worker-using-helm) to execute flow runs.
4. **Execution Flow**:
...
- **High Availability**: If self-hosting, use **PostgreSQL** and consider running **multiple API replicas behind a load balancer**. Prefect Cloud handles availability automatically.
...
- **Notifications & Alerts**: Set up **failure alerts** via **Slack, email, or Twilio** using **Prefect Automations** to ensure timely issue resolution.
...
- **Configuration Management**: Replace Airflow **Variables/Connections** with **Prefect Blocks**, storing secrets in **CLI, UI, or version-controlled JSON**.
...
For more details on operating Prefect in production, see the [Operate](/v3/manage/index) guide.
...
- **Run flows locally** - Execute `@flow` functions directly (`etl_pipeline_flow()`) to validate logic.
