...
description: "Migrating from Apache Airflow to Prefect: A Comprehensive Step-by-Step Tutorial"
---

## Introduction
Migrating from Apache Airflow to Prefect simplifies orchestration, reduces overhead, and enables a more Pythonic workflow. Prefect's flexible **library-based approach** lets you write, test, and run workflows like regular code—without the complexity of schedulers, executors, or metadata databases.
...
| [**Executors**](#executors-to-work-pools)  | [**Work Pools & Workers**](#executors-to-work-pools)    | Prefect decouples task execution using lightweight **workers** polling **work pools**. |
...
| [**Sensors**](#airflow-sensors) | [**Triggers & Event-Driven Flows**](#using-external-triggers) | Prefect uses external event triggers or lightweight polling flows. |
...
**Set up Prefect alongside Airflow:** Prepare a **parallel Prefect environment** for testing before fully migrating
- Install Prefect (`pip install prefect`) and launch a [**Prefect Server**](/v3/manage/self-host) or sign up for [**Prefect Cloud**](https://app.prefect.cloud/).
- Configure an [**execution environment**](/v3/deploy/infrastructure-concepts/work-pools) (local, Docker, Kubernetes)
- Consider running **Airflow and Prefect side-by-side temporarily** for validation, but avoid maintaining both long-term—disable Airflow DAGs once Prefect replacements are stable
...
In this phase, you will **rewrite your Airflow DAGs as Prefect flows and tasks**. The goal is to replicate each workflow's logic in Prefect, while simplifying wherever possible. Prefect's API is quite ergonomic - many Airflow users find they can express the same logic with *less code and more flexibility*. Let's break down the conversion process step-by-step, and walk through a concrete example.
...
In this Airflow DAG, we define three tasks using `PythonOperator`, then specify that they run sequentially (`extract` then `transform` then `load`). The DAG is scheduled daily. If we wanted 

### Create equivalent Prefect tasks
In Prefect, we'll take the core logic of `extract_fn`, `transform_fn`, `load_fn` and turn each into a `@task` function. The code inside can remain largely the same (minus any Airflow specific calls). For example:
...
This Prefect flow function replaces the Airflow DAG, automatically executing tasks in order based on data flow—no need for `>>` dependencies or XComs. **Tasks pass outputs directly as function arguments**, ensuring a clear, Pythonic workflow.  
...
Many Airflow DAGs include branches (using `BranchPythonOperator`) or conditionally skipping tasks (`ShortCircuitOperator`, trigger rules, etc.). In Prefect, you implement branching with normal Python `if/else` or loops inside the flow function. For example, suppose after extraction you choose one of two paths to run (Branch A or Branch B), and later join. In Airflow you'd use a BranchPythonOperator and perhaps DummyOperators to join branches. In Prefect, you can do:
...
The `if/else` will conditionally call the appropriate tasks. Prefect will only run the tasks that were called (the other branch tasks won't even execute, no need to “skip” them explicitly). After the conditional, you can continue with common downstream tasks. In our example, we could call `final_task()` after the branches - it will run after whichever branch was taken, effectively acting like a join. This approach is much simpler than Airflow's branching operators. 
...
For each task and special case, including subDAGs and TaskGroups, implement them as subflows or Python functions in Prefect. When transitioning from Airflow's TaskFlow API, keep in mind that Prefect's `@task` decorator serves a similar purpose but does not rely on XComs.
...
Migrating your code is a big step, but ensuring your workflows run smoothly in Prefect is just as important. Prefect's **flexible execution** makes this easier, supporting local machines, VMs, containers, and Kubernetes with less setup. This section maps Airflowss executors to **Prefect Work Pools and Workers**, while also covering sensors, hooks, logging, and state management to complete your migration.

### Airflow Executors
**Airflow Executors vs Prefect Work Pools/Workers:** Airflow's executor setting determines how tasks are distributed. Prefect's equivalent concept is the [**work pool**](/v3/deploy/infrastructure-concepts/work-pools) (with one or more [**workers**](/v3/deploy/infrastructure-concepts/workers) polling it). Here's a mapping of typical setups:
...
Prefect **work pools** function similarly to **Celery queues**, allowing multiple workers to process tasks concurrently. Multiple **work queues** can be created within a pool for **priority-based task routing**, without the overhead of maintaining a Celery broker or backend.

#### Airflow KubernetesExecutor
Using the **Airflow KubernetesExecutor** each task launches in its own Kubernetes Pod.

Prefect approaches this a bit differently. Prefect has a **Kubernetes work pool** that can launch each *flow run* in a Kubernetes Job/Pod. Instead of each task being a pod, Prefect will typically execute the entire flow in a pod (which can itself spawn parallel threads or sub-tasks if needed). This achieves isolation and scalability similar to KubernetesExecutor but with less fragmentation and easier config. 

You can set up a Kubernetes work pool with the desired pod template (image, resources, etc.), and when you deploy your flow you'd specify that work pool. Then Prefect's Kubernetes **worker** (which could be running in-cluster) will create a new pod for each run. The pod spins up, executes the flow's tasks, then terminates - no long-running scheduler in the loop. This simplifies what in Airflow often required a lot of YAML and `airflow.cfg` tweaks.
...
For setup details, refer to [Prefect's serverless execution documentation](/v3/deploy/infrastructure-examples/serverless).
...
The general rule is: **Anything you did with a custom Airflow operator or hook can be achieved in Prefect by writing a task that uses the appropriate Python library**. You're no longer constrained to what Airflow provides - you have the whole Python ecosystem at your disposal within tasks. This is powerful because you might eliminate entire categories of Airflow workaround code. For example, if you had an Airflow BashOperator to run a shell script that calls an API (because maybe the Airflow Python environment didn't have the needed package), in Prefect you could install that package in your flow's environment and call it directly from a task, avoiding the indirection of a shell script.
...
- **Flow cancellation** (`Cancelling` state).  
- **Resuming failed flows** (with manual steps).  
...
To ensure logs appear correctly in Prefect’s UI, use:  
```python
...
In Airflow, monitoring is typically managed through the UI, email alerts on task failures, and external monitoring of the scheduler. Prefect provides similar capabilities through **Notifications and Automations**, which can be configured to trigger alerts via **Slack, email, or webhooks** based on specific events.  
...
### Prefect Deployment
**From Airflow DAG schedules to Prefect Deployment:** In Airflow, deployment usually meant placing your DAG code on the Airflow scheduler (e.g., by committing to a Git repo that the scheduler reads, or copying files to the DAGs directory). There wasn't a formal deployment artifact beyond the Python files. Prefect, by contrast, treats deployments as first-class objects. You will create a deployment for each flow (or for each distinct configuration of a flow you want to run). This can be done via code (calling `flow.deploy()`), via CLI (`prefect deployment`), or by writing a YAML (`prefect.yaml`) that describes the deployment. 
...
This can all be scripted. In fact, Prefect provides guidance on using [GitHub Actions or similar tooling to do this](v3/deploy/infrastructure-concepts/deploy-ci-cd). By integrating Prefect's deployment steps into CI, you ensure that any change in your flow code gets reflected in Prefect's orchestrator, much like updating DAG code in Airflow.

Altenatively, if your deployment is set to pull the workflow code from your git repository each time, you only need to push the latest workflow code, and automatically next time your deployment runs it will pull the latest workflow code.
...
Once deployed, Prefect will begin scheduling and orchestrating according to your deployments. Here are best practices for running Prefect in prod, analogous to Airflow's production setup considerations:

- **Ensure high availability of Prefect orchestration**: If you self-host, run Prefect with a reliable backing database (Postgres recommended) and possibly in a highly available mode (e.g., multiple replicas behind a load balancer for the API,). If using Prefect Cloud, the reliability is handled by the service - you just worry about your agents/workers.
- **Keep Prefect workers running**: Just like Airflow had scheduler/worker processes always on, you need your Prefect workers active to pick up scheduled runs. You might run them as systemd services, Docker containers in a cluster, or Kubernetes deployments. 
- **Configure logging and observability**: Decide if you'll use Prefect's UI for all log access or also pipe logs to an external system. You might, for instance, set up an S3 block so that flow run logs are additionally saved to S3 for long-term storage.
- **Notifications and alerts**: As mentioned, set up Prefect notifications for failures. Prefect Cloud lets you configure Alerts easily (Slack, Email, etc.), and Prefect Server can use Automations (which might need an outbound notification block like Twilio or SMTP configured). This ensures that if a flow fails at 2 AM, the relevant people get notified.
- **CI/CD for Prefect flows**: We covered deploying flows via CI. It's good practice to also run tests on your flow functions as part of CI (even if Airflow DAGs were hard to test, now you can test Prefect tasks like any Python function!). Also, you might incorporate a **Prefect preview** or dry-run in CI – for example, use `prefect deployment build --skip-upload` just to validate that the deployment YAML is well-formed and the flow is importable, etc. This catches errors before they hit production.
- **Managing configuration**: In Airflow, you might use Variables or connections in the UI. In Prefect, you can use **Blocks** in the UI/CLI to store things like credentials, and reference them in your flows. After migration, audit what Airflow Variables/Connections you had and create equivalent Prefect Blocks or Secrets. Prefect Blocks can be created via CLI or UI, and you can even store them in code (as JSON) and apply via CI for consistency.
- **Security**: If your Airflow was self-hosted, you likely secured the webserver and had roles. Prefect Cloud has built-in authentication and role-based access. 
- **Decommissioning Airflow**: Once you've migrated all DAGs, you can gradually tear down the Airflow infrastructure. It's common to overlap for a few days or weeks during cutover (with Airflow DAGs disabled but available if needed). Eventually, you can archive the DAG code and shut down Airflow components, reducing ops burden.

## Testing & Validation

Thorough testing and validation are critical during the migration to ensure your new Prefect flows perform just like the old Airflow DAGs. Because we aim for a **full migration** (not running Airflow and Prefect together long-term), you'll want high confidence in Prefect before decommissioning Airflow. This section covers strategies for testing Prefect flows and running side-by-side comparisons, as well as common issues and troubleshooting tips.

**Test Prefect flows in isolation:** One advantage of Prefect's design is that you can test flows and tasks as plain Python. Make use of this! For each migrated DAG:
- **Unit test task logic:** Since tasks are simple functions, write unit tests for them with various inputs. This wasn't always easy with Airflow operators tied to Airflow context, but now it is. Ensure that given certain inputs, your task returns or produces expected outcomes.
- **Dry-run flows locally:** You can call the `@flow` function directly in a development environment to execute the workflow immediately. This is great for validation. For example, run `etl_pipeline_flow()` on your laptop or a dev pod and see that it completes and produces the correct result (perhaps writing to a dev database or a sandbox bucket). This executes outside the Prefect server, but it uses the same code, so it's a good smoke test. Keep in mind when you call a flow function directly, it will run *synchronously* in your local process (unless tasks use concurrent task runners). That's fine for testing.
- **Use Prefect's local orchestration for testing:** You can also start a local Prefect server (`prefect server start`) and actually trigger the flow via Prefect (register a deployment and run it). This tests the full pipeline with Prefect in the loop and will show you the flow in the UI. It's a bit more overhead than calling the function directly, but it mirrors production execution more closely (especially if your deployment uses Docker/K8s, etc., you can simulate that).
- **Side-by-side output comparison:** Run the Prefect flow for a given date or input, and run the Airflow DAG for the same date/input (Airflow's backfill or manual trigger can be used to run it on demand). Compare the outputs: e.g., did both write the same number of rows to the database? Do files have the same content? This can be manual or automated depending on what your pipelines do, but it's a very important step. If any discrepancies arise, debug them now.

### Validation Phase
**Temporary parallel running (shadow mode):** During a validation phase, you might keep the Airflow DAG active (but not scheduled) and trigger it in parallel with Prefect flows for critical pipelines. *Do not run them both on the same schedule against production data simultaneously*, as that could double-process data or cause conflicts. Instead, you can:
- Pause the Airflow DAG's schedule (so it doesn't auto-run).
- Whenever the Prefect flow runs, manually trigger the Airflow DAG (or vice versa) for that execution date, but perhaps point it to write output to a test location.
- Compare results and performance. This overlapping period should be as short as possible - just enough to verify correctness and performance parity.

For example, if migrating a daily batch job, you might run the first few days on Prefect and also still run Airflow (maybe writing to a temp table) to confirm results match, then stop Airflow runs. This builds trust that nothing was lost in translation.

### Decommissioning Airflow
**Retiring Airflow DAGs:** Once a Prefect flow is confirmed stable, **disable the Airflow DAG** to avoid confusion. In Airflow, you can pause the DAG or remove it. The Prefect “replacement” should be clearly documented as the new source of truth. This one-by-one migration ensures you're never maintaining two versions of the pipeline for long. It's tempting to leave the Airflow DAG as backup, but that can lead to accidentally running it or outdated logic floating around. Safer is to remove or archive it once Prefect is live.
...
- **Credentials & access:** Verify that Prefect tasks have the same permissions as Airflow (e.g., service accounts, IAM roles). If using Kubernetes, ensure pods can access necessary databases and APIs.  
...
- **Use subflows for modularity**: Instead of chaining DAGs, use **subflows** to orchestrate dependencies within a single flow.
- **Optimize parallelism**: Use **dynamic task mapping** (`task.map(items)`) to process large datasets efficiently.
...
- **Scale efficiently**: Prefect naturally distributes workloads across **Work Pools and Workers**, eliminating Airflow's scheduler bottlenecks.
...
### Congratulations!
By completing this migration, you've moved to a more scalable, efficient orchestration system. Prefect allows your team to **focus on engineering**—iterating faster, improving reliability, and scaling seamlessly. **Happy flowing!**