...
In [Set up a platform for data pipelines](/v3/tutorials/platform), you used Prefect Cloud to set up a platform for data pipelines.
...
Flows commonly experience multiple failures in a row.
When this happens, it is usually best to inspect the first failure in the series.
...
You can now deploy the fix to the production workspace as well.
