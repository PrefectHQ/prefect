...
description: Use S3 events to trigger an ML training flow on SageMaker
---
...
sequenceDiagram
    ...
    Note right of Prefect Cloud: Start model training
    ...
    activate SageMaker
    Note left of SageMaker: Provision instance
    ...
    S3->>S3: Trigger EventBridge rule
    S3->>Prefect Cloud: Send event to webhook
    Note right of Prefect Cloud: Start model inference
    Prefect Cloud->>Prefect Cloud: Trigger automation
    Prefect Cloud->>Prefect Cloud: Run deployment
...
* Trigger whenever you see that event 1 time within 1 minutes (this ensures your model will train only once when a batch of files are uploaded).
...
* An IAM role named `prefect-tutorial` with the `Amazon SageMaker Full Access` and `Amazon S3 Full Access` policies. Note the role ARN, as you'll need this later.
