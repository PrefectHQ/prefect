...
* Create a `prefect-data` bucket to store training and test data.
...
Create an EventBridge rule named `training-data-changed` to trigger model training whenever a new object is uploaded to the `prefect-data` bucket.
...
```
* Target:  
An API destination named `model-training-webhook`, set to the endpoint for the `model-training` webhook that you created in Prefect Cloud.
  * To authenticate with the webhook, create a new Connection, configure its authorization to use **API Key**, and set the **API key name** to `Authorization` and the **Value** to `Bearer <your-prefect-service-account-api-key>`.
...
```
* Target:  
Create an API destination named `model-inference-webhook`, set to the endpoint of the `model-inference` webhook that you created earlier.
...
Use the AWS Console to upload the following files to the `prefect-data` bucket:
