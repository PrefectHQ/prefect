...
TODO: Create a new Docker image that has the necessary dependencies (python, pip, prefect[aws]) and use that image instead of the base nvidia/cuda one (or the Amazon ECS GPU-optimized kernel 5.10 AMI if needed).
...
Finally, edit the `my-ecs-pool` work pool definition in Prefect Cloud and:

1. Set the **Base Job Template** / **Task Definition Arn** setting to the ARN of the task definition you just registered.
1. Set the **Base Job Template** / **Launch Type** setting to `EC2`
