...
First, clone the repository with the flow code you'll be deploying to Prefect Cloud.
...
In order to trigger the deployments you just created, you need to create webhooks in Prefect Cloud.
These webhooks will receive events from S3.

#### Model training webhook

Create a webhook named `s3-model-webhook`:
...
Note the endpoint for this webhook, as you'll need it later.

#### Model inference webhook

Create a webhook named `s3-data-webhook`:
...
Finally, create automations which are triggered by each webhook you just created.
These automations run the deployments you created earlier.

#### Model training automation

Create an automation named `train-model`:

* Trigger on any event matching `webhook.called` from the `s3-data-webhook` webhook.
* Trigger whenever you see that event 1 time within 0 days (this ensures your model will train every time the event occurs).
* Run the `train-iris-model > model-training` deployment whenever this automation is triggered.

#### Model inference automation

Create an automation named `run-inference`:

* Trigger on any event matching `webhook.called` from the `s3-model-webhook` webhook.
* Trigger whenever you see that event 1 time within 0 days (this ensures your model will train every time the event occurs).
* Run the `run-inference > model-inference` deployment whenever this automation is triggered.

## Set up AWS resources

This tutorial uses S3 to store the data and models, EventBridge to trigger the Prefect webhooks, and SageMaker to train the model.

### Create S3 buckets

* Create a `prefect-tutorial` bucket to store training and test data.
* Create a `prefect-model` bucket to store the trained model.

<Note>
You must turn on EventBridge notifications for all events in these buckets.
</Note>
...
Create an EventBridge rule named `training-data-changed` to trigger model training whenever a new object is uploaded to the `prefect-tutorial` bucket.
...
```
* Target:  
An API destination named `prefect-tutorial`, set to the endpoint for the `s3-data-webhook` webhook that you created in Prefect Cloud.
  * To make authentication work, create a new Connection, configure its authorization to use **API Key**, and set the **API key name** to `Authorization` and the **Value** to `Bearer <your-prefect-service-account-api-key>`.
    * The service account API key is the same as the API key you created for the `s3` service account in Prefect Cloud.
...
```
* Target:  
Create an API destination named `prefect-model-webhook`, set to the endpoint of the `s3-model-webhook` webhook that you created earlier.
...
* A SageMaker role with the `Amazon SageMaker Full Access` policy. Note the role ARN, as you'll need this later.

Next, create the following Prefect Blocks:
...
* An `S3 Bucket` Block named `s3-bucket-block`.  
This Block can use the `aws-credentials` Block you created earlier.
* A `Secret` Block named `sagemaker-role-arn`.
This Block stores the SageMaker role ARN that you created earlier.

Now, when you run the flows in Prefect Cloud, they can use these Blocks to authenticate with AWS.

## Test the ML pipeline

Before you can test the ML pipeline, you need to create a work pool and start a worker.
To keep this simple, create a new Process work pool named `my-process-pool` in Prefect Cloud.
...
Use the AWS Console to upload the following files to the `prefect-tutorial` bucket:
...
After the files are uploaded, the `train-iris-model` flow will automatically run.
