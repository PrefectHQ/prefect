...
2. Install the latest Prefect client:
   ```bash
   pip install -U prefect
   ```

3. Create a new namespace to work from (all commands in this quick-start will use this namespace):
   ```bash
   kubectl create namespace prefect-server
   kubectl config set-context --current --namespace=prefect-server
   ```
...
2. For a basic deployment with default values, using the latest Prefect version and no auth:
   ```bash
   helm install prefect-server prefect/prefect-server
   ```

3. For a customized deployment, create a values.yaml file for Prefect Server (see [values.yaml template](https://github.com/PrefectHQ/prefect-helm/blob/main/charts/prefect-server/values.yaml)).

4. Create a secret for the API basic auth:
   ```bash
   kubectl create secret generic server-auth-secret --from-literal auth-string='admin:password123'
   ```

5. Example `server-values.yaml` file for this tutorial:
   ```yaml
   server:
     basicAuth:
       enabled: true
       existingSecret: "server-auth-secret"  # Must pre-exist if existingSecret is used; use authString otherwise
     autoscaling:
       enabled: true
     livenessProbe:
       enabled: true
...
6. Install Prefect Server with custom values:
   ```bash
...
2. Create a `worker-values.yaml` file for this tutorial:
   ...
   worker:
     ...
     selfHostedServerApiConfig:
       ...
       # apiUrl = <service>.<namespace> if in same cluster, or FQDN if external
     basicAuth:
       enabled: true
       existingSecret: worker-auth-secret
   ```

3. Install the Prefect Worker:
   ```bash
