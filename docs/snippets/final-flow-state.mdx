...
@flow 
def my_flow():
    # avoided the raising of an exception with return_state flag
    state = add_one("1", return_state=True)
    assert state.is_failed()

    # the flow function returns succesfully!
    return
```

If in this example the `state` was returned from the flow function, the run would be marked as `FAILED`.
