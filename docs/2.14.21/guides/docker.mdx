...
In the [Deployments](/2.14.21/tutorial/deployments/) tutorial, we looked at serving a flow that enables scheduling or creating flow runs via the Prefect API.
...
If creating a deployment with a `prefect.yaml` file, the build step makes it easy to customize your Docker image and push it to the registry of your choice. See an example [here](/2.14.21/guides/deployment/kubernetes/#define-a-deployment).
...
- A Python script that defines and serves a flow.
  - We'll use the flow script and deployment from the [Deployments](/2.14.21/tutorial/deployments/) tutorial.
...
In this directory, we'll create a sub-directory named `flows` and put our flow script from the [Deployments](/2.14.21/tutorial/deployments/) tutorial in it.
...
See the [Work pool deployment guide](/2.14.21/guides/prefect-deploy/) for discussion of how Prefect can help you build custom images with dependencies specifiied in a `requirements.txt` file.

By default, Prefect [work pools](/2.14.21/concepts/work-pools) that use containers refer to the `2-latest` image.
...
We only served a single flow in this guide, but you can extend this setup to serve multiple flows in a single Docker image by updating your Python script to using `flow.to_deployment` and `serve` to [serve multiple flows or the same flow with different configuration](/2.14.21/concepts/flows#serving-multiple-flows-at-once).

To learn more about deploying flows, check out the [Deployments](/2.14.21/concepts/deployments/) concept doc!

For advanced infrastructure requirements, such as executing each flow run within its own dedicated Docker container, learn more in the [Work pool deployment guide](/2.14.21/guides/prefect-deploy/).
