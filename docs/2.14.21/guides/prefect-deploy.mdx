...
- [Docker](/2.14.21/guides/deployment/docker/) - runs flows in an ephemeral Docker container.
- [Kubernetes](/2.14.21/guides/deployment/kubernetes/) - runs flows as a Kubernetes Job.
- [Serverless Cloud Provider options](/2.14.21/guides/deployment/serverless-workers/) - runs flows in a Docker container in a serverless cloud provider environment, such as AWS ECS, Azure Container Instance, Google Cloud Run, or Vertex AI.
...
!!!note "Additional work pool options available with Prefect Cloud"
    ...
    - [Push Work Pools](/2.14.21/guides/deployment/push-work-pools) - serverless cloud options that don't require a worker because Prefect Cloud submits them to your serverless cloud infrastructure on your behalf. Prefect can auto-provision your cloud infrastructure for you and set it up to use your work pool.
    
    - [Managed Execution](/2.14.21/guides/managed-execution/) Prefect Cloud submits and runs your deployment on serverless infrastructure. No cloud provider account required.
...
- What instructions does a [worker](/2.14.21/concepts/work-pools/) need to set up an execution environment for our flow? For example, a flow may have Python package requirements, unique Kubernetes settings, or Docker networking configuration.
- How should the flow code be accessed?

## Creating work pool-based deployments

The [tutorial](/2.14.21/tutorial/deployments/) shows how you can create a deployment with a long-running process using `.serve` and how to move to a [work-pool-based deployment](/2.14.21/tutorial/workers/) setup with `.deploy`.
See the discussion of when you might want to move to work-pool-based deployments [there](/2.14.21/tutorial/workers/#why-workers-and-work-pools).
...
=== ".deploy"
    ...
    Make sure you have the [work pool](/2.14.21/concepts/work-pools/) created in the Prefect Cloud workspace you are authenticated to or on your running self-hosted server instance.  
    ...
    Note that we used a [Prefect Secret block](/2.14.21/concepts/blocks/) to load the URL configuration for the artifact registry above.
    ...
    If you need to provide additional configuration, such as specifying a private repository, you can provide a [`GitRepository`](/2.14.21/api-ref/prefect/flows/#prefect.runner.storage.GitRepository) object instead of a URL:
    ...
    If you need additional configuration for your cloud-based storage - for example, with a private S3 Bucket -  we recommend using a [storage block](/2.14.21/concepts/blocks/). 
    ...
    The values passed in override default values in the specified work pool's [base job template](/2.14.21/concepts/work-pools/#base-job-template).
...
=== "prefect.yaml"
    ...
    As discussed above, if you are using [blocks](/2.14.21/concepts/blocks/), the credentials section can be templated with a block reference for secure and dynamic credentials access:
    ...
    - **blocks**: [Prefect blocks](/2.14.21/concepts/blocks) can also be referenced with the special syntax `{{ prefect.blocks.block_type.block_slug }}`. It is highly recommended that you use block references for any sensitive information (such as a GitHub access token or any credentials) to avoid hardcoding these values in plaintext
    - **variables**: [Prefect variables](/2.14.21/concepts/variables) can also be referenced with the special syntax `{{ prefect.variables.variable_name }}`. Variables can be used to reference non-sensitive, reusable pieces of information such as a default image name or a default work pool name.
    ...
    !!! note "Docker step"
        The most commonly used build step is [`prefect_docker.deployments.steps.build_docker_image`](/2.14.21/guides/deployment/docker/) which produces both the `image_name` and `tag` fields.

        For an example, [check out the deployments tutorial](/2.14.21/guides/deployment/docker/).
...
=== "prefect.yaml"
    ...
    | `schedule`                                 | An optional [schedule](/2.14.21/concepts/schedules) to assign to the deployment. Fields for this section are documented in the [Schedule Fields](#schedule-fields) section.                                                                                                                      |
    | `triggers`                                  | An optional array of [triggers](/2.14.21/concepts/deployments/#create-a-flow-run-with-an-event-trigger) to assign to the deployment |
    ...
    For more information about schedules, see the [Schedules](/2.14.21/concepts/schedules/#creating-schedules-through-a-deployment-yaml-files-schedule-section) concept doc.
    ...
    | `job_variables`                                | Values used to override the default values in the specified work pool's [base job template](/2.14.21/concepts/work-pools/#base-job-template). Maps directly to a created deployments `infra_overrides` attribute. |
...
Now that you are familiar with creating deployments, you may want to explore [push work pools](/2.14.21/guides/deployment/kubernetes) or [Kubernetes work pools](/2.14.21/guides/deployment/kubernetes).
