...
A [worker](/2.14.21/concepts/work-pools/#worker-overview) is the fusion of an [agent](/2.14.21/concepts/agents/) with an [infrastructure block](/2.14.21/concepts/infrastructure/). Like agents, workers poll a work pool for flow runs that are scheduled to start. Like infrastructure blocks, workers are typed - they work with only one kind of infrastructure, and they specify the default configuration for jobs submitted to that infrastructure.

Accordingly, workers are not a drop-in replacement for agents. **Using workers requires deploying flows differently.** In particular, deploying a flow with a worker does not involve specifying an infrastructure block. Instead, infrastructure configuration is specified on the [work pool](/2.14.21/concepts/work-pools/) and passed to each worker that polls work from that pool.
...
- Work pools allow greater customization and governance of infrastructure parameters for deployments via their [base job template](/2.14.21/concepts/work-pools/#base-job-template).
- Prefect Cloud [push work pools](/2.14.21/guides/deployment/push-work-pools/) enable flow execution in your environment without needing to host a worker.
...
- Both options allow you to [deploy multiple flows](/2.14.21/concepts/deployments/#working-with-multiple-deployments) with a single command.
- Both options allow you to build Docker images for your flows to create portable execution environments.
- The YAML-based API supports [templating](/2.14.21/concepts/deployments/#templating-options) to enable [dryer deployment definitions](/2.14.21/concepts/deployments/#reusing-configuration-across-deployments).
...
1. **Deployment CLI and Python API:**
    ...
    `prefect deployment build <entrypoint>`/`prefect deployment apply` --> [`prefect deploy`](/2.14.21/concepts/deployments/#deployment-declaration-reference)

    Prefect will now automatically detect flows in your repo and provide a [wizard](/2.14.21/#step-5-deploy-the-flow) 🧙 to guide you through setting required attributes for your deployments.

    `Deployment.build_from_flow` --> [`flow.deploy`](/2.14.21/api-ref/prefect/flows/#prefect.flows.Flow.deploy)

2. **Configuring remote flow code storage:**

    storage blocks --> [pull action](/2.14.21/concepts/deployments/#the-pull-action)

    When using the YAML-based deployment API, you can configure a pull action in your `prefect.yaml` file to specify how to retrieve flow code for your deployments. You can use configuration from your existing storage blocks to define your pull action [via templating](/2.14.21/guides/prefect-deploy/#templating-options).
...
3. **Configuring flow run infrastructure:**
    ...
    infrastructure blocks --> [typed work pool](/2.14.21/concepts/work-pools/#worker-types)
...
4. **Managing multiple deployments:**
    ...
    Create and/or update many deployments at once through a [`prefect.yaml`](/2.14.21/concepts/deployments/#working-with-multiple-deployments) file or use the [`deploy`](/2.14.21/api-ref/prefect/deployments/runner/#prefect.deployments.runner.deploy) function.
...
- Deployment-level infrastructure overrides operate in much the same way.
    ...
    `infra_override` -> [`job_variable`](/2.14.21/concepts/deployments/#work-pool-fields)

- The process for starting an agent and [starting a worker](/2.14.21/concepts/work-pools/#starting-a-worker) in your environment are virtually identical.
...
1. **[Create a work pool](/2.14.21/concepts/work-pools/#work-pool-configuration)** 

    This new work pool will replace your infrastructure block. 
    
    You can use the [`.publish_as_work_pool`](/2.14.21/api-ref/prefect/infrastructure/#prefect.infrastructure.Infrastructure.publish_as_work_pool) method on any infrastructure block to create a work pool with the same configuration.
    ...
    !!! Tip "Serving flows"
        ...
        Check out our [Docker guide](/2.14.21/guides/docker/) for how to build a served flow into a Docker image and host it in your environment.

2. **[Start a worker](/2.14.21/concepts/work-pools/#starting-a-worker)**
...
3. **Deploy your flows to the new work pool**
    ...
    === ".deploy"
        ...
        - Replace `storage` with a call to [`flow.from_source`](/2.14.21/api-ref/prefect/flows/#prefect.flows.Flow.deploy). 
    ...
    === "prefect deploy"
        ...
        !!! warning "Always run `prefect deploy` commands from the **root** level of your repo!"
            With agents, you might have had multiple `deployment.yaml` files, but under worker deployment patterns, each repo will have a single `prefect.yaml` file located at the **root** of the repo that contains [deployment configuration](/2.14.21/concepts/deployments/#working-with-multiple-deployments) for all flows in that repo.
        ...
        !!! Note "For step 4, select `y` on the last prompt to save the configuration for the deployment."
            Saving the configuration for your deployment will result in a `prefect.yaml` file populated with your first deployment. You can use this YAML file to edit and [define multiple deployments](/2.14.21/concepts/deployments/#working-with-multiple-deployments) for this repo.

        You can add more [deployments](/2.14.21/concepts/deployments/#deployment-declaration-reference) to the `deployments` list in your `prefect.yaml` file and/or by continuing to use the deployment creation wizard.

For more information, check out our [in-depth guide for deploying flows to work pools](/2.14.21/guides/prefect-deploy/).
