...
Prefect Cloud can also schedule flow runs through event-driven [automations](/2.14/concepts/automations/).

Schedules tell the Prefect API how to create new flow runs for you automatically on a specified cadence.

You can add a schedule to any flow [deployment](/2.14/concepts/deployments/).
...
- Via a the `cron`, `interval`, or `rrule` parameters if building your deployment via the [`serve` method](/2.14/concepts/flows/#serving-a-flow) of the `Flow` object or [the `serve` utility](/2.14/concepts/flows/#serving-multiple-flows-at-once) for managing multiple flows simultaneously
- If using [worker-based deployments](/2.14/concepts/work-pools/)
...
![Prefect UI with Interval button selected](/2.14/img/ui/interval-schedule.png)
...
As seen in the [Quickstart](/2.14/getting-started/quickstart/#step-5-add-a-schedule), you can create a schedule by passing a `cron`, `interval`, or `rrule` parameters to the `Flow.serve` method or the `serve` utility.

## Creating schedules through the interactive `prefect deploy` command

If you are using [worker-based deployments](/2.14/concepts/work-pools/), you can create a schedule through the interactive `prefect deploy` command.
...
See the [Settings docs](/2.14/concepts/settings/) for more information on altering your settings.
