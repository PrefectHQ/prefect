...
Once a flow run reaches a terminal state ([detailed in the chart here](/2.14/concepts/states/#state-types)), it will be retained until the end of the flow run retention period.
...
This retention policy applies to all [details about a flow run](/2.14/ui/flow-runs/#inspect-a-flow-run), including its task runs.
