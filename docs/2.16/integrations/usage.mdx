...
Once the Prefect Integration is installed, [register the blocks](/2.16/concepts/blocks/#registering-blocks-for-use-in-the-prefect-ui) within the integration to view them in the Prefect Cloud UI:
...
!!! tip "Loading a block in code"
    To use the `load` method on a Block, you must already have a block document [saved](/2.16/concepts/blocks/#saving-blocks) either through code or through the Prefect UI.
