...
Potential triggers include the occurrence of events from changes in a flow run's state - or the absence of such events. You can even define your own custom trigger to fire based on an [event](/2.16/cloud/events/) created from a webhook or a custom event defined in Python code.
...
!!! cloud-ad "Automations are only available in Prefect Cloud"
    [Notifications](/2.16/concepts/notifications/) in an open-source Prefect server provide a subset of the notification message-sending features available in Automations.
...
![Viewing automations for a workspace in Prefect Cloud.](/2.16/img/ui/automations.png)
...
![Configuring a trigger for an automation in Prefect Cloud.](/2.16/img/ui/automations-trigger.png)
...
![Configuring an action for an automation in Prefect Cloud.](/2.16/img/ui/automations-action.png)
...
![Viewing a custom trigger for automations for a workspace in Prefect Cloud.](/2.16/img/ui/automations-custom.png)
...
At deployment time, this will create a linked automation that is triggered by events matching your chosen [grammar](/2.16/concepts/events/#event-grammar), which will pass the templatable `event` as a parameter to the deployment's flow run.
...
![Configuring notifications for an automation in Prefect Cloud.](/2.16/img/ui/automations-notifications.png)
...
- [flow_run](/2.16/api-ref/server/schemas/core/#prefect.server.schemas.core.FlowRun)
- [flow](/2.16/api-ref/server/schemas/core/#prefect.server.schemas.core.Flow)
- [deployment](/2.16/api-ref/server/schemas/core/#prefect.server.schemas.core.Deployment)
- [work_queue](/2.16/api-ref/server/schemas/core/#prefect.server.schemas.core.WorkQueue)
- [work_pool](/2.16/api-ref/server/schemas/core/#prefect.server.schemas.core.WorkPool)
...
![Configuring notifications for an automation in Prefect Cloud.](/2.16/img/ui/templated-notification.png)
