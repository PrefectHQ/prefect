...
Users may specify an [infrastructure](/2.8/api-ref/prefect/infrastructure/) block when creating a deployment. This block will be used to specify infrastructure for flow runs created by the deployment at runtime.

Infrastructure can only be used with a [deployment](/2.8/concepts/deployments/). When you run a flow directly by calling the flow yourself, you are responsible for the environment in which the flow executes.
...
- [`Process`](/2.8/api-ref/prefect/infrastructure/#prefect.infrastructure.process.Process) runs flows in a local subprocess.
- [`DockerContainer`](/2.8/api-ref/prefect/infrastructure/#prefect.infrastructure.docker.DockerContainer) runs flows in a Docker container.
- [`KubernetesJob`](/2.8/api-ref/prefect/infrastructure/#prefect.infrastructure.kubernetes.KubernetesJob) runs flows in a Kubernetes Job.
...
!!! question "What about tasks?"
    ...
    Tasks use task runners. For more on how task runners work, see [Task Runners](/2.8/concepts/task-runners/).

## Using infrastructure

You may create customized infrastructure blocks through the Prefect UI or Prefect Cloud [Blocks](/2.8/ui/blocks/) page or create them in code and save them to the API using the blocks [`.save()`](/2.8/api-ref/prefect/blocks/core/#prefect.blocks.core.Block.save) method.
...
In this example we specify the `DockerContainer` infrastructure in addition to a preconfigured AWS S3 bucket [storage](/2.8/concepts/storage/) block.
...
See [Deployments](/2.8/concepts/deployments/) for more information about deployment build options.
...
[`Process`](/2.8/api-ref/prefect/infrastructure/#prefect.infrastructure.process.Process) infrastructure runs a command in a new process.
...
[`DockerContainer`](/2.8/api-ref/prefect/infrastructure/#prefect.infrastructure.docker.DockerContainer) infrastructure  executes flow runs in a container.

Requirements for `DockerContainer`:

- Docker Engine must be available.
- You must configure remote [Storage](/2.8/concepts/storage/). Local storage is not supported for Docker.
...
- The ephemeral Prefect API won't work with Docker and Kubernetes. You must have a Prefect server or Prefect Cloud API endpoint set in your [agent's configuration](/2.8/concepts/work-pools/).
...
| image_registry | A [`DockerRegistry`](/2.8/api-ref/prefect/infrastructure/#prefect.infrastructure.docker.DockerRegistry) block containing credentials to use if `image` is stored in a private image registry. |
...
[`KubernetesJob`](/2.8/api-ref/prefect/infrastructure/#prefect.infrastructure.kubernetes.KubernetesJob) infrastructure executes flow runs in a Kubernetes Job.

Requirements for `KubernetesJob`:

- `kubectl` must be available.
- You must configure remote [Storage](/2.8/concepts/storage/). Local storage is not supported for Kubernetes.
- The ephemeral Prefect API won't work with Docker and Kubernetes. You must have an Prefect server or Prefect Cloud API endpoint set in your [agent's configuration](/2.8/concepts/work-pools/).
...
- The ephemeral Prefect API won't work with ECS directly. You must have a Prefect server or Prefect Cloud API endpoint set in your [agent's configuration](/2.8/concepts/work-pools/).
...
- You must configure remote [Storage](/2.8/concepts/storage/). Local storage is not supported for ECS tasks. The most commonly used type of storage with `ECSTask` is S3. If you leverage that type of block, make sure that [`s3fs`](https://s3fs.readthedocs.io/en/latest/) is installed within your agent and flow run environment. The easiest way to satisfy all the installation-related points mentioned above is to include the following commands in your Dockerfile:  
