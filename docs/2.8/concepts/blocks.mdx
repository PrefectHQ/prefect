...
You can access blocks for both configuring flow [deployments](/2.8/concepts/deployments/) and directly from within your flow code.

Prefect provides some built-in block types that you can use right out of the box. Additional blocks are available through [Prefect Collections](/2.8/collections/catalog/). To use these blocks you can `pip install` the collection, then register the blocks you want to use with Prefect Cloud or a Prefect server.
...
!!! tip "Blocks and parameters"
    ...
    For configuration that will change between flow runs, we recommend using [parameters](/2.8/concepts/flows/#parameters).
...
| --- | --- | --- |
| [Azure](/2.8/concepts/filesystems/#azure) | `azure` | Store data as a file on Azure Datalake and Azure Blob Storage. |
| [Date Time](/2.8/api-ref/prefect/blocks/system/#prefect.blocks.system.DateTime) | `date-time` | A block that represents a datetime. |
| [Docker Container](/2.8/api-ref/prefect/infrastructure/#prefect.infrastructure.DockerContainer) | `docker-container` | Runs a command in a container. |
| [Docker Registry](/2.8/api-ref/prefect/infrastructure/#prefect.infrastructure.docker.DockerRegistry) | `docker-registry` | Connects to a Docker registry.  Requires a Docker Engine to be connectable. |
| [GCS](/2.8/concepts/filesystems/#gcs) | `gcs` | Store data as a file on Google Cloud Storage. |
| [GitHub](/2.8/concepts/filesystems/#github) | `github` | Interact with files stored on public GitHub repositories. |
| [JSON](/2.8/api-ref/prefect/blocks/system/#prefect.blocks.system.JSON) | `json` | A block that represents JSON. |
| [Kubernetes Cluster Config](/2.8/api-ref/prefect/blocks/kubernetes/#prefect.blocks.kubernetes.KubernetesClusterConfig) | <span class="no-wrap">`kubernetes-cluster-config`</span> | Stores configuration for interaction with Kubernetes clusters. |
| [Kubernetes Job](/2.8/api-ref/prefect/infrastructure/#prefect.infrastructure.KubernetesJob) | `kubernetes-job` | Runs a command as a Kubernetes Job. |
| [Local File System](/2.8/concepts/filesystems/#local-filesystem) | `local-file-system` | Store data as a file on a local file system. |
| [Microsoft Teams Webhook](/2.8/api-ref/prefect/blocks/notifications/#prefect.blocks.notifications.MicrosoftTeamsWebhook) | `ms-teams-webhook` | Enables sending notifications via a provided Microsoft Teams webhook. |
| [Opsgenie Webhook](/2.8/api-ref/prefect/blocks/notifications/#prefect.blocks.notifications.OpsgenieWebhook) | `opsgenie-webhook` | Enables sending notifications via a provided Opsgenie webhook. |
| [Pager Duty Webhook](/2.8/api-ref/prefect/blocks/notifications/#prefect.blocks.notifications.PagerDutyWebHook) | `pager-duty-webhook` | Enables sending notifications via a provided PagerDuty webhook. |
| [Process](/2.8/concepts/infrastructure/#process) | `process` | Run a command in a new process. |
| [Remote File System](/2.8/concepts/filesystems/#remote-file-system) | `remote-file-system` | Store data as a file on a remote file system.  Supports any remote file system supported by `fsspec`.  |
| [S3](/2.8/concepts/filesystems/#s3) | `s3` | Store data as a file on AWS S3. |
| [Secret](/2.8/api-ref/prefect/blocks/system/#prefect.blocks.system.Secret) | `secret` | A block that represents a secret value. The value stored in this block will be obfuscated when this block is logged or shown in the UI. |
| [Slack Webhook](/2.8/api-ref/prefect/blocks/notifications/#prefect.blocks.notifications.SlackWebhook) | `slack-webhook` | Enables sending notifications via a provided Slack webhook. |
| [SMB](/2.8/concepts/filesystems/#smb) | `smb` | Store data as a file on a SMB share. |
| [String](/2.8/api-ref/prefect/blocks/system/#prefect.blocks.system.String) | `string` | A block that represents a string. |
| [Twilio SMS](/2.8/api-ref/prefect/blocks/notifications/#prefect.blocks.notifications.TwilioSMS) | `twilio-sms` | Enables sending notifications via Twilio SMS. |
| [Webhook](/2.8/api-ref/prefect/blocks/webhook/#prefect.blocks.webhook.Webhook) | `webhook` | Block that enables calling webhooks. |

## Blocks in Prefect Collections

Blocks can also be created by anyone and shared with the community. You'll find blocks that are available for consumption in many of the published [Prefect Collections](/2.8/collections/catalog/). The following table provides an overview of the blocks available from our most popular Prefect Collections.
...
!!! tip "Utilizing the UI"
    Blocks documents can also be created and updated via the [Prefect UI](/2.8/ui/blocks/).
...
!!! tip "Sharing Blocks"
    Blocks can also be loaded by fellow Workspace Collaborators, available on [Prefect Cloud](/2.8/ui/cloud/).
...
This command is useful for registering all blocks found in the credentials module within [Prefect Collections](/2.8/collections/catalog/).
...
The registered block will then be available in the [Prefect UI](/2.8/ui/blocks/) for configuration.
