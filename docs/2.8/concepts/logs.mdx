...
You can view and filter logs in the [Prefect UI](/2.8/ui/flow-runs/#inspect-a-flow-run) or Prefect Cloud, or access log records via the API.
...
You can see logs for the flow run in the Prefect UI by navigating to the [**Flow Runs**](/2.8/ui/flow-runs/#inspect-a-flow-run) page and selecting a specific flow run to inspect.
...
For more information, see [Log schema](/2.8/api-ref/server/schemas/core/#prefect.server.schemas.core.Log) in the API documentation.
