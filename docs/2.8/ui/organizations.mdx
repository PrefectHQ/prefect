...
- Expanded and customizable [Flow Run Retention Policy](/2.8/ui/flow-runs/#flow-run-retention-policy).
- [Single Sign-on (SSO)](/2.8/ui/sso/) authentication using your identity provider (Enterprise plans).
...
**Workspaces** shows you a list of [workspaces you can access within the organization](/2.8/ui/workspaces/). If you have been given the organization Admin role, you can create and manage workspaces here.
...
[Service accounts](/2.8/ui/service-accounts/) enable you to create a Prefect Cloud API key that is not associated with a user account. Service accounts are typically used to configure API access for running agents or executing flow runs on remote infrastructure.
...
See the [Workspace sharing](/2.8/ui/workspaces/#workspace-sharing) documentation for more information.

## Organization and workspace roles

Prefect Cloud enables you to configure both [organization and workspace roles](/2.8/ui/roles/) for users.
...
See the [Roles (RBAC)](/2.8/ui/roles/) documentation for more information on default and custom role permissions.
...
See the [Single Sign-on (SSO)](/2.8/ui/sso/) documentation for more information on default and custom role permissions.