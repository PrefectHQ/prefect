...
### Worker logs
<Warning>This feature is experimental as of Prefect version 3.X.X</Warning>

<span class="badge cloud"></span>Workers can now send logs directly to Prefect Cloud. To enable this feature:

1. Set the environment variable:

```bash
PREFECT_EXPERIMENTS_WORKER_LOGGING_TO_API_ENABLED=True
```
...
- Each flow run log includes a direct link to its associated worker's logs
...
- Runtime Information
  - Current Prefect version
  - Active worker configuration

- Integrations
  - Installed packages (e.g., `prefect-aws`, `prefect-gcp`)
  - Integration-specific configurations and status if available

- Worker Status
   - Live worker logs (if logging is enabled)
   - Current worker health and performance metrics

You can access any worker's detail by clicking on the worker's name in the Work Pool list.
