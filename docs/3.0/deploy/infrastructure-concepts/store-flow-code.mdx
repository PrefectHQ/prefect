...
A few lines of metadata were excluded for brevity.
...
In the examples above, we elected not to push the image to a remote location.

In both cases, any pip packages specified in a `requirements.txt` file will be installed into the image.
...
If you don't store your flow code in a git provider storage or a Docker image, you can stored it in AWS S3, Azure Blob Storage, or GCP GCS.
...
<Tabs>
  <Tab title="AWS S3 bucket">
    ...
    ```python my_s3_deploy_no_credentials.py
    from prefect import flow
    ...
    ```python my_s3_deploy_credentials_s3bucket.py
    ...
    ```python my_s3_deploy_credentials_aws_credentials.py
    ...
    ```python my_s3_deploy_aws_secret_block.py
    ...
    1. If you use the credentials to create an s3Bucket block, you can use the s3Bucket block directly as shown in the example above.
