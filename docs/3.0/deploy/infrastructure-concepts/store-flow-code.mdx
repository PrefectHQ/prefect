...
<Tabs>
  <Tab title="AWS S3 bucket">
    ...
    if __name__ == "__main__":
        source = S3Bucket.load("my-code-storage-block")
    ...
    1. Create an [AWS Credentials block](/integrations/prefect-aws/index#save-credentials-to-an-aws-credentials-block) in code or the Prefect UI. In addition to the block name, most users will fill in the *AWS Access Key ID* and *AWS Access Key Secret* fields.
