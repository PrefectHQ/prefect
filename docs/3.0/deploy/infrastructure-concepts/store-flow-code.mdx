...
We just show the Credentials block example for Azure Blob Storage and GCS, but the same pattern works for any of the cloud-based storage options.

The `prefect.yaml` example includes the additional single line of code needed to reference an AWS Credentials block if using that to authenticate. 
...
<Tabs>
  ...
  <Tab title="GCP GCS bucket">
    ...
    ```
     ...
     To create an `GcpCredentials` block:
...
If you are using a Process work pool, you can use one of the remote code storage options shown above, or you can store your flow code in a local folder.
...
The Python `serve` method is a way to create a deployment and a local long-running process to poll for flow runs at the same time.
...
Read more about the `serve` method [here](/3.0/develop/deployments/serve/).
