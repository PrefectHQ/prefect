...
To create a `prefect.yaml` file interactively, you can run `prefect deploy` from the CLI and select the appropriate storage option.

Python-based deployment creation requires you to use the `flow.from_source` method and pass `source` and `entrypoint` arguments. 
The `entrypoint` is the path to the file the flow is located in and the function name, separated by a colon.
The `source` is the URL to the repository or an object that contains the repository URL and credentials.
...
Alternatively, for a private repository, you can create a Secrete block or a credentials block specific to the git-based platform to store you credentials and reference the block in the Python `deploy` method or the `prefect.yaml` file pull step.
...
<Tabs>
  <Tab title="GitHub">
    ...
    ```python my_gh_deploy_no_credentials_passed_explicitly.py
  ...
  <Tab title="Bitbucket">
    ...
    ```python my_bb_deploy_no_credentials_passed_explicitly.py
    from prefect import flow
    ...
    ```python my_private_bb_deploy_secret_block.py
