...
You can use create a deployment through Python code or a YAML specification defined in a `prefect.yaml` file.

To create a `prefect.yaml` file interactively, you can run `prefect deploy` and select the appropriate storage option.
...
Alternatively, for a private repository, you can create a block to store you credentials and reference it in your deployment. 
You can create a Secret block or use a credentials block for the specific git-based remote storage platform.

The examples below show each of these options.


Alternatively, instead of creating a Secret block, you can create a credentials block specific to the remote storage platform ahead of time and reference it in the `prefect.yaml` pull step or the Python `deploy` method.
...
<Tabs>
  <Tab title="GitHub">
    ...
    ```python my_gh_repo_no_credentials_passed_explicitly.py
...
To pass credentials explicitly to authenticate to your storage location, you can use any of the following block types:
...
For Azure Blob Storage and GCS we just show passing the credentials block explicitly, but the same pattern works for any of the cloud-based storage options.

The `prefect.yaml` example includes the additional line to reference a credentials block if authenticating through that option. 
...
<Tabs>
  <Tab title="AWS S3 bucket">
    ...
    if __name__ == "__main__":
        flow.from_source(
            source=S3Bucket.load(
                "my-code-storage-block"), 
            entrypoint="my_file.py:my_flow"
        ).deploy(name="my-aws-s3-deployment", 
        work_pool_name="my-work-pool"
    )
    ```
...
The Python [`serve` method](/3.0/develop/deployments/serve/) is a way to create a deployment and a local long-running process to poll for flow runs at the same time.
