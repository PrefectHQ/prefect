...
If you are using a private repository, we suggest creating a [personal access token](https://docs.github.com/en/authentication/keeping-your-account-and-data-secure/managing-your-personal-access-tokens) 
...
To create a deployment with Docker-based flow code storage:
...
build:
  - prefect_docker.deployments.steps.build_docker_image:
        requires: prefect-docker>=0.6
        image_name: my-image
        image_tag: "{{ get-commit-hash.stdout }}"
        dockerfile: auto
        push: true    # push image to a Docker registry, Docker Hub by default 
...
- prefect.deployments.steps.pip_install_requirements:  # optional section 
...
<Note>
    If you don't specify an image as part of your deployment creation, the image specified in the work pool runs your flow.
</Note>

With the Python `deploy` method, if a requirements.txt file is present in the directory, it will be discovered and packages will be installed into the image with pip.


## Cloud-provider storage

Flow code can be stored in an AWS S3, Azure Blob Storage, or GCP GCS.

If the storage is publicly available or if you are authenticated in the environment where you are creating and running your deployment, you can reference storage location directly.  `from_source`  `push` and `pull` steps of your `prefect.yaml` file.
