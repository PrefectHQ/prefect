...
Each of these options is popular.
Your choice will depend upon your team's needs and tools.
...
Alternatively, for a private repository, you can create a Secret block or create a credentials block specific to your git-based version control platform to store you credentials. 
...
<Tab title="GitLab">
    ...
    if __name__ == "__main__":
        flow.from_source(
            source="https://gitlab.com/org/my-public-repo.git",
...
- Prefect integration library storage blocks, such as the `prefect-aws` library's `S3Bucket` block.
- Prefect integration library credentials blocks, such as the `prefect-aws` library's `AwsCredentials` block 
- Secret blocks
...
<Tabs>
  <Tab title="AWS S3 bucket">
    ...
    ```python s3_credentials_block.py
  ...
  <Tab title="Azure Blob Storage container">
    ...
    ```python azure_credentials_block.py
    from prefect import flow
  ...
  <Tab title="GCP GCS bucket">
    ...
    ```python gcs_credentials_block.py
    from prefect import flow
