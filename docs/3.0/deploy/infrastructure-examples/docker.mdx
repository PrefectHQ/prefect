...
and then you may execute your deployment via the Prefect API (i.e. SDK, CLI, UI, etc).

You must have [Docker](https://docs.docker.com/engine/install/) installed and running on your machine.

<Note>
**Want to run flows in a long-lived container?**

This guide shows you how to run a flows in ephemeral containers that are cleaned up after the flow run completes.
To learn how to run flows in a static, long-lived container, see [this](/3.0/deploy/static-infrastructure-examples/docker/) guide.
...
If `my-docker-pool` does not already exist, the above command will create it for you automatically with the default settings for that work pool type, in this case `docker`.
</Tip>
