...
The server process runs in a single process and waits for flow runs to be triggered.

If you want to run your flow on remote infrastructure, need more control over the execution environment, or want to create a template for other deployments to use, use a deployment with a work pool.

### Create a work pool-based Deployment

### Create a deployment with a Python `flow.deploy` method

Prefect offers two primary methods for creating deployments that use a work pool.
...
Prefect also provides [CI/CD options](/3.0/deploy/infrastructure-concepts/deploy-ci-cd) for automatically creating deployments. 

### Work pools
[Work pools](/3.0/deploy/infrastructure-concepts/work-pools/) are used to easily switch between different types of infrastructure and create a template for deployments to use.
...
The deployment inherits the infrastructure configuration from the work pool, and can be overrun at deployment creation time or runtime. 

### Work pools that require a worker

To kick off a scheduled deployment run, if your work pool is a hybrid type, you must start a [`prefect worker`](/3.0/deploy/infrastructure-concepts/workers/). 
