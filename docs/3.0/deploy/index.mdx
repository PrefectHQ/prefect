...
This example also create a schedule so that the flow will run every minute of every day.

For slightly more configuration you can create a deployment that uses a work pool.
Reasons to create a work-pool based deployment include:

- Wanting to run your flow dynamically on remote infrastructure
- Needing more control over the execution environment
- Creating a template for other deployments
