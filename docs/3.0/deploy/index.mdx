...
Some of the most common reasons to use an orchestration tool such as Prefect are for
[scheduling](/3.0/automate/add-schedules/) and [event-based triggering](/3.0/automate/events/automations-triggers/).
In addition to manually triggering and managing flow runs, deploying a flow exposes an API and
...
- remotely configure [schedules](/3.0/automate/add-schedules) and [automation rules](/3.0/automate/events/automations-triggers) for deployments
- dynamically provision infrastructure with [work pools](/3.0/deploy/infrastructure-concepts/work-pools)

## Create a deployment 

### Create a deployment with a local `serve` method

A deployment can be defined in Python with the `.serve` method. 
This is a great way to get started and to learn about deployments.
...
This methods creates a deployment object that can be used to trigger flow runs.
It starts a server and registers the flow with the Prefect API.
The process runs locally and waits to pick up triggered flow runs on the server

If you want to run your flow on remote infrastructure, need more control over the execution environment, or want to create a template for other deployments, use a deployment with a work pool.

### Create a work pool-based deployment

Prefect offers two primary options for creating deployments that use a work pool: a Python method and a YAML file.

### Create a deployment with a Python `flow.deploy` method

To define a deployment through Python code, use the `flow.deploy` method.
...
Prefect also provides [CI/CD options](/3.0/deploy/infrastructure-concepts/deploy-ci-cd) for automatically creating YAML-baseddeployments. 
...
2. Chain the `from_source` method to a flow object or flow function and specify one of the following:
    1. the git-based cloud provider location (for example, GitHub) 
    2. the cloud provider storage location (for example, AWS S3)
    3. the local path

See the [Retrieve code from storage docs](/3.0/deploy/infrastructure-concepts/store-flow-code) for more information.
...
To kick off a scheduled deployment run, if your work pool is a hybrid type, you must start a [worker](/3.0/deploy/infrastructure-concepts/workers/). 

A [Prefect worker](/3.0/deploy/infrastructure-concepts/workers) is a client-side process that checks for scheduled flow runs in the work pool.
When a scheduled run is found, the worker kicks off a flow run on the specified infrastructure and monitors the flow run until it completes.
...
This is key to the Prefect hybrid model used with hybrid work pool types: there's a boundary between your proprietary assets,
