...
The `.serve` method allows you to easily elevate a flow to a deployment, immediately executing scheduled work [as a local process](/3.0/deploy/run-flows-in-local-processes).

However, this _"local"_ process does not need to be on your local machine.

## Overview
In this example, you will set up:

- a simple flow that retrieves the number of stars for some GitHub repositories
- a `Dockerfile` that packages up your flow code and dependencies into a container image that can be run on any machine

You will need to have [Docker](https://docs.docker.com/engine/install/) installed and running on the machine where you want to run your flow.

## Writing the flow

Say we have a flow that retrieves the number of stars for a GitHub repository:

```python serve_retrieve_github_stars.py {16-20}
import httpx
...
Now that we have a flow and a Dockerfile, we can build the the image from the Dockerfile and run a container from this image.

### Build (and push) the image

We can build the image using the `docker build` command, using the `-t` flag to specify a name for the image.

```bash
docker build -t my-flow-image .
```

At this point, you may also want to push the image to a container registry like [Docker Hub](https://hub.docker.com/) or [GitHub Container Registry](https://docs.github.com/en/packages/working-with-a-github-packages-registry/working-with-the-container-registry). Please see the registries' respective documentation for details on authentication and registry naming conventions.

### Run the container

In practice, you'll likely want to inject some environment variables into your container, so let's define a `.env` file:
...
Then, run the container in [detached mode](https://docs.docker.com/engine/reference/commandline/run/#detached-d) (i.e. in the background):
...
You may now easily deploy this container to other infrastructures, like:
