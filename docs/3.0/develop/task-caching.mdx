...
Configuring a cache policy with a `storage` argument allows cache records to be stored separately from task results. 

When cache storage is configured, persisted task results will only include the return value of your task and cache records can be deleted or modified 
...
cache_policy = (TASK_SOURCE + INPUTS).configure(storage="/path/to/cache/storage")
...
cache_policy = (TASK_SOURCE + INPUTS).configure(storage=S3Bucket.load("my-bucket"))
