...
By default, cache records are collocated with task results. This means that files containing task results will also include metadata used for caching. 
Configuring a cache policy with a `storage` argument will cause cache records to be stored separately from task results. 

This separation means that persisted task results will only include the return value of your task and cache records can be deleted or modified 
without effecting your task results.

You can configure where cache records are stored by using the `.configure` method with a `storage` argument on a cache policy.
The `storage` argument accepts either a path to a local directory or a storage block.
...
@task(cache_policy=(TASK_SOURCE + INPUTS).configure(storage="/path/to/cache/storage"))
def my_cached_task(x: int):
    return x + 42
```
...
To store cache records in a remote object store like S3, pass a storage block instead:
...
@task(cache_policy=(TASK_SOURCE + INPUTS).configure(storage=S3Bucket.load("my-bucket")))
def my_cached_task(x: int):
    return x + 42
```
...
By default, cache records operate with a `READ_COMMITTED` isolation level. This guarantees that reading a cache record will see the latest committed value,
