...
These types are what separate blocks from [Prefect variables](/3.0/develop/variables/), which are unstructured JSON documents.
...
For example, an `S3` block contains methods for downloading data from, or upload data to, an S3 bucket; a `SnowflakeConnector` block contains methods for querying Snowflake databases.
...
<Warning>
It is very important to note that the class itself is _not_ stored server-side when registering block types and block documents. 
For this reason, we highly recommend loading block documents by first importing the block class and then calling its `load` method with the relevant document name.
</Warning>
...
To update a prevously saved block document, overwrite the existing document by passing `overwrite=True`:
...
Alternatively, we can also load from previously saved credentials:
