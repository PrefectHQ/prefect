...
Prefect 3 introduces exciting new features and improvements while maintaining compatibility with most Prefect 2 workflows. 
To learn more about the enhanced performance and new features, see [What's new in Prefect 3](/resources/whats-new-prefect-3).

For the majority of users, upgrading to Prefect 3 will be a seamless process that requires few or no code changes. 
This guide highlights key changes that you may need to consider when upgrading.

<Note>
[Prefect Cloud](/3.0/manage/cloud) is compatible with both Prefect 3 and Prefect 2 clients. 
</Note>
...
In Prefect 2, it was possible to call native `async` tasks from synchronous flows, a pattern that is not normally supported in Python. Prefect 3 removes this behavior to reduce complexity and potential issues and edge cases. If you relied on asynchronous tasks in synchronous flows, you must either make your flow asynchronous or use a task runner that supports asynchronous execution. 
