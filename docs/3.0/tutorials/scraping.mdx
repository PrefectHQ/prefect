...
@task
def fetch_page_of_issues(repo: str, page: int = 1) -> Optional[List[dict]]:
...
Using subflows and tasks can help you distribute tasks more efficiently and aid with debugging.

* Use subflows for more complex operations that involve multiple steps.
* Use tasks for simpler, atomic operations.

Here's an example of how to use subflows and tasks:
...
def fetch_repo_issues(repo: str):
    """Subflow: Fetch all data for a single repository"""
...
def calculate_response_times(issues: List[dict]) -> float:
    """Task: Calculate average time to first response for issues"""
...
def analyze_repo_health(repos: List[str]):
    ...
    for repo in repos:
        # Fetch sample issues
        issues = fetch_page_of_issues(repo)
        
        # Calculate metrics
        metrics = calculate_resolution_rate(issues)
...
@task
def fetch_page_of_issues(repo: str) -> List[dict]:
