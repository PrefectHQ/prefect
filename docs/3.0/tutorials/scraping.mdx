...
description: Use Prefect to deal with the complexity of fetching and analyzing an indeterminate amount of data
---

In the [Build a data pipeline](/3.0/tutorials/pipelines) tutorial, you learned how to create resilient and performant data pipelines.
Now you'll learn how to handle unstructured data and complex API responses by building a GitHub issue analysis pipeline.

The real world presents additional challenges when dealing with web data:

- API responses can contain missing or malformed data
- You need to make multiple dependent API calls
- You need to handle pagination and rate limits
- You want to process data in parallel when possible
...
def analyze_repo_health(repos: [str]):
    """Analyze issue health metrics for a GitHub repository"""
    for repo in repos:
        try:
            issue = fetch_issue_details(repo, 1)
...
def fetch_issue_details(repo: str, issue_number: int) -> dict:
    ...
    response.raise_for_status() # Throw an exception if you don't get a 2xx response
...
Use pagination to fetch large amounts of data and run tasks in parallel to analyze it efficiently:
...
def analyze_repo_health(repos: [str]):
    ...
    # Run issue analysis tasks in parallel
    for issue in all_issues:
        analyze_issue.submit(issue)
...
<Expandable title="full example">
Run the following code to see pagination and parallel tasks in action:
...
def analyze_repo_health(repos: [str]):
    ...
    # Run issue analysis tasks in parallel
    issue_details = []
    for issue in all_issues[:5]:  # Analyze first 5 issues to avoid hitting rate limits
...
@task
def fetch_issues(repo: str, page: int = 1) -> Optional[List[dict]]:
...
if __name__ == "__main__":
    analyze_repo_health(["PrefectHQ/prefect"])
```
</Expandable>

## Use dependent tasks to calculate metrics

Add dependent tasks to analyze the issue data and calculate health metrics:

```python
@task
def calculate_metrics(issues: List[dict]):
    """Calculate basic repository health metrics"""
    total = len(issues)
    closed = sum(1 for issue in issues if issue["state"] == "closed")
    resolution_rate = (closed / total) * 100 if total > 0 else 0
    return {"total": total, "resolution_rate": resolution_rate}
```
...
def analyze_repo_health(repos: List[str]):
    ...
    for repo in repos:
        ...
        response_times = calculate_response_times(issues)
        
        print(f"\nRepository Health Metrics for {repo}:")
        print(f"Total Issues: {metrics['total']}")
        print(f"Resolution Rate: {metrics['resolution_rate']:.1f}%")
        print(f"Average Response Time: {response_times:.1f} hours")
...
def analyze_repo_health(repos: List[str]):
    ...
    for repo in repos:
        ...
        issues = analyze_single_repo(repo)
...
@flow(log_prints=True)
def analyze_single_repo(repo: str):
    """Analyze all issues for a single repository"""
    all_issues = []
    page = 1
...
def fetch_issues(repo: str, page: int = 1) -> Optional[dict]:
    """Fetch a page of issues for a GitHub repository"""
...
10:59:27.632 | INFO    | Flow run 'robust-kangaroo' - Analyzing tensorflow/tensorflow...
...
...
- Modularized workflows with dependent subflows and tasks
- Efficient ingestion and processing of large data with pagination and parallel tasks
