...
```bash
# This allows 3 tasks to run concurrently, with a 1-second delay before the next slot is freed up.
prefect gcl create github-api --limit 3 --slot-decay-per-second 1.0
```
...
In this tutorial, you built a resilient and performant data pipeline using retries, parallel tasks, concurrency limits, and caching.
