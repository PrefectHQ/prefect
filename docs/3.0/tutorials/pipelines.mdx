...
## Enable concurrency for slow tasks
...
<Expandable title="full example">
Run the following code to see concurrency in action:
...
To avoid violating the rate limits of whatever API you're using, use Prefect to set a global concurrency limit.
...
def show_stars(github_repos: list[str]):
    ...
    repo_stats = []
    for repo in github_repos:
        # Apply a rate limit to this loop
...
<Expandable title="full example">
Run the following code to see rate limits in action:
