...
In the [Quickstart](/3.0/get-started/quickstart), you created a Prefect flow to get stars for a list of GitHub repositories.
...
If individual API requests are slow, you can speed them up in aggregate by making multiple requests in parallel.
...
def show_stars(github_repos: list[str]):
    ...
    # Task 1: Make HTTP requests in parallel
    repo_stats = []
    for repo in github_repos:
        repo_stats.append({
            'repo': repo,
    ...
    # Task 2: Once all parallel tasks have completed, show the results
...
<Expandable title="full example">
Run the following code to see parallel tasks in action:
...
One consequence of running tasks in parallel is that you're more likely to hit the rate limits of whatever API you're using.
...
def show_stars(github_repos: list[str]):
    ...
    # Task 1: Make HTTP requests in parallel while respecting concurrency limits
    repo_stats = []
    for repo in github_repos:
        rate_limit("github-api")
        repo_stats.append({
...
09:08:12.374 | INFO    | Flow run 'laughing-nightingale' - tensorflow/tensorflow: 186319 stars
...
- [Retries](/3.0/develop/write-tasks#retries) to handle transient errors
- [Parallel tasks](/3.0/develop/task-runners) to speed up slow tasks
- [Concurrency limits](/3.0/develop/global-concurrency-limits) to avoid hitting the rate limits of your APIs
- [Caching](/3.0/develop/task-caching) to skip repeated tasks

Next, learn how to [handle data dependencies and ingest large amounts of data](/3.0/tutorials/scraping).
