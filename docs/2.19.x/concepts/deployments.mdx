...
Every Prefect deployment references one and only one "entrypoint" flow (though that flow may itself call any number of nested flows). Different deployments may reference the same underlying flow, a useful pattern when developing or promoting workflow changes through staged environments.
...
If you call `run_deployment` from within a flow or task, the scheduled flow run will be linked to the calling flow run (or the calling task's flow run) as a nested flow run by default.

Nested flow runs have different behavior than regular flow runs. For example, a nested flow run can't be suspended independently of its parent flow. If you'd rather not link the scheduled flow run to the calling flow or task run, you can disable this behavior by passing `as_subflow=False`:
...
@flow
def my_flow():
    # The scheduled flow run will not be linked to this flow as a nested flow.
