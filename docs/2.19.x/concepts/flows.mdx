...
Flows can include calls to [tasks](https://docs.prefect.io/concepts/tasks/) as well as to other flows, which Prefect calls ["nested flows"](#composing-flows) in this context. Flows may be defined within modules and imported for use as nested flows in your flow definitions.
...
You may call any number of other tasks, nested flows, and even regular Python functions within your flow. You can pass parameters to your flow function that will be used elsewhere in the workflow, and Prefect will report on the progress and [final state](#final-state-determination) of any invocation.
...
A _subflow_ run is created when a flow function is called inside the execution of another flow. The primary flow is the "parent" flow. The flow created within the parent is the "child" flow or "nested flow."
...
<Note>
**Nested flows can't be suspended independently of their parent run**
