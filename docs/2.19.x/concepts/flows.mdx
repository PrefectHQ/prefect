...
Subflow runs behave like normal flow runs. There is a full representation of the flow run in the backend as if it had been called separately. When a nested flow starts, it will create a new [task runner](https://docs.prefect.io/concepts/task-runners/) for tasks within the nested flow. When the nested flow completes, the task runner is shut down.

Subflows will block execution of the parent flow until completion. However, asynchronous nested flows can be run concurrently by using [AnyIO task groups](https://anyio.readthedocs.io/en/stable/tasks.html) or [asyncio.gather](https://docs.python.org/3/library/asyncio-task.html#id6).
...
Here's a parent flow that imports and uses `my_subflow()` as a nested flow:
...
15:19:22.107 | INFO    | Flow run 'daft-cougar' - Created nested flow run 'ninja-duck' for flow 'Subflow'
...
Subflows enable you to group related tasks within your workflow. Here are some scenarios where you might choose to use a nested flow rather than calling tasks individually:

*   Observability: Subflows, like any other flow run, have first-class observability within the Prefect UI and Prefect Cloud. You'll see nested flow status in the **Flow Runs** dashboard rather than having to dig down into the tasks within a specific flow run. See [Final state determination](#final-state-determination) for some examples of leveraging task state within flows.
...
*   Task runners: Subflows enable you to specify the task runner used for tasks within the flow. For example, if you want to optimize parallel execution of certain tasks with Dask, you can group them in a nested flow that uses the Dask task runner. You can use a different task runner for each nested flow.
