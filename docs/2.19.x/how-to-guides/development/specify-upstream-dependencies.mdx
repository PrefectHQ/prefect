...
Results from a task can be provided to other tasks (or nested flows) as upstream dependencies. Prefect uses upstream dependencies in two ways:
...
**Only results from tasks** inform Prefect's ability to determine dependencies. Return values from functions without task decorators, including nested flows, do not carry the same information about their origin as task results.
</Note>
...
A task or nested flow's upstream dependencies can be inferred automatically via its inputs, or stated explicitly via the `wait_for` parameter.
...
By default, deployments started from `run_deployment` will also appear as nested flows for tracking purposes. This behavior can be disabled by setting the `as_subflow` parameter for `run_deployment` to `False`.
