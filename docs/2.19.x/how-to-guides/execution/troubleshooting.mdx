...
Keep in mind that [integrations](https://docs.prefect.io/integrations/) are versioned and released independently of the core Prefect library. They should be upgraded simultaneously with the core library, using the same method.
</Note>

Logs
-------------------------------

In many cases, there will be an informative stack trace in Prefect's [logs](https://docs.prefect.io/concepts/logs/). **Read it carefully**, locate the source of the error, and try to identify the cause.
...
Note: requests to nested URLs may exhibit the _opposite_ behavior and require no trailing slash:
