...
This retention policy applies to all [details about a flow run](https://docs.prefect.io/ui/flow-runs/#inspect-a-flow-run), including its task runs. 
Nested flow runs are retained independently from their parent flow runs. Each nested flow run is deleted based on the time it reached a terminal state.
