...
This retention policy applies to all [details about a flow run](https://docs.prefect.io/ui/flow-runs/#inspect-a-flow-run), including its task runs. Nested flow runs follow the retention policy independently from their parent flow runs, and are removed based on the time each nested flow run reaches a terminal state.
