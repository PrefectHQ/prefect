...
|  `--override TEXT`       | One or more optional infrastructure overrides provided as a dot delimited path. For example, specify an environment variable: `env.env_key=env_value`. For Kubernetes, specify customizations: `customizations='[{"op": "add","path": "/spec/limits", "value": {"memory": "8Gi","cpu": "4000m"}}]'` (note the string format).|
