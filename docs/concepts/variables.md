...
Variables are dynamic-named, mutable string values, much like environment variables. Variables are scoped to a Prefect Server instance or a single workspace in Preflect Cloud. Varibles can be created or modified at any time. While Variables are most commonly interpolated at during flow runtime via Prefect's Python SDK, they can be interpolated at any time, such that they can be used to pass configuration information to Prefect services, such as workers.
...
Variables can be created via the Prefect UI or REST API. To create a new variable, navigate to the Variables page 
