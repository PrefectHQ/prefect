...
9. **Mark Cloud-only features.** Features available only in Prefect Cloud (SSO, audit logs, incidents, SLAs, the `send-email-notification` automation action, etc.) must be visibly marked so they are discoverable from the open source docs. Use a `<span class="badge cloud"></span>` badge on its own line above the page or section it marks (it renders a standalone "Prefect Cloud" label; put a blank line before the following prose), or a `<Note>This is only available in Prefect Cloud.</Note>` callout for inline cases (e.g. a single table row). Cloud-only how-to guides belong under `v3/how-to-guides/cloud/`. See `contribute/docs-contribute.mdx`.