...
!!! tip "[Choosing Between `flow.deploy()` and `flow.serve()`](/2.18.3/concepts/deployments/#two-approaches-to-deployments)"
...
- Work pools can be used to prioritize (or limit) flow runs through the use of [work queues](/2.18.3/concepts/work-pools/#work-queues).

Prefect provides several [types of work pools](/2.18.3/concepts/work-pools/#work-pool-types).
...
!!! note "Prefect Cloud"
    ...
    If you are not using Prefect Cloud, please learn about work pools below and then proceed to the [next tutorial](/2.18.3/tutorial/workers/) that uses worker-based work pools.
...
See the [Managed Execution guide](/2.18.3/guides/managed-execution/) for more details.
...
See the [schedules](/2.18.3/concepts/schedules/) docs for more information on scheduling options.

See the [Push Work Pool guide](/2.18.3/guides/deployment/push-work-pools/) for more details and example commands for each cloud provider.

## Next step

Congratulations!
You've learned how to deploy flows to work pools.
If these work pool options meet all of your needs, we encourage you to go deeper with the [concepts docs](/2.18.3/concepts/) or explore our [how-to guides](/2.18.3/guides/) to see examples of particular Prefect use cases.

However, if you need more control over your infrastructure, want to run your workflows in Kubernetes, or are running a self-hosted Prefect server instance, we encourage you to see the [next section of the tutorial](/2.18.3/tutorial/workers/).
