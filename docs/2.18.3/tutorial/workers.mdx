...
Different [worker types](/2.18.3/concepts/work-pools/#worker-types) create different types of flow run infrastructure.</sup>
...
!!! tip "Other work pool types"
    There are [work pool types](/2.18.3/concepts/work-pools/#worker-types) for serverless computing environments such as AWS ECS, Azure Container Instances, Google Cloud Run, and Vertex AI.
    Kubernetes is also a popular work pool type.

    These options are expanded upon in various [How-to Guides](/2.18.3/guides/).
...
Therefore, in a production setting this worker should run as a [daemonized or managed process](/2.18.3/guides/deployment/daemonize/).
...
1. [A flow](/2.18.3/tutorial/flows/)
2. A work pool
3. A worker
...
The `job_variables` section allows you to fine-tune the infrastructure settings for a specific deployment. These values override default values in the specified work pool's [base job template](/2.18.3/concepts/work-pools/#base-job-template).
...
- Go deeper with deployments and learn about configuring deployments in YAML with [`prefect.yaml`](/2.18.3/guides/prefect-deploy/).
- [Concepts](/2.18.3/concepts/) contain deep dives into Prefect components.
- [Guides](/2.18.3/guides/) provide step-by-step recipes for common Prefect operations including:
  - [Deploying flows on Kubernetes](/2.18.3/guides/deployment/kubernetes/)
  - [Deploying flows in Docker](/2.18.3/guides/deployment/docker/)
  - [Deploying flows on serverless infrastructure](/2.18.3/guides/deployment/serverless-workers/)
  - [Daemonizing workers](/2.18.3/guides/deployment/daemonize/)
