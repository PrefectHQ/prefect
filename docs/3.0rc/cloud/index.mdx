...
!!! success "Getting Started with Prefect Cloud"
    Ready to jump right in and start running with Prefect Cloud? See the [Quickstart](/3.0rc/getting-started/quickstart/) and follow the instructions on the **Cloud** tabs to write and deploy your first Prefect Cloud-monitored flow run.
...
!!! cloud-ad "Prefect Cloud features"
    ...
    - [Automations](/3.0rc/cloud/automations/) &mdash; configure triggers, actions, and notifications in response to real-time monitoring events.
    - [Email notifications](/3.0rc/cloud/automations/) &mdash; send email alerts from Prefect's server based on automation triggers.
    - [Service accounts](/3.0rc/cloud/users/service-accounts/) &mdash; configure API access for running workers or executing flow runs on remote infrastructure.
    - [Custom role-based access controls (RBAC)](/3.0rc/cloud/users/roles/) &mdash; assign users granular permissions to perform certain activities within an account or a workspace.
    - [Single Sign-on (SSO)](/3.0rc/cloud/users/sso/) &mdash; authentication using your identity provider.
    - [Audit Log](/3.0rc/cloud/users/audit-log/) &mdash; a record of user activities to monitor security and compliance.
    ...
    - [Push work pools](/3.0rc/guides/deployment/push-work-pools/) &mdash; run flows on your serverless infrastructure without running a worker.
...
Custom plans have [object-level access control lists](/3.0rc/cloud/users/object-access-control-lists/), [custom roles](/3.0rc/cloud/users/roles/), [teams](/3.0rc/cloud/users/teams/), and [single sign-on (SSO)](#single-sign-on-(sso) with [Directory Sync/SCIM provisioning](/3.0rc/cloud/users/sso/#scim-provisioning).
...
- [Flow runs](/3.0rc/concepts/flows/) and task runs executed in an environment that is [syncing with the workspace](/3.0rc/cloud/workspaces/)
- [Flows](/3.0rc/concepts/flows/) associated with flow runs and deployments observed by the Prefect Cloud API
- [Deployments](/3.0rc/concepts/deployments/)
- [Work pools](/3.0rc/concepts/work-pools/)
- [Blocks](/3.0rc/concepts/blocks/)
- [Events](/3.0rc/cloud/events/)
- [Automations](/3.0rc/concepts/automations/)
- [Incidents](/3.0rc/cloud/incidents/)

![Viewing a workspace dashboard in the Prefect Cloud UI.](/3.0rc/img/ui/cloud-new-workspace.png)

## Events

Prefect Cloud allows you to see your [events](/3.0rc/cloud/events/). Events provide information about the state of your workflows, and can be used as [automation](/3.0rc/concepts/automations/) triggers.

![Prefect UI](/3.0rc/img/ui/event-feed.png)

## Automations

Prefect Cloud [automations](/3.0rc/concepts/automations/) provide additional notification capabilities beyond those in a self-hosted open-source Prefect server.
...
Prefect Cloud's [incidents](/3.0rc/cloud/incidents/) help teams identify, rectify, and document issues in mission-critical workflows.
Incidents are formal declarations of disruptions to a workspace.
With [automations](/3.0rc/cloud/incidents/#incident-automations)), activity in that workspace can be paused when an incident is created and resumed when it is resolved.
...
See the [service accounts](/3.0rc/cloud/users/service-accounts/) documentation for more information about creating and managing service accounts.
...
See the [role-based access controls (RBAC)](/3.0rc/cloud/users/roles/) documentation for more information about managing user roles in a Prefect Cloud account.

## Single Sign-on (SSO) <span class="badge pro"></span> <span class="badge custom"></span>

Prefect Cloud's [Pro and Custom plans](https://www.prefect.io/pricing) offer [single sign-on (SSO)](/3.0rc/cloud/users/sso/) authentication integration with your team’s identity provider.
...
Prefect Cloud's [Pro and Custom plans](https://www.prefect.io/pricing) offer [Audit Logs](/3.0rc/cloud/users/audit-log/) for compliance and security.
...
Then follow the steps in the UI to deploy your first Prefect Cloud-monitored flow run. For more details, see the [Prefect Quickstart](/3.0rc/getting-started/quickstart/) and follow the instructions on the **Cloud** tabs.
