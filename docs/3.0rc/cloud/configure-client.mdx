...
If using a [worker](/3.0rc/deploy/dynamic-infra/control-runs/) that can create flow runs for deployments in remote environments, 
...
A common example is setting the `PREFECT_API_URL`, which you may need to change when interacting with different Prefect server 
