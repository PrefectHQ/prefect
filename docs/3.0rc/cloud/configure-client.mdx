...
!!! tip "`PREFECT_API_URL` setting for workers"
    ...
    If using a [worker](/3.0rc/deploy/dynamic-infra/control-runs/) that can create flow runs for deployments in remote environments,  [`PREFECT_API_URL`](/3.0rc/cloud/configure-client/) must be set for the environment in which your worker is running.
...
!!! tip "Running the Prefect UI behind a reverse proxy"
    ...
    You can also set [`PREFECT_API_URL`](/3.0rc/cloud/configure-client/#prefect.settings.PREFECT_API_URL) to the API URL. This setting is a fallback if `PREFECT_UI_API_URL` is not set.
