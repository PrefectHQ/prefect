...
After installing Prefect, you have a Python SDK client that can communicate with either [Prefect Cloud](/3.0rc/cloud/) or a self-hosted Prefect server, backed by a database and a UI.
...
- [Email notifications](/3.0rc/react/understand-events/automations-triggers/)&mdash;send email alerts from Prefect's servers based on automation triggers
...
- [Custom role-based access controls (RBAC)](/3.0rc/cloud/manage-users/manage-roles/)&mdash;assign users granular permissions to perform activities within an account or workspace
...
Your self-hosted server must meet certain requirements and be properly configured. 

### SQLite

Hosting your own Prefect server instance requires a PostgreSQL database, but you can use SQLite 2.24 or newer for development. 
...
You can add the certificate to your system bundle and point your tools to use that bundle by configuring the `SSL_CERT_FILE` environment variable.
...
Whether you are using Prefect Cloud or hosting your own Prefect server instance, set `HTTPS_PROXY` and `SSL_CERT_FILE` in your environment, and the underlying network libraries route Prefect’s requests appropriately.

Alternatively, the Prefect library connects to the API via any proxies you have listed in the `HTTP_PROXY` or `ALL_PROXY` environment variables.
...
You can save the API server address in a [Prefect profile](/3.0rc/cloud/configure-client/). Whenever that profile is active, the API endpoint is at that address.

See [Profiles & Configuration](/3.0rc/cloud/configure-client/) for more information on profiles and configurable Prefect settings.
...
[Prefect Cloud](/3.0rc/cloud/) gives you access to a hosted platform with Workspace & User controls, Events, and Automations. Prefect Cloud has an option for automation notifications. The more limited Notifications option is provided for the self-hosted Prefect server.

Notifications enable you to set up alerts that are sent when a flow enters any state you specify. When your flow and task runs changes [state](/3.0rc/develop/observe-workflows/manage-states/), Prefect notes the state change and checks whether the new state matches any notification policies. If it does, a new notification is queued.
...
!!! cloud-ad "Notifications in Prefect Cloud"
    Prefect Cloud uses the robust [Automations](/3.0rc/react/understand-events/automations-triggers/) interface to enable notifications related to flow run state changes and work pool status.
