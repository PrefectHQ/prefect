...
**Additional work pool options available with Prefect Cloud**
    ...
    - **[Managed Execution](/3.0rc/guides/managed-execution/)**: Prefect Cloud submits and runs your 
...
- What instructions does a [worker](/3.0rc/deploy/dynamic-infra/control-runs/) require to set up an execution environment for your flow? 
...
[Serve workflows](/3.0rc/deploy/serve-workflows/) shows how to create a deployment with a long-running process using `.serve`, 
...
Make the [work pool](/3.0rc/deploy/dynamic-infra/control-runs/) exists in the Prefect Cloud workspace you are 
...
found in the current environment. Each step can specify an `id` to reference outputs 
later. The additional fields map directly to Python keyword arguments to the step function. Within a given section, 
...
Note that in the build step example above, you relied on the `prefect-docker` package; in cases that deal with external services, 
...
Inspecting our newly created `prefect.yaml` file you find that the `push` and `pull` sections have been templated out for us as follows:
...
flows for a deployment run. These instructions will be executed each time a deployment created within this folder is run through a worker.
...
So long as our `build` steps produce fields called `image_name` and `tag`, every time you deploy a new version of our deployment, 
...
**CLI Options When Deploying Multiple Deployments**
    ...
    When deploying more than one deployment with a single `prefect deploy` command, any additional attributes provided through 
...
In the above example, you are using YAML aliases to reuse work pool, schedule, and build configuration across multiple deployments:
...
- Next, all variable and block references are resolved with the deployment declaration.  All flags provided through the `prefect deploy` CLI are then overlaid on the values loaded from the file.
