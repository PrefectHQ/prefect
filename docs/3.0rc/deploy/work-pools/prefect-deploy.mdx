...
A deployment turns your flow into an application you can manage through the Prefect API.
...
Work pool-based deployments contain information about the infrastructure type and configuration for your flow execution.
...
While baking code into Docker images is a popular deployment option, many teams store their flow code in git-based 
...
This is useful if using a monorepo approach to your flows.
...
- **variables**: you can reference [Prefect variables](/3.0rc/concepts/variables) with the  
...
For more information, see the [Schedules](/3.0rc/deploy/serve-flows/add-schedules/#creating-schedules-through-a-deployment-yaml-files-schedule-section).
