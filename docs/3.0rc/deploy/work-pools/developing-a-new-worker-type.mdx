...
This guide is for users who want to extend the Prefect framework. Completing this successfully  
requires deep knowledge of Prefect concepts. For standard use cases, we recommend using one of the 
...
This configuration class populate the `job_configuration` section of the resulting base job template.
...
For example, a worker that executes flow runs in AWS Fargate need AWS credentials. As a worker developer, you can use 
...
Prefect discovers this entry point and load your work module in the specified module. The entry point allows the 
