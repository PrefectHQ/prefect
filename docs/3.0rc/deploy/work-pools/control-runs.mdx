...
When a [deployment](/3.0rc/deploy/serve-workflows/) creates a flow run, it is submitted to a specific work pool for scheduling. 
...
[Managed work pools](/3.0rc/guides/managed-execution) are administered by Prefect and handle the submission and execution of code on your behalf.
...
Work pools have three statuses: , , and .
...
If your deployment's infrastructure block supports it, you can use work pools to temporarily send runs to a worker 
running on your local machine for debugging. Do this by running `prefect worker start -p my-local-machine` and updating the deployment's 
work pool to `my-local-machine`.
...
**`PREFECT_API_URL` and `PREFECT_API_KEY`settings for workers**
    ...
    with the `Worker` role, which you caan configure by setting the `PREFECT_API_KEY`.
</Tip>
...
Workers poll for work every 15 seconds by default. You can configure this interval in your [profile settings](/3.0rc/cloud/configure-client/) 
with the
...
- Allow work pools to prioritize (or limit) flow runs through the use of [work queues](/3.0rc/deploy/dynamic-infra/control-runs/#work-queues).
...
- [A flow](/3.0rc/develop/write-workflows/)
- A work pool

Update your `repo_info.py` file to create a deployment in Prefect Cloud with these updates:
...
See the [schedules](/3.0rc/deploy/serve-workflows/add-schedules/) guide for more information on scheduling options.
