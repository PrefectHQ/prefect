...
[Deployments](/3.0rc/deploy/static-infra/push-runs-remote/) are server-side representations of flows. 
...
- dynamically provision infrastructure using [workers](/3.0rc/deploy/static-infra/push-runs-remote)
...
[Work pools](/3.0rc/deploy/work-pools/) and [workers](/3.0rc/deploy/workers/) are an advanced deployment pattern that 
...
[Work pools](/3.0rc/deploy/work-pools/) allow Prefect to exercise greater control 
of the infrastructure on which flows run.
Options for [serverless work pools](/3.0rc/deploy/dynamic-infra/push-runs-serverless/) allow you to 
