...
Prefect Cloud can schedule flow runs through event-driven [automations](/3.0rc/react/understand-events/automations-triggers/).

Schedules instruct the Prefect API to automatically create new flow runs on a set cadence.
You can add a schedule to any [deployment](/3.0rc/deploy/serve-workflows/).
The Prefect `Scheduler` service periodically reviews every deployment and creates new flow runs according to the schedule. 
...
You can specify a schedule with a [`cron`](https://en.wikipedia.org/wiki/Cron) pattern. You may also provide a timezone to 
enforce DST behaviors.
...
both, as in the SQL statement `SELECT * FROM employees WHERE first_name = 'Xiāng' OR last_name = 'Brookins';`.
...
For example, the same schedule, when `day_or` is `False`, runs a flow on every minute on the **3rd Tuesday** in January. 
This behavior matches `fcron` instead of `cron`.

<Tip>
**Supported `croniter` features**

While Prefect supports most features of `croniter` for creating `cron`-like schedules, we do not support "R" 
...
For example, when clocks are set back, this results in a two-hour pause as the schedule will fire _the first time_ 1am is 
reached and _the first time_ 2am is reached, 120 minutes later. 
    
Longer schedules, such as one that fires at 9am every morning, adjust for DST automatically.
</Note>
...
For example, an hourly schedule will fire every UTC hour, even across DST boundaries. When clocks are set back, this results 
in two runs that _appear_ to be scheduled for 1am local time, even though they are an hour apart in UTC time. 
    
For longer intervals, like a daily schedule, the interval schedule adjusts for DST boundaries so that the clock-hour remains 
constant. 
...
<Note>
**RRule restrictions**
The max supported character length of an `rrulestr` is 6500 characters
...
A 9am daily schedule with a DST-aware start date maintains a local 9am time through DST boundaries. A 9am daily schedule 
with a UTC start date maintains a 9am UTC time.
</Note>
...
The **Schedules** section appears in the sidebar on the right side of the page on wider displays. On narrower displays, 
it appears on the **Details** tab of the page.
...
<Note>
**What about RRule?**
The UI does not support creating RRule schedules. However, the UI will display RRule schedules that you've created througb the 
command line.
</Note>

The new schedule appears on the **Deployment** page where you created it. In addition, the schedule is viewable in 
human-friendly text in the list of deployments on the **Deployments** page.

After you create a schedule, new scheduled flow runs are visible in the **Upcoming** tab of the **Deployment** page 
...
Here's an example of creating an interval schedule with `serve` for a deployment flow that runs every 10 minutes with an 
anchor date and a timezone:
...
If you are using [worker-based deployments](/3.0rc/deploy/dynamic-infra/control-runs/), you can create a schedule through the 
...
The `Scheduler` service starts automatically when `prefect server start` is run. It is a built-in service of Prefect Cloud.

By default, the `Scheduler` service visits deployments on a 60-second loop, though recently-modified deployments are visited 
more frequently.
The `Scheduler` evaluates each deployment's schedules and creates new runs appropriately.
For typical deployments, it creates the next three runs, though more runs are scheduled if the next three all start in 
the next hour.

The `Scheduler` tries to create the smallest number of runs that satisfy the following constraints, in order:
...
See the [Settings docs](/3.0rc/cloud/configure-client/) for more information on altering your settings.

These settings mean that if a deployment has an hourly schedule, the default settings will create runs for the next four days 
(or 100 hours).
...
To remove all scheduled runs for a flow deployment, you can remove the schedule through the UI.
