...
- [`Cron`](#cron) is best for users who are already familiar with `cron` from other systems.
- [`Interval`](#interval) is best suited for deployments that need to run at a regular cadence that isn't related to absolute time.
...
!!! tip "Supported `croniter` features"
    While Prefect supports most features of `croniter`, but it does not support "R" random or "H" hashed keyword expressions or the schedule jittering possible with those expressions.
...
An `Interval` schedule creates new flow runs on a regular interval measured in seconds. Intervals are computed using an optional `anchor_date`. For example, you can specify an interval schedule for every 10 minutes in a `prefect.yaml` file:
...
The `anchor_date` does not indicate a "start time" for the schedule, but rather a fixed point in time from which to compute intervals.
If the anchor date is in the future, then schedule dates are computed by subtracting the `interval` from it.
In this example, we import the [Pendulum](https://pendulum.eustace.io/) Python package for easy datetime manipulation. Pendulum isn’t required, but it’s a useful tool for specifying dates.
...
RRules are appropriate for any kind of calendar-date representation of a schedule, including simple repetition, irregular intervals, exclusions, week day or day-of-month adjustments, and more. RRules can specify complex logic like:
...
For example, the following RRule schedule in a [block-based deployment](/3.0rc/deploy/serve-workflows/#block-based-deployments) YAML file creates flow runs on Monday, Wednesday, and Friday until July 30, 2024.
...
- Through the `cron`, `interval`, or `rrule` parameters if building your deployment via the [`serve` method](/3.0rc/develop/write-workflows/#serving-a-flow) of the `Flow` object or [the `serve` utility](/3.0rc/develop/write-workflows/#serving-multiple-flows-at-once) for managing multiple flows simultaneously
...
!!! note "What about RRule?"
    The UI does not support creating RRule schedules, but will display RRule schedules created elsewhere.
...
After you added a schedule to a deployed flow, new scheduled flow runs will be visible in the **Upcoming** tab of the **Deployment** page where you created it.
...
When you create a deployment in a Python file with `flow.serve()`, `serve`, `flow.deploy()`, or `deploy` you can specify schedules. Just add the keyword argument `cron`, `interval`, or `rrule`.
...
For example, when `my_flow` is served with this cron scheule, it will run every minute of every day:

```python
my_flow.serve(name="flowing", cron="* * * * *")
```

When `my_flow` is served with this interval schedule, it will run every 10 minutes beginning at midnight on January, 1, 2024 in the `America/Chicago` timzone:

```python
from datetime import timedelta, datetime
from prefect.client.schemas.schedules import IntervalSchedule

my_flow.serve(name="flowing", schedule=IntervalSchedule(interval=timedelta(minutes=10), anchor_date=datetime(2024, 1, 1, 0, 0), timezone="America/Chicago"))
```
...
If you are using [worker-based deployments](/3.0rc/deploy/dynamic-infra/control-runs/), you can create a schedule through the interactive `prefect deploy` command.
...
Prefect's `Scheduler` service evaluates each deployment's schedules and creates new runs appropriately. It starts automatically when `prefect server start` is run and it is a built-in service of Prefect Cloud.
...
Typically, it will create the next three runs, though more runs will be scheduled if the next 3 would all start in the next hour.
...
These behaviors can all be adjusted through the relevant settings, which can be viewed with the terminal command `prefect config view --show-defaults`:
