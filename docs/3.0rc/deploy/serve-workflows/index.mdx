...
- **`schedules`**: a list of [schedule objects](/3.0rc/concepts/schedules/).
Most of the convenient interfaces for creating deployments allow users to avoid creating this object themselves.
For example, when [updating a deployment schedule in the UI](/3.0rc/concepts/schedules/#creating-schedules-through-the-ui) basic information such as a cron string or interval is all that's required.
- **`trigger`** (Cloud-only): triggers allow you to define event-based rules for running a deployment.
For more information see [Automations](/3.0rc/concepts/automations/).
...
!!! tip "Using the Prefect client"
    ...
    REST API, see [our guide](/3.0rc/guides/using-the-client/).
...
This metadata is available during execution through [Prefect runtime](/3.0rc/guides/runtime-context/).
...
[Workers and work pools](/3.0rc/concepts/work-pools/) are an advanced deployment pattern that allow you to dynamically provision infrastructure for each flow run.
...
Prefect provides several [types of work pools](/3.0rc/concepts/work-pools/#work-pool-types) corresponding to different types of infrastructure.
...
When you have several flows running regularly, [the `serve` method](/3.0rc/concepts/flows/#serving-a-flow) of the `Flow` object or [the `serve` utility](/3.0rc/concepts/flows/#serving-multiple-flows-at-once) is a great option for managing multiple flows simultaneously.
...
[Work pools](/3.0rc/concepts/work-pools/) allow Prefect to exercise greater control of the infrastructure on which flows run.
Options for [serverless work pools](/3.0rc/guides/deployment/serverless-workers/) allow you to scale to zero when workflows aren't running.
Prefect even provides you with the ability to [provision cloud infrastructure via a single CLI command](/3.0rc/guides/deployment/push-work-pools/#automatically-creating-a-new-push-work-pool-and-provisioning-infrastructure), if you use a Prefect Cloud push work pool option.
