---
title: Manage deployments with the API
...
When a [deployment](/3.0rc/deploy/dynamic-infra/prefect-deploy/) creates a flow run, it is submitted to a specific work pool for scheduling. 
...
- **[Docker](/3.0rc/deploy/static-infra/push-runs-remote/)**: - runs flows in an ephemeral Docker container.
- **[Kubernetes](/3.0rc/deploy/dynamic-infra/deploy-kubernetes/)**: - runs flows as a Kubernetes Job.
- **[Serverless Cloud Provider options](/3.0rc/deploy/dynamic-infra/push-runs-serverless/)**: runs flows in a 
...
**Additional work pool options available with Prefect Cloud**
    ...
    - **[Managed Execution](/3.0rc/deploy/dynamic-infra/managed-execution/)**: Prefect Cloud submits and runs your 
...
- What instructions does a [worker](/3.0rc/deploy/dynamic-infra/workers/) require to set up an execution environment for your flow? 
...
[Serve flows](/3.0rc/deploy/dynamic-infra/prefect-deploy/) shows how to create a deployment with a long-running process using `.serve`, 
and how to move to a [work-pool-based deployment](/3.0rc/deploy/static-infra/push-runs-remote/) setup with `.deploy`.
Learn more about when to move to [work-pool-based deployments](/3.0rc/deploy/dynamic-infra/work-pools/).
...
Ensure the [work pool](/3.0rc/deploy/dynamic-infra/work-pools/) exists in the Prefect Cloud workspace you are 
...
[base job template](/3.0rc/deploy/dynamic-infra/work-pools/#base-job-template).
...
| `triggers`                                  | An optional array of [triggers](/3.0rc/deploy/dynamic-infra/prefect-deploy/#create-a-flow-run-with-an-event-trigger) to assign to the deployment |
...
| `job_variables`                                | Values used to override the default values in the specified work pool's [base job template](/3.0rc/deploy/dynamic-infra/work-pools/#base-job-template). Maps directly to a created deployments `infra_overrides` attribute. |
