...
Push [work pools](/3.0rc/deploy/dynamic-infra/control-runs/#work-pool-overview) are a special type of work pool. They allow 
Prefect Cloud to submit flow runs for execution to serverless computing infrastructure without running a worker.
...
- to have authenticated with your cloud provider
...
<Tabs>
  <Tab title="AWS ECS">
    ...
    Each step has several optional fields that are detailed in the
     [work pools documentation](/3.0rc/deploy/dynamic-infra/control-runs/). 
 ...
 <Tab title="Google Cloud Run">
    ...
    Each step has several optional fields that are detailed in the 
    [work pools guide](/3.0rc/deploy/dynamic-infra/control-runs/). 
...
pools and workers](/3.0rc/guides/prefect-deploy/#creating-work-pool-based-deployments-with-deploy) guide.