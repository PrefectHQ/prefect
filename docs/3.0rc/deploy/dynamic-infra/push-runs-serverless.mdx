...
Push [work pools](/3.0rc/deploy/dynamic-infra/control-runs/#work-pool-overview) are a special type of work pool that allows Prefect Cloud to submit flow runs for execution to serverless computing infrastructure without running a worker.
...
=== "AWS ECS"
    ...
    Each step has several optional fields that are detailed in the [work pools documentation](/3.0rc/deploy/dynamic-infra/control-runs/). 
...
Deployment details are described in the deployments [concept section](/3.0rc/deploy/serve-workflows/).
