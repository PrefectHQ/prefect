---
title: Run flows with Kubernetes
...
3. Python and Prefect [installed](/3.0rc/get-started/install/)
...
[Work pools](/3.0rc/deploy/dynamic-infra/control-runs/) allow you to manage deployment 
infrastructure.
...
The recommended method for deploying a worker is with [Prefect Helm Chart](https://github.com/PrefectHQ/prefect-helm/tree/main/charts/prefect-worker).
...
We use the `tags` context manager to tag the flow run as `local`.
