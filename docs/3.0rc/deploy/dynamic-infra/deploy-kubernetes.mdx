...
3. [Install](/3.0rc/get-started/install/) Python and Prefect
...
[Work pools](/3.0rc/deploy/dynamic-infra/control-runs/) allow you to manage deployment infrastructure.
...
To learn about the Python deployment creation method with `flow.deploy` refer to the [Workers & Work Pools tutorial page](/3.0rc/deploy/dynamic-infra/push-runs-remote/).

The [`prefect.yaml`](/3.0rc/deploy/serve-workflows/#managing-deployments) file is used by the `prefect deploy` command to deploy our flows.
