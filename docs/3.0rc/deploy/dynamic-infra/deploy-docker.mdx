...
The [Deployments](/3.0rc/deploy/serve-workflows/) guide served a flow to enable scheduling or 
creating flow runs through the Prefect API.

With a Python script, you can build a Docker image for that script, allowing you to serve your flow 
in various remote environments. This guide uses Kubernetes, but you can use any Docker-compatible 
infrastructure.
...
from the [Deployments](/3.0rc/deploy/serve-workflows/) tutorial in it.
...
If your flow's container ever crashes, Kubernetes automatically restarts it, 
...
In the above manifest, you are passing in the Prefect API URL and API key as environment variables. 
...
You now have a deployment manifest. To deploy your flow to the cluster, run:

```bash
kubectl apply -f deployment-manifest.yaml
```

To monitor the status of your Kubernetes deployment, run:
...
You're now serving your flow in your cluster. To trigger a flow, run:
...
(3.8, 3.9, 3.10, 3.11) with both standard Python and Conda.
...
See the [Work pool deployment guide](/3.0rc/guides/prefect-deploy/) 
to build custom images with dependencies specified in a `requirements.txt` file.

By default, Prefect [work pools](/3.0rc/deploy/dynamic-infra/control-runs) 
...
`prefecthq/prefect` image and set `prefect.deployments.steps.pip_install_requirements:` 
in the `pull`step to install these dependencies at runtime.
...
- If your flow (or flows) require extra dependencies or shared libraries, 
...
not when the flow source changes.
