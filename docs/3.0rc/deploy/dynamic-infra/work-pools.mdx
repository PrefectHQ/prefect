...
[Managed work pools](/3.0rc/deploy/dynamic-infra/managed-execution) are administered by Prefect and handle the submission and execution of code on your behalf.
...
<Tip>
    **[Choosing Between `flow.deploy()` and `flow.serve()`](/3.0rc/deploy/dynamic-infra/prefect-deploy/#two-approaches-to-deployments)**
...
- Allow work pools to prioritize (or limit) flow runs through the use of [work queues](/3.0rc/deploy/dynamic-infra/work-pools/#work-queues).

Prefect provides several [types of work pools](/3.0rc/deploy/dynamic-infra/work-pools/#work-pool-types).
...
See the [Managed Execution guide](/3.0rc/deploy/dynamic-infra/managed-execution) for more details.
...
server instance, see the Workers [guide](/3.0rc/deploy/static-infra/push-runs-remote/). You will learn how 
