...
The simplest way to create a [deployment](/3.0rc/deploy/dynamic-infra/push-runs-remote/) for your 
flow is by calling its `serve` method.

<Warning>
Prefect does not recommend using the `serve` method for production deployments.
</Warning>
