...
2. [Push work pools](/3.0rc/deploy/infrastructure-examples/serverless): These submit runs directly to serverless infrastructure providers.
3. [Managed work pools](/3.0rc/deploy/infrastructure-examples/managed): These are administered by Prefect and handle both submission and execution of code.
...
Each work pool has a base job template that controls the of the behavior of the worker executing flow runs from the work pool.
...
Each work pool type is configured with a default base job template, which is a good place to get started. 
...
For example, you see these configuration options available in the Prefect UI for `process` work pool:
![process work pool configuration options](/3.0rc/img/ui/process-work-pool-config.png)

The default base job template allow you to:
...
Learn more about [customizing job variables](/3.0rc/deploy/infrastructure-concepts/customize).
...
Work queues offer advanced control over how runs are executed. Each work pool has a "default" queue which is used if a specific work queue name is not specified. 
...
- Learn more about [workers](/3.0rc/deploy/infrastructure-concepts/workers) and how they interact with work pools
- Learn how to [deploy flows](3.0rc/deploy/infrastructure-concepts/prefect-yaml) that run in work pools
- Learn how to set up work pools for:
 - [Kubernetes](3.0rc/deploy/infrastructure-examples/kubernetes)
 - [Docker](3.0rc/deploy/infrastructure-examples/docker)
 - [Serverless platforms](3.0rc/deploy/infrastructure-examples/serverless)
 - [Infrastructure managed by Prefect Cloud](3.0rc/deploy/infrastructure-examples/managed)
