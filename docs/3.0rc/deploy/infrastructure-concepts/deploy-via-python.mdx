...
and follow the [Connect to Prefect Cloud](/3.0rc/manage/cloud/connect-to-cloud/) guide.

2. A Prefect flow: You should have a flow defined in your Python script. If you haven't created a flow yet, refer to the 
[Write Flows](/3.0rc/develop/write-flows/) guide.
...
the Prefect UI or using the Prefect CLI. For more information, see the [Work Pools](/3.0rc/deploy/infrastructure-concepts/work-pools/) guide. 
...
4. Docker: Install Docker on your local machine. Docker will be used to build and store the image containing your flow code. You can download 
Docker from the [official Docker website](https://www.docker.com/get-started/).
...
In this example, we set `push=False` to skip pushing the image to a registry. This is useful for local development, and you can push your image to a registry like 
Docker Hub for more production environments.
...
- `schedules`
    ...
    Learn more about schedules [here](/3.0rc/automate/add-schedules).


## Use remote code storage

In addition to using Docker images, Prefect also supports deploying your code using remote storage. This approach allows you to 
...
Learn more about remote code storage [here](/3.0rc/deploy/infrastructure-concepts/store-flow-code).
...
Job variables can be used to customize environment variables, resources limits, and other infrastructure options
 allowing fine-grained control over your infrastructure on a per-deployment or per-flow-run basis.
Any variable defined in the base job template of the associated work pool can be overridden by a job variable.

You can learn more about job variables [here](3.0rc/deploy/infrastructure-concepts/customize).
...
- [Store Flow Code](/3.0rc/deploy/infrastructure-concepts/store-flow-code/)
- [Customize Infrastructure](/3.0rc/deploy/infrastructure-concepts/customize/)
- [Schedules](/3.0rc/automate/add-schedules/)
