...
To create deployments with Python code, see the [work pools guide](/3.0rc/deploy/infrastructure-concepts/work-pools).
...
1. You will see a series of prompts. Select yes creating a new deployment.
1. Select the flow code entrypoint
...
? Is https://github.com/some_username/some_repo.git the correct URL to pull your flow code from? [y/n] (y): 
...
This token will be saved as a Secret block via the Prefect API: "123_abc_this_is_some_token"
```

Verify that you have a new Secret block in your active workspace named in the format 
"deployment-some-deployment-some-flow-name-repo-token".
...
pull:
    - prefect.deployments.steps.git_clone:
        repository: https://gitlab.com/org/some-private-repo.git
        access_token: "{{ prefect.blocks.secret.some-block-name }}"
```
...
<Tabs>
  <Tab title="GitHub">
    ...
    1. Create a GitHub Credentials block in code or the Prefect UI and reference it as shown:

    ```yaml

    pull:
        - prefect.deployments.steps.git_clone:
            repository: https://github.com/org/some-private-repo.git
            credentials: "{{ prefect.blocks.github-credentials.some-block-name }}"
    ```
  </Tab>  
  <Tab title="Bitbucket">
    ...
    1. Create a Bitbucket credentials block in code or the Prefect UI and reference it as shown:

    ```yaml

    pull:
        - prefect.deployments.steps.git_clone:
            repository: https://bitbucket.org/org/some-private-repo.git
            credentials: "{{ prefect.blocks.bitbucket-credentials.some-block-name }}"
    ```
  </Tab>
<Tab title="GitLab">
    ...
    pull:
        - prefect.deployments.steps.git_clone:
            repository: https://gitlab.com/org/some-private-repo.git
            credentials: "{{ prefect.blocks.gitlab-credentials.some-block-name }}"
    ```
  </Tab>
</Tabs>
...
1. Run `prefect init` in the root of your repository and choose `docker` for the project name. Answer the prompts to 
create a `prefect.yaml` file with a build step that creates a Docker image with the flow code built in. 
See the [Workers and Work Pools page of the tutorial](/3.0rc/deploy/infrastructure-examples/docker/) for more info.
...
<Tabs>
  <Tab title="AWS S3 bucket">
    ...
    - prefect_aws.deployments.steps.push_to_s3:
        ...
        bucket: some-bucket
        folder: some-folder
        credentials: "{{ prefect.blocks.aws-credentials.some-credentials-block }}" # if private
  ...
  <Tab title="Azure Blob Storage container">
    ...
    - prefect_azure.deployments.steps.push_to_azure_blob_storage:
        ...
        container: some-prefect-azure-container
        folder: some-folder
        credentials: "{{ prefect.blocks.azure-blob-storage-credentials.some-credentials-block }}" # if private
  ...
  <Tab title="GCP GCS bucket">
    ...
    - prefect_gcp.deployment.steps.push_to_gcs:
        ...
        bucket: some-bucket
        folder: some-folder
        credentials: "{{ prefect.blocks.gcp-credentials.some-credentials-block }}" # if private 
    ...
    1. Create a GCP Credentials block in code or the Prefect UI. Enter a name for the block and paste the entire contents of 
    the JSON key file into the *Service Account Info* field.
    1. Reference the block as shown in the push and pull steps above.
    </Tab>
</Tabs>

Another authentication option is to give the [worker](/3.0rc/deploy/infrastructure-concepts/workers/) access 
