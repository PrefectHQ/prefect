...
If not set, the latest Prefect 3 image is used (for example, `prefecthq/prefect:3rc-latest`).
...
[Workers](/3.0rc/deploy/infrastructure-examples/docker/).

The [`prefect.yaml`](/3.0rc/deploy/infrastructure-concepts/prefect-yaml/#managing-deployments) file is used 
