...
pool for flow runs to execute. [Push work pools](/3.0rc/deploy/infrastructure/serverless) submit runs directly to your 
...
[Managed work pools](/3.0rc/deploy/infrastructure/managed) are administered by Prefect and handle the submission and execution of code on your behalf.
...
- What instructions does a [worker](/3.0rc/deploy/workers/) require to set up an execution environment for your flow? 
...
and how to move to a [work pool deployment](/3.0rc/deploy/infrastructure/docker/) setup with `.deploy`.
...
If you prefer to learn about using a YAML-based alternative for managing deployment configuration, skip to [Create deployments with YAML](/3.0rc/deploy/prefect-yaml).
...
Learn more about [overriding job variables](/3.0rc/deploy/customize).
...
**Advanced customization of the base job template**
    ...
    See more information about [overriding a work pool's job variables](/3.0rc/deploy/customize).
</Tip>
...
- Allow work pools to prioritize (or limit) flow runs through the use of [work queues](/3.0rc/deploy/work-pools/#work-queues).

Prefect provides several [types of work pools](/3.0rc/deploy/work-pools/#work-pool-types).
...
See the [Managed Execution guide](/3.0rc/deploy/infrastructure/managed) for more details.
...
See the [Push Work Pool guide](/3.0rc/deploy/infrastructure/serverless/) for more details and example commands for each 
cloud provider.
...
server instance, see the Workers [guide](/3.0rc/deploy/infrastructure/docker/). You will learn how 
