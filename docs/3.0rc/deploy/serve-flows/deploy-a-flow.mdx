...
[scheduling](/3.0rc/deploy/serve-flows/add-schedules/) and [event-based triggering](/3.0rc/react/understand-events/automations-triggers/).
...
Whether you use Prefect Cloud or host a Prefect server instance, you can run flows in the 
...
class Deployment:
    ...
    # flow scheduling and parametrization
...
For example, if a deployment references a flow defined within a Docker image, the `path` is the  
absolute path to the parent directory where that flow will run anytime the deployment is triggered. 
...
configuration, and prepares the runtime environment for flow execution.

Pull steps allow users to highly decouple their flow architecture.
...
scale to zero when flows aren't running.
