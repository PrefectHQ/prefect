...
See the [install guide](/3.0rc/getting-started/installation/) for more detailed installation instructions, if needed.
...
!!! note "Self-hosted Prefect server instance"
    If you would like to host a Prefect server instance on your own infrastructure, see the [tutorial](/3.0rc/tutorial/) and select the "Self-hosted" tab.
...
[Flows](/3.0rc/concepts/flows/) are the core observable, deployable units in Prefect and are the primary entrypoint to orchestrated work.
...
We can tell Prefect where we want to run our workflow by creating a [work pool](/3.0rc/concepts/work-pools/).

We can have Prefect Cloud run our flow code for us with a Prefect Managed work pool.

Let's create a [Prefect Managed work pool](/3.0rc/guides/managed-execution/) so that Prefect can run our flows for us.
...
Let's package both of these things, along with the location for where to find our flow code, into a [deployment](/3.0rc/concepts/deployments/) so that we can schedule our workflow to run remotely.
...
!!! note "Code storage options"
    ...
    Read more in [this guide](/3.0rc/guides/prefect-deploy/#creating-work-pool-based-deployments).
...
To learn how to run flows on your own infrastructure, customize the Docker image where your flow runs, and gain more orchestration and observation benefits check out the [tutorial](/3.0rc/tutorial/).
