...
This guide is for users who want to extend the Prefect framework. Completing this successfully requires deep knowledge of 
Prefect concepts. For standard use cases, we recommend using one of the [available workers](/3.0rc/deploy/dynamic-infra/control-runs/#worker-types) instead.
</Tip>
...
The work pool that a worker polls for flow runs has a [base job template](/3.0rc/deploy/dynamic-infra/control-runs/#base-job-template) 
associated with it. 
...
A worker developer defines their worker's configuration to function with a class extending 
[`BaseJobConfiguration`](/3.0rc/api-ref/prefect/workers/base/#prefect.workers.base.BaseJobConfiguration).
...
the values of these attributes, use the 
[`prepare_for_flow_run`](/3.0rc/api-ref/prefect/workers/base/#prefect.workers.base.BaseJobConfiguration.prepare_for_flow_run) method.
...
To see other examples of worker implementations, see the [`ProcessWorker`](/3.0rc/api-ref/prefect/workers/process/) and 
[`KubernetesWorker`](https://prefecthq.github.io/prefect-kubernetes/worker/) implementations.
