...
*   To extend a CLI application, for example, `prefect.cli`.
...
Attempt to avoid circular dependencies. This often reveals entanglement in the design.

You should place all deferred imports at the top of the function.
...
The Prefect client can be run separately from the Prefect orchestration server and communicate entirely via an API. 
Among other things, the Prefect client includes anything that runs task or flow code, (e.g. agents, and the Python client) 
or any consumer of Prefect metadata, (e.g. the Prefect UI, and CLI). The Prefect server stores this metadata and serves it
via the REST API.

### API version header

Sometimes we have to make breaking changes to the API (for good reasons). To check a Prefect client's compatibility
...
A breaking change means that your code needs to change to use a new version of Prefect. We strive to avoid breaking 
...
Prefect increases the major version when significant and widespread changes are made to the core product. It is very 
unlikely that the major version will change without extensive warning.
