...
When running workflow apps, it's helpful to create long-running processes that
...
[Prefect worker](/3.0rc/deploy/infrastructure-concepts/workers/) or
...
to target the `prefect` app in the `bin` subdirectory of your virtual environment.
