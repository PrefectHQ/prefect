...
A [worker](/3.0rc/deploy/workers/) is the fusion of an 
...
[work pool](/3.0rc/deploy/work-pools/) and passed to each worker that polls work 
from that pool.
...
through their [base job template](/3.0rc/deploy/work-pools/#base-job-template).
...
3. **Configuring flow run infrastructure:**
    ...
    infrastructure blocks --> [typed work pool](/3.0rc/deploy/workers/#worker-types)
...
- The process for starting an agent and [starting a worker](/3.0rc/deploy/workers/#start-a-worker) 
...
1. **[Create a work pool](/3.0rc/deploy/work-pools/#work-pool-configuration)**
...
2. **[Start a worker](/3.0rc/deploy/workers/#start-a-worker)**
