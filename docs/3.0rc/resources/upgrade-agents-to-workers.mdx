...
- Both options allow you to [deploy multiple flows](/3.0rc/deploy/serve-flows/#working-with-multiple-deployments) 
with a single command.
- Both options allow you to build Docker images for your flows to create portable execution environments.
- The YAML-based API supports [templating](/3.0rc/deploy/serve-flows/#templating-options) 
to enable [dryer deployment definitions](/3.0rc/deploy/serve-flows/#reusing-configuration-across-deployments).

## Upgrade changes

1. **Deployment CLI and Python SDK:**

    `prefect deployment build <entrypoint>`/`prefect deployment apply` --> [`prefect deploy`](/3.0rc/deploy/serve-flows/#deployment-declaration-reference)
...
2. **Configuring remote flow code storage:**
    ...
    storage blocks --> [pull action](/3.0rc/deploy/serve-flows/#the-pull-action)
...
4. **Managing multiple deployments:**
    ...
    [`prefect.yaml`](/3.0rc/deploy/serve-flows/#working-with-multiple-deployments) 
...
that contains [deployment configuration](/3.0rc/deploy/serve-flows/#working-with-multiple-deployments) 
...
with your first deployment. You can use this YAML file to edit and [define multiple deployments](/3.0rc/deploy/serve-flows/#working-with-multiple-deployments) 
for this repo.
</Note>

You can add more [deployments](/3.0rc/deploy/serve-flows/#deployment-declaration-reference) 
