...
A [worker](/3.0rc/deploy/dynamic-infra/workers/) is the fusion of an 
...
[work pool](/3.0rc/deploy/dynamic-infra/work-pools/) and passed to each worker that polls work 
from that pool.
...
through their [base job template](/3.0rc/deploy/dynamic-infra/work-pools/#base-job-template).
...
- Prefect Cloud [managed work pools](/3.0rc/deploy/dynamic-infra/managed-execution/) allow you to run flows on 
...
- Both options allow you to [deploy multiple flows](/3.0rc/deploy/dynamic-infra/prefect-deploy/#working-with-multiple-deployments) 
with a single command.
- Both options allow you to build Docker images for your flows to create portable execution environments.
- The YAML-based API supports [templating](/3.0rc/deploy/dynamic-infra/prefect-deploy/#templating-options) 
to enable [dryer deployment definitions](/3.0rc/deploy/dynamic-infra/prefect-deploy/#reusing-configuration-across-deployments).

## Upgrade changes

1. **Deployment CLI and Python SDK:**

    `prefect deployment build <entrypoint>`/`prefect deployment apply` --> [`prefect deploy`](/3.0rc/deploy/dynamic-infra/prefect-deploy/#deployment-declaration-reference)
...
2. **Configuring remote flow code storage:**
    ...
    storage blocks --> [pull action](/3.0rc/deploy/dynamic-infra/prefect-deploy/#the-pull-action)
    ...
    existing storage blocks to define your pull action [through templating](/3.0rc/deploy/dynamic-infra/prefect-deploy/#templating-options).
...
3. **Configuring flow run infrastructure:**
    ...
    infrastructure blocks --> [typed work pool](/3.0rc/deploy/dynamic-infra/workers/#worker-types)
...
4. **Managing multiple deployments:**
    ...
    [`prefect.yaml`](/3.0rc/deploy/dynamic-infra/prefect-deploy/#working-with-multiple-deployments) 
    file or use the [`deploy`](/3.0rc/deploy/dynamic-infra/push-runs-remote) 
    function.
...
- Deployment-level infrastructure overrides operate in much the same way.
    ...
    `infra_override` -> [`job_variable`](/3.0rc/deploy/dynamic-infra/prefect-deploy/#work-pool-fields)

- The process for starting an agent and [starting a worker](/3.0rc/deploy/dynamic-infra/workers/#start-a-worker) 
...
1. **[Create a work pool](/3.0rc/deploy/dynamic-infra/work-pools/#work-pool-configuration)**
...
(or aren't using an infrastructure and storage block at all), you can use [`flow.serve`](/3.0rc/deploy/dynamic-infra/index) 
...
2. **[Start a worker](/3.0rc/deploy/dynamic-infra/workers/#start-a-worker)**
...
- Replace `storage` with a call to [`flow.from_source`](/3.0rc/deploy/dynamic-infra/index).
...
that contains [deployment configuration](/3.0rc/deploy/dynamic-infra/prefect-deploy/#working-with-multiple-deployments) 
...
with your first deployment. You can use this YAML file to edit and [define multiple deployments](/3.0rc/deploy/dynamic-infra/prefect-deploy/#working-with-multiple-deployments) 
for this repo.
</Note>

You can add more [deployments](/3.0rc/deploy/dynamic-infra/prefect-deploy/#deployment-declaration-reference) 
to the `deployments` list in your `prefect.yaml` file and/or by continuing to use the deployment 
creation wizard.

For more information on deployments, check out our [in-depth guide for deploying flows to work pools](/3.0rc/deploy/dynamic-infra/prefect-deploy/).