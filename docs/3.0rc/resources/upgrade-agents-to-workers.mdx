...
!!! warning "This guide is for users who are on `prefect<3.0` who are upgrading from agents to workers."
    If you are new to Prefect, we recommend starting with the [Prefect Cloud Quickstart](/3.0rc/get-started/quickstart/) guide.
...
A [worker](/3.0rc/deploy/dynamic-infra/control-runs/#worker-overview) is the fusion of an [agent](https://docs.prefect.io/2.19.3/concepts/agents/) with an [infrastructure block](https://docs.prefect.io/2.19.3/concepts/infrastructure/). Like agents, workers poll a work pool for flow runs that are scheduled to start. Like infrastructure blocks, workers are typed - they work with only one kind of infrastructure, and they specify the default configuration for jobs submitted to that infrastructure.

Accordingly, workers are not a drop-in replacement for agents. **Using workers requires deploying flows differently.** In particular, deploying a flow with a worker does not involve specifying an infrastructure block. Instead, infrastructure configuration is specified on the [work pool](/3.0rc/deploy/dynamic-infra/control-runs/) and passed to each worker that polls work from that pool.
...
- Work pools allow greater customization and governance of infrastructure parameters for deployments via their [base job template](/3.0rc/deploy/dynamic-infra/control-runs/#base-job-template).
- Prefect Cloud [push work pools](/3.0rc/deploy/dynamic-infra/push-runs-serverless/) enable flow execution in your cloud provider environment without needing to host a worker.
...
- Both options allow you to [deploy multiple flows](/3.0rc/deploy/serve-workflows/#working-with-multiple-deployments) with a single command.
- Both options allow you to build Docker images for your flows to create portable execution environments.
- The YAML-based API supports [templating](/3.0rc/deploy/serve-workflows/#templating-options) to enable [dryer deployment definitions](/3.0rc/deploy/serve-workflows/#reusing-configuration-across-deployments).
...
1. **Deployment CLI and Python SDK:**
    ...
    `prefect deployment build <entrypoint>`/`prefect deployment apply` --> [`prefect deploy`](/3.0rc/deploy/serve-workflows/#deployment-declaration-reference)
...
2. **Configuring remote flow code storage:**
    ...
    storage blocks --> [pull action](/3.0rc/deploy/serve-workflows/#the-pull-action)
...
3. **Configuring flow run infrastructure:**
    ...
    infrastructure blocks --> [typed work pool](/3.0rc/deploy/dynamic-infra/control-runs/#worker-types)
...
4. **Managing multiple deployments:**
    ...
    Create and/or update many deployments at once through a [`prefect.yaml`](/3.0rc/deploy/serve-workflows/#working-with-multiple-deployments) file or use the [`deploy`](/3.0rc/api-ref/prefect/deployments/runner/#prefect.deployments.runner.deploy) function.
...
- Deployment-level infrastructure overrides operate in much the same way.
    ...
    `infra_override` -> [`job_variable`](/3.0rc/deploy/serve-workflows/#work-pool-fields)

- The process for starting an agent and [starting a worker](/3.0rc/deploy/dynamic-infra/control-runs/#starting-a-worker) in your environment are virtually identical.
...
1. **[Create a work pool](/3.0rc/deploy/dynamic-infra/control-runs/#work-pool-configuration)**
...
```python fixture:my_k8s_job notest
...
!!! Tip "Serving flows"
    ...
    Check out our [Docker guide](/3.0rc/deploy/dynamic-infra/deploy-docker/) for how to build a served flow into a Docker image and host it in your environment.

2. **[Start a worker](/3.0rc/deploy/dynamic-infra/control-runs/#starting-a-worker)**
...
if __name__ == "__main__":
    Deployment.build_from_flow(
        my_flow,
        name="my-deployment",
        storage=GitHubRepository.load("demo-repo"),
        parameters=dict(name="Marvin"),
    )
```
...
if __name__ == "__main__":
    flow.from_source(
        source=GitHubRepository.load("demo-repo"),
...
```python fixture:demo_repo fixture:my_k8s_job notest
from prefect import flow
...
!!! warning "Always run `prefect deploy` commands from the **root** level of your repo!"
    With agents, you might have had multiple `deployment.yaml` files, but under worker deployment patterns, each repo will have a single `prefect.yaml` file located at the **root** of the repo that contains [deployment configuration](/3.0rc/deploy/serve-workflows/#working-with-multiple-deployments) for all flows in that repo.
...
!!! Note "For step 4, select `y` on the last prompt to save the configuration for the deployment."
    Saving the configuration for your deployment will result in a `prefect.yaml` file populated with your first deployment. You can use this YAML file to edit and [define multiple deployments](/3.0rc/deploy/serve-workflows/#working-with-multiple-deployments) for this repo.

You can add more [deployments](/3.0rc/deploy/serve-workflows/#deployment-declaration-reference) to the `deployments` list in your `prefect.yaml` file and/or by continuing to use the deployment creation wizard.

For more information on deployments, check out our [in-depth guide for deploying flows to work pools](/3.0rc/guides/prefect-deploy/).
