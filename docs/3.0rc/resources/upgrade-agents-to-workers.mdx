...
[Prefect Cloud Quickstart](/3.0rc/get-started/quickstart/) guide.
</Note>

## About workers and agents
A [worker](/3.0rc/deploy/dynamic-infra/control-runs/#worker-overview) is the fusion of an 
[agent](https://docs.prefect.io/2.19.3/concepts/agents/) with an [infrastructure block](https://docs.prefect.io/2.19.3/concepts/infrastructure/). 
...
[work pool](/3.0rc/deploy/dynamic-infra/control-runs/) and passed to each worker that polls work 
from that pool.
...
through their [base job template](/3.0rc/deploy/dynamic-infra/control-runs/#base-job-template).
...
- Prefect Cloud [managed work pools](/3.0rc/guides/managed-execution/) allow you to run flows on 
...
- Both options allow you to [deploy multiple flows](/3.0rc/deploy/serve-workflows/#working-with-multiple-deployments) 
with a single command.
- Both options allow you to build Docker images for your flows to create portable execution environments.
- The YAML-based API supports [templating](/3.0rc/deploy/serve-workflows/#templating-options) 
to enable [dryer deployment definitions](/3.0rc/deploy/serve-workflows/#reusing-configuration-across-deployments).
...
2. **Configuring remote flow code storage:**
    ...
    existing storage blocks to define your pull action [through templating](/3.0rc/guides/prefect-deploy/#templating-options).
...
4. **Managing multiple deployments:**
    ...
    [`prefect.yaml`](/3.0rc/deploy/serve-workflows/#working-with-multiple-deployments) 
    file or use the [`deploy`](/3.0rc/api-ref/prefect/deployments/runner/#prefect.deployments.runner.deploy) 
    function.
...
- The process for starting an agent and [starting a worker](/3.0rc/deploy/dynamic-infra/control-runs/#starting-a-worker) 
...
(or aren't using an infrastructure and storage block at all), you can use [`flow.serve`](/3.0rc/api-ref/prefect/flows/#prefect.flows.Flow.deploy) 
...
Check out our [Docker guide](/3.0rc/deploy/dynamic-infra/deploy-docker/) for how to build a 
...
that contains [deployment configuration](/3.0rc/deploy/serve-workflows/#working-with-multiple-deployments) 
...
with your first deployment. You can use this YAML file to edit and [define multiple deployments](/3.0rc/deploy/serve-workflows/#working-with-multiple-deployments) 
for this repo.
</Note>

You can add more [deployments](/3.0rc/deploy/serve-workflows/#deployment-declaration-reference) 
