...
- Prefect Cloud [push work pools](/3.0rc/deploy/infrastructure/serverless/) enable flow 
execution in your cloud provider environment without the need to host a worker.
- Prefect Cloud [managed work pools](/3.0rc/deploy/infrastructure/managed/) allow you to run flows on 
...
- Both options allow you to [deploy multiple flows](/3.0rc/deploy/prefect-yaml/#work-with-multiple-deployments-with-prefect-yaml) 
with a single command.
- Both options allow you to build Docker images for your flows to create portable execution environments.
- The YAML-based API supports [templating](/3.0rc/deploy/prefect-yaml/#templating-options) 
to enable [dryer deployment definitions](/3.0rc/deploy/infrastructure/docker/#reusing-configuration-across-deployments).

## Upgrade changes

1. **Deployment CLI and Python SDK:**

    `prefect deployment build <entrypoint>`/`prefect deployment apply` --> [`prefect deploy`](/3.0rc/deploy/prefect-yaml/#deployment-declaration-reference)
...
2. **Configuring remote flow code storage:**
    ...
    storage blocks --> [pull action](/3.0rc/deploy/prefect-yaml/#the-pull-action)
    ...
    existing storage blocks to define your pull action [through templating](/3.0rc/deploy/prefect-yaml/#templating-options).
...
4. **Managing multiple deployments:**
    ...
    [`prefect.yaml`](/3.0rc/deploy/prefect-yaml/#work-with-multiple-deployments-with-prefect-yaml) 
    file or use the [`deploy`](/3.0rc/deploy/infrastructure/docker) 
    function.
...
- Deployment-level infrastructure overrides operate in much the same way.
    ...
    `infra_override` -> [`job_variable`](/3.0rc/deploy/prefect-yaml/#work-pool-fields)
...
that contains [deployment configuration](/3.0rc/deploy/prefect-yaml/#work-with-multiple-deployments-with-prefect-yaml) 
...
with your first deployment. You can use this YAML file to edit and [define multiple deployments](/3.0rc/deploy/prefect-yaml/#work-with-multiple-deployments-with-prefect-yaml) 
for this repo.
</Note>

You can add more [deployments](/3.0rc/deploy/prefect-yaml/#deployment-declaration-reference) 
to the `deployments` list in your `prefect.yaml` file and/or by continuing to use the deployment 
creation wizard.

For more information on deployments, check out our [in-depth guide for deploying flows to work pools](/3.0rc/deploy/infrastructure/docker/).