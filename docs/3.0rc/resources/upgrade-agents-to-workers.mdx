...
- The process for starting an agent and [starting a worker](/3.0rc/deploy/control-runs/workerz/#start-a-worker) 
...
(or aren't using an infrastructure and storage block at all), you can use [`flow.serve`](/3.0rc/deploy/serve-flows/deploy-a-flow) 
...
2. **[Start a worker](/3.0rc/deploy/control-runs/workers/#starting-a-worker)**
...
- Replace `storage` with a call to [`flow.from_source`](/3.0rc/deploy/serve-flows/deploy-a-flow).
