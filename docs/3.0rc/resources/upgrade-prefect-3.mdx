...
So, *you do not need to use the `await` keyword in front of your `.submit` call*:
...
In Prefect 3, `PrefectFuture`s resulting from submitted task runs are not automatically awaited/resolved before exiting a flow, unless they’re a dependency of other flows or tasks, so *you must wait for a task’s `PrefectFuture` resolution before the end of a flow*:
...
@flow
def my_flow():
    # Prefect will resolve these futures to states
    # because the flow returned them, so you
    # don't have to call `wait()`.
...
Prefect 3 supports transactional semantics - tasks runs can be grouped into larger transactions, enabling more resilient workflows.
In keeping with the transactional model, Prefect 3 encourages task runs to be idempotent - intended to run only once per flow run.
...
This means that tasks that may have re-run with the same inputs in Prefect 2 will not re-run in Prefect 3 unless the cache key is modified to be more precise.
...
This caching behavior will only take effect when result persistence is enabled.
...
## Variables have replaced certain blocks
...
In Prefect 3, variables can be any JSON compatible type, such that variables can now store all of these data types and more.
Any single-value blocks created in Prefect 2 will still be accessible as such in Prefect 3, but you cannot create new single-value blocks.
