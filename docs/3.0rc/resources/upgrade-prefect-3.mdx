...
Prefect 3 introduces client-side task run orchestration as the default. Task creation and state updates now happen locally, dramatically reducing API calls to the Prefect server during execution. This change brings a <b>10x performance improvement for task executions</b> many scenarios. 
