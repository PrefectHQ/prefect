...
## Client Side Task Orchestration
Prefect 3 introduces client-side task run orchestration as the default. Task creation and state updates now happen locally, dramatically reducing API calls to the Prefect server during execution. This change brings a <b>10x performance improvement for task executions</b> in many scenarios. 

The reduced server dependency also means your workflows are more resilient to network issues or server load, continuing to run even if there's a temporary loss of connection to the Prefect server.

Task updates are now logged in batch, greatly improving efficiency and enabling Prefect 3 to handle much larger and more complex workflows. You can create and run workflows with thousands of tasks efficiently, supporting large-scale data processing and complex orchestration scenarios. 

State changes and logs in the Prefect UI are now eventually consistent model for task states, this trade-off enables the substantial performance gains and increased workflow scale that Prefect 3 delivers.
...
# The enclosing flow must be async as well
@flow
async def my_flow():
...
If you [self-host a Prefect server instance](/3.0rc/manage/self-host), run the following command to update your database for Prefect 3:

```bash
prefect server database upgrade -y
```

## Flow run final state determination
...
1. Allow exceptions to propagate by not using `raise_on_failure=False`.
