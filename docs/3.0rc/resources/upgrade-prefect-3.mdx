...
In Prefect 2, the final state of a flow run was influenced by the states of its task runs, i.e. if any task run failed, the flow run was marked as failed.
...
1. The `return` value of the flow function (same as in Prefect 2):
   ...
   - Any explicit `State` that is returned will be considered the final state of the flow run.
