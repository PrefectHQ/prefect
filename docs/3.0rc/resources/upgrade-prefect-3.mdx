...
## Eager caching enabled by default
...
Note that result persistence is still disabled by default, so the new caching behavior will only take effect
when result persistence has been enabled.
...
The cache key can be configured through the new global `cache_policy` setting, or through the `cache_key_fn` on a particular task.
