...
Objects imported from changed modules will work until their six month deprecation period ends, but will raise deprecation warnings. 
...
## Type enforcement for custom blocks

If you have registered custom blocks with a self-hosted Prefect Server, type enforcement for those block schemas must be upgraded from Pydantic V1 to Pydantic V2 to work with Prefect 3. 
This upgrade does not require changes for any built-in Prefect interfaces or supported integrations. 
...
## Self-hosted Prefect server database upgrade

If you are [self-hosting a Prefect server instance](/3.0rc/manage/self-host), run the following code to update your database for Prefect 3:
