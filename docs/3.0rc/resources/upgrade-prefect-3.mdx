...
Some modules have been renamed, reorganized, or removed for clarity. If you use a module that has changed, it will continue to work, but it will raise deprecation warnings. You will need to update these modules before the six month deprecation period is up:
...
When executing tasks asynchronously in Prefect 3, you must follow conventional asynchronous Python programing principles. 
...
<CardGroup cols={2}>
  ...
  <Card title="Prefect 3" icon="square-2">
    ```python
    @task
    async def my_task():
	    ...
...
So, *you must wait for the future to complete, then retrieve the state separately*:
...
The notifications feature in Prefect 2 has been vastly expanded to enable all kinds of “if this, then that” logic that can be defined and executed by a Prefect server instance. 
See the [Automations documentation](https://docs-3.prefect.io/3.0rc/automate/events/automations-triggers) for more information.

## Integrations must be upgraded

Prefect 2 integrations cannot be used with Prefect 3 and vice versa. 
You must update all of your integrations packages in conjunction with upgrading from Prefect 2 to Prefect 3. 
The easiest way to keep your integrations in sync is to use the extras on the `prefect` package to ensure that you get the correct version (e.g. `pip install prefect[aws]`).
...
However, if you have registered custom blocks with a self hosted Prefect Server, type enforcement for those block schemas will need to be upgraded to Pydantic v2 to work with Prefect 3. 
See the the [Pydantic migration guide](https://www.notion.so/Prefect-2-3-Upgrade-Guide-e43313731c1d4adbb4e4175548058b51?pvs=21) for more information.
