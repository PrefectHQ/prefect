...
In keeping with the transactional model, Prefect 3 enrourages task runs to be idempotent - intended to run only once per flow run.
...
To store and retrive typed values in Prefect 2, you have to use certain single-value blocks, icluding JSON, Date Time, and String blocks.
In Prefect 3, variables can be any JSON compatible type, such that varaibles can now store all of these data types and more.
