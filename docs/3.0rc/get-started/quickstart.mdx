...
Connect to the Prefect API on Cloud or spin up a local API server:
...
If you have any issues with browser-based authentication, see the [Prefect Cloud docs](/3.0rc/manage/cloud/manage-users/api-keys/) to learn how to authenticate with a manually created API key.
  ...
  <Tab title="OSS">
1. Use the Prefect CLI to start a local server:

   ```bash
   prefect server start
   ```

1. Open the Prefect UI in your browser at [http://localhost:4200](http://localhost:4200).
  </Tab>
</Tabs>
...
1. View your new work pool on the **Work Pools** page of the UI.
  ...
  <Tab title="OSS">
Deploy your flow to your local server using a Docker type work pool.

1. Install Docker on your machine.
1. Create a Docker type work pool:
...
A [deployment](/3.0rc/deploy/serve-flows/) is used to determine when, where, and how a flow function should run.
...
1. When you're done, click the **Remove** button in the top right of the **Deployment** page to stop further scheduled runs.
