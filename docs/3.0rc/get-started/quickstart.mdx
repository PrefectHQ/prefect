...
We can tell Prefect where we want to run our workflow by creating a [work pool](/3.0rc/deploy/dynamic-infra/control-runs/).
...
Let's package both of these things, along with the location for where to find our flow code, into a [deployment](/3.0rc/deploy/serve-workflows/) so that we can schedule our workflow to run remotely.
