...
If you have any issues with browser-based authentication, see the [Prefect Cloud docs](/3.0rc/manage/cloud/manage-users/api-keys/) to learn how to authenticate with a manually created API key.

<Note>
If you would like to run a Prefect server instance on your own infrastructure, see the [Host a Prefect server instance](/3.0rc/manage/self-host/) docs.
...
Running a flow locally is a good start, but a remote destination is used for production. Tell Prefect where to run the workflow by creating a [work pool](/3.0rc/deploy/work-pools/control-runs/).
