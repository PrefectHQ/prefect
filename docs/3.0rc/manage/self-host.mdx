...
either [Prefect Cloud](/3.0rc/cloud/) or a self-hosted Prefect server, backed by a database and a UI.
...
- [Workspaces](/3.0rc/cloud/workspaces/): isolated environments to organize your flows, deployments, and flow runs
- [Automations](/3.0rc/react/understand-events/automations-triggers/): configure triggers, actions, and 
notifications in response to real-time monitoring events
- [Email notifications](/3.0rc/react/understand-events/automations-triggers/): send email alerts from Prefect's 
servers based on automation triggers
- [Service accounts](/3.0rc/cloud/manage-users/service-accounts/): configure API access for running workers or 
executing flow runs on remote infrastructure
- [Custom role-based access controls (RBAC)](/3.0rc/cloud/manage-users/manage-roles/): assign users granular 
permissions to perform activities within an account or workspace
- [Single Sign-on (SSO)](/3.0rc/cloud/manage-users/configure-sso/): authentication using your identity provider
- [Audit Logs](/3.0rc/cloud/manage-users/audit-logs/): a record of user activities to monitor security and compliance
...
You can save the API server address in a [Prefect profile](/3.0rc/cloud/configure-client/). Whenever that profile is 
active, the API endpoint is at that address.

See [Profiles and configuration](/3.0rc/cloud/configure-client/) for more information on profiles and configurable Prefect settings.
...
[Prefect Cloud](/3.0rc/cloud/) gives you access to a hosted platform with Workspace and User controls, Events, and Automations. 
...
Prefect Cloud uses the robust [Automations](/3.0rc/react/understand-events/automations-triggers/) interface to 
