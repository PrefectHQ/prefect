...
Read more about [managing API keys](/3.0rc/cloud/manage-users/api-keys/). 
...
If you're using a [worker](/3.0rc/deploy/dynamic-infra/control-runs/) to run flows in remote environments, 
[`PREFECT_API_URL`](/3.0rc/cloud/configure-client/) must be set for the environment where your worker is running.
...
You can also set [`PREFECT_API_URL`](/3.0rc/cloud/configure-client/#prefect.settings.PREFECT_API_URL) to the API URL. 
...
default [Prefect SQLite database](/3.0rc/concepts/database/).
...
If running a self-hosted Prefect server instance, there are several [database configuration settings](/3.0rc/cloud/self-host/) to consider.
