...
<Note>To help prevent accidental account lockouts, updates to an allowlist requires the requestor's current IP address to be on the list.</Note>

```bash
prefect cloud ip-allowlist add <your-ip-address> --description "My home IP address"
```

The allowlist has a limit to how many entries can be added; 
...
Once enabled with at least 1 enabled IP entry, the allowlist will be enforced for all incoming requests to Prefect Cloud from the UI and API.
