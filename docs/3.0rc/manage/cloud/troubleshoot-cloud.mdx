---
title: Troubleshoot Prefect Cloud
description: Learn tips and tricks and common solutions for troubleshooting Prefect Cloud.
---
...
and check out the [Release Notes](https://github.com/PrefectHQ/prefect/blob/main/RELEASE-NOTES.md). 
...
[Integrations](/3.0rc/integrations/) are versioned and released independently of the core Prefect library. 
...
to the `DEBUG` level. [Settings](/3.0rc/api-ref/prefect/settings/) such as the logging level are propagated 
...
[Service accounts](/3.0rc/manage/cloud/manage-users/service-accounts/) (sometimes referred to as bots),  
...
Actors can be members of [workspaces](/3.0rc/manage/cloud/workspaces/). An actor attempting an action in a 
...
an actor has a [role](/3.0rc/manage/cloud/manage-users/manage-roles/) which grants them certain permissions. 
...
behavior by executing the flow locally with the `-m` flag (i.e. `python -m flow` instead of `python flow.py`). 
