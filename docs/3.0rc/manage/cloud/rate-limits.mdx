---
title: Rate limits and metadata retention
...
Nested flow runs are retained independently from their parent flow runs. They are removed based on 
the time each nested flow run reaches a terminal state.
