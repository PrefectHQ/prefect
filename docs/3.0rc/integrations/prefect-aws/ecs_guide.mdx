...
### 4. [Deploy](https://docs.prefect.io/deploy/serve-workflows/#create-a-deployment) the flow to the Prefect Cloud or your self-managed server instance, specifying the ECS work pool when prompted
...
1. Now that you are confident your ECS worker is healthy, you can experiment with different work pool configurations.
    ...
    These infrastructure configuration values can be set on your ECS work pool or they can be overridden on the deployment level through [job_variables](/3.0rc/deploy/serve-workflows/#workers-and-work-pools) if desired.
