...
After installing Prefect, you have a Python SDK client that can communicate with [Prefect Cloud](https://app.prefect.cloud), and an [API server](/3.0rc/api-ref/) instance backed by a database and a UI.
...
- [Email notifications](/3.0rc/cloud/automations/) &mdash; send email alerts from Prefect's servers based on automation triggers.
...
- [Custom role-based access controls (RBAC)](/3.0rc/cloud/users/roles/) &mdash; assign users granular permissions to perform activities within an account or workspace.
...
Read more about Prefect Cloud in the [Cloud](/3.0rc/cloud/) section.
...
!!! tip "`PREFECT_API_URL` required when running Prefect inside a container"
    ...
    You can save the API server address in a [Prefect profile](/3.0rc/concepts/settings/). Whenever that profile is active, the API endpoint is at that address.

    See [Profiles & Configuration](/3.0rc/concepts/settings/) for more information on profiles and configurable Prefect settings.
...
Notifications enable you to set up alerts that are sent when a flow enters any state you specify. When your flow and task runs changes [state](/3.0rc/concepts/states/), Prefect notes the state change and checks whether the new state matches any notification policies. If it does, a new notification is queued.
...
!!! cloud-ad "Notifications in Prefect Cloud"
    Prefect Cloud uses the robust [Automations](/3.0rc/cloud/automations/) interface to enable notifications related to flow run state changes and work pool status.
