...
- A basic understanding of Prefect flows. Start with the [Getting Started](/3.0rc/getting-started/quickstart/) guide if necessary.
- A recent version of Prefect installed in your command line environment. Follow these [instructions](/3.0rc/getting-started/installation/) if you have any issues.
...
## Deploy with `serve`
To run shell commands on a schedule, the `serve` command creates a Prefect [deployment](/3.0rc/concepts/deployments/) for regular execution. This is a quick way to create a deployment served by Prefect.
...
- **Configurable execution:** Customize execution frequency, [concurrency limits](/3.0rc/guides/global-concurrency-limits/), and other parameters to suit your project's needs and resources.
