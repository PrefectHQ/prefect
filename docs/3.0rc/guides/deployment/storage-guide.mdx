...
Another option for authentication is for the [worker](/3.0rc/concepts/work-pools/#worker-overview) to have access to the storage location at runtime via SSH keys.
...
In earlier versions of Prefect [storage blocks](/3.0rc/concepts/blocks/) were the recommended way to store flow code.
