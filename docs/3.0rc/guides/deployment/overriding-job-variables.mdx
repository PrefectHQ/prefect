...
As described in the [Deploying Flows to Work Pools and Workers](/3.0rc/guides/prefect-deploy/) guide, there are two ways to deploy flows to work pools: using a `prefect.yaml` file or using the `.deploy()` method.
...
![Job variables via custom run](/3.0rc/img/ui/deployment-job-variables.png)
...
![Job variables via automation action](/3.0rc/img/ui/automations-action-job-variable.png)
