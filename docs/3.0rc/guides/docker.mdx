...
The [Deployments](/3.0rc/tutorial/deployments/) tutorial served a flow that enabled scheduling or creating flow runs through the Prefect API.
...
If creating a deployment with a `prefect.yaml` file, the build step makes it easy to customize your Docker image and push it to the registry of your choice. See an [example](/3.0rc/guides/deployment/kubernetes/#define-a-deployment).
...
- A Python script that defines and serves a flow.
  - This guide uses the flow script and deployment from the [Deployments](/3.0rc/tutorial/deployments/) tutorial.
...
In this directory, create a sub-directory named `flows` and put your flow script from the [Deployments](/3.0rc/tutorial/deployments/) tutorial in it.
...
By default, Prefect [work pools](/3.0rc/concepts/work-pools) that use containers refer to the `3-latest` image.
