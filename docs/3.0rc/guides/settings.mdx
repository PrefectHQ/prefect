...
The `PREFECT_API_KEY` value specifies the [API key](/3.0rc/cloud/cloud-api-keys/#create-an-api-key) used to authenticate with Prefect Cloud.
...
Read more about [managing API keys](/3.0rc/cloud/users/api-keys/). 
...
!!! tip "`PREFECT_API_URL` setting for workers"
    ...
    If using a [worker](/3.0rc/concepts/work-pools/) (agent and block-based deployments are legacy) that can create flow runs for deployments in remote environments,  [`PREFECT_API_URL`](/3.0rc/concepts/settings/) must be set for the environment in which your worker is running.
...
!!! tip "Running the Prefect UI behind a reverse proxy"
    ...
    You can also set [`PREFECT_API_URL`](/3.0rc/concepts/settings/#prefect.settings.PREFECT_API_URL) to the API URL. This setting is a fallback if `PREFECT_UI_API_URL` is not set.

### PREFECT_HOME

The `PREFECT_HOME` value specifies the local Prefect directory for configuration files, profiles, and the location of the default [Prefect SQLite database](/3.0rc/concepts/database/).
...
If running a self-hosted Prefect server instance, there are several [database configuration settings](/3.0rc/guides/host/) to consider.

### Logging settings

Prefect provides several [logging configuration settings](/3.0rc/guides/logs/).
