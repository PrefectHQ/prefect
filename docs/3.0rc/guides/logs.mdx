...
View and filter logs in the [UI](/3.0rc/cloud/flow-runs/#inspect-a-flow-run) or access log records through the API.
...
See logs for a flow run in the UI by navigating to the [**Flow Runs**](/3.0rc/cloud/flow-runs/#inspect-a-flow-run) page and selecting a specific flow run to inspect.
...
Prefect provides several settings to configure [logging level and loggers](/3.0rc/guides/logs/).
...
Override any logging configuration by setting an environment variable or [Prefect Profile](/3.0rc/concepts/settings/) setting using the syntax `PREFECT_LOGGING_[PATH]_[TO]_[KEY]`, with `[PATH]_[TO]_[KEY]` corresponding to the nested address of any setting.
...
To access the Prefect logger, import `from prefect.logging import get_run_logger`. You can send messages to the logger in both flows and tasks.
...
For more information, see [Log schema](/3.0rc/api-ref/server/schemas/core/#prefect.server.schemas.core.Log) in the API documentation.
...
[Settings](/3.0rc/concepts/settings/) for more details about how to use settings.
