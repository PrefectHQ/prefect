...
2. To determine execution order for [concurrently submitted](/3.0rc/concepts/task-runners) units of work that depend on each other
...
When using non-sequential task runners such as the [`ConcurrentTaskRunner`](/3.0rc/api-ref/prefect/task-runners/#prefect.task_runners.ConcurrentTaskRunner) or [`DaskTaskRunner`](https://prefecthq.github.io/prefect-dask/), the order of execution for submitted tasks is not guaranteed unless you specify their dependencies.
...
For more complex workflows, parts of your logic may require additional resources, different infrastructure, or independent parallel execution. A typical approach for addressing these needs is to execute that logic as separate [deployment](/3.0rc/concepts/deployments) runs from within a flow.

Compose deployment runs into a flow and treat them as upstream dependencies by calling [`run_deployment`](/3.0rc/api-ref/prefect/deployments/deployments/#prefect.deployments.deployments.run_deployment) from within a task.
