...
To run tasks in a separate process or container if you schedule them with `delay`, start a task server, similar to how you would run a Celery worker or an arq worker.
...
Check out additional examples in the [deferred tasks github repository](https://github.com/PrefectHQ/prefect-background-task-examples.git).