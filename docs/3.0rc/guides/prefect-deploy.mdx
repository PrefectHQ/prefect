...
- [Docker](/3.0rc/guides/docker/) - runs flows in an ephemeral Docker container.
...
!!! note "Docker step"
    The most commonly used build step is [`prefect_docker.deployments.steps.build_docker_image`](/3.0rc/guides/docker/) which produces both the `image_name` and `tag` fields.

    For an example, [check out the deployments tutorial](/3.0rc/guides/docker/).
