...
- [Serverless Cloud Provider options](/3.0rc/deploy/dynamic-infra/push-runs-remote/) - runs flows in a Docker container in a serverless cloud provider environment, such as AWS ECS, Azure Container Instance, Google Cloud Run, or Vertex AI.
...
!!!note "Additional work pool options available with Prefect Cloud"
    ...
    - [Push Work Pools](/3.0rc/deploy/dynamic-infra/push-runs-serverless) - serverless cloud options that don't require a worker because Prefect Cloud submits them to your serverless cloud infrastructure on your behalf. Prefect can auto-provision your cloud infrastructure for you and set it up to use your work pool.
    
    - [Managed Execution](/3.0rc/guides/managed-execution/) Prefect Cloud submits and runs your deployment on serverless infrastructure. No cloud provider account required.
...
- What instructions does a [worker](/3.0rc/deploy/dynamic-infra/control-runs/) need to set up an execution environment for our flow? For example, a flow may have Python package requirements, unique Kubernetes settings, or Docker networking configuration.
- How should the flow code be accessed?

The [tutorial](/3.0rc/deploy/serve-workflows/) shows how you can create a deployment with a long-running process using `.serve` and how to move to a [work-pool-based deployment](/3.0rc/deploy/dynamic-infra/push-runs-remote/) setup with `.deploy`.
See the discussion of when you might want to move to work-pool-based deployments [there](/3.0rc/deploy/dynamic-infra/push-runs-remote/#why-workers-and-work-pools).
...
Make sure you have the [work pool](/3.0rc/deploy/dynamic-infra/control-runs/) created in the Prefect Cloud workspace you are authenticated to or on your running self-hosted server instance.  
...
```python hl_lines="2 18-22" title="private-package.py" notest
from prefect import flow
...
Note that we used a [Prefect Secret block](/3.0rc/develop/connect-third-party/) to load the URL configuration for the artifact registry above.
...
If you need to provide additional configuration, such as specifying a private repository, you can provide a [`GitRepository`](/3.0rc/api-ref/prefect/flows/#prefect.runner.storage.GitRepository) object instead of a URL:
...
If you need additional configuration for your cloud-based storage - for example, with a private S3 Bucket -  we recommend using a [storage block](/3.0rc/develop/connect-third-party/).
...
The values passed in override default values in the specified work pool's [base job template](/3.0rc/deploy/dynamic-infra/control-runs/#base-job-template).
...
```python hl_lines="5" title="job_var_image_pull.py" notest
if __name__ == "__main__":
...
```python hl_lines="5" title="job_var_env_vars.py" notest
if __name__ == "__main__":
    get_repo_info.deploy(
        name="my-deployment-never-pull",
        work_pool_name="my-docker-pool",
...
For more information on overriding job variables see this [guide](/3.0rc/deploy/dynamic-infra/customize).
...
As discussed above, if you are using [blocks](/3.0rc/develop/connect-third-party/), the credentials section can be templated with a block reference for secure and dynamic credentials access:
...
- **blocks**: [Prefect blocks](/3.0rc/develop/connect-third-party) can also be referenced with the special syntax `{{ prefect.blocks.block_type.block_slug }}`. It is highly recommended that you use block references for any sensitive information (such as a GitHub access token or any credentials) to avoid hardcoding these values in plaintext
- **variables**: [Prefect variables](/3.0rc/concepts/variables) can also be referenced with the special syntax `{{ prefect.variables.variable_name }}`. Variables can be used to reference non-sensitive, reusable pieces of information such as a default image name or a default work pool name.
...
!!! note "Docker step"
    The most commonly used build step is [`prefect_docker.deployments.steps.build_docker_image`](/3.0rc/deploy/dynamic-infra/deploy-docker/) which produces both the `image_name` and `tag` fields.

    For an example, [check out the deployments tutorial](/3.0rc/deploy/dynamic-infra/deploy-docker/).
...
| `schedule`                                 | An optional [schedule](/3.0rc/deploy/serve-workflows/add-schedules) to assign to the deployment. Fields for this section are documented in the [Schedule Fields](#schedule-fields) section.                                                                                                                      |
| `triggers`                                  | An optional array of [triggers](/3.0rc/deploy/serve-workflows/#create-a-flow-run-with-an-event-trigger) to assign to the deployment |
...
For more information about schedules, see the [Schedules](/3.0rc/deploy/serve-workflows/add-schedules/#creating-schedules-through-a-deployment-yaml-files-schedule-section) concept doc.
...
| `job_variables`                                | Values used to override the default values in the specified work pool's [base job template](/3.0rc/deploy/dynamic-infra/control-runs/#base-job-template). Maps directly to a created deployments `infra_overrides` attribute. |
...
- [Managed work pools](/3.0rc/guides/managed-execution/)
