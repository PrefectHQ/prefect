...
Artifacts are visually rich annotations on Prefect objects. They are visual metadata intended to be produced in flows or tasks but consumed by humans. They come in standardized formats such as tables, progress idicators, images, Markdown, or links. They are stored on Prefect Cloud or a Prefect server instance and rendered in the Prefect UI.
Artifacts make it easy to visualize outputs or side effects that your flows produce and capture updates over time.

![Markdown artifact sales report screenshot](/3.0rc/img/ui/md-artifact-info.png)

Artifacts are associated with a particular Prefect object, usually a task run or flow run.

Common use cases for artifacts include:

- Progress indicators: Artifacts can be used to publish progress indicators for long-running tasks. This can help you monitor the progress of your tasks and flows and ensure that they are running as expected.
...
Currently, you can render five artifact types: links, Markdown, progress, images, and tables.
...
Progress artifacts are useful for tracking the progress of long-running tasks. They render dynamically on the flow run timeline in the Prefect UI, providing a visually rich indicator of a task run's progress.

To create a progress artifact, use the `create_progress_artifact()` function. To update the progress of a progress artifact, use the `update_progress_artifact()` function.

![Progress artifact example](/3.0rc/img/ui/progress-artfiact-example.png)
...
Note that progress artifacts are updated using a separate `update_progress_artifact()` function. This is because we're not tracking revisions between versions of an artifact, we're updating an existing artifact in place.
...
Image artifacts render publically available images in the Prefect UI for easy visualization & association with a task or flow run. To create an image artifact, use the `create_image_artifact()` function.

![Image artifact example](/3.0rc/img/ui/image-artfiact-example.png)
...
Note: The image URL must be publicly accessible for the Prefect UI to render it. If you would like to create an artifact for a private image, you can use the `create_link_artifact()` function instead.
