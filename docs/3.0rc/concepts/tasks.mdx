...
Tasks also take advantage of automatic Prefect [logging](/3.0rc/guides/logs/) to capture details about task runs such as runtime, tags, and final state.
...
```pythonfrom prefect import task, flow
...
Depending on how you call tasks, they can return different types of results and optionally engage the use of a [task runner](/3.0rc/concepts/task-runners/).

Any task can return:

- Data , such as `int`, `str`, `dict`, `list`, and so on &mdash;  this is the default behavior any time you call `your_task()`.
- [`PrefectFuture`](/3.0rc/api-ref/prefect/futures/#prefect.futures.PrefectFuture) &mdash;  this is achieved by calling [`your_task.submit()`](/3.0rc/concepts/task-runners/#using-a-task-runner). A `PrefectFuture` contains both _data_ and _State_
...
To run your task with a [task runner](/3.0rc/concepts/task-runners/), you must call the task with `.submit()`.

See [state returned values](/3.0rc/concepts/task-runners/#using-results-from-submitted-tasks) for examples.
...
Note, if you are not using `asyncio.gather`, calling [`.submit()`](/3.0rc/concepts/task-runners/#using-a-task-runner) is required for asynchronous execution on the `ConcurrentTaskRunner`.
...
Task tag limits are checked whenever a task run attempts to enter a [`Running` state](/3.0rc/concepts/states/).
