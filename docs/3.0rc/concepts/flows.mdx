...
Flows can include calls to [tasks](/3.0rc/concepts/tasks/) as well as to other flows, which Prefect calls ["subflows"](#composing-flows) in this context.
Flows may be defined within modules and imported for use as subflows in your flow definitions.

[Deployments](/3.0rc/concepts/deployments/) elevate individual workflows from functions that you call manually to API-managed entities.
...
- Creating a [deployment](/3.0rc/concepts/deployments/) on Prefect Cloud or a locally run Prefect server.
...
| `task_runner`                                      | An optional [task runner](/3.0rc/concepts/task-runners/) to use for task execution within the flow when you `.submit()` tasks. If not provided and you `.submit()` tasks, the `ConcurrentTaskRunner` will be used.         |
...
When a subflow starts, it will create a new [task runner](/3.0rc/concepts/task-runners/) for tasks within the subflow.
...
Whether running locally or via a [deployment](/3.0rc/concepts/deployments/), you must indicate which flow is the entrypoint for a flow run.
...
Note that parameter values can be provided to a flow via API using a [deployment](/3.0rc/concepts/deployments/).
...
!!! note "Prerequisite"
    Read the documentation about [states](/3.0rc/concepts/states) before proceeding with this section.
...
The simplest way to create a [deployment](/3.0rc/concepts/deployments/) for your flow is by calling its [`serve` method](/3.0rc/api-ref/prefect/flows/#prefect.flows.Flow.serve).
...
!!! note "The difference between pausing and suspending a flow run"
    ...
    However, when the flow run resumes, the flow code will execute again from the beginning of the flow, so you should use [tasks](/3.0rc/concepts/tasks/) and [task caching](/3.0rc/concepts/tasks/#caching) to avoid recomputing expensive operations.
...
For more in-depth information on receiving input from users when pausing and suspending flow runs, see the [Creating interactive workflows](/3.0rc/guides/creating-interactive-workflows/) guide.
