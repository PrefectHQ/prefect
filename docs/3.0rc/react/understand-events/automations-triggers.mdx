...
Automations are a Prefect Cloud feature that enable you to configure [actions](#actions) that Prefect executes automatically based on 
...
You can even define your own custom trigger to fire based on an [event](/3.0rc/react/understand-events/) created from a 
...
Importantly, triggers can be configured not only in reaction to events, but also proactively: to fire in the absence of 
an expected event.
...
A trigger could kick off an action if the flow stays in a running state for more than 30 minutes. This action could be 
taken on the flow itself, such as canceling or restarting it. Or the action could take the form of a notification so 
someone can take manual remediation steps. Or you could set both actions to to take place when the trigger occurs.
...
Selected targets are simple, and useful for when you know exactly what object your action should act on &mdash; for 
example, the case of a cleanup flow you want to run or a specific notification you’d like to send.
...
flow run, the flow run to cancel is inferred as the stuck run that caused the trigger to fire.
...
When you need a trigger that doesn't quite fit the templates in UI trigger builder, you can define a custom trigger in JSON. 
With custom triggers, you have access to the full capabilities of Prefect's automation system - allowing you to react to many 
kinds of events and metrics in your workspace.

Each automation has a single trigger that, when fired, will cause all of its associated actions to run.  That single 
...
Event triggers are the most common types of trigger, and they are intended to react to the presence or absence of an event 
happening in your workspace.  Event triggers are indicated with `{"type": "event"}`.
...
Prefect objects (like flows, deployment, blocks, work pools, tags, etc) as well as resources you have defined in any 
events you emit yourself.  The `match` and `match_related` fields control which events a trigger considers for evaluation 
...
Consider the `resource` and `related` fields on the following `prefect.flow-run.Completed` event, truncated for the sake 
of example. Its primary resource is a flow run, and since that flow run was started via a deployment, it is related to 
...
The following configuration will filter for any events whose primary resource is a flow run, _and_ that flow run has a 
...
This configuration, on the other hand, will filter for any events for which this specific deployment is a related 
resource.
...
possibly undesired events through the filter as well. `match` and `match_related` can be combined for more restrictive 
filtering:
...
Once an event has passed through the `match` filters, it must be decided if this event should be counted toward the 
trigger's `threshold`. Whether that is the case is determined by the event names present in `expect`.
...
above 1 will also require use of `within` to define a range of time in which multiple events are seen. The following 
configuration will expect two occurrences of `prefect.flow-run.Completed` within 60 seconds.
...
<Tip>
    Note how `match` and `match_related` are used to ensure the trigger only evaluates events that are relevant to its 
    purpose.
</Tip>
...
A proactive trigger will fire when its `threshold` has _not_ been met by the end of the window of time defined by `within`. 
Proactive triggers must have a `within` value of at least 10 seconds.

The following trigger will fire if a `prefect.flow-run.Completed` event is not seen within 60 seconds after a 
...
`prefect.resource.id` will cause this trigger to be evaluated separately for each flow run id associated with these events.
...
Metric triggers (`{"type": "metric"}`) fire when the value of a metric in your workspace crosses a threshold you've defined. 
...
last day exceeds 5 minutes late, and that number hasn't gotten better for the last 10 minutes, use a trigger like this:
...
This trigger will fire once it sees at least one of each of the underlying event triggers fire within the time 
frame specified. Then the trigger will reset its state and fire the next time these three events all happen. 
...
In this case, the trigger will only fire if it sees the `daily-export-initiator` flow complete, and then the three 
files written by the other flows.

The `within` parameter for compound and sequence triggers constrains how close in time (in seconds) the child triggers 
...
and you don't want a stray older event to be included in the evaluation of the trigger.  If this isn't a concern for you, 
you may omit the `within` period, in which case there is no limit to how far apart in time the child triggers occur.

Any type of trigger may be composed into higher-order composite triggers, including proactive event triggers and metric 
triggers.  In the following example, the compound trigger will fire if any of the following events occur: a flow run 
...
specified trigger definitions will create linked automations that are triggered by events matching your chosen 
[grammar](/3.0rc/concepts/events/#event-grammar). Each trigger definition may include a [jinja template](https://en.wikipedia.org/wiki/Jinja_(template_engine)) 
...
This deployment will create a flow run when an `external.resource.pinged` event _and_ an `external.resource.replied` 
...
You can pass one or more `--trigger` arguments to `prefect deploy`, which can be either a JSON string or a 
...
Automation notifications support sending notifications via any predefined block that is capable of and configured 
to send a message. That includes, for example:
...
Automation actions can access templated variables through [Jinja](https://palletsprojects.com/p/jinja/) syntax. 
...
- A [work pool](/3.0rc/deploy/dynamic-infra/control-runs/) set up to handle the deployments
...
Next, send a notification based off a completed state outcome. Configure a notification that shows when to look into our 
workflow logic. 
...
5. Finally, create the actions for when the trigger is hit. In this case, create a notification to showcase the 
completion. 
...
Keep in mind, you do not need to create a deployment to trigger your automation, where a state outcome of a local 
flow run helped trigger this notification block. You are not required to create a deployment to trigger a notification.
</Tip>
...
automation by taking advantage of Prefect's REST API.  
...
Marvin is great at embedding data science and data analysis applications within your pre-existing data engineering workflows. 
In this case, you can use [Marvin's AI functions](https://www.askmarvin.ai/docs/text/functions/) to help make the dataset more information rich. 
...
You've seen how to create automations with the UI, REST API, and a trigger defined in a `prefect.yaml` deployment 
definition.

To learn more about Prefect events, which can trigger automations, see the [events docs](/3.0rc/concepts/events/). 
Prefect Cloud can receive external events webhooks. See the [webhooks guide](/3.0rc/react/understand-events/webhook-triggers/) 
