...
The data cleaning flow has visibility into each step, and sends a list of names to the next step of the pipeline.
...
In a few quick changes, you can programmatically create an automation that deploys flows with custom parameters. 
...
By exposing a webhook endpoint, you can kick off flows that trigger deployments, all from an event created from an 
HTTP request. 
