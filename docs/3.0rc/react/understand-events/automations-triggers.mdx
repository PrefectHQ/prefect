...
Potential actions include kicking on flow runs, pausing schedules, and sending custom notifications.
...
Importantly, you can configure the triggers not only in reaction to events, but also proactively: to fire in the absence of 
an expected event.
...
Automation notifications support sending notifications through any predefined block that is capable of and configured 
...
You do not need to create a deployment to trigger your automation, where a state outcome of a local 
...
- See the [webhooks guide](/3.0rc/react/understand-events/webhook-triggers/) 
