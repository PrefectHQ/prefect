...
The [Automations concept page](/3.0rc/react/understand-events/automations-triggers/) shows you how to configure one within the UI. 
...
To learn more about Prefect events, which can trigger automations, see the [events docs](/3.0rc/concepts/events/). Prefect Cloud can receive external events webhooks. See the [webhooks guide](/3.0rc/react/understand-events/webhook-triggers/) to learn how to create them.
...
Potential triggers include the occurrence of events from changes in a flow run's state - or the absence of such events. You can even define your own custom trigger to fire based on an [event](/3.0rc/react/understand-events/) created from a webhook or a custom event defined in Python code.
