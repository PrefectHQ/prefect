...
<Tip>
Tag-based task concurrency is different from [Global Concurrency Limits](/3.0rc/develop/global-concurrency-limits), though they can be used to achieve similar outcomes. Global Concurrency Limits are a more general way to control concurrency for any python-based operation, whereas tag-based concurrency limits are specific to Prefect tasks.
</Tip>
...
Tags without explicit limits are considered to have unlimited concurrency, and **a concurrency limit of 0 will abort task runs**. That is to say, if the concurrency limit is set to 0 for a tag, any attempt to run a task with that tag will abort instead of delay.
...
While task run concurrency limits are configured through tags (as shown below), [flow run concurrency limits](/3.0rc/develop/global-concurrency-limits) are configured through work pools and/or work queues.
</Tip>
