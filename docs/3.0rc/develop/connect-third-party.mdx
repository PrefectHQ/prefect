...
[Prefect integration libraries](/3.0rc/integrations/).
...
Find available block types in many of the published [Prefect integrations libraries](/3.0rc/integrations/).
...
<Tip>
You can also create and update blocks through the [Prefect UI](/3.0rc/cloud/blocks/).
</Tip>
...
Saving block values like this links the values of the two blocks. This means any changes 
to the values stored for the `AWSCredentials` block with the name `my_aws_credentials` 
are seen when the block values for the `S3Bucket` block named `my_s3_bucket` is loaded.
