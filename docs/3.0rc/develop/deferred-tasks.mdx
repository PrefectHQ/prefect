...
app and distribute concurrent execution across multiple processes or machines.

For example, if you have a web app, deferred tasks allow you to offload processes such as sending emails, processing 
...
workflows. For example, by calling `my_task.delay()` within a web app.
</Note>
...
- create a basic FastAPI app that defers tasks when you hit an endpoint
...
Create a file named `greeter.py` and save the following code in it, or run the existing file in the 
...
This example starts a task worker and runs deferred tasks in the background.  

To run tasks in a separate process or container, you need to start a task worker, similar to how you would run a 
...
We return a "future" from the `delay` method. You can use this object to wait for the task to complete with 
