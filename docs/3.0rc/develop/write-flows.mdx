...
Flows are defined as Python functions. Like Python functions, flows can take inputs, perform work, and return an output.
...
When a function becomes a flow, its behavior changes, giving it the following abilities:
...
- A flow can be [deployed](/3.0rc/deploy/serve-flows/), which exposes an API for interacting with it remotely.
...
A _flow run_ is a record of a single execution of a flow.

You can create a flow run by calling a flow's entrypoint function just as you would a normal Python function.
For example, by running a Python script or importing a flow into an interactive session and calling it.
...
However you run the flow, Prefect monitors the flow run, capturing flow run state for observability.
...
A single flow function could contain all of your workflow's code.
However, if you put all of your workflow logic in a single flow function and any line of code fails, the entire flow fails 
and must be retried from the beginning. 
Organizing your workflow code into smaller flow and task units lets you take advantage of Prefect features like retries, 
...
We reccomend "small tasks." 
Each task should be a single discrete piece of work in your workflow. 
For example, calling an API, performing a database operation, analyzing or transforming a data point. 
The more granular you make your tasks, your workflows can better recover from failures and you can more easily find and fix issues.
...
A _nested_ flow run is created when a flow function is called inside the execution of another flow.
Nested flow runs behave just as all flow runs do.
...
The relationship between a child and parent flow run is tracked by creating a special task run in the parent flow run.
This task run mirrors the state of the child flow run.

A task that represents a nested flow run is annotated in its `state_details` with the presence of a `child_flow_run_id` field.
A nested flow run is identified with the presence of a `parent_task_run_id` on `state_details`.
...
Nested flow runs cannot be cancelled without cancelling their parent flow run.
