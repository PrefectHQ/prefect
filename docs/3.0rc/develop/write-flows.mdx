...
- A flow can be [deployed](/3.0rc/deploy/infrastructure/docker/), which exposes an API for interacting with it remotely.
...
- Triggering a [deployment](/3.0rc/deploy/infrastructure/docker/) of that flow on Prefect Cloud or a self-hosted Prefect server instance
...
Note that you can provide parameter values to a flow through the API using a [deployment](/3.0rc/deploy/infrastructure/docker/).
...
Whether running locally or through a [deployment](/3.0rc/deploy/infrastructure/docker/), you must indicate which flow is the entrypoint for a flow run.
...
- Using external schedulers such as `cron` to invoke a flow function
- Creating a [deployment](/3.0rc/deploy/infrastructure/docker/) on Prefect Cloud or a locally run Prefect server
