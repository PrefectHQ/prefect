...
Turn any function into a Prefect flow by adding the `@flow` decorator to it:
...
Create a flow run by calling a flow by its function name, just as you would a normal Python function in a script.
...
The script below uses the HTTPX client library to fetch statistics about the [main Prefect repository](https://github.com/PrefectHQ/prefect).
...
Prefect also supports asynchronous exocution. 
...
A _flow run_ represents a single execution of a flow.

In adddition to create a flow run by calling the flow manually, you can create a flow run by using an external scheduler such as `cron` to invoke a flow function. 
More commonly, most users run flows by create a [deployment](/3.0rc/deploy/) on Prefect Cloud or Prefect server and then scheduling a flow run for the deployment through a schedule, the Prefect UI, or the Prefect API.
...
[Nested flows](/3.0rc/develop/write-flows/#composing-flows) allow you to efficiently manage, track, and version common multi-task logic.
...
If no description is provided, a flow function's docstring becomes the description.
...
Running this flow results in a **Completed** final state because the flow returns the future of the task that succeeds:
...
Running this flow fails because one of the three returned futures failed. 
Note that the states of each of the returned futures are included in the flow run state:
...
To enable retries, pass `retries` to the flow.
...
- Organize a flow with [tasks](/3.0rc/develop/write-tasks/) to break down the workflow's complexity and make it more performant and observable.
