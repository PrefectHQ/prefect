---
title: Task run concurrency limits
description: Prevent too many tasks from running simultaneously.
---

Task concurrency limits help prevent too many tasks from running simultaneously.
For example, if many tasks across multiple flows are designed to interact with a database that only allows 10 connections.

Task concurrency limits use [task tags](#tags). You can specify an optional concurrency limit as the maximum number of concurrent 
