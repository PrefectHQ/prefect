...
Prefect uses a [`ThreadPoolTaskRunner`](/3.0rc/api-ref/prefect/task-runners/#prefect.task_runners.ThreadPoolTaskRunner), which runs tasks concurrently, by default.
Tasks are submitted to a thread pool for execution.
