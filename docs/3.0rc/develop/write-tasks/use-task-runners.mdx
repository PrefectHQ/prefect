...
To submit work to the runner, use the `.submit()` method of your task.
...
Prefect future objects must be resolved explicitly before returning from your flow. 
...
In this case, you can create [subflows](/3.0rc/develop/write-flows/#composing-flows) 
...
Prefect also provides a `.map()` implementation that automatically submits a new task run for each element of its 
input data. 
This can be useful for easily submitting a lot of work to a task runner simultaneously.
