...
description: Learn how to run tasks in a desired order.
---

You can provide results from a task to other tasks (or subflows) as upstream dependencies. 
Prefect uses upstream dependencies in two ways:

1. To populate dependency arrows in the flow run graph
2. To determine execution order for [concurrently submitted](/3.0rc/develop/write-tasks/use-task-runners) 
units of work that depend on each other

<Note>

**Tasks vs. other functions**

Only results from tasks inform Prefect's ability to determine dependencies. Return values from functions 
without task decorators, including subflows, do not carry the same information about their origin as task results.
</Note>

When using task runners, such as the default [`ThreadPoolTaskRunner`](/3.0rc/api-ref/prefect/task-runners/#prefect.task_runners.ThreadPoolTaskRunner) or 
...
You can infer a task or subflow's upstream dependencies automatically through its inputs, or stated explicitly with the `wait_for` parameter.

### Automatic

When a result from a task is used as input for another task, Prefect automatically recognizes the task that result 
originated from as an upstream dependency.

This applies to every way you can run tasks with Prefect, whether you're calling the task function directly,
 calling [`.submit()`](/3.0rc/api-ref/prefect/tasks/#prefect.tasks.Task.submit), or calling 
 [`.map()`](/3.0rc/api-ref/prefect/tasks/#prefect.tasks.Task.map). Subflows similarly recognize tasks results as upstream dependencies.
...
You can inform tasks of their upstream dependencies that do not share data through the `wait_for` parameter. 
Just as with automatic dependencies, this applies to direct task function calls, 
[`.submit()`](/3.0rc/api-ref/prefect/tasks/#prefect.tasks.Task.submit), [`.map()`](/3.0rc/api-ref/prefect/tasks/#prefect.tasks.Task.map), and subflows.

<Warning>

**Differences with `.map()`**

Manually defined upstream dependencies apply to all tasks submitted by `.map()`,
so each mapped task must wait for _all_ upstream dependencies passed into `wait_for` to finish. 
This is distinct from automatic dependencies for mapped tasks, where each mapped task must only 
wait for the upstream tasks whose results it depends on.
</Warning>
...
For more complex workflows, parts of your logic may require additional resources, different infrastructure, 
or independent parallel execution. A typical approach for addressing these needs is to execute that logic as 
separate [deployment](/3.0rc/deploy/serve-flows) runs from within a flow.

Compose deployment runs into a flow and treat them as upstream dependencies by calling 
[`run_deployment`](/3.0rc/api-ref/prefect/deployments/deployments/#prefect.deployments.deployments.run_deployment) from within a task.
...
By default, deployments started from `run_deployment` also appear as subflows for tracking purposes. 
Disable this behavior by setting the `as_subflow` parameter for `run_deployment` to `False`.
