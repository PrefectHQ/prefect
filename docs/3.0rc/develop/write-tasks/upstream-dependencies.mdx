...
You can provide results from a task to other tasks (or nested flows) as upstream dependencies. 
...
without task decorators, including nested flows, do not carry the same information about their origin as task results.
</Note>
...
You can infer a task or nested flow's upstream dependencies automatically through its inputs, or stated explicitly with the `wait_for` parameter.
...
This applies to every way you can run tasks with Prefect, whether you're calling the task function directly,
 ...
 [`.map()`](/3.0rc/api-ref/prefect/tasks/#prefect.tasks.Task.map). Nested flows similarly recognize tasks results as upstream dependencies.
...
[`.submit()`](/3.0rc/api-ref/prefect/tasks/#prefect.tasks.Task.submit), [`.map()`](/3.0rc/api-ref/prefect/tasks/#prefect.tasks.Task.map), and nested flows.
...
By default, deployments started from `run_deployment` also appear as nested flows for tracking purposes. 
