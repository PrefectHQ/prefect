...
```python
from prefect import task
from prefect.cache_policies import TASKDEF
...
This task will run once and only once until the underlying code is altered:
...
```python
from prefect import task
from prefect.cache_policies import TASKDEF, INPUTS

@task(cache_policy=TASKDEF + INPUTS)
