---
title: Create Interactive Workflows
...
As an additional step, you can use an [automation](/3.0rc/react/understand-events/automations-triggers) or [notification](/3.0rc/concepts/notifications/) to alert the user to the error.
...
An iterator helps keep track of the inputs your flow has already received. If you want your flow to suspend and then resume later, save the keys of the inputs you've seen so the flow can read them back out when it resumes. You might use a [Block](/3.0rc/develop/connect-third-party/), such as a `JSONBlock`.
