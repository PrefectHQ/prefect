---
title: Create interactive flows
description: Learn how to create interactive flows with Prefect.
---
...
This guide explains how to use these features to build _interactive flows_.
...
additional information or feedback before resuming a flow. These flows are often called 
...
Approval flows that pause to ask a human to confirm whether a workflow should continue are very common in the business world. 
...
If you require more complex validation, use Pydantic [validators](https://docs.pydantic.dev/1.10/usage/validators/).
...
In the example, we use Pydantic's `validator` decorator to define a custom validation method for the `color` field. 
...
As an additional step, you can use an [automation](/3.0rc/react/understand-events/automations-triggers) or 
