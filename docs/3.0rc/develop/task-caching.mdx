...
There are many situations in which multiple tasks need to always run together as an atomic unit.
...
together. 
And after a successful execution both will be indefinitely cached until the cache key is updated.
For more information on transactions, see [here](/3.0rc/develop/transactions). 
