...
In the next section, you'll supercharge this flow by using [tasks](/3.0rc/develop/write-tasks/) to break down the workflow's complexity and make it more performant and observable.
...
Flows can include calls to [tasks](/3.0rc/develop/write-tasks/) as well as to other flows, which Prefect calls ["subflows"](#composing-flows) in this context.
Flows may be defined within modules and imported for use as subflows in your flow definitions.

[Deployments](/3.0rc/deploy/serve-workflows/) elevate individual workflows from functions that you call manually to API-managed entities.
