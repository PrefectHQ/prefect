...
States are rich objects that contain information about the status of a particular [task](/3.0rc/develop/write-tasks) run or [flow](/3.0rc/develop/write-workflows/) run. While you don't need to know the details of the states to use Prefect, you can give your workflows superpowers by taking advantage of it.
...
```python hl_lines="1-3" continuation
@flow 
def subflow():
    return 42 
...
```python hl_lines="3-4" continuation
@flow 
def my_flow():
...
```python hl_lines="4-5" continuation
@flow 
def my_flow():
    state = add_one(1, return_state=True) # return State
...
```python hl_lines="7-8" continuation
@flow 
def subflow():
    return 42 
...
```python hl_lines="3-5" continuation
@flow 
def my_flow():
...
See the [Final state determination](/3.0rc/develop/write-workflows/#final-state-determination) section of the [Flows](/3.0rc/develop/write-workflows/) documentation for further details and examples.
