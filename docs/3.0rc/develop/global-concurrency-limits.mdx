...
You may create, read, edit, and delete concurrency limits through the Prefect UI, CLI, or Python SDK.
...
### Slot decay
To achieve rate limiting behavior, you can configure "slot decay". It controls how quickly occupied slots become available again for reuse.
...
To use slot decay, set the `slot_decay_per_second` parameter when creating or updating a concurrency limit. This value determines how quickly slots refresh:
...
Choose a decay rate that balances your need for frequent task execution with the desire to limit overall system load. This allows you to fine-tune your workflow's performance and resource usage.
