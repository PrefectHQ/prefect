...
Override any logging configuration by setting an environment variable or [Prefect profile](/3.0rc/manage/configure-client/) 
...
By default, tasks and nested flows inherit the `log_prints` setting from their parent flow, unless opted out with their 
...
Configure this behavior globally for all Prefect flows, tasks, and nested flows.
...
For more information, see [Log schema](/3.0rc/api-ref/server/schemas/core/#prefect.server.schemas.core.Log).
...
[Settings](/3.0rc/manage/configure-client/) for more details about how to use settings.
