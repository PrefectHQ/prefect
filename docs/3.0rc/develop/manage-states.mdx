...
The dinstinct between types and names is subtle: state _types_ are typically used for backing orchestration logic, whereas state _names_ are more for visual display and bookkeeping.
...
A task will be placed into a `Completed` state if it returns essentially _any_ Python object, with one exception:
...
def example():
    ...
    # similarly, the flow will fail because we return a `Failed` state
