...
description: Results represent the data returned by a flow or a task.
---
...
When submitting tasks to a runner, retrieve the result with the `Future.result()` method:
...
Prefect captures all exceptions to report states to the orchestrator, 
but we do not hide them from you (unless you ask us to) as your program needs to 
know if an unexpected error has occurred.
...
If you retrieve the result from a failed state, the exception is raised. 
For this reason, it's often best to check if the state is failed first.
...
To use this behavior automatically, you may enable the `PREFECT_ASYNC_FETCH_STATE_RESULT` setting.
...
The Prefect API does not store your results [except in special cases](#storage-of-results-in-prefect). 
Instead, the result is _persisted_ to a storage location in your infrastructure and 
Prefect stores a _reference_ to the result.
...
Persistence of results requires a [**serializer**](#result-serializer) and a [**storage** location](#result-storage-location). 
Prefect sets defaults which you can adjust for custom behavior. 
You can configure results on the `flow` and `task` decorators with the following options:
...
Configure the persistence of the result of a task or flow with the `persist_result` option. 
The `persist_result` option defaults to a null value, which automatically enables persistence if it is 
...
Flow retries do not require the flow's result to be persisted.
...
You may also change Prefect's default persistence behavior with the `PREFECT_RESULTS_PERSIST_BY_DEFAULT` setting. 
...
Task and flows with `persist_result=False` will not persist their results even 
if `PREFECT_RESULTS_PERSIST_BY_DEFAULT` is `true`.

#### Result storage location

Configure the result [storage location](#result-storage-types) with the `result_storage` option. 
The `result_storage` option defaults to a null value, which infers storage from the context.
...
Configure the path of the result file in the result storage with the `result_storage_key`. 
The `result_storage_key` option defaults to a null value, which generates a unique identifier for each result.
...
Result storage keys are formatted with access to all of the modules in `prefect.runtime` and the run's `parameters`. 
The following example runs a flow with three runs of the same task. 
Each task run will write its result to a unique file based on the `name` parameter.
...
You can only configure result storage keys on tasks at this time.

#### Result serializer

You can configure the [result serializer](#result-serializer-types) with the `result_serializer` option. 
The `result_serializer` option defaults to a null value, which infers the serializer from the context.
...
Prefect provides a `CompressedSerializer` which can _wrap_ other serializers to provide 
compression over the bytes they generate. The compressed serializer uses `lzma` compression by default. 
We test other compression schemes provided in the Python standard library such as `bz2` and `zlib`, 
but any compression library that provides `compress` and `decompress` methods should work.
...
There are a few cases where Prefect _will_ store your results directly in the database. 
This is an optimization to reduce the overhead of reading and writing to result storage.

The following data types is stored by the API without persistence to storage:
...
The Prefect API tracks metadata about your results. The value of your result is only stored in 
[specific cases](#storage-of-results-in-prefect). View result metadata in the UI on the "Results" page for flows.
...
When running your workflows, Prefect keeps the results of all tasks and flows in memory 
...
When `cache_result_in_memory` is disabled, the result of your flow or task will be persisted by default. 
...
Result storage is responsible for reading and writing serialized data to an external location.
You can use any file system block for result storage.

## Result serializer types

A result serializer is responsible for converting your Python object to and from bytes. 
This is necessary to store the object outside of Python and retrieve it later.

### Pickle serializer

Pickle is a standard Python protocol for encoding arbitrary Python objects. 
We supply a custom pickle serializer at `prefect.serializers.PickleSerializer`. 
Prefect's pickle serializer uses the [cloudpickle](https://github.com/cloudpipe/cloudpickle) 
...
Specifically, we add support for all types supported by [Pydantic](https://pydantic-docs.helpmanual.io/).

By default, we use the standard Python `json` library. You can specify alternative JSON libraries, such as:
...
Prefect uses internal result types to capture information about the result attached to a state. 
...
All result types include a `get()` method that can be called to return the value of the result. 
This is done behind the scenes when the `result()` method is used on states or futures.

### Unpersisted results

Unpersisted results represent results that are not persisted beyond the current flow run. 
The value associated with the result is stored in memory, but is not available later. 
Result metadata is attached to this object for storage in the API and representation in the UI.

### Literal results

Literal results represent [results stored in the Prefect database](#storage-of-results-in-prefect). 
The values contained by these results must be JSON serializable.
...
- Storage: A reference to the [result storage](#result-storage-types) to read the serialized result.
...
The `get()` operation caches the resolved object to reduce the overhead of subsequent calls.

#### Persisted result blob

When results are persisted to storage, they are always written as a JSON document. The schema for this is described 
by the `PersistedResultBlob` type. The document contains:

- The serialized data of the result.
- A full description of [result serializer](#result-serializer-types) to deserialize the result data.
