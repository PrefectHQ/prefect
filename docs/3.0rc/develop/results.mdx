...
Results are the bedrock of many Prefect features - most notably [transactions](/3.0rc/develop/transactions) 
and [caching](/3.0rc/develop/task-caching) - and are foundational to the resilient execution paradigm that Prefect enables.
...
- [whether to persist results at all](/3.0rc/develop/results#enabling-result-persistence): this is configured through
various keyword arguments and the `PREFECT_RESULTS_PERSIST_BY_DEFAULT` setting.
- [what filesystem to persist results to](/3.0rc/develop/results#result-storage): this is configured through the `result_storage` 
keyword and the `PREFECT_DEFAULT_RESULT_STORAGE_BLOCK` setting.
- [how to serialize and deserialize results](/3.0rc/develop/results#result-serialization): this is configured through 
the `result_serializer` keyword and the `PREFECT_RESULTS_DEFAULT_SERIALIZER` setting.
- [what filename to use](/3.0rc/develop/results#result-filenames): this is configured through one of 
...
through any of the mechanisms [described below](/3.0rc/develop/results#enabling-result-persistence) - Prefect's default 
...
- `cache_policy`: a [cache policy](/3.0rc/develop/task-caching#cache-policies) specifying the behavior of the task's cache.
- `cache_key_fn`: [a function](/3.0rc/develop/task-caching#cache-key-functions) that configures a custom cache policy.
...
a string. See [task caching documentation](/3.0rc/develop/task-caching#cache-key-functions) for more information.
...
To learn more about caching mechanics in Prefect, see the [caching documentation](/3.0rc/develop/task-caching).
