...
To edit an existing schedule, select the **Edit** from the three-dot menu next to a schedule on a **Deployment** page.
...
This `serve` method will create a deployment of `my_flow` with a cron schedule that creates runs every minute of every day:
