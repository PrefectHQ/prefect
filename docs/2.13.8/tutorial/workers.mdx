...
!!! tip "[Choosing Between `flow.deploy()` and `flow.serve()`](/2.13.8/concepts/deployments/#two-approaches-to-deployments)"
...
- Work pools can be used to prioritize (or limit) flow runs through the use of [work queues](/2.13.8/concepts/work-pools/#work-queues)
...
!!! tip "Other work pool types"
    There are [work pool types](/2.13.8/concepts/work-pools/#worker-types) for serverless computing environments such as AWS ECS, Azure Container Instances, and GCP Cloud Run. Kubernetes is also a popular type of work pool.
    
    These are expanded upon in the [Guides](/2.13.8/guides) section.
...
1. [A flow](/2.13.8/tutorial/flows/)
2. A work pool
3. A worker
...
The `job_variables` section allows you to fine-tune the infrastructure settings for a specific deployment. These values override default values in the specified work pool's [base job template](/2.13.8/concepts/work-pools/#base-job-template).
...
- Learn about deploying multiple flows and CI/CD with our [`prefect.yaml`](/2.13.8/guides/prefect-deploy/#managing-deployments).
- Check out some of our other [work pools](/2.13.8/concepts/work-pools/).
- [Concepts](/2.13.8/concepts/) contain deep dives into Prefect components.
- [Guides](/2.13.8/guides/) provide step-by-step recipes for common Prefect operations including:
    - [Deploying on Kubernetes](/2.13.8/guides/deployment/helm-worker/)
    - [Deploying flows in Docker](/2.13.8/guides/deployment/docker/)
    - [Writing tests](/2.13.8/guides/testing)
