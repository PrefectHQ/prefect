...
Events enable observability into your data stack via the [event feed](/2.13.8/ui/events/#event-feed), and the configuration of Prefect's reactivity via [automations](/2.13.8/ui/automations/).

![Prefect UI](/2.13.8/img/ui/event-feed.png)

## Event Specification

Events adhere to a structured [specification](https://app.prefect.cloud/api/docs#tag/Events).

![Prefect UI](/2.13.8/img/ui/event-spec.png)
...
Prefect Cloud offers [programmable webhooks](/2.13.8/guides/webhooks/) to receive HTTP requests from other systems and translate them into events within your workspace.  Webhooks can emit [pre-defined static events](/2.13.8/guides/webhooks/#static-webhook-events), dynamic events that [use portions of the incoming HTTP request](/2.13.8/guides/webhooks/#dynamic-webhook-events), or events derived from [CloudEvents](/2.13.8/guides/webhooks/#accepting-cloudevents).

Events emitted from any source will appear in the event feed, where you can visualize activity in context and configure [automations](/2.13.8/concepts/automations/) to react to the presence or absence of it in the future.
...
![Event feed](/2.13.8/img/ui/event-feed.png)
...
![Automation from event](/2.13.8/img/ui/automation-from-event.png)

The default trigger configuration will fire every time it sees an event with a matching resource identifier. Advanced configuration is possible via [custom triggers](/2.13.8/cloud/automations/).
