...
- Prefect Cloud [API keys](/2.13.8/cloud/users/api-keys/)
- Organization [roles](/2.13.8/cloud/users/roles/) and permissions

Using service account credentials, you can [configure an execution environment](/2.13.8/cloud/connecting/#configure-a-local-execution-environment) to interact with your Prefect Cloud organization workspaces without a user having to manually log in from that environment.
...
![Viewing service accounts for an organization in Prefect Cloud.](/2.13.8/img/ui/service-accounts.png)
...
See [workspace sharing](/2.13.8/cloud/workspaces/#workspace-sharing) for more information.
...
!!! note "Service account roles"
    A service account may only be a Member of an organization.
    You may apply any valid _workspace-level_ role to a service account when it is [added to a workspace](/2.13.8/cloud/workspaces/#workspace-sharing).
