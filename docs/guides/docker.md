...
1. Images with the release packaged are built for each supported Python version (3.8, 3.9, 3.10, 3.11, 3.12) with both standard Python and Conda.
...
6. Finally, for users who want the latest `2.x.y` release without specifying a Python version, we update `2-latest` to the image for our highest supported Python version, which in this case would be equivalent to `prefect:2.1.1-python3.11`.
...
| 2-python3.12          | most recent v2 PyPi version |            3.12 |
...
| 2-python3.12-conda          | most recent v2 PyPi version |            3.12 |
