...
!!! warning "Deployments have changed since beta"
    Deployments based on `DeploymentSpec` are no longer supported. Instead, you can define deployments by using either the [`prefect deployment` CLI command](#create-a-deployment) or [the `Deployment` Python object](/2.6.9/api-ref/prefect/deployments/).
...
!!! info "Your flow code and the Prefect hybrid model"
    ...
    This is the heart of the Prefect hybrid model: there's always a boundary between your code, your private infrastructure, and the Prefect backend, such as [Prefect Cloud](/2.6.9/ui/cloud/). Even if you're using a self-hosted Prefect Orion API, you only register the deployment metadata on the backend allowing for a clean separation of concerns.

When creating a deployment, a user must answer *two* basic questions:

- What instructions does an [agent](/2.6.9/concepts/work-queues/) need to set up an execution environment for my workflow? For example, a workflow may have Python requirements, unique Kubernetes settings, or Docker networking configuration.
...
- Assign one or more tags to organize your deployments and flow runs. You can use those tags as filters in the [Prefect UI](/2.6.9/ui/flow-runs/).
...
- Define a [`Deployment`](/2.6.9/api-ref/prefect/deployments/) Python object, specifying the deployment options as properties of the object, then building and applying the object using methods of `Deployment`.
...
|  `-i`, `--infra`                   | The [infrastructure type](/2.6.9/concepts/infrastructure/) to use. (Default is `Process`) |
...
|  `--cron TEXT`    | A cron string that will be used to set a [`CronSchedule`](/2.6.9/concepts/schedules/) on the deployment. For example, `--cron "*/1 * * * *"` to create flow runs from that deployment every minute. |
|  `--interval INTEGER`     | An integer specifying an interval (in seconds) that will be used to set an [`IntervalSchedule`](/2.6.9/concepts/schedules/) on the deployment. For example, `--interval 60` to create flow runs from that deployment every minute. |
|  `--rrule TEXT`     | An `RRule` that will be used to set an [`RRuleSchedule`](/2.6.9/concepts/schedules/) on the deployment. For example, `--rrule 'FREQ=HOURLY;BYDAY=MO,TU,WE,TH,FR;BYHOUR=9,10,11,12,13,14,15,16,17'` to create flow runs from that deployment every hour but only during business hours. |
...
Note that the appropriate library supporting the storage filesystem must be installed prior to building a deployment with a storage block. For example, the AWS S3 Storage block requires the [`s3fs`](https://s3fs.readthedocs.io/en/latest/) library. See [Storage](/2.6.9/concepts/storage/) for more information.
...
![Configuring custom parameter values for an ad-hoc flow run](/2.6.9/img/concepts/custom-parameters.png)
...
Once the deployment has been created, you'll see it in the [Prefect UI](/2.6.9/ui/flow-runs/) and can inspect it using the CLI.
...
![Viewing deployments in the Prefect UI](/2.6.9/img/concepts/deployments.png)
...
[Agents and work queues](/2.6.9/concepts/work-queues/) enable the Prefect orchestration engine and API to run deployments in your local execution environments. To execute deployed flow runs you need to configure at least one agent.
...
!!! note "Scheduled flow runs"
    ...
    Scheduled flow runs will not run unless an appropriate [agent and work queue](/2.6.9/concepts/work-queues/) are configured.
...
Create a new deployment with a pre-defined [storage block](/2.6.9/concepts/storage/) and an [infrastructure](/2.6.9/concepts/infrastructure/) override:
...
In the [Prefect UI](/2.6.9/ui/deployments/), you can click the **Run** button next to any deployment to execute an ad hoc flow run for that deployment.
...
!!! tip "`PREFECT_API_URL` setting for agents"
    You'll need to configure [agents and work queues](/2.6.9/concepts/work-queues/) that can create flow runs for deployments in remote environments. [`PREFECT_API_URL`](/2.6.9/concepts/settings/#prefect_api_url) must be set for the environment in which your agent is running.
