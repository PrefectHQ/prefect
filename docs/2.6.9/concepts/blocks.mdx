...
Blocks are useful for configuration that needs to be shared across flow runs and between flows. For configuration that will change between flow runs, we recommend using [parameters](/2.6.9/concepts/flows/#parameters).
...
Blocks can also be created by anyone and shared with the community. You'll find blocks that are available for consumption in many of the published [Prefect Collections](/2.6.9/collections/catalog).
...
!!! tip "Utilizing the UI"
    Blocks documents can also be created and updated via the [Prefect UI](/2.6.9/ui/blocks/).
...
!!! tip "Sharing Blocks"
    Blocks can also be loaded by fellow Workspace Collaborators, available on [Prefect Cloud](/2.6.9/ui/cloud/).
...
This command is useful for registering all blocks found in the credentials module within [Prefect Collections](/2.6.9/collections/catalog/).
...
The registered block will then be available in the [Prefect UI](/2.6.9/ui/blocks/) for configuration.
