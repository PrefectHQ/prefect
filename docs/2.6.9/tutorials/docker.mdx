...
In the [Deployments](/2.6.9/tutorials/deployments/) and [Storage and Infrastructure](/2.6.9/tutorials/storage/) tutorials, we looked at creating configuration that enables creating flow runs via the API and with code that was uploaded to a remotely accessible location.  
...
- We'll use the flow script and deployment from the [Deployments](/2.6.9/tutorials/deployments/) tutorial. 
- We'll also use the remote storage block created in the [Storage and Infrastructure](/2.6.9/tutorials/storage/) tutorial.
...
!!! note "Run Prefect Orion server"
    This tutorial assumes you're already running a Prefect Orion server with `prefect orion start`, as described in the [Deployments](/2.6.9/tutorials/deployments/#run-a-prefect-orion-server) tutorial. 
...
![Viewing a list of infrastructure block types in the Prefect UI](/2.6.9/img/tutorials/infrastructure-blocks.png)
...
![Viewing a list of infrastructure block types in the Prefect UI](/2.6.9/img/tutorials/docker-infrastructure.png)
...
In the [Storage and Infrastructure](/2.6.9/tutorials/storage/) tutorial, recall that we needed to `pip install s3fs` the library for an S3 storage block. You'll need to include the same command in the configuration of the Docker Container infrastructure block. When the agent spins up a container for a flow run, it will know to install the `s3fs` package before starting the flow run.

As a convenience, we can use the [`EXTRA_PIP_PACKAGES` environment variable](/2.6.9/concepts/infrastructure/#installing-extra-dependencies-at-runtime) to install dependencies at runtime. If defined, `pip install ${EXTRA_PIP_PACKAGES}` is executed before the flow run starts.
...
```
If you defined a different type of storage block, such as Azure or GCS, you'll need to specify the relevant storage library. See the [Prerequisites section of the Storage tutorial](/2.6.9/tutorials/storage/#prerequisites) for details.

![Configuring a new Docker Container infrastructure block in the Prefect UI](/2.6.9/img/tutorials/docker-tutorial-block.png)
...
To demonstrate using an infrastructure block, we'll create a new variation of the deployment for the `log_flow` example from the [deployments tutorial](/2.6.9/tutorials/deployments/). For this deployment, we'll include the following options to the `prefect deployment build` command:

- Use the storage block created in the [Storage and Infrastructure](/2.6.9/tutorials/storage/) tutorial by passing `-sb s3/log-test` or `--storage-block s3/log-test`.
...
![Viewing the new Docker deployment in the Prefect UI](/2.6.9/img/tutorials/docker-deployment.png)
...
![Viewing the Docker deployment details in the Prefect UI](/2.6.9/img/tutorials/docker-deployment-details.png)
...
![Editing the Docker deployment details in the Prefect UI](/2.6.9/img/tutorials/edit-docker-deployment.png)
...
!!! note "Run a Prefect agent"
    This tutorial assumes you're already running a Prefect agent with `prefect agent start`, as described in the [Deployments](/2.6.9/tutorials/deployments/#agents-and-work-queues) tutorial. 
...
![Running the Docker deployment from the Prefect UI](/2.6.9/img/tutorials/run-docker-deployment.png)
...
![Log messages from the deployment flow run.](/2.6.9/img/tutorials/docker-flow-log.png)
