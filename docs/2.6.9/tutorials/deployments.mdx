...
[Deployments](/2.6.9/concepts/deployments/) take your flows to the next level: adding the information needed for scheduling flow runs or triggering a flow run via an API call. Deployments elevate workflows from functions that you call manually to API-managed entities.

## Components of a deployment

You need just a few ingredients to turn a flow definition into a deployment:

- A Python script that contains a function [decorated with `@flow`](/2.6.9/tutorials/flow-task-config/#basic-flow-configuration)

That's it. To create flow runs based on the deployment, you need a few more pieces:

- Prefect orchestration engine, either [Prefect Cloud](/2.6.9/ui/cloud/) or a local Prefect Orion server started with `prefect orion start`.
- An [agent and work queue](/2.6.9/concepts/work-queues/).
...
Optionally, you can configure [storage](/2.6.9/concepts/storage/) for packaging and saving your flow code and dependencies. 
...
You can pass other option flags. For example, you may specify multiple tags by providing a `-t tag` parameter for each tag you want applied to the deployment. Options are described in the [Deployments](/2.6.9/concepts/deployments/#deployment-build-options) documentation or via the `prefect deployment build --help` command.
...
Now we can _apply_ the settings in `log_flow-deployment.yaml` to create the deployment object on the Prefect Orion server API &mdash; or on a [Prefect Cloud workspace](/2.6.9/ui/cloud-getting-started/) if you had configured the Prefect Cloud API as your backend. 
...
!!! tip "Use profiles to switch between 'PREFECT_API_URL' settings"
    You can create [configuration profiles](/2.6.9/concepts/settings/#configuration-profiles) to save commonly used settings. 
...
[Agents and work queues](/2.6.9/concepts/work-queues/) are the mechanisms by which Prefect orchestrates deployment flow runs in remote execution environments.
...
You can also see your flow in the [Prefect UI](/2.6.9/ui/overview/). Open the Prefect UI at [http://127.0.0.1:4200/](http://127.0.0.1:4200/). You'll see your deployment's flow run in the UI.

![Deployment flow run on the Flow Runs page of the Prefect UI](/2.6.9/img/tutorials/my-first-deployment.png)
...
![The Deployments page displays a list of deployments created in Prefect](/2.6.9/img/tutorials/orion-deployments.png)

Now select **log-flow/log-simple** to see details for the deployment you just created.

![Viewing details of a single deployment](/2.6.9/img/tutorials/deployment-details.png)

Select **Parameters** to see the default parameters you specified in the deployment definition.

![Viewing deployment parameters](/2.6.9/img/tutorials/deployment-parameters.png)
...
![Deployment run options in the UI](/2.6.9/img/tutorials/deployment-run-options.png)
...
![Configuring custom flow run settings](/2.6.9/img/tutorials/custom-flow-run.png)
...
![Configuring custom flow run settings](/2.6.9/img/tutorials/custom-parameter.png)
...
![The deployment flow run is shown in the UI run history](/2.6.9/img/tutorials/deployment-run.png)
...
![The flow run logs show the expected Hello Trillian! log message](/2.6.9/img/tutorials/dep-flow-logs.png)
...
!!! tip "Next steps: Storage and infrastructure"
    Deployments get interesting when you can execute flow runs in environments other than your local machine. To do that, you'll need to configure [Storage and Infrastructure](/2.6.9/tutorials/storage/), which is covered in our next tutorial.
