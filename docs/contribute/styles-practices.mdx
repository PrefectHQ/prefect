...
The user-facing versioning policy lives in [Versioning and compatibility](/v3/get-started/versioning). It covers
how Prefect increments major, minor, and patch versions (which does not strictly follow semantic versioning), the
deprecation window, and which client versions are compatible with a given server or Prefect Cloud.