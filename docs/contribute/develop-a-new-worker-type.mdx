...
Prefect. For standard needs, we recommend using one of the [available worker types](deploy/infrastructure-concepts/workers/#worker-types).
</Tip>
...
The work pool that a worker polls for flow runs has a [base job template](deploy/infrastructure-concepts/work-pools/#base-job-template) 
associated with it. 
...
The [Docker worker](https://prefect-python-sdk-docs.netlify.app/prefect_docker/worker/) is an example of a worker that uses this pattern.
...
The [Kubernetes worker](https://prefect-python-sdk-docs.netlify.app/prefect_kubernetes/worker/) is an example of a worker that uses this pattern.
