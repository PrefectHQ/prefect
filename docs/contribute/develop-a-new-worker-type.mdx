...
Prefect. For standard needs, we recommend using one of the [available worker types](/3.0rc/deploy/infrastructure-concepts/workers/#worker-types).
</Tip>
...
The work pool that a worker polls for flow runs has a [base job template](/3.0rc/deploy/infrastructure-concepts/work-pools/#base-job-template) 
associated with it. 
