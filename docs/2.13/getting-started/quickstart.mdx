...
See the [install guide](/2.13/getting-started/installation/) for more detailed installation instructions.

## Step 2: Connect to Prefect's API

Much of Prefect's functionality is backed by an API.
If [self-hosting](/2.13/guides/host/), you'll need to start the Prefect webserver and related services yourself, or if you'd rather use a hosted version of the API with a bunch of additional features such as automations, collaborators, and error summaries powered by Marvin AI, sign up for a forever free [Prefect Cloud account](/2.13/cloud/).

=== "Cloud"

    1. Sign in with an existing account or create a new account at [https://app.prefect.cloud/](https://app.prefect.cloud/).
    2. If setting up a new account, [create a workspace](/2.13/cloud/workspaces/#create-a-workspace) for your account.
    3. Use the `prefect cloud login` CLI command to [log into Prefect Cloud](/2.13/cloud/users/api-keys) from your environment.
...
The fastest way to get started with Prefect is to add a `@flow` decorator to any Python function and call its `serve` method to create a deployment. [Flows](/2.13/concepts/flows/) are the core observable, deployable units in Prefect and are the primary entrypoint to orchestrated work. [Deployments](/2.13/concepts/deployments/) elevate flows to remotely configurable entities that have their own API, as we will see shortly.

Here is an example flow named "Repo Info" that contains two [tasks](/2.13/concepts/tasks/), which are the smallest unit of observed and orchestrated work in Prefect:
...
![Flow run timeline](/2.13/img/ui/flow-run-diagram.png)
...
!!! tip "Getting started tips"
    ...
    - [Flows](/2.13/concepts/flows) can be called inside of other flows (Prefect designates these ["subflows"](/2.13/concepts/flows/#composing-flows)) but a task _cannot_ be run inside of another task or from outside the context of a flow
    - Elevating a flow to a [deployment](/2.13/concepts/deployments/) exposes a Prefect-backed API for remotely managing and configuring your workflow with things such as [scheduling rules](/2.13/concepts/schedules/), [trigger rules](/2.13/cloud/automations/), and cancellation
...
We can now configure our deployment as we like - for example, let's add a [schedule](/2.13/concepts/schedules/) to our deployment.  We could do this in one of two ways:
...
![Deployment schedule](/2.13/img/ui/deployment-cron-schedule.png)

## Next steps

To learn more, try our [tutorial](/2.13/tutorial) and [guides](/2.13/guides), or go deeper with [concepts](/2.13/concepts).
