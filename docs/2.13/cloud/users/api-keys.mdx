...
After you grant approval to connect, you'll be redirected to the CLI and the API key will be saved to your local [Prefect profile](/2.13/guides/settings/).
...
![Viewing and editing API keys in the Cloud UI.](/2.13/img/ui/cloud-api-keys.png)

Select the **+** button to create a new API key.
Provide a name for the key and an expiration date.

![Creating an API key in the Cloud UI.](/2.13/img/ui/cloud-new-api-key.png)
...
Service accounts are a feature of Prefect Cloud [organizations](/2.13/cloud/organizations/) that enable you to create a Prefect Cloud API key that is not associated with a user account.
