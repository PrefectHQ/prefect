...
1. [Created a flow](/2.13/tutorial/flows/)
2. [Added tasks to it](/2.13/tutorial/tasks/)
3. [Deployed and run the flow](/2.13/tutorial/deployments/)
...
4. [Creating a work pool & worker](/2.13/tutorial/workers/)

If you're looking for examples of more advanced operations (like [deploying on Kubernetes](/2.13/guides/deployment/helm-worker/)), check out Prefect's [guides](/2.13/guides/).

## Prerequisites

1. Before you start, make sure you have Python installed, then install Prefect: `pip install -U prefect`
      1. See the [install guide](/2.13/getting-started/installation/) for more detailed instructions.

2. This tutorial requires the Prefect API, so sign up for a forever free [Prefect Cloud Account](https://app.prefect.cloud/) or, alternatively, self-host a [Prefect Server](/2.13/host/).
...
## [First steps: Flows](/2.13/tutorial/flows/)

Let's begin by learning how to create your first Prefect flow - [click here to get started](/2.13/tutorial/flows/).
