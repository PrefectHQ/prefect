...
Flows can include calls to [tasks](/2.13/concepts/tasks/) as well as to other flows, which Prefect calls ["subflows"](#composing-flows) in this context. Flows may be defined within modules and imported for use as subflows in your flow definitions.

[Deployments](/2.13/concepts/deployments/) elevate individual workflows from functions that you call manually to API-managed entities.
...
- Creating a [deployment](/2.13/concepts/deployments/) on Prefect Cloud or a locally run Prefect server.
...
![Prefect UI](/2.13/img/ui/timeline-flows.png)
...
| `task_runner`                                      | An optional [task runner](/2.13/concepts/task-runners/) to use for task execution within the flow when you `.submit()` tasks. If not provided and you `.submit()` tasks, the `ConcurrentTaskRunner` will be used.         |
...
![A simple flow visualized with the .visualize() method](/2.13/img/orchestration/hello-flow-viz.png)
...
![A flow with return values visualized with the .visualize() method](/2.13/img/orchestration/viz-return-value-tracked.png)
...
You can define multiple flows within the same file. Whether running locally or via a [deployment](/2.13/concepts/deployments/), you must indicate which flow is the entrypoint for a flow run.

!!! warning "Cancelling subflow runs"
    In line subflow runs, i.e. those created without `run_deployment`, cannot be cancelled without cancelling their parent flow run. If you may need to cancel a subflow run independent of its parent flow run, we recommend deploying it separately and starting it using the [run_deployment](/2.13/api-ref/prefect/deployments/deployments/#prefect.deployments.run_deployment) method.
...
!!! note "Prerequisite"
    Read the documentation about [states](/2.13/concepts/states) before proceeding with this section.
...
You can take this further and serve multiple flows with the same process using the [`serve`](/2.13/api-ref/prefect/runner/#prefect.runner.serve) utility along with the `to_deployment` method of flows:
...
Flows can be retrieved from remote storage using the [`flow.from_source`](/2.13/api-ref/prefect/flows/#prefect.flows.Flow.from_source) method.
...
If you need additional configuration, such as specifying a private repository, you can provide a [`GitRepository`](/2.13/api-ref/prefect/flows/#prefect.runner.storage.GitRepository) instead of URL:
...
Prefect enables pausing an in-progress flow run for manual approval. Prefect exposes this functionality via the [`pause_flow_run`](/2.13/api-ref/prefect/engine/#prefect.engine.pause_flow_run) and [`resume_flow_run`](/2.13/api-ref/prefect/engine/#prefect.engine.resume_flow_run) functions, as well as the Prefect UI.
...
!!! warning "A deployment is required"
    Flow run cancellation requires the flow run to be associated with a [deployment](#serving-a-flow). A monitoring process must be running to enforce the cancellation. Inline subflow runs, i.e. those created without `run_deployment`, cannot be cancelled without cancelling the parent flow run. If you may need to cancel a subflow run independent of its parent flow run, we recommend deploying it separately and starting it using the [run_deployment](/2.13/api-ref/prefect/deployments/deployments/#prefect.deployments.run_deployment) method.
...
![Prefect UI](/2.13/img/ui/flow-run-cancellation-ui.png)
