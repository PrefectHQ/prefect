...
!!! info "Deployments do not contain code definitions"
    ...
    This is the heart of the Prefect hybrid model: there's a boundary between your proprietary assets, such as your flow code, and the Prefect backend (including [Prefect Cloud](/2.13/cloud/)). 
...
- **`schedule`**: a [schedule object](/2.13/concepts/schedules/).
Most of the convenient interfaces for creating deployments allow users to avoid creating this object themselves. 
For example, when [updating a deployment schedule in the UI](/2.13/concepts/schedules/#creating-schedules-through-the-ui) basic information such as a cron string or interval is all that's required.
- **`trigger`** (Cloud-only): triggers allow you to define event-based rules for running a deployment. 
For more information see [Automations](/2.13/concepts/automations/).
...
This metadata is available during execution through [Prefect runtime](/2.13/guides/runtime-context/).
...
[Workers and work pools](/2.13/concepts/work-pools/) are an advanced deployment pattern that allow you to dynamically provision infrastructure for each flow run. 
...
For more information see [the guide to deploying with a worker](/2.13/guides/prefect-deploy/).
...
In the other setup, you do [a little extra up-front work to setup a work pool and a base job template that defines how individual flow runs will be submitted to infrastructure](/2.13/guides/prefect-deploy). 
...
The simplest way to deploy a Prefect flow is to use [the `serve` method](/2.13/concepts/flows/#serving-a-flow) of the `Flow` object or [the `serve` utility](/2.13/concepts/flows/#serving-multiple-flows-at-once) for managing multiple flows simultaneously. 
...
[Work pools and workers](/2.13/concepts/work-pools/) allow Prefect to exercise greater control of the infrastructure on which flows run. 
