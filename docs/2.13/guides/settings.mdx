...
!!! tip "`PREFECT_API_URL` setting for workers"
    When using [workers and work pools](/2.13/concepts/work-pools/) (agent and block-based deployments are legacy) that can create flow runs for deployments in remote environments,  [`PREFECT_API_URL`](/2.13/concepts/settings/) must be set for the environment in which your worker is running.
...
The `PREFECT_HOME` value specifies the local Prefect directory for configuration files, profiles, and the location of the default [Prefect SQLite database](/2.13/concepts/database/).
...
Prefect provides several self-hosting database configuration settings you can read about [here](/2.13/host/).

### Logging settings

Prefect provides several logging configuration settings that you can read about in the [logging docs](/2.13/concepts/logs/).
