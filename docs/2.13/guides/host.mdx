...
After you install Prefect you have a Python SDK client that can communicate with [Prefect Cloud](https://app.prefect.cloud), the platform hosted by Prefect. You also have an [API server](/2.13/api-ref/) backed by a database and a UI.
...
![Viewing the dashboard in the Prefect UI.](/2.13/img/ui/self-hosted-server-dashboard.png)
...
- [Email notifications](/2.13/cloud/automations/) &mdash; send email alerts from Prefect's servers based on automation triggers.
...
You can read more about Prefect Cloud in the [Cloud](/2.13/cloud/) section.
...
!!! tip "`PREFECT_API_URL` required when running Prefect inside a container"
    ...
    You can save the API server address in a [Prefect profile](/2.13/concepts/settings/). Whenever that profile is active, the API endpoint will be be at that address.

    See [Profiles & Configuration](/2.13/concepts/settings/) for more information on profiles and configurable Prefect settings.
...
See the [contributing docs](/2.13/contributing/overview/#adding-database-migrations) for information on how to create new database migrations.

## Notifications

When you use [Prefect Cloud](/2.13/cloud/) you gain access to a hosted platform with Workspace & User controls, Events, and Automations. Prefect Cloud has an option for automation notifications. The more limited Notifications option is provided for the self-hosted Prefect server.

Notifications enable you to set up alerts that are sent when a flow enters any state you specify. When your flow and task runs changes [state](/2.13/concepts/states/), Prefect notes the state change and checks whether the new state matches any notification policies. If it does, a new notification is queued.
...
!!! cloud-ad "Notifications in Prefect Cloud"
    Prefect Cloud uses the robust [Automations](/2.13/cloud/automations/) interface to enable notifications related to flow run state changes and work pool status.
...
![Creating a notification in the Prefect UI](/2.13/img/ui/create-email-notification.png)
...
![Viewing all configured notifications in the Prefect UI](/2.13/img/ui/notifications.png)
