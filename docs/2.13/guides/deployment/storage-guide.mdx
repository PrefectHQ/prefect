...
Another option for authentication is for the [worker](/2.13/concepts/work-pools/#worker-overview) to have access to the the storage location at runtime via SSH keys.
...
In earlier versions of Prefect [storage blocks](/2.13/concepts/blocks/) were the recommended way to store flow code. 
...
Check out more [guides](/2.13/guides/) to reach your goals with Prefect.
