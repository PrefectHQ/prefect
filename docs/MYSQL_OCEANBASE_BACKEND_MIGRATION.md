...
- `server/models/task_runs.py`（`create_task_run` → `INSERT IGNORE` + 再查询）
- `server/models/flow_runs.py`（带 **idempotency key** 的 flow run 插入 → `IGNORE`）
- `server/services/task_run_recorder.py`（`TaskRun` 批量 upsert → `on_duplicate_key_update` + 条件更新；**`TaskRunState`：MySQL 下逐行插入，仅忽略主键重复—不可用 `INSERT IGNORE`**，否则会因 **`UNIQUE(task_run_id, timestamp)`** 静默丢弃后续状态行，表现为 task 卡在 **RUNNING**。）

**后续**若新接口 500 且堆栈为 **`'Insert' object has no attribute 'on_conflict_...'`**，按上表在同一文件补 MySQL 分支即可。

---

## 8. TaskRunRecorder 与 task_run 图（`start_time` / `end_time`）

走 API 的 `set_task_run_state` 时，orchestration 会写入 **`start_time` / `end_time`**。**`TaskRunRecorder`** 只根据客户端事件里的 `task_run` 做 upsert，不会自动跑同一套策略。

客户端在 `emit_task_run_state_change_event` 中对 `task_run` 使用 **`exclude_none=True`**，再经服务端 **`model_dump_for_orm(..., exclude_unset=True)`**，`start_time` / `end_time` 常常**根本不会出现在 upsert 列清单里**，库里长期为 **NULL**，UI 瀑布图/时间轴会异常（`state_timestamp` 仍有值）。

本仓库在 **`task_run_recorder.py`** 中：

- 按当前状态与 **`state.timestamp`** 在写入前**补全**时间与可推导的 **`total_run_time`**（与 `SetStartTime` / `SetEndTime` 语义对齐）；
- 同一 flush 内对**同一 task** 多条事件：**合并**组内候选时间（如取最早开始、最晚结束），避免只保留最后一条时丢掉 RUNNING 的 `start_time`。

**仅影响新写入**。历史行若已为 NULL，需单独数据修复或依赖新运行自然覆盖。
...
- **时间戳列**：ORM `Timestamp` 在 MySQL 上映射为 **`DATETIME(6)`**（微秒），降低同一秒内多状态与 **`uq_task_run_state__task_run_id_timestamp_desc`** 冲突的概率。已在运库的表若仍为秒精度，可按需在 OceanBase 上对 `task_run_state.timestamp`（及同类列）做 **`ALTER ... DATETIME(6)`**。
