...
- A [Prefect Cloud](https://app.prefect.cloud) account or [Prefect self-managed instance](/v3/concepts/server).
...
<Steps>
    <Step title={"Create the task definition"}>
        ...
        * `<prefect-api-key>` with your Prefect API key.
        <Accordion title={"Finding your PREFECT_API_KEY"} icon={"circle-question"}>
            ...
            <AccordionGroup>
                ...
                <Accordion title={"Self-hosted Prefect server"}>
                    ...
                    You should either:
                    1. Remove the `PREFECT_API_KEY` environment variable from your task definition (if your server has no authentication)
                    ...
                    2. Replace it with `PREFECT_API_AUTH_STRING` containing your basic auth credentials (if your server uses [basic authentication](/v3/advanced/security-settings#basic-authentication))
