...
<Note>
Prefect also includes a built-in `SlackWebhook` block in `prefect.blocks.notifications` that requires no extra dependencies. The two blocks have different capabilities:

- **`prefect.blocks.notifications.SlackWebhook`** (block type slug: `slack-webhook`): Uses Apprise under the hood. Includes `notify_type` and `allow_private_urls` options. Supports Slack GovCloud.
- **`prefect_slack.SlackWebhook`** (block type slug: `slack-incoming-webhook`): Uses the official Slack SDK. Provides `get_client()` for advanced Slack SDK access.
