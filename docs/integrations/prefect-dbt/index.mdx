...
<Tip>
The `PrefectDbtRunner` is inspired by the `DbtRunner` from dbt Core, and its `invoke` method accepts the same arguments. Refer to the [`DbtRunner` documentation](https://docs.getdbt.com/reference/programmatic-invocations/dbt-runner) for more information on how to call `invoke`.
</Tip>
...
The `PrefectDbtSettings` class, based on Pydantic's `BaseSettings`, automatically detects common `DBT_`-prefixed environment variables, like `DBT_PROFILES_DIR` and `DBT_PROJECT_DIR`.
If no environment variables are set, dbt's established defaults are used.
...
The `PrefectDbtRunner` supports templating in your `profiles.yml` file, allowing you to reference Prefect blocks and variables that will be resolved at runtime.
...
For example, a Prefect variable called `target` can have a different value in development and production workspaces - `"dev"` and `"prod"`, respectively.
This allows you to use the same `profiles.yml` file for both environments, automatically referencing Snowflake blocks when running in production.
...
The `PrefectDbtRunner` maps all dbt log levels to standard Python logging levels, so filtering for log levels like `WARNING` or `ERROR` in the Prefect UI applies to dbt's logs.
It also selects the appropriate logging target based on the invocation context: logging to a Prefect run logger when executed within a flow or task, and logging directly to the terminal when run outside a flow or task.

#### Failure handling

The `PrefectDbtRunner` raises exceptions with standardized messages containing detailed information about the failure.
...
This enables you to set up alerting for specific node states through Prefect's automations system. To be notified when this model is skipped, the following automation trigger could be set up:
...
The `PrefectDbtRunner` automatically emits lineage events describing the relationships between the resources in your dbt project.
Resources can be viewed in Prefect Cloud on the flow run page. Clicking "View graph" will display a lineage graph with nodes directly upstream and downstream of the selected resource.
