...
`DbtHookContext` includes the hook event, command, owner, args, node metadata, node status, and run result details. `post_model` and `on_run_end` hooks can optionally be filtered with `select="..."` on both `PrefectDbtRunner` and `PrefectDbtOrchestrator`; `on_run_start` is always run-level and does not accept a selector.
