...
- A [dbt adapter](https://docs.getdbt.com/docs/supported-data-platforms) for your target database if using dbt Core (for example, `dbt-duckdb`, `dbt-snowflake`, or `dbt-bigquery`). Install it alongside `prefect-dbt`:

```bash
pip install "prefect[dbt]" dbt-duckdb    # for DuckDB
pip install "prefect[dbt]" dbt-snowflake  # for Snowflake
pip install "prefect[dbt]" dbt-bigquery   # for BigQuery
```

<Info>
`prefect-dbt` depends on `dbt-core` but does **not** include a database adapter.
Without one you will see an error like `Could not find adapter type <type>!` at runtime.
The `[snowflake]`, `[bigquery]`, and `[postgres]` extras for `prefect-dbt` **do** include the corresponding dbt adapter alongside Prefect Block types, but adapters without an extra (such as `dbt-duckdb`) must be installed separately.
</Info>
...
`PrefectDbtRunner` supports optional Python lifecycle hooks for invocation, selector, and node events.
...
def log_node_start(node_id, manifest_node, **_):
    print(f"Starting {node_id} ({manifest_node.name})")


def alert_on_failure(node_id, status, **_):
    print(f"{node_id} failed with status {status}")
...
def run_dbt():
    runner = PrefectDbtRunner(
        hooks={
            "before_invoke": [lambda args, **_: print(f"Running: {args}")],
            "before_node": [log_node_start],
            "on_node_failure": [alert_on_failure],
        }
    )
    runner.invoke(["build", "--select", "tag:nightly"])
```

Supported `PrefectDbtRunner` hook names:
...
`PrefectDbtOrchestrator` also accepts a `hooks` mapping for higher-level orchestration events such as `before_orchestration`, `after_orchestration`, `before_wave`, `after_wave`, `before_node_orchestration`, `after_node_orchestration`, and `on_orchestration_failure`.
