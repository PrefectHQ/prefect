...
Using [dbt's native retry functionality](https://docs.getdbt.com/reference/commands/retry) to retry failed nodes is recommended.
...
The `PrefectDbtRunner`'s `raise_on_failure` option can be set to `False` to suppress failures in dbt to cause the failure of the flow or task in which `.invoke()` is called.
