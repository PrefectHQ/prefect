...
`prefect-dbt` 0.7.0 and later ship two interfaces for running dbt Core from a Prefect flow. Both coexist and can be mixed within the same project — choose based on how much control you want Prefect to have over dbt's execution.
...
See the dedicated guides for each interface. For the legacy 0.6.6-and-earlier CLI API, see below.
