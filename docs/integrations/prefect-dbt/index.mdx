...
```bash
13:37:11.000 | INFO    | Flow run 'vigilant-bullfinch' - Found 8 models, 3 seeds, 18 data tests, 543 macros
13:37:11.014 | INFO    | Flow run 'vigilant-bullfinch' - 
13:37:11.026 | INFO    | Flow run 'vigilant-bullfinch' - Concurrency: 1 threads (target='dev')
13:37:11.038 | INFO    | Flow run 'vigilant-bullfinch' - 
13:37:13.665 | INFO    | Task run 'model my_first_dbt_model' - 1 of 29 OK created sql table model main.my_first_dbt_model ..................... [OK in 0.18s]
13:37:13.671 | INFO    | Task run 'model my_first_dbt_model' - Finished in state Completed()
```
...
Using dbt's native retry functionality to retry failed nodes is recommended.
...
def run_dbt():
    PrefectDbtRunner(
        settings=PrefectDbtSettings(
            project_dir="test",
            profiles_dir="examples/run_dbt",
            log_level=EventLevel.DEBUG, # explicitly choose a log level for dbt
        )
    ).invoke(["build"])
...
By default, node execution failures will cause raises exceptions with standardized messages containing detailed information about the failure.
