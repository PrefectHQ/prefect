...
If no environment variables are set, dbt's defaults are used, which are `$HOME/.dbt` for both profiles dir and project dir.
...
def run_dbt():
    PrefectDbtRunner(
        settings=PrefectDbtSettings(
            ...
            profiles_dir=Path("dbt/dbt_profiles").resolve()
        )
    ).invoke(["build"])
...
It's recommended to use absolute paths for `project_dir` and `profiles_dir` to avoid unwanted issues with relative paths.
