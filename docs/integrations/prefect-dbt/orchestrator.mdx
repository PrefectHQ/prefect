...
<Info>
The orchestrator automatically runs `dbt deps` before the first build to install packages declared in `packages.yml`. To skip this (for example when packages are pre-installed in an image), pass a custom executor:
...
Each wave of independent nodes runs as a single `dbt build` invocation. Lowest overhead, but if any node in a wave fails the entire wave is marked failed and downstream waves are skipped.
...
| `skipped_nodes`          | Nodes filtered out by selectors or source-freshness checks, with reason.                                  |
| `estimated_parallelism`  | Width of the largest wave — the maximum concurrent nodes.                                                 |
...
By default, `run_build()` creates a Prefect markdown artifact summarizing results — nodes executed, statuses, and timings. You can optionally attach compiled SQL or write a dbt-compatible `run_results.json`:

```python
orchestrator = PrefectDbtOrchestrator(
    settings=settings,
    create_summary_artifact=True,   # default — set False to disable
    include_compiled_code=True,     # attach compiled SQL to the artifact
...
- `run_results.json` is written to `<project_dir>/target/run_results.json`.
...
<Info>
When you pass a custom `executor`, the orchestrator's `settings`, `state_path`, `defer`, `defer_state_path`, and `favor_state` parameters are ignored — those are specific to `DbtCoreExecutor`. All other orchestrator features (execution modes, test strategies, caching, retries, artifacts, asset lineage) work identically.
</Info>
