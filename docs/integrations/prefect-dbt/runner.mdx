...
description: Reactive dbt Core execution — dbt drives, Prefect observes
...
The runner is *reactive*: dbt drives execution with its own threading, and Prefect observes each node as a task. If you need per-node retries, cross-run caching, or fine-grained concurrency control, see [`PrefectDbtOrchestrator`](/integrations/prefect-dbt/orchestrator) instead.
...
- [`PrefectDbtOrchestrator`](/integrations/prefect-dbt/orchestrator) — proactive per-node and per-wave orchestration with retries, caching, and dbt Cloud support.
