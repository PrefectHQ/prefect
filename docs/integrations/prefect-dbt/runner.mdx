...
|---|---|---|---|
| `@runner.on_run_start` | Run | No | Before dbt begins executing any nodes |
| `@runner.post_model` | Node | Yes (optional) | After each model node finishes |
| `@runner.on_run_end` | Run | Yes (optional) | After all nodes have finished and dbt returns a result |
...
or [Prefect automations](/v3/how-to-guides/automations/create-actions) for critical failure responses.
