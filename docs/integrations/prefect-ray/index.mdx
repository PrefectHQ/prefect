...
A couple of other sharp edges teams hit when running real workloads over `ray://`:
...
- **The Ray head node can end up needing to import your flow's dependencies.** When a flow module imports heavy or environment-specific packages at the top level (decorators, type hints, closures), it's possible to see `ModuleNotFoundError` raised on the head even when your workers have the packages installed. The mitigations — baking every flow's dependencies into the head image, doing runtime installs on the head, running one head per flow stack — are all costly. If you run into this, the simplest fix is usually to move the driver *inside* the cluster so the Ray Client path is out of the picture entirely.
...
Here the Prefect worker (and therefore the flow's driver process) runs on a node that already has a Ray instance started on it; `ray.init(address="auto")` attaches to that local raylet. No Ray Client connection; no off-cluster driver to keep fed with the right dependencies.

There are two common ways to get your Prefect worker onto a cluster node:

- **Sidecar container.** Run the Prefect worker as a second container in the Ray head pod (or a Ray worker-group pod) with a shared `/tmp/ray` `emptyDir` volume. The worker polls Prefect; when a flow runs, its driver finds the local raylet through the shared socket. If the always-on sidecar is a cost concern, pair it with something like [KEDA](https://keda.sh/) for scale-to-zero.
- **Submit the flow script as a Ray Job.** `ray job submit --runtime-env-json='...' -- python run_flow.py` places the entrypoint on a cluster node, sets `RAY_ADDRESS` in its environment, and — unlike the `ray://` case above — applies `runtime_env` to the entrypoint script itself. Inside `run_flow.py`, `RayTaskRunner(address="auto")` attaches to the same cluster. This is Ray's recommended pattern for long-running workloads.

Both avoid the Ray Client sharp edges above; pick based on how you want to schedule the driver process. The ephemeral "one Ray cluster per flow run" pattern (e.g. via KubeRay) is a third option when you want full per-run isolation, at the cost of cluster start-up time on every run.
