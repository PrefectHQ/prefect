...
prefect-ray currently requires Prefect < 3.0.

With Prefect < 3.0 there are a few limitations with Ray::
...
We recommend using the [Prefect UI to configure a storage block](https://docs.prefect.io/develop/connect-third-party/) to use for remote results storage.
