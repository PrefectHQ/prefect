...
Once you move beyond a local, temporary Ray instance, the `address` argument you pass to `RayTaskRunner` determines where the *driver* (the process running the flow engine and submitting tasks to Ray) sits relative to the Ray cluster. A few things in Ray's own documentation are worth knowing before you pick one.

### What Ray's docs say about the options

**Ray Client (`ray://...`).** Ray's [Ray Client page](https://docs.ray.io/en/latest/cluster/running-applications/job-submission/ray-client.html) notes that Ray Client "has architectural limitations and may not work as expected when using Ray for ML workloads (like Ray Tune or Ray Train)" and recommends the Ray Jobs API for interactive development on ML projects. It also warns that a Ray Client connection "will terminate the workload if the connection is lost for more than 30 seconds." The [Ray Jobs overview](https://docs.ray.io/en/latest/cluster/running-applications/job-submission/index.html) lists Ray Client under "interactive running" and marks it **"(For Experts only)"**, recommending instead that you "run your script directly on a cluster node."

**`runtime_env` scope.** Ray's [dependency management docs](https://docs.ray.io/en/latest/ray-core/handling-dependencies.html) state: *"If `runtime_env` is specified from `ray.init(runtime_env=...)`, the runtime env is only applied to all children Tasks and Actors, not the entrypoint script (Driver) itself."* Since `RayTaskRunner` calls `ray.init` under the hood, `init_kwargs={"runtime_env": ...}` installs dependencies on Ray workers but not on the process that holds the Ray Client connection. Whatever your flow module imports at the top level must already be available to the driver process.

**`address="auto"`.** [Ray's starting-Ray docs](https://docs.ray.io/en/latest/ray-core/starting-ray.html) describe this as attaching to a Ray instance that was started on the same machine via `ray start`. For running a flow inside the cluster, Ray's guidance is to "run your script directly on a cluster node."
...
@flow(task_runner=RayTaskRunner())
def my_flow():
    return [f.result() for f in square.map(range(4))]
...
No `address` specified. `prefect-ray` starts a Ray instance in the same process. Best for local development, tests, and single-machine parallelism.
...
The driver runs on whatever machine you launch the flow from; it connects to the Ray head over `ray://`. Easy to set up for interactive use. The two constraints Ray documents above both apply: the `runtime_env` you pass here installs into workers but not into your driver process, and a 30-second network hiccup will terminate the workload. Ray itself recommends the Jobs API rather than Ray Client for long-running workloads.

### Pattern 3 — In-cluster driver with `address="auto"`
...
Run the Prefect worker (and therefore the flow's driver process) on a node that already has a Ray instance started via `ray start` — for example, as a sidecar container in the Ray head pod sharing `/tmp/ray` via an `emptyDir` volume, or by submitting the flow script as a Ray Job so Ray places the entrypoint on a cluster node. `ray.init(address="auto")` then attaches to that local raylet. This matches Ray's own recommendation for non-interactive use ("run your script directly on a cluster node") and avoids the Ray Client limitations listed above.

If you submit via the Ray Jobs API, the `runtime_env` you pass to `ray job submit` *does* install on the entrypoint script as well as its children — see Ray's [Jobs quickstart](https://docs.ray.io/en/latest/cluster/running-applications/job-submission/quickstart.html).

### Summary

| Pattern | Address | Driver process location | Best for |
| --- | --- | --- | --- |
| Local temporary | omitted | In-process with your flow | development, tests |
| Ray Client | `ray://host:10001` | Outside the cluster | interactive use (Ray docs mark this "For Experts only") |
| In-cluster driver | `"auto"` | On a cluster node with a local raylet | long-running and production workloads |
