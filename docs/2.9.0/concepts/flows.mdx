...
All workflows are defined within the context of a flow. Flows can include calls to [tasks](/2.9.0/concepts/tasks/) as well as to other flows, which we call ["subflows"](#composing-flows) in this context. Flows may be defined within modules and imported for use as subflows in your flow definitions.

Flows are required for [deployments](/2.9.0/concepts/deployments/) &mdash; every deployment points to a specific flow as the entrypoint for a flow run.
...
- Creating a [deployment](/2.9.0/concepts/deployments/) on Prefect Cloud or a locally run Prefect server.
...
| `task_runner` | An optional [task runner](/2.9.0/concepts/task-runners/) to use for task execution within the flow when you `.submit()` tasks. If not provided and you `.submit()` tasks, the `ConcurrentTaskRunner` will be used. |
...
You can define multiple flows within the same file. Whether running locally or via a [deployment](/2.9.0/concepts/deployments/), you must indicate which flow is the entrypoint for a flow run.
...
!!! note "Prerequisite"    
    Read the documentation about [states](/2.9.0/concepts/states) before proceeding with this section. 
...
Prefect enables pausing an in-progress flow run for manual approval. Prefect exposes this functionality via the [`pause_flow_run`](/2.9.0/api-ref/prefect/engine/#prefect.engine.pause_flow_run) and [`resume_flow_run`](/2.9.0/api-ref/prefect/engine/#prefect.engine.resume_flow_run) functions, as well as via the Prefect server or Prefect Cloud UI. 
