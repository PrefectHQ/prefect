...
One of the most common reasons to use a tool like Prefect is [scheduling](/2.10.21/concepts/schedules). You want your flows running on production infrastructure in a consistent and predictable way. Up to this point, we’ve demonstrated running Prefect flows as scripts, but this means *you* have been the one triggering flow runs. In order to schedule flow runs or trigger them based on [events](/2.10.21/cloud/events/) or certain conditions, you’ll need to [deploy](/2.10.21/concepts/deployments/) them.
...
Deploying a flow is the act of specifying where, when, and how it will run. This information is encapsulated and sent to Prefect as a [Deployment](/2.10.21/concepts/deployments/) which contains the crucial metadata needed for orchestration. Deployments elevate workflows from functions that you call manually to API-managed entities.
...
!!! tip "Other work pool types"
    ...
    These are expanded upon in the [Guides](/2.10.21/guides) section.
...
When you click into the `my-process-pool`, select the "Work Queues" tab. You should see a red status icon listed for the default work queue signifying that this queue is not ready to submit work. Work queues are an advanced feature. You can learn more about them in the [work queue documentation.](/2.10.21/concepts/work-pools/#work-queues) 
...
- Learn about deploying multiple flows and CI/CD with our [`prefect.yaml`](/2.10.21/concepts/projects/#the-prefect-yaml-file)
- Check out some of our other [work pools](/2.10.21/concepts/work-pools/)
- [Concepts](/2.10.21/concepts/) contain deep dives into Prefect components.
- [Guides](/2.10.21/guides/) provide step by step recipes for common Prefect operations including:
    - [Deploying on Kubernetes](/2.10.21/guides/deployment/helm-worker/)
    - [Deploying flows in Docker](/2.10.21/guides/deployment/docker/)
    - [Writing tests](/2.10.21/guides/testing)
