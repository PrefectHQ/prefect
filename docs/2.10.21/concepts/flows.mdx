...
All workflows are defined within the context of a flow. Flows can include calls to [tasks](/2.10.21/concepts/tasks/) as well as to other flows, which we call ["subflows"](#composing-flows) in this context. Flows may be defined within modules and imported for use as subflows in your flow definitions.

Flows are required for [deployments](/2.10.21/concepts/deployments/) &mdash; every deployment points to a specific flow as the entrypoint for a flow run.
...
- Creating a [deployment](/2.10.21/concepts/deployments/) on Prefect Cloud or a locally run Prefect server.
...
![Prefect UI](/2.10.21/img/ui/prefect-dashboard.png)
...
| `task_runner`                                      | An optional [task runner](/2.10.21/concepts/task-runners/) to use for task execution within the flow when you `.submit()` tasks. If not provided and you `.submit()` tasks, the `ConcurrentTaskRunner` will be used.         |
...
You can define multiple flows within the same file. Whether running locally or via a [deployment](/2.10.21/concepts/deployments/), you must indicate which flow is the entrypoint for a flow run.

!!! warning "Cancelling subflow runs"
    In line subflow runs, i.e. those created without `run_deployment`, cannot be cancelled without cancelling their parent flow run. If you may need to cancel a subflow run independent of its parent flow run, we recommend deploying it separately and starting it using the [run_deployment](/2.10.21/api-ref/prefect/deployments/deployments/#prefect.deployments.run_deployment) method.
...
!!! note "Prerequisite"
    Read the documentation about [states](/2.10.21/concepts/states) before proceeding with this section.
...
Prefect enables pausing an in-progress flow run for manual approval. Prefect exposes this functionality via the [`pause_flow_run`](/2.10.21/api-ref/prefect/engine/#prefect.engine.pause_flow_run) and [`resume_flow_run`](/2.10.21/api-ref/prefect/engine/#prefect.engine.resume_flow_run) functions, as well as the Prefect server or Prefect Cloud UI.
...
!!! warning "An agent is required"
    Flow run cancellation requires the flow run to be submitted by an agent or worker. The agent or worker must be running to enforce the cancellation. In line subflow runs, i.e. those created without `run_deployment`, cannot be cancelled without cancelling the parent flow run. If you may need to cancel a subflow run independent of its parent flow run, we recommend deploying it separately and starting it using the [run_deployment](/2.10.21/api-ref/prefect/deployments/deployments/#prefect.deployments.run_deployment) method.
...
![Prefect UI](/2.10.21/img/ui/flow-run-cancellation-ui.png)


# content from old Flows UI page
# Flows

A [flow](/2.10.21/concepts/flows/) contains the instructions for workflow logic, including the `@flow` and `@task` functions that define the work of your workflow.

The **Flows** page in the Prefect UI lists any flows that have been observed by a Prefect API. This may be your [Prefect Cloud](/2.10.21/ui/cloud/) workspace API, a local Prefect server, or the Prefect ephemeral API in your local development environment.

![View a list of flows observed by Prefect in the Prefect UI.](/2.10.21/img/ui/flows.png)

For each flow, the **Flows** page lists the flow name and displays a graph of activity for the flow.

You can see additional details about the flow by [selecting the flow name](#inspect-a-flow). You can see detail about the flow run by [selecting the flow run name](/2.10.21/ui/flow-runs/#inspect-a-flow-run).

## Inspect a flow

If you select the name of a flow on the **Flows** page, the UI displays details about the flow.

![Details for a flow in the Prefect UI](/2.10.21/img/ui/flow-details.png)
...
- Check [work pools](/2.10.21/ui/work-pools/) to make sure there's a queue that can service the flow run based on tags, deployment, or flow runner.
- Make sure an [agent](/2.10.21/concepts/work-pools/) is running in your execution environment and is configured to pull work from an appropriate work pool.
