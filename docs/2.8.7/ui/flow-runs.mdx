...
You can select any flow name or flow run name on the list to display further details. See the [Flows and Tasks](/2.8.7/ui/flows-and-tasks/) documentation for more information.
...
- [State](/2.8.7/concepts/states/) of the flow run
...
- Flow version
- Run count
- [Infrastructure](/2.8.7/concepts/infrastructure/) used by the flow run
- Tags
- Logs
- Task runs
- Subflow runs
...
**Logs** displays all log messages for the flow run. See [Logging](/2.8.7/concepts/logs/) for more information about configuring and customizing log messages. Logs are the default view for a flow run.
...
- Check [work pools](/2.8.7/ui/work-pools/) to make sure there's a queue that can service the flow run based on tags, deployment, or flow runner.
- Make sure an [agent](/2.8.7/concepts/work-pools/) is running in your execution environment and is configured to pull work from an appropriate work pool.
...
The flow run retention policy represents the number of days each flow run is available in the Prefect Cloud UI, and via the Prefect CLI and API after it ends. Once a flow run reaches a terminal state ([detailed in the chart here](/2.8.7/concepts/states/#state-types)), it will be retained until the end of the flow run retention period. 
...
This retention policy applies to all [details about a flow run](/2.8.7/ui/flow-runs/#inspect-a-flow-run), including its task runs. Subflow runs follow the retention policy independently from their parent flow runs, and are removed based on the time each subflow run reaches a terminal state. 
