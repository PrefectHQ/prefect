...
One of the configuration options demonstrated in the [Flow and task configuration](/2.8.7/tutorials/flow-task-config/) tutorial was setting a task runner that enables different execution capabilities for tasks _within a single flow run_. 
...
You can also run tasks using parallel or distributed execution by using the Dask or Ray task runners available through [Prefect Collections](/2.8.7/collections/catalog/). 
...
Read more about using Dask in the [Dask task runner tutorial](/2.8.7/tutorials/dask-ray-task-runners/#running-parallel-tasks-with-dask).
...
Any given flow run &mdash; meaning in this case your workflow including any subflows and tasks &mdash; executes in its own environment using the infrastructure configured for that environment. In these examples, that infrastructure is probably your local computing environment. But for flow runs based on [deployments](/2.8.7/concepts/deployments/), that infrastructure might be a server in a datacenter, a VM, a Docker container, or a Kubernetes cluster.

The ability to execute flow runs in a non-blocking or parallel manner is subject to execution infrastructure and the configuration of [agents and work pools](/2.8.7/concepts/work-pools/) &mdash; advanced topics that are covered in other tutorials.
...
!!! tip "Next steps: Flow orchestration with Prefect"
    The next step is learning about [the components of Prefect](/2.8.7/tutorials/orchestration/) that enable coordination and orchestration of your flow and task runs.
