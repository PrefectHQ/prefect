...
- Has [tasks](/2.8.7/concepts/tasks/) and [flows](/2.8.7/concepts/flows/).
- Orchestrates your flow runs and provides observability into their execution [states](/2.8.7/concepts/states/).
...
| Configuration that determines how and where to execute your flow runs. | Run configuration such as `flow.run_config = DockerRun()`. | Create an infrastructure block such as a [Docker Container](/2.8.7/tutorials/docker/) and specify it as the infrastructure when creating a deployment. | [How can I run my flow in a Docker container?](https://discourse.prefect.io/t/how-can-i-run-my-flow-in-a-docker-container/64) |
...
To deploy your Prefect 1 flows, you have to send flow metadata to the backend in a step called registration. Prefect 2 no longer requires flow pre-registration. Instead, you create a [Deployment](/2.8.7/concepts/deployments/) that specifies the entry point to your flow code and optionally specifies:
...
- [Caching docs](/2.8.7/concepts/tasks/#caching)
