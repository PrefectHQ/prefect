...
If you'd like to create your deployments using Python code, see the discussion of flow code storage on the `.deploy` tab of [Deploying Flows to Work pools and Workers guide](/2.15/guides/prefect-deploy/#creating-work-pool-based-deployments).
...
Another option for authentication is for the [worker](/2.15/concepts/work-pools/#worker-overview) to have access to the storage location at runtime via SSH keys.
...
In earlier versions of Prefect [storage blocks](/2.15/concepts/blocks/) were the recommended way to store flow code. 
...
Check out more [guides](/2.15/guides/) to reach your goals with Prefect.
