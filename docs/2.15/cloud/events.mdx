...
Events enable observability into your data stack via the [event feed](/2.15/ui/events/#event-feed), and the configuration of Prefect's reactivity via [automations](/2.15/concepts/automations/).
...
![Prefect UI](/2.15/2.15/img/ui/event-spec.png)
...
Prefect Cloud offers [programmable webhooks](/2.15/guides/webhooks/) to receive HTTP requests from other systems and translate them into events within your workspace.  Webhooks can emit [pre-defined static events](/2.15/guides/webhooks/#static-webhook-events), dynamic events that [use portions of the incoming HTTP request](/2.15/guides/webhooks/#dynamic-webhook-events), or events derived from [CloudEvents](/2.15/guides/webhooks/#accepting-cloudevents).

Events emitted from any source will appear in the event feed, where you can visualize activity in context and configure [automations](/2.15/concepts/automations/) to react to the presence or absence of it in the future.
...
![Event feed](/2.15/2.15/img/ui/event-feed.png)
...
From an event page, you can configure an [automation](/2.15/concepts/automations) to trigger on the observation of matching events or a lack of matching events by clicking the automate button in the overflow menu:

![Automation from event](/2.15/2.15/img/ui/automation-from-event.png)

The default trigger configuration will fire every time it sees an event with a matching resource identifier. Advanced configuration is possible via [custom triggers](/2.15/cloud/automations/).
