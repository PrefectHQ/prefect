...
- API keys: Create and view [API keys](/2.15.0/cloud/users/api-keys/) for connecting to Prefect Cloud from the CLI or other environments.
...
Admins on Pro and Enterprise tier Prefect Cloud accounts can grant members of the account [roles](/2.15.0/cloud/users/roles/) in a workspace, such as Runner or Viewer.
Custom roles are available on Enterprise tier accounts.

## API keys

[API keys](/2.15.0/cloud/users/api-keys/) enable you to authenticate an environment to work with Prefect Cloud.

## Service accounts <span class="badge pro"></span> <span class="badge enterprise"></span>

[Service accounts](/2.15.0/cloud/users/service-accounts/) enable you to create a Prefect Cloud API key that is not associated with a user account.

## Single sign-on (SSO) <span class="badge pro"></span> <span class="badge enterprise"></span>

Pro and Enterprise plans offer [single sign-on (SSO)](/2.15.0/cloud/users/sso/) integration with your team’s identity provider. Enterprise tier accounts provide additional options with [directory sync and SCIM provisioning](/2.15.0/cloud/users/sso/#directory-sync).

## Audit log <span class="badge pro"></span> <span class="badge enterprise"></span>

[Audit logs](/2.15.0/cloud/users/audit-log/) provide a chronological record of activities performed by Prefect Cloud users who are members of an account.
...
Users of Enterprise tier Prefect Cloud accounts can be added to [Teams](/2.15.0/cloud/users/teams/) to simplify access control governance.
