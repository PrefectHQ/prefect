...
<span class="badge enterprise"></span> Prefect Cloud incidents help you manage inevitable workflow disruptions, minimize their impact, and ensure operational continuity. 
...
* **Through an [automation](/3.1/automate/events/automations-triggers/)**, set up incident creation as an automated response to selected triggers.
...
See [event triggers](/3.1/automate/events/automations-triggers/#custom-triggers) for more information on custom triggers, and check out your event feed to see the event types emitted by your incidents and other resources (that is, the events that you can react to).
