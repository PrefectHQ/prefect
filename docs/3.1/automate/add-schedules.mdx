...
You can add one or more schedules to any [served or deployed](/3.1/deploy/run-flows-in-local-processes) flow.
...
[`serve` method](/3.1/develop/write-flows/#serving-a-flow) of the `Flow` object or
[the `serve` utility](/3.1/develop/write-flows/#serving-multiple-flows-at-once) for managing multiple flows simultaneously
- If using [worker-based deployments](/3.1/deploy/infrastructure-concepts/workers/)
...
See the [Settings docs](/3.1/manage/settings-and-profiles/) for more information on altering your settings.
