...
In Prefect Cloud, events power [audit logs](/3.1/manage/cloud/manage-users/audit-logs/).

Events enable observability into your data stack through the [event feed in the UI](#events-in-the-cloud-ui) and the 
configuration of Prefect's reactivity through [automations](/3.1/automate/events/automations-triggers/).

## Event specification

Events adhere to a structured [specification](https://app.prefect.cloud/api/docs#tag/Events).

![Prefect UI](/3.1/img/ui/event-spec.png)
...
Prefect Cloud offers [programmable webhooks](/3.1/automate/events/webhook-triggers/) to receive HTTP requests 
from other systems and translate them into events within your workspace. 
Webhooks can emit [pre-defined static events](/3.1/automate/events/webhook-triggers/#static-webhook-events), 
dynamic events that [use portions of the incoming HTTP request](/3.1/automate/events/webhook-triggers/#dynamic-webhook-events), or events derived from [CloudEvents](/3.1/automate/events/webhook-triggers/#accepting-cloudevents).
...
![Event feed](/3.1/img/ui/event-feed.png)
...
From an event page, you can configure an [automation](/3.1/automate/events/automations-triggers/) to trigger on 
the observation of matching events—or a lack of matching events—by clicking the automate button in the overflow menu:

![Automation from event](/3.1/img/ui/automation-from-event.png)

The default trigger configuration fires every time it sees an event with a matching resource identifier. 
Advanced configuration is possible through [custom triggers](/3.1/automate/events/custom-triggers/).
