...
We recommend using [tasks](/3.1/develop/write-tasks/) and [task caching](/3.1/develop/task-caching) to avoid recomputing expensive operations.
...
see [Send and receive flow run inputs](/3.1/develop/inputs/).