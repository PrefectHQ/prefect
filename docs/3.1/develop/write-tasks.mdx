...
- Use automatic [logging](/3.1/develop/logging/) to capture runtime details, tags, 
and final state
...
(see all [task settings](/3.1/develop/write-tasks/#task-configuration) and 
[flow settings](/3.1/develop/write-flows/#flow-settings))
...
<Tip>
Tasks are always executed in the main thread by default, unless a specific [task runner](/3.1/develop/task-runners) is used to execute them on different threads, processes, or infrastructure. This facilitates native Python debugging and profiling.
</Tip>
...
- Filter task runs by tag in the UI and through the [Prefect REST API](/3.1/api-ref/rest-api/#filtering).
