...
For configuration that will change between flow runs, we recommend using [parameters](/3.1/develop/write-flows/#parameters).
</Tip>
...
![The block catalogue in the UI](/3.1/img/ui/block-library.png)

These types separate blocks from [Prefect variables](/3.1/develop/variables/), which are unstructured JSON documents.
...
The JSON, DateTime, and String blocks are deprecated in favor of [Variables](/3.1/develop/variables/).
</Warning>
