...
- a Docker [**work pool**](/3.1/deploy/infrastructure-concepts/work-pools/): stores the infrastructure configuration for your deployment
- a Docker [**worker**](/3.1/deploy/infrastructure-concepts/workers/): process that polls the Prefect API for flow runs to execute as Docker containers
- a [**deployment**](/3.1/deploy/index/): a flow that should run according to the configuration on your Docker work pool
...
To instead learn how to run flows in a static, long-lived container, see [this](/3.1/deploy/static-infrastructure-examples/docker/) guide.
...
In a production setting this worker should run as a [daemonized or managed process](/3.1/resources/daemonize-processes/).
...
Note that you used a [Prefect Secret block](/3.1/develop/blocks/) to load the URL configuration for
...
See the [Store flow code](/3.1/deploy/infrastructure-concepts/store-flow-code/) guide for more flow code storage options.
...
[base job template](/3.1/deploy/infrastructure-concepts/work-pools/#base-job-template).
...
See [Override work pool job variables](/3.1/deploy/infrastructure-concepts/customize) for more information about how to customize these variables.
...
- [Deploy flows on Kubernetes](/3.1/deploy/infrastructure-examples/kubernetes/)
- [Deploy flows on serverless infrastructure](/3.1/deploy/infrastructure-examples/serverless/)
- [Daemonize workers](/3.1/resources/daemonize-processes/)
