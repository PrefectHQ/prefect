...
You can create a deployment through [Python code with the `flow.deploy` method](/3.1/deploy/infrastructure-concepts/deploy-via-python) or through a [YAML specification defined in a `prefect.yaml` file](/3.1/deploy/infrastructure-concepts/prefect-yaml).
...
For more information, see [this discussion of custom Docker images](/3.1/deploy/infrastructure-examples/docker/#automatically-build-a-custom-docker-image-with-a-local-dockerfile).
...
Another authentication option is to give the [worker](/3.1/deploy/infrastructure-concepts/workers/) access to the storage location at runtime through SSH keys.
...
Read more about when to consider using `serve` [here](/3.1/deploy/infrastructure-concepts/deploy-via-python#when-to-consider-flow-deploy-over-flow-serve).
