...
The simplest way to create a [deployment](/3.1/deploy/infrastructure-examples/docker/) for your flow is by calling its `serve` method.
...
[schedules](/3.1/automate/add-schedules/) for more advanced scheduling options
...
For more ways to store and access flow code, see the [Retrieve code from storage page](/3.1/deploy/infrastructure-concepts/store-flow-code).
...
Learn more about work-pool based deployments on the [Configure dynamic infrastructure with work pools page](/3.1/deploy/infrastructure-concepts/work-pools)