...
<Tip>
See [Install Prefect](/3.1/get-started/install/) for more details on installation.
</Tip>
...
If you have any issues with browser-based authentication, you can [authenticate with a manually created API key](/3.1/manage/cloud/manage-users/api-keys/) instead.
  </Tab>
</Tabs>
...
This will create a corresponding [flow](/3.1/develop/write-flows/). 

Adding `@task` decorators to any functions called by the flow converts them to [tasks](/3.1/develop/write-tasks/). 
...
A [work pool](/3.1/deploy/infrastructure-concepts/work-pools/) is the most common interface for deploying flows to remote infrastructure.
...
1. Create a [managed work pool](/3.1/deploy/infrastructure-concepts/work-pools):
...
A [deployment](/3.1/deploy/infrastructure-examples/docker/) is used to determine when, where, and how a flow should run.
...
1. Create a deployment in code:
   ...
   See [Where to store your flow code](/3.1/deploy/infrastructure-concepts/store-flow-code) for more details.
   </Tip>
...
1. Schedule a run for the deployment:
   ...
   ![Flow run graph and logs](/3.1/img/ui/qs-flow-run.png)
...
* [Write tasks](/3.1/develop/write-tasks)
* [Cloud and server](/3.1/manage)
* [Manage infrastructure with work pools](/3.1/deploy/infrastructure-concepts/work-pools) to learn about running workflows on Kubernetes, Docker, and serverless infrastructure.
