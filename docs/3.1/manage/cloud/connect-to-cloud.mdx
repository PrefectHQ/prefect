...
2. [Install Prefect](/3.1/get-started/install/) in the environment where you want to execute flow runs.
...
You can authenticate by manually pasting an [API key](/3.1/manage/cloud/manage-users/api-keys/) or through a browser-based approval that auto-generates an API key with a 30-day expiration.
...
See [Settings](/3.1/manage/settings-and-profiles/) for details.

<Note>
**Environment variables**

You can set `PREFECT_API_URL` and `PREFECT_API_KEY` just like any other environment variable. 
See [Overriding defaults with environment variables](/3.1/manage/settings-and-profiles/) 
