...
After you install Prefect you have a Python SDK client that can communicate with [Prefect Cloud](https://app.prefect.cloud), the platform hosted by Prefect. You also have an [API server](/2.11/api-ref/) backed by a database and a UI. 

In this section you'll learn how to host your own Prefect server.

![Prefect Server UI](/2.11/img/ui/flow-run-page-server.png)
...
![Viewing the orchestrated flow runs in the Prefect UI.](/2.11/img/tutorial/first-steps-ui.png)
...
- [Workspaces](/2.11/cloud/workspaces/) &mdash; isolated environments to organize your flows, deployments, and flow runs.
- [Automations](/2.11/cloud/automations/) &mdash; configure triggers, actions, and notifications in response to real-time monitoring events.
- [Email notifications](/2.11/cloud/automations/) &mdash; send email alerts from Prefect's servers based on automation triggers.
- [Organizations](/2.11/cloud/organizations/) &mdash; user and workspace management features that enable collaboration for larger teams.
- [Service accounts](/2.11/cloud/users/service-accounts/) &mdash; configure API access for running agents or executing flow runs on remote infrastructure.
- [Custom role-based access controls (RBAC)](/2.11/cloud/users/roles/) &mdash; assign users granular permissions to perform certain activities within an organization or a workspace.
- [Single Sign-on (SSO)](/2.11/cloud/users/sso/) &mdash; authentication using your identity provider.
- [Audit Log](/2.11/cloud/users/audit-log/) &mdash; a record of user activities to monitor security and compliance.
- Collaborators &mdash; invite others to work in your [workspace](/2.11/cloud/workspaces/#workspace-collaborators) or [organization](/2.11/cloud/organizations/#organization-members).

You can read more about Prefect Cloud in the [Cloud](/2.11/cloud/) section. 
...
!!! tip "`PREFECT_API_URL` required when running Prefect inside a container"
    ...
    You can save the API server address in a [Prefect profile](/2.11/concepts/settings/). Whenever that profile is active, the API endpoint will be be at that address.

    See [Profiles & Configuration](/2.11/concepts/settings/) for more information on profiles and configurable Prefect settings.
...
See the [contributing docs](/2.11/contributing/overview/#adding-database-migrations) for information on how to create new database migrations.


## Notifications

When you use [Prefect Cloud](/2.11/cloud/) you gain access to a hosted platform with Workspace & User controls, Events, and Automations. Prefect Cloud has an option for automation notifications. The more limited Notifications option is provided for the self-hosted Prefect server.

Notifications enable you to set up alerts that are sent when a flow enters any state you specify. When your flow and task runs changes [state](/2.11/concepts/states/), Prefect notes the state change and checks whether the new state matches any notification policies. If it does, a new notification is queued.
...
!!! cloud-ad "Notifications in Prefect Cloud"
    Prefect Cloud uses the robust [Automations](/2.11/cloud/automations/) interface to enable notifications related to flow run state changes and work queue health.
...
![Creating a notification in the Prefect UI](/2.11/img/ui/create-slack-notification.png)
...
![Viewing all configured notifications in the Prefect UI](/2.11/img/ui/notifications.png)
