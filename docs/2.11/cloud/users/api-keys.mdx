...
See [Configure Local Environments](/2.11/ui/cloud-local-environment/) for details on setting up access to Prefect Cloud from a local development environment or remote execution environment.
...
![Viewing and editing API keys in the Cloud UI.](/2.11/img/ui/cloud-api-keys.png)
...
![Creating an API key in the Cloud UI.](/2.11/img/ui/cloud-new-api-key.png)
...
Service accounts are a feature of Prefect Cloud [organizations](/2.11/cloud/organizations/) that enable you to create a Prefect Cloud API key that is not associated with a user account. 
