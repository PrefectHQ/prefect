...
- [`PrefectFuture`](/2.11/api-ref/prefect/futures/#prefect.futures.PrefectFuture): A Prefect object that contains both _data_ and _State_.

Returning data  is the default behavior any time you call `your_task()`.

Returning Prefect [`State`](/2.11/api-ref/server/schemas/states/) occurs anytime you call your task or flow with the argument `return_state=True`.

Returning [`PrefectFuture`](/2.11/api-ref/prefect/futures/#prefect.futures.PrefectFuture) is achieved by calling `your_task.submit()`.
...
See the [Final state determination](/2.11/concepts/flows/#final-state-determination) section of the [Flows](/2.11/concepts/flows/) documentation for further details and examples.
...
- [Send a notification when a flow run fails](/2.11/guides/state-change-hooks/#send-a-notification-when-a-flow-run-fails)
- [Delete a Cloud Run job when a flow crashes](/2.11/guides/state-change-hooks/#delete-a-cloud-run-job-when-a-flow-crashes)
