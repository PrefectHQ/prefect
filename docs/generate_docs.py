...
def format_header(obj, level=1):
    ...
    call_sig = f' {header} {is_class} {class_name}```({class_sig})```{get_source(obj)}\n'
    return call_sig
...
def collect_items(page):
    ...
    fname = page["page"]
    module = page["module"]
    assert hasattr(module, '__all__'), "Cannot document a module without specifying __all__"
    ...
    for page in OUTLINE:
        ...
        with open(fname, 'w') as f:
            for obj in items:
                ...
                # document methods
                for name, method in inspect.getmembers(obj, predicate=lambda x: inspect.isroutine(x) and obj.__name__ in x.__qualname__):
