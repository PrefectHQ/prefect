...
You can view and filter logs in the [Prefect UI](/2.7.11/ui/flow-runs/#inspect-a-flow-run) or Prefect Cloud, or access log records via the API.
...
You can see logs for the flow run in the Prefect UI by navigating to the [**Flow Runs**](/2.7.11/ui/flow-runs/#inspect-a-flow-run) page and selecting a specific flow run to inspect.
...
For more information, see [Log schema](/2.7.11/api-ref/orion/schemas/core/#prefect.orion.schemas.core.Log) in the API documentation.
