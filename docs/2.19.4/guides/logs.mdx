...
You can view and filter logs in the [Prefect UI](/2.19.4/ui/flow-runs/#inspect-a-flow-run) or Prefect Cloud, or access log records via the API.
...
You can see logs for a flow run in the Prefect UI by navigating to the [**Flow Runs**](/2.19.4/ui/flow-runs/#inspect-a-flow-run) page and selecting a specific flow run to inspect.

![Viewing logs for a flow run in the Prefect UI](/2.19.4/img/ui/flow-run-details.png)
...
Prefect provides several settings for configuring [logging level and loggers](/2.19.4/concepts/logs/).
...
You can override any logging configuration by setting an environment variable or [Prefect Profile](/2.19.4/concepts/settings/) setting using the syntax `PREFECT_LOGGING_[PATH]_[TO]_[KEY]`, with `[PATH]_[TO]_[KEY]` corresponding to the nested address of any setting.
...
For more information, see [Log schema](/2.19.4/api-ref/server/schemas/core/#prefect.server.schemas.core.Log) in the API documentation.
...
[Settings](/2.19.4/concepts/settings/) for more details about how to use settings.
