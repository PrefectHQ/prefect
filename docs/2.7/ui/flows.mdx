...
A [flow](/2.7.11/concepts/flows/) contains the instructions for workflow logic, including the `@flow` and `@task` functions that define the work of your workflow. 

The **Flows** page in the Prefect UI lists any flows that have been observed by a Prefect API. This may be your [Prefect Cloud](/2.7.11/ui/cloud/) workspace API, a local Prefect Orion API server, or the Prefect ephemeral API in your local development environment.
...
You can see additional details about the flow by [selecting the flow name](#inspect-a-flow). You can see detail about the flow run by [selecting the flow run name](/2.7.11/ui/flow-runs/#inspect-a-flow-run).
