...
In Prefect, [blocks](/2.7.11/concepts/blocks/) are a primitive that enable you to specify configuration for interacting with external systems.

[Storage](/2.7.11/concepts/storage/) blocks contain configuration for interacting with file storage such as a remote filesystem, AWS S3, and so on.

[Infrastructure](/2.7.11/concepts/infrastructure/) blocks contain settings that [agents](/2.7.11/concepts/work-queues/) use to stand up execution infrastructure for a flow run.
...
In this example we've specified a storage location that could be used by the flow example from the [deployments tutorial](/2.7.11/tutorials/deployments/).
...
To demonstrate using a storage block, we'll create a new variation of the deployment for the `log_flow` example from the [deployments tutorial](/2.7.11/tutorials/deployments/). For this deployment, we'll specify using the storage block created earlier by passing `-sb s3/log-test` or `--storage-block s3/log-test` to the `prefect deployment build` command.
...
Similar to storage blocks, infrastructure [blocks](/2.7.11/concepts/blocks/) contain configuration for interacting with external systems. Specifically, infrastructure includes settings that [agents](/2.7.11/concepts/work-queues/) use to create an execution environment for a flow run.
...
- Providing a value for `path` to `Deployment.build_from_flow` or at `Deployment` initialization (see [the API reference for more details](/2.7.11/api-ref/prefect/deployments/#prefect.deployments.Deployment)).
...
As before, we can configure all of this via Python instead of the CLI by modifying our `deployment.py` file [created in the previous tutorial](/2.7.11/tutorials/deployments/#deployment-creation-with-python):
...
!!! tip "Next steps: Flow runs with Docker"
    Continue on to the [Docker](/2.7.11/tutorials/docker/) tutorial where we'll put storage, infrastructure, and deployments together to run a flow in a Docker container.
