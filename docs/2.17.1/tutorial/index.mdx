...
1. [Created a flow](/2.17.1/tutorial/flows/)
2. [Added tasks to it](/2.17.1/tutorial/tasks/)
3. [Deployed and run the flow locally](/2.17.1/tutorial/deployments/)
4. [Created a work pool and run the flow remotely](/2.17.1/tutorial/work-pools/)
...
5. [Using a worker-based deployment](/2.17.1/tutorial/workers/)

If you're looking for examples of more advanced operations (like [deploying on Kubernetes](/2.17.1/guides/deployment/kubernetes/)), check out Prefect's [guides](/2.17.1/guides/).

## Prerequisites

1. Before you start, make sure you have Python installed, then install Prefect: `pip install -U prefect`

See the [install guide](/2.17.1/getting-started/installation/) for more detailed instructions, if needed.
...
1. Use the `prefect cloud login` CLI command to [authenticate to Prefect Cloud](/2.17.1/cloud/users/api-keys/) from your environment.
...
As an alternative to using Prefect Cloud, you can self-host a [Prefect server instance](/2.17.1/host/).
...
## [First steps: Flows](/2.17.1/tutorial/flows/)

Let's begin by learning how to create your first Prefect flow - [click here to get started](/2.17.1/tutorial/flows/).
