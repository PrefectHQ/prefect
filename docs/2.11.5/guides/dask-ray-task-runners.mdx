...
The default task runner is the [`ConcurrentTaskRunner`](/2.11.5/api-ref/prefect/task-runners/#prefect.task_runners.ConcurrentTaskRunner). 
...
Many real-world data workflows benefit from true parallel, distributed task execution. For these use cases, the following Prefect-developed task runners for parallel task execution may be installed as [Prefect Integrations](/2.11.5/integrations/catalog/). 
...
!!! note "Remote storage"
    We recommend configuring [remote file storage](/2.11.5/concepts/storage/) for task execution with `DaskTaskRunner` or `RayTaskRunner`. This ensures tasks executing in Dask or Ray have access to task result storage, particularly when accessing a Dask or Ray instance outside of your execution environment.
...
Let's start with the [`SequentialTaskRunner`](/2.11.5/api-ref/prefect/task-runners/#prefect.task_runners.SequentialTaskRunner). This task runner runs all tasks synchronously and may be useful when used as a debugging tool in conjunction with async code.
...
Now run `ray_flow.py` `RayTaskRunner` automatically creates a local Ray instance, then immediately starts executing all of the tasks in parallel. If you have an existing Ray instance, you can provide the address as a parameter to run tasks in the instance. See [Running tasks on Ray](/2.11.5/concepts/task-runners/#running_tasks_on_ray) for details.
...
Because task runners are specified on flows, you can assign different task runners to tasks by using [subflows](/2.11.5/concepts/flows/#composing-flows) to organize those tasks.
