...
You can add a schedule to any flow [deployment](/2.11.5/concepts/deployments/). The Prefect `Scheduler` service periodically reviews every deployment and creates new flow runs according to the schedule configured for the deployment.
...
You can add, modify, and view schedules by selecting **Edit** under the three dot menu next to a Deployment in the **Deployments** tab of the [Prefect UI](/2.11.5/ui/overview/). 

![Deployment edit button](/2.11.5/img/concepts/edit-schedule-callout.png)

To create a schedule from the UI, select **Add**. 

![Prefect UI with Add button called out under Scheduling heading](/2.11.5/img/concepts/add-schedule-callout.png)

Then select **Interval** or **Cron** to create a schedule.

![Prefect UI with Interval button selected](/2.11.5/img/concepts/interval-schedule.png)
...
Alternatively, you can edit the `schedule` section of the [deployment YAML file](/2.11.5/concepts/deployments/#deploymentyaml). 
...
!!! info "Daylight saving time considerations"
  ...
  See the Python [CronSchedule class docs](/2.11.5/api-ref/server/schemas/schedules/#prefect.server.schemas.schedules.CronSchedule) for more information. 
...
See the Python [IntervalSchedule class docs](/2.11.5/api-ref/server/schemas/schedules/#prefect.server.schemas.schedules.IntervalSchedule) for more information. 
...
See the Python [RRuleSchedule class docs](/2.11.5/api-ref/server/schemas/schedules/#prefect.server.schemas.schedules.RRuleSchedule) for more information. 
...
See the [Settings docs](/2.11.5/concepts/settings/) for more information on altering your settings.
