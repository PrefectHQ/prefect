...
1. Run `prefect init` in the root of your repository and choose `docker` for the project name and answer the prompts to create a `prefect.yaml` file with a build step that will create a Docker image with the flow code built in. See the [Docker guide](/2.12.1/guides/deployment/docker/) for more info.
...
Another option for authentication is for the [worker](/2.12.1/concepts/work-pools/#worker-overview) to have access to the the storage location at runtime via SSH keys.
...
In earlier versions of Prefect [storage blocks](/2.12.1/concepts/blocks/) were the recommended way to store flow code. 
...
Check out more [guides](/2.12.1/guides/) to reach your goals with Prefect.
