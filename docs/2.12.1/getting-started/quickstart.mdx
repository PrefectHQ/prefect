...
See the [install guide](/2.12.1/getting-started/installation/) for more detailed installation instructions.

### Step 2: Connect to Prefect's API

Much of Prefect's functionality is backed by an API - if [self-hosting](/2.12.1/guides/host/), you'll need to start the Prefect webserver and related services yourself, or if you'd rather use a hosted version of the API you can sign up for a forever free [Prefect Cloud account](/2.12.1/cloud/).
...
=== "Cloud"
    ...
    2. If setting up a new account, [create a workspace](/2.12.1/cloud/workspaces/#create-a-workspace) for your account.
    3. Use the `prefect cloud login` Prefect CLI command to [log into Prefect Cloud](/2.12.1/cloud/users/api-keys) from your environment.
...
The fastest way to get started with Prefect is to add a `@flow` decorator to any Python function and call its `serve` method to create a deployment. [Flows](/2.12.1/concepts/flows/) are the core observable, deployable units in Prefect and are the primary entrypoint to orchestrated work. [Deployments](/2.12.1/concepts/deployments/) elevate flows to remotely configurable entities that have their own API, as we will see shortly.

Here is an example flow named "Repo Info" that contains two [tasks](/2.12.1/concepts/tasks/), which are the smallest unit of observed and orchestrated work in Prefect:
...
![Flow run timeline](/2.12.1/img/ui/flow-run-diagram.jpg)
...
!!! tip "Getting started tips"
    ...
    - [Flows](/2.12.1/concepts/flows) can be called inside of other flows (Prefect designates these ["subflows"](/2.12.1/concepts/flows/#composing-flows)) but a task _cannot_ be run inside of another task or from outside the context of a flow
    - Elevating a flow to a [deployment](/2.12.1/concepts/deployments/) exposes a Prefect-backed API for remotely managing and configuring your workflow with things such as [scheduling rules](/2.12.1/concepts/schedules/), [trigger rules](/2.12.1/cloud/automations/), and cancellation 
...
We can now configure our deployment as we like - for example, let's add a [schedule](/2.12.1/concepts/schedules/) to our deployment.  We could do this in one of two ways:
...
![Deployment schedule](/2.12.1/img/ui/deployment-cron-schedule.png)

## Next Steps

To learn more, try our [tutorial](/2.12.1/tutorial) and [guides](/2.12.1/guides), or go deeper with [concepts](/2.12.1/concepts).
