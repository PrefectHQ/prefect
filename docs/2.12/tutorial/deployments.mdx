...
!!! note "Reminder to start a Prefect API"
    Some features in this tutorial, such as scheduling, require a Prefect server to be running; if using a self-hosted setup, simply run `prefect server start` to run both the webserver and UI; if using Prefect Cloud, make sure you have [successfully configured your local profile](/2.12/cloud/cloud-quickstart/).

## Why deployments?

One of the most common reasons to use a tool like Prefect is [scheduling](/2.12/concepts/schedules) or [event-based triggering](/2.12/concepts/automations/). 
...
- trigger new runs, [cancel active runs](/2.12/concepts/flows/#cancel-a-flow-run), pause scheduled runs, customize parameters, and more
- remotely configure schedules and automation rules for your deployments
- dynamically provision infrastructure using [workers](/2.12/tutorials/workers/)

## What is a deployment?

Deploying a flow is the act of specifying when, where, and how it will run. 
This information is encapsulated and sent to Prefect as a [deployment](/2.12/concepts/deployments/) that contains the crucial metadata needed for remote orchestration. 
...
- `cron`: a keyword that allows us to set a cron string schedule for the deployment; see [schedules](/2.12/concepts/schedules/) for more advanced scheduling options
...
- learn how to dynamically provision infrastructure with [the workers and work pools tutorial](/2.12/tutorial/workers/)
- review the [deployments concept documentation](/2.12/concepts/deployments/)
