...
Agents poll for work every 15 seconds by default. This interval is configurable in your [profile settings](/2.10/concepts/settings/) with the `PREFECT_AGENT_QUERY_INTERVAL` setting.
...
![The UI displays a list of configured work pools](/2.10/img/ui/work-pool-list.png)
...
![process work pool configuration options](/2.10/img/ui/process-work-pool-config.png)
...
| --- | --- | --- |
| [`process`](/2.10/api-ref/prefect/workers/process/) | Executes flow runs in subprocesses | |
...
If you don’t see a worker type that meets your needs, consider [developing a new worker type](/2.10/guides/deployment/developing-a-new-worker-type/)!
...
### Polling for work
Workers poll for work every 15 seconds by default. This interval is configurable in your [profile settings](/2.10/concepts/settings/) with the
