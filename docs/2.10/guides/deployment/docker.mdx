...
In the [Deployments](/2.10/tutorial/deployments/) tutorial, we looked at creating configuration that enables creating flow runs via the API and with code that was uploaded to a remotely accessible location.  
...
- We'll use the flow script and deployment from the [Deployments](/2.10/tutorial/deployments/) tutorial. 
...
!!! note "Run a Prefect server"
    This guide assumes you're already running a Prefect server with `prefect server start`, as described in the [Deployments](/2.10/tutorial/deployments/) tutorial.
...
![Viewing a list of infrastructure block types in the Prefect UI](/2.10/img/tutorial/infrastructure-blocks.png)
...
![Viewing a list of infrastructure block types in the Prefect UI](/2.10/img/tutorial/docker-infrastructure.png)
...
As a convenience, Prefect base images check the [`EXTRA_PIP_PACKAGES` environment variable](/2.10/concepts/infrastructure/#installing-extra-dependencies-at-runtime) to install dependencies at runtime. If defined, `pip install ${EXTRA_PIP_PACKAGES}` is executed before the flow run starts.
...
![Configuring a new Docker Container infrastructure block in the Prefect UI](/2.10/img/guides/docker-tutorial-block.png)
...
To demonstrate using an infrastructure block, we'll create a new variation of the deployment for the `log_flow` example from the [deployments tutorial](/2.10/tutorial/deployments/).
...
![Viewing the new Docker deployment in the Prefect UI](/2.10/img/guides/docker-deployment.png)
...
![Viewing the Docker deployment details in the Prefect UI](/2.10/img/guides/docker-deployment-details.png)
...
![Editing the Docker deployment details in the Prefect UI](/2.10/img/guides/edit-docker-deployment.png)
...
!!! note "Run a Prefect agent"
    This guide assumes you're already running a Prefect agent with `prefect agent start`, as described in the [Deployments](/2.10/tutorial/deployments/#agents-and-work-pools) tutorial.
...
![Running the Docker deployment from the Prefect UI](/2.10/img/guides/run-docker-deployment.png)
...
![Log messages from the deployment flow run.](/2.10/img/guides/docker-flow-log.png)
