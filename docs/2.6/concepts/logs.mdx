...
You can view and filter logs in the [Prefect UI](/2.6/ui/flow-runs/#inspect-a-flow-run) or Prefect Cloud, or access log records via the API or CLI.
...
You can see logs for the flow run in the Prefect UI by navigating to the [**Flow Runs**](/2.6/ui/flow-runs/#inspect-a-flow-run) page and selecting a specific flow run to inspect.

![Viewing logs for a flow run in the Prefect UI](/2.6/img/ui/orion-flow-run-details.png)
...
For more information, see [Log schema](/2.6/api-ref/orion/schemas/core/#prefect.orion.schemas.core.Log) in the API documentation.
