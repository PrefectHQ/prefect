...
- [`PrefectFuture`](/2.6/api-ref/prefect/futures/#prefect.futures.PrefectFuture): A Prefect object that contains both _data_ and _State_.

Returning data  is the default behavior any time you call `your_task()`.

Returning Prefect [`State`](/2.6/api-ref/orion/schemas/states/) occurs anytime you call your task or flow with the argument `return_state=True`.

Returning [`PrefectFuture`](/2.6/api-ref/prefect/futures/#prefect.futures.PrefectFuture) is achieved by calling `your_task.submit()`.
...
See the [Final state determination](/2.6/concepts/flows/#final-state-determination) section of the [Flows](/2.6/concepts/flows/) documentation for further details and examples.
