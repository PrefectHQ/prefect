...
All workflows are defined within the context of a flow. Flows can include calls to [tasks](/2.6/concepts/tasks/) as well as to other flows, which we call ["subflows"](#composing-flows) in this context. Flows may be defined within modules and imported for use as subflows in your flow definitions.

Flows are required for [deployments](/2.6/concepts/deployments/) &mdash; every deployment points to a specific flow as the entrypoint for a flow run.
...
- Creating a [deployment](/2.6/concepts/deployments/) on Prefect Cloud or a locally run Prefect Orion server
...
| `task_runner` | An optional [task runner](/2.6/concepts/task-runners/) to use for task execution within the flow when you `.submit()` tasks. If not provided and you `.submit()` tasks, the `ConcurrentTaskRunner` will be used. |
...
You can define multiple flows within the same file. Whether running locally or via a [deployment](/2.6/concepts/deployments/), you must indicate which flow is the entrypoint for a flow run.
...
!!! note "Prerequisite"    
    Read the documentation about [states](/2.6/concepts/states) before proceeding with this section. 
