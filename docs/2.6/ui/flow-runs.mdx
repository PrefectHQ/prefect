...
![Prefect UI dashboard.](/2.6/img/ui/orion-dashboard.png)
...
![Highlighting the filters section of the dashboard.](/2.6/img/ui/orion-dash-filters.png)
...
![Highlighting the run history section of the dashboard.](/2.6/img/ui/orion-dash-history.png)

## Flow run details

The flow run details area of the page provides a listing of flow runs that are scheduled or have attempted to execute. 

You can select any flow name or flow run name on the list to display further details. See the [Flows and Tasks](/2.6/ui/flows-and-tasks/) documentation for more information.

![Highlighting the details section of the flow runs page.](/2.6/img/ui/orion-dash-details.png)
...
![Information displayed for a flow run in the UI](/2.6/img/ui/orion-flow-run-examples.png)
...
- [State](/2.6/concepts/states/) of the flow run
...
![Details for a flow in the Prefect UI](/2.6/img/ui/orion-flow-details.png)
...
![Display details for a specific flow run.](/2.6/img/ui/orion-flow-run-details.png)
...
- Flow version
- Run count
- [Infrastructure](/2.6/concepts/infrastructure/) used by the flow run
- Tags
- Logs
- Task runs
- Subflow runs
...
**Logs** displays all log messages for the flow run. See [Logging](/2.6/concepts/logs/) for more information about configuring and customizing log messages. Logs are the default view for a flow run.
...
![Display all task runs.](/2.6/img/ui/orion-task-runs.png)

**Sub Flow Runs** displays any subflows for the flow run.

![Display any subflow runs for the parent flow run.](/2.6/img/ui/orion-subflows.png)
...
![Radar view of flow and task relationships.](/2.6/img/ui/orion-flow-radar.png)
...
- Check [work queues](/2.6/ui/work-queues/) to make sure there's a queue that can service the flow run based on tags, deployment, or flow runner.
- Make sure an [agent](/2.6/concepts/work-queues/) is running in your execution environment and is configured to pull work from an appropriate work queue.
...
The flow run retention policy represents the number of days each flow run is available in the Prefect Cloud UI, and via the Prefect CLI and API after it ends. Once a flow run reaches a terminal state ([detailed in the chart here](/2.6/concepts/states/#state-types)), it will be retained until the end of the flow run retention period. 
...
This retention policy applies to all [details about a flow run](/2.6/ui/flow-runs/#inspect-a-flow-run), including its task runs. Subflow runs follow the retention policy independently from their parent flow runs, and are removed based on the time each subflow run reaches a terminal state. 
