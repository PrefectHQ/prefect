...
Tasks also take advantage of automatic Prefect [logging](/2.16.9/concepts/logs/) to capture details about task runs such as runtime, tags, and final state.
...
See all possible parameters in the [Python SDK API docs](/2.16.9/api-ref/prefect/tasks/#prefect.tasks.task).
...
- Filtering task runs by tag in the UI and via the [Prefect REST API](/2.16.9/api-ref/rest-api/#filtering).
- Setting [concurrency limits](#task-run-concurrency-limits) on task runs by tag.

Tags may be specified as a keyword argument on the [task decorator](/2.16.9/api-ref/prefect/tasks/#prefect.tasks.task).
...
You can also provide tags as an argument with a [`tags` context manager](/2.16.9/api-ref/prefect/context/#prefect.context.tags), specifying tags when the task is called rather than in its definition.
...
Additionally, you can pass a callable that accepts the number of retries as an argument and returns a list. Prefect includes an [`exponential_backoff`](/2.16.9/api-ref/prefect/tasks/#prefect.tasks.exponential_backoff) utility that will automatically generate a list of retry delays that correspond to an exponential backoff retry strategy. The following flow will wait for 10, 20, then 40 seconds before each retry.
...
Depending on how you call tasks, they can return different types of results and optionally engage the use of a [task runner](/2.16.9/concepts/task-runners/).

Any task can return:

- Data , such as `int`, `str`, `dict`, `list`, and so on &mdash;  this is the default behavior any time you call `your_task()`.
- [`PrefectFuture`](/2.16.9/api-ref/prefect/futures/#prefect.futures.PrefectFuture) &mdash;  this is achieved by calling [`your_task.submit()`](/2.16.9/concepts/task-runners/#using-a-task-runner). A `PrefectFuture` contains both _data_ and _State_
- Prefect [`State`](/2.16.9/api-ref/server/schemas/states/)  &mdash; anytime you call your task or flow with the argument `return_state=True`, it will directly return a state you can use to build custom behavior based on a state change you care about, such as task or flow failing or retrying.

To run your task with a [task runner](/2.16.9/concepts/task-runners/), you must call the task with `.submit()`.

See [state returned values](/2.16.9/concepts/task-runners/#using-results-from-submitted-tasks) for examples.
...
To create a dependency between two tasks that do not exchange data, but one needs to wait for the other to finish, use the special [`wait_for`](/2.16.9/api-ref/prefect/tasks/#prefect.tasks.Task.submit) keyword argument:
...
Note, if you are not using `asyncio.gather`, calling [`.submit()`](/2.16.9/concepts/task-runners/#using-a-task-runner) is required for asynchronous execution on the `ConcurrentTaskRunner`.
...
Task tag limits are checked whenever a task run attempts to enter a [`Running` state](/2.16.9/concepts/states/).
...
To remove all concurrency limits on a tag, use [`PrefectClient.delete_concurrency_limit_by_tag`](/2.16.9/api-ref/prefect/client/orchestration/#prefect.client.orchestration.PrefectClient.delete_concurrency_limit_by_tag/), passing the tag:
...
If you wish to query for the currently set limit on a tag, use [`PrefectClient.read_concurrency_limit_by_tag`](/2.16.9/api-ref/prefect/client/orchestration/#prefect.client.orchestration.PrefectClient.read_concurrency_limit_by_tag), passing the tag:

To see _all_ of your limits across all of your tags, use [`PrefectClient.read_concurrency_limits`](/2.16.9/api-ref/prefect/client/orchestration/#prefect.client.orchestration.PrefectClient.read_concurrency_limits).
