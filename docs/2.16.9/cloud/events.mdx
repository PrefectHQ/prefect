...
Events enable observability into your data stack via the [event feed](/2.16.9/ui/events/#event-feed), and the configuration of Prefect's reactivity via [automations](/2.16.9/concepts/automations/).

![Prefect UI](/2.16.9/img/ui/event-feed.png)

## Event specification

Events adhere to a structured [specification](https://app.prefect.cloud/api/docs#tag/Events).

![Prefect UI](/2.16.9/img/ui/event-spec.png)
...
Prefect Cloud offers [programmable webhooks](/2.16.9/guides/webhooks/) to receive HTTP requests from other systems and translate them into events within your workspace.  Webhooks can emit [pre-defined static events](/2.16.9/guides/webhooks/#static-webhook-events), dynamic events that [use portions of the incoming HTTP request](/2.16.9/guides/webhooks/#dynamic-webhook-events), or events derived from [CloudEvents](/2.16.9/guides/webhooks/#accepting-cloudevents).

Events emitted from any source will appear in the event feed, where you can visualize activity in context and configure [automations](/2.16.9/concepts/automations/) to react to the presence or absence of it in the future. 
...
![Event feed](/2.16.9/img/ui/event-feed.png)
...
From an event page, you can configure an [automation](/2.16.9/concepts/automations) to trigger on the observation of matching events or a lack of matching events by clicking the automate button in the overflow menu:

![Automation from event](/2.16.9/img/ui/automation-from-event.png)

The default trigger configuration will fire every time it sees an event with a matching resource identifier. Advanced configuration is possible via [custom triggers](/2.16.9/cloud/automations/).
