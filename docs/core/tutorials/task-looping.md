...
Previously, [task mapping](https://docs.prefect.io/core/core_concepts/mapping.html) allowed users to elevate parallelizable for-loops into first class parallel pipelines at runtime. Task looping offers much the same benefit, but for situations that require a while-loop pattern.
...
As a matter of best practice, we opted to elevate `M` to a [Prefect Parameter](https://docs.prefect.io/core/core_concepts/parameters.html) instead of hardcoding its value.  This way we can experiment with small values and eventually increase the value without recompiling our Flow.
