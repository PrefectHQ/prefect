...
States are rich objects that contain information about the status of a particular [task](/2.19/concepts/tasks) run or [flow](/2.19/concepts/flows/) run. While you don't need to know the details of the states to use Prefect, you can give your workflows superpowers by taking advantage of it.
...
- [`PrefectFuture`](/2.19/api-ref/prefect/futures/#prefect.futures.PrefectFuture): A Prefect object that contains both _data_ and _State_.

Returning data  is the default behavior any time you call `your_task()`.

Returning Prefect [`State`](/2.19/api-ref/server/schemas/states/) occurs anytime you call your task or flow with the argument `return_state=True`.

Returning [`PrefectFuture`](/2.19/api-ref/prefect/futures/#prefect.futures.PrefectFuture) is achieved by calling `your_task.submit()`.
...
See the [Final state determination](/2.19/concepts/flows/#final-state-determination) section of the [Flows](/2.19/concepts/flows/) documentation for further details and examples.
...
- [Send a notification when a flow run fails](/2.19/guides/state-change-hooks/#send-a-notification-when-a-flow-run-fails)
- [Delete a Cloud Run job when a flow crashes](/2.19/guides/state-change-hooks/#delete-a-cloud-run-job-when-a-flow-crashes)
