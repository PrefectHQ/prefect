...
This guide focusses on deployments created with the interactive CLI experience or a prefect.yaml file. If you'd like to create your deployments using Python code, see the discussion of flow code storage in the [`.deploy` section of Deploying Flows to Work pools and Workers guide](/2.19/guides/prefect-deploy/#creating-work-pool-based-deployments-with-deploy).
...
Another option for authentication is for the [worker](/2.19/concepts/work-pools/#worker-overview) to have access to the storage location at runtime via SSH keys.
...
In earlier versions of Prefect [storage blocks](/2.19/concepts/blocks/) were the recommended way to store flow code.
