...
- an [API server](/2.19/api-ref/) instance backed by a database and a UI
...
![Viewing the dashboard in the Prefect UI.](/2.19/img/ui/self-hosted-server-dashboard.png)
...
- [Workspaces](/2.19/cloud/workspaces/) &mdash; isolated environments to organize your flows, deployments, and flow runs.
- [Automations](/2.19/cloud/automations/) &mdash; configure triggers, actions, and notifications in response to real-time monitoring events.
- [Email notifications](/2.19/cloud/automations/) &mdash; send email alerts from Prefect's servers based on automation triggers.
- [Service accounts](/2.19/cloud/users/service-accounts/) &mdash; configure API access for running workers or executing flow runs on remote infrastructure.
- [Custom role-based access controls (RBAC)](/2.19/cloud/users/roles/) &mdash; assign users granular permissions to perform activities within an account or workspace.
- [Single Sign-on (SSO)](/2.19/cloud/users/sso/) &mdash; authentication using your identity provider.
- [Audit Log](/2.19/cloud/users/audit-log/) &mdash; a record of user activities to monitor security and compliance.

Read more about Prefect Cloud in the [Cloud](/2.19/cloud/) section.
...
**`PREFECT_API_URL` required when running Prefect inside a container**
    ...
    You can save the API server address in a [Prefect profile](/2.19/concepts/settings/). Whenever that profile is active, the API endpoint is at that address.

    See [Profiles & Configuration](/2.19/concepts/settings/) for more information on profiles and configurable Prefect settings.
...
See the [contributing docs](/2.19/contributing/overview/#adding-database-migrations) to create new database migrations.

## Notifications

[Prefect Cloud](/2.19/cloud/) gives you access to a hosted platform with Workspace & User controls, Events, and Automations. Prefect Cloud has an option for automation notifications. The more limited Notifications option is provided for the self-hosted Prefect server.

Notifications enable you to set up alerts that are sent when a flow enters any state you specify. When your flow and task runs changes [state](/2.19/concepts/states/), Prefect notes the state change and checks whether the new state matches any notification policies. If it does, a new notification is queued.
...
**Notifications in Prefect Cloud**
    ...
    Prefect Cloud uses the robust [Automations](/2.19/cloud/automations/) interface to enable notifications related to flow run state changes and work pool status.
...
![Creating a notification in the Prefect UI](/2.19/img/ui/create-email-notification.png)
...
![Viewing all configured notifications in the Prefect UI](/2.19/img/ui/notifications.png)
