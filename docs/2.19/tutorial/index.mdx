...
1. [Create a flow](/2.19/tutorial/flows/)
2. [Add tasks to it](/2.19/tutorial/tasks/)
3. [Deploy and run the flow locally](/2.19/tutorial/deployments/)
4. [Create a work pool and run the flow on remote infrastructure](/2.19/tutorial/work-pools/)
...
5.[Using a worker-based deployment](/2.19/tutorial/workers/)

If you're looking for examples of more advanced operations (such as [deploying on Kubernetes](/2.19/guides/deployment/kubernetes/)), check out Prefect's [guides](/2.19/guides/).

Compared to the [Quickstart](/2.19/getting-started/quickstart/), this tutorial is a more in-depth guide to Prefect's functionality.
...
See the [install guide](/2.19/getting-started/installation/) for more detailed instructions.
...
1. Use the `prefect cloud login` CLI command to [authenticate to Prefect Cloud](/2.19/cloud/users/api-keys/) from your environment.
...
If you have any issues with browser-based authentication, see the [Prefect Cloud docs](/2.19/cloud/users/api-keys/) to learn how to authenticate with a manually created API key.

As an alternative to using Prefect Cloud, you can self-host a [Prefect server instance](/2.19/host/).
If you choose this option, run `prefect server start` to start a local Prefect server instance.

## [First steps: Flows](/2.19/tutorial/flows/)

Let's begin by learning how to create your first Prefect flow - [click here to get started](/2.19/tutorial/flows/).
