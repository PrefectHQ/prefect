...
See the [install guide](/2.19/getting-started/installation/) for more detailed installation instructions, if needed.
...
**Self-hosted Prefect server instance**
    ...
    If you would like to host a Prefect server instance on your own infrastructure, see the [tutorial](/2.19/tutorial/) and select the "Self-hosted" tab.
...
[Flows](/2.19/concepts/flows/) are the core observable, deployable units in Prefect and are the primary entrypoint to orchestrated work.
...
We can tell Prefect where we want to run our workflow by creating a [work pool](/2.19/concepts/work-pools/).

We can have Prefect Cloud run our flow code for us with a Prefect Managed work pool.

Let's create a [Prefect Managed work pool](/2.19/guides/managed-execution/) so that Prefect can run our flows for us.
...
Let's package both of these things, along with the location for where to find our flow code, into a [deployment](/2.19/concepts/deployments/) so that we can schedule our workflow to run remotely.
...
**Code storage options**
    ...
    Read more in [this guide](/2.19/guides/prefect-deploy/#creating-work-pool-based-deployments).
...
![Managed flow run graph and logs](/2.19/img/ui/qs-flow-run.png)
...
To learn how to run flows on your own infrastructure, customize the Docker image where your flow runs, and gain more orchestration and observation benefits check out the [tutorial](/2.19/tutorial/).
