...
async def main():
    async with get_client() as client:
        ...
        print(
            f"  Deployment has concurrency_limit: {deployment_before.concurrency_limit}"
        )
        if deployment_before.global_concurrency_limit:
            print(
                f"  Active slots: {deployment_before.global_concurrency_limit.active_slots}"
            )
        ...
        print(
            f"  Deployment has concurrency_limit: {deployment_after.concurrency_limit}"
        )
        if deployment_after.global_concurrency_limit:
            ...
            if active_slots == 2:
                print(
                    "  ✅ Active slots = 2 (fix is working! RUNNING flows acquired slots)"
                )
            elif active_slots == 0:
                print(
                    "  ❌ Active slots = 0 (fix NOT working - RUNNING flows didn't acquire slots)"
                )
            else:
        ...
        if new_run_state.state_type.value == "SCHEDULED":
            if active_slots == 2:
                print(
                    "✅ New flow run is blocked AND RUNNING flows have slots (fix working!)"
                )
            else:
                print(
                    "⚠️  New flow run is blocked, but RUNNING flows don't have slots (may be blocking for wrong reason)"
                )
        else:
