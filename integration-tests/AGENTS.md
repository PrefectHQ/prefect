...
- The global `pytest-timeout` is 90s (`pyproject.toml`). A test whose polls/waits can sum above that gets killed mid-run before it can report its own failure — add `@pytest.mark.timeout(N)` for scenarios that legitimately need longer.
